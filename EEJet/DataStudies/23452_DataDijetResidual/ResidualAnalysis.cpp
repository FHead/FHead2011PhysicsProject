...
struct Event
{
   ...
   bool operator <(const Event &other) const
   {
      ...
      if(Ref.InnerGetP() < other.Ref.InnerGetP())       return true;
      if(Ref.InnerGetP() > other.Ref.InnerGetP())       return false;
      if(Probe.InnerGetP() < other.Probe.InnerGetP())   return true;
      if(Probe.InnerGetP() > other.Probe.InnerGetP())   return false;
      return false;
   }
};
...
int main(int argc, char *argv[])
{
   ...
   out << "{1 JetTheta 1 JetP [0]*([1]+[2]*x+[3]*x*x+[4]*x*x*x) RelativeResidual}" << endl;
...
vector<BinResult> GetR(PdfFileHelper &PdfFile, string FileName, double AlphaMax)
{
   ...
   for(int iE = 0; iE < EntryCount; iE++)
   {
      ...
      double Alpha = 0;
      if(N >= 3)
         Alpha = JetPT[2] / cosh(JetEta[2]) / (JRef.GetP() + JProbe.GetP());
...
pair<double, double> GetExtrapolatedR(PdfFileHelper &PdfFile, vector<Event> &E, double ThetaMin, double ThetaMax)
{
   ...
   TH1D HRefP("HRefP", ";Ref Jet P;", 100, 0, 75);
   TH1D HProbeP("HProbeP", ";Probe Jet P;", 100, 0, 75);
   ...
   for(int i = 0; i < N; i++)
   {
      HRefP.Fill(E[i].Ref.GetP());
      HProbeP.Fill(E[i].Probe.GetP());
      HAlpha.Fill(E[i].Alpha);
      HDPhi.Fill(GetDPhi(E[i].Ref, E[i].Probe));
   }

   PdfFile.AddPlot(HRefP, "", true);
   PdfFile.AddPlot(HProbeP, "", true);
   ...
   for(int iB = 0; iB < BinCount; iB++)
   {
      ...
      {
         double R = E[i].Probe.GetP() / E[i].Ref.GetP();
