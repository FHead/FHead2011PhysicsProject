...
vector<BinResult> GetR(PdfFileHelper &PdfFile, string FileName, double AlphaMax = 0.15);
...
struct Event
{
   ...
   bool operator <(const Event &other) const
   {
      ...
      if(Ref[0] < other.Ref[0])       return true;
      if(Ref[0] > other.Ref[0])       return false;
      if(Probe[0] < other.Probe[0])   return true;
      if(Probe[0] > other.Probe[0])   return false;
      return false;
   }
};
...
int main(int argc, char *argv[])
{
   ...
   double AlphaMax     = CL.GetDouble("AlphaMax", 0.15);

   PdfFileHelper PdfFile(PdfFileName);
   PdfFile.AddTextPage("Residual check");
   
   vector<BinResult> RData = GetR(PdfFile, DataFileName, AlphaMax);
   vector<BinResult> RMC   = GetR(PdfFile, MCFileName, AlphaMax);

   DrawSummary(PdfFile, RData, RMC);

   ofstream out("JECRelativeResidual.txt");

   out << "{1 JetTheta 1 JetE [0]*([1]+[2]*x+[3]*x*x+[4]*x*x*x) RelativeResidual}" << endl;
...
vector<BinResult> GetR(PdfFileHelper &PdfFile, string FileName, double AlphaMax)
{
   ...
   for(int iE = 0; iE < EntryCount; iE++)
   {
      ...
      if(J1.GetTheta() > RefThetaMin && J1.GetTheta() <= RefThetaMax)
         J1InReference = true;
      if(J2.GetTheta() > RefThetaMin && J2.GetTheta() <= RefThetaMax)
         J2InReference = true;
...
pair<double, double> GetExtrapolatedR(PdfFileHelper &PdfFile, vector<Event> &E, double ThetaMin, double ThetaMax)
{
   ...
   TH1D HRefE("HRefE", ";Ref Jet E;", 100, 0, 75);
   TH1D HProbeE("HProbeE", ";Probe Jet E;", 100, 0, 75);
   ...
   for(int i = 0; i < N; i++)
   {
      HRefE.Fill(E[i].Ref[0]);
      HProbeE.Fill(E[i].Probe[0]);
      HAlpha.Fill(E[i].Alpha);
      HDPhi.Fill(GetDPhi(E[i].Ref, E[i].Probe));
   }

   PdfFile.AddPlot(HRefE, "", true);
   PdfFile.AddPlot(HProbeE, "", true);
   ...
   for(int iB = 0; iB < BinCount; iB++)
   {
      ...
      {
         double R = E[i].Probe[0] / E[i].Ref[0];
...
void DrawSummary(PdfFileHelper &PdfFile, vector<BinResult> &RData, vector<BinResult> &RMC)
{
   ...
   GData.GetXaxis()->SetRangeUser(0, 1);
   ...
   Latex.SetTextAlign(12);
   Latex.SetNDC();
   Latex.DrawLatex(0.20, 0.22, "#color[1]{Data} #color[2]{MC}");

   PdfFile.AddCanvas(Canvas);

   GRatio.Draw("ap");

   GRatio.GetXaxis()->SetRangeUser(0, 1);
