...
int main(int argc, char *argv[])
{
   ...
   out << "{1 JetTheta 1 JetP " << Formula << " L2Relative}" << endl;
...
vector<double> JECFit(PdfFileHelper &PdfFile, TTree *Tree, double ThetaMin, double ThetaMax, double R)
{
   ...
   double GenPT, GenTheta, RecoPT, RecoTheta, Angle;
   ...
   for(int i = 0; i < EntryCount; i++)
   {
      ...
      Data.push_back(pair<double, double>(GenP, RecoP));
   }
   ...
   H.SetStats(0);
   TProfile P("P", ";GenP;Response", BinCount, Bins);
   P.SetStats(0);

   TH1D HR("HR", "GenP = 30-40 GeV;RecoP/GenP;", 100, 0, 2);
   // HR.SetStats(0);
   ...
   TF1 F1("F1", "pol2",               0, MaxP * 1.2);
   TF1 F2("F2", "[0]+[1]*exp([2]*x)", 0, MaxP * 1.2);
   TF1 F3("F3", "pol3",               0, MaxP * 1.2);
   TF1 F4("F4", "pol4",               0, MaxP * 1.2);
