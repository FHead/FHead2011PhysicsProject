...
void GenerateEvents(PdfFileHelper &PdfFile, string OutputFileName, int NEvent, double EPythia, double EBall);
...
int main(int argc, char *argv[])
{
   ...
   PdfFileHelper PdfFile("Test.pdf");
   PdfFile.AddTextPage("Test");

   GenerateEvents(PdfFile, "PythiaTestPythia.root",   1000000, 90, 0);
   GenerateEvents(PdfFile, "PythiaTestBall.root",     1000000, 45, 45);
   GenerateEvents(PdfFile, "PythiaTestOnlyBall.root", 1000000, 0, 90);
...
void GenerateEvents(PdfFileHelper &PdfFile, string OutputFileName, int NEvent, double EPythia, double EBall)
{
   ...
   for(int iEntry = 0; iEntry < NEvent; iEntry++)
   {
      ...
      double ParticleMean = 0.40000;  // mean momentum for the thermal distribution
...
double DrawMomentum(double Mean)
{
   return DrawLogNormal(Mean, sqrt(Mean));
}
