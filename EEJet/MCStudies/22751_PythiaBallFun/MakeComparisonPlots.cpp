...
void AppendPlot(PdfFileHelper &PdfFile, vector<string> &Input, vector<string> &Label, string HistogramName)
{
   ...
   int Colors[] = {kBlack, kRed, kBlue, kGreen, kCyan, kMagenta};
   Colors[0] = TColor::GetColor("#E74C3C");
   Colors[1] = TColor::GetColor("#3498DB");
   Colors[2] = TColor::GetColor("#F1C40F");
   Colors[3] = TColor::GetColor("#2ECC71");
   Colors[4] = TColor::GetColor("#7F7F7F");
   Colors[5] = TColor::GetColor("#8E44AD");
   ...
   for(int i = 0; i < N; i++)
   {
      ...
      Histograms[i]->SetLineColor(Colors[i%6]);
      Histograms[i]->SetMarkerColor(Colors[i%6]);
