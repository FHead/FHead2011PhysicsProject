...
enum ObservableType {ObservableNone, ObservableJetP, ObservableZG, ObservableRG, ObservableThrust, ObservableTheta};
...
public:
      ...
      double Shift = 0, double Smear = 1)
   {
      ...
      if(Type == ObservableTheta)
      {
         if(Step == Gen && Item < GenJetTheta->size())
            return (*GenJetTheta)[Item];
         if(Step == Reco && Item < RecoJetTheta->size())
            return (*RecoJetTheta)[Item];
         if(Step == Matched && Item < MatchedJetTheta->size())
            return (*MatchedJetTheta)[Item];
      }
...
int main(int argc, char *argv[])
{
   ...
   bool CheckThetaGap             = CL.GetBool("CheckThetaGap", true);
   ...
   for(int iE = 0; iE < EntryCount; iE++)
   {
      ...
      {
         double GenTheta = MMC.GetValue(Gen, ObservableTheta, 0, iJ);
         if(CheckThetaGap == true && ((GenTheta < ThetaGap) || (GenTheta > M_PI - ThetaGap)))
            continue;
      ...
      for(int iJ = 0; iJ < NJet; iJ++)
      {
         ...
         if(CheckThetaGap == true && ((GenTheta < ThetaGap) || (GenTheta > M_PI - ThetaGap)))
            continue;
         double MatchedTheta = MMC.GetValue(Matched, ObservableTheta, 0, iJ);
         if(CheckThetaGap == true && ((MatchedTheta < ThetaGap) || (MatchedTheta > M_PI - ThetaGap)))
            continue;
      ...
      for(int iJ = 0; iJ < NJet; iJ++)
      {
         double RecoTheta = MMC.GetValue(Reco, ObservableTheta, 0, iJ);
         if(CheckThetaGap == true && ((RecoTheta < ThetaGap) || (RecoTheta > M_PI - ThetaGap)))
            continue;
   ...
   for(int iE = 0; iE < EntryCount; iE++)
   {
      ...
      for(int iJ = 0; iJ < NJet; iJ++)
      {
         double RecoTheta = MData.GetValue(Reco, ObservableTheta, 0, iJ);
         if(CheckThetaGap == true && ((RecoTheta < ThetaGap) || (RecoTheta > M_PI - ThetaGap)))
            continue;
