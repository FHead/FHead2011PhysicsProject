...
int main(int argc, char *argv[])
{
   ...
   TH2D HReco1OneDownXY("HReco1OneDownXY", ";x;y", 100, -2, 2, 100, -2, 2);
   TH2D HReco2OneDownXY("HReco2OneDownXY", ";x;y", 200, -4, 4, 200, -4, 4);

   TH2D HReco2OneDownDistanceVsCentrality("HReco2OneDownDistanceVsCentrality",
      ";centrality;reco 2 one up distance", 100, 0, 1, 100, 0, 10);
   TH2D HReco2OneDownDistanceVsJetPT("HReco2OneDownDistanceVsJetPT",
      ";JetPT;reco 2 one up distance", 100, 80, 500, 100, 0, 10);
   TH2D HReco2OneDownDistanceVsJetEta("HReco2OneDownDistanceVsJetEta",
      ";JetEta;reco 2 one up distance", 100, -1.5, 1.5, 100, 0, 10);
   
   TProfile PGoodReco2OneDownVsCentrality("PGoodReco2OneDownVsCentrality",
      ";centrality;P(reco 2 one up distance < 1.0)", 100, 0, 1);
   TProfile PGoodReco2OneDownVsJetPT("PGoodReco2OneDownVsJetPT",
      ";JetPT;P(reco 2 one up distance < 1.0)", 100, 80, 500);
   TProfile PGoodReco2OneDownVsJetEta("PGoodReco2OneDownVsJetEta",
      ";JetEta;P(reco 2 one up distance < 1.0)", 100, -1.5, 1.5);

   TH2D HRecoOneDownDistance("HRecoOneDownDistance",
      ";reco1 one down distance;reco 2 one down distance", 100, 0, 5, 100, 0, 5);
   ...
   for(int iEntry = 0; iEntry < EntryCount; iEntry++)
   {
      ...
      for(int i = 0; i < MSDJet.JetCount; i++)
      {
         ...
         if(HSDJet.Region[i] != 2)   // we want region II for this study
            continue;
         if(HSDJet.Inside[i] == false)   // we want inside jets
            continue;
         ...
         Node *OneDownGen = GroomedGen->Child1;

         XY OneDownGenMean;    OneDownGenMean.SetAsAverage(OneDownGen->Child1->P, OneDownGen->Child2->P);
         Matrix22 OneDownGenMatrix(OneDownGen->Child1->P, OneDownGen->Child2->P);

         XY Reco1OneDownXY(OneDownGenMatrix, OneDownGenMean, GroomedReco->Child1->P);
         XY Reco2OneDownXY(OneDownGenMatrix, OneDownGenMean, GroomedReco->Child2->P);

         double Reco1OneDownDistance = (Reco1OneDownXY - XY(+1, 0)).Norm();
         double Reco2OneDownDistance = (Reco2OneDownXY - XY(-1, 0)).Norm();

         HReco1OneDownXY.Fill(Reco1OneDownXY.X, Reco1OneDownXY.Y);
         HReco2OneDownXY.Fill(Reco2OneDownXY.X, Reco2OneDownXY.Y);
         HReco2OneDownDistanceVsCentrality.Fill(Centrality,   Reco2OneDownDistance);
         HReco2OneDownDistanceVsJetPT.Fill(MSDJet.JetPT[i],   Reco2OneDownDistance);
         HReco2OneDownDistanceVsJetEta.Fill(MSDJet.JetEta[i], Reco2OneDownDistance);
         PGoodReco2OneDownVsCentrality.Fill(Centrality,       ((Reco2OneDownDistance < 1.0) ? 1 : 0));
         PGoodReco2OneDownVsJetPT.Fill(MSDJet.JetPT[i],       ((Reco2OneDownDistance < 1.0) ? 1 : 0));
         PGoodReco2OneDownVsJetEta.Fill(MSDJet.JetEta[i],     ((Reco2OneDownDistance < 1.0) ? 1 : 0));

         HRecoOneDownDistance.Fill(Reco1OneDownDistance, Reco2OneDownDistance);
   ...
   HReco1OneDownXY.Write();
   HReco2OneDownXY.Write();
   HReco2OneDownDistanceVsCentrality.Write();
   HReco2OneDownDistanceVsJetPT.Write();
   HReco2OneDownDistanceVsJetEta.Write();
   PGoodReco2OneDownVsCentrality.Write();
   PGoodReco2OneDownVsJetPT.Write();
   PGoodReco2OneDownVsJetEta.Write();

   HRecoOneDownDistance.Write();
