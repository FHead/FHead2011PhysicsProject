...
int main(int argc, char *argv[])
{
   ...
   for(int iEntry = 0; iEntry < EntryCount; iEntry++)
   {
      ...
      for(int iJ = 0; iJ < MSDJet.JetCount; iJ++)
      {
         ...
         double TotalPT = Rho * 0.4 * 0.4 * PI * DrawGaussian(1, 0.2);
         while(TotalPT > 0)
         {
         ...
         while(Candidates.size() > 0 && Ghosts.size() > 0)
         {
            int BestI = 0, BestJ = 0;
            double BestDR = -1;
            ...
            for(int i = 0; i < (int)Candidates.size(); i++)
            {
               ...
               {
                  double DR = GetDR(Candidates[i], Ghosts[j]);
            ...
            FourVector CP = Candidates[BestI];
            FourVector GP = Ghosts[BestJ];

            if(CP.GetPT() > GP.GetPT())
            {
               Candidates[BestI].SetPtEtaPhi(CP.GetPT() - GP.GetPT(), CP.GetEta(), CP.GetPhi());
               Ghosts.erase(Ghosts.begin() + BestJ);
            }
            else
            {
               Candidates.erase(Candidates.begin() + BestI);
               Ghosts[BestJ].SetPtEtaPhi(GP.GetPT() - CP.GetPT(), GP.GetEta(), GP.GetPhi());
            }
         }
