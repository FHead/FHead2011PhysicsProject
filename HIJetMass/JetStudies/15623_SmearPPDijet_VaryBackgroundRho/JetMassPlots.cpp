...
int main(int argc, char *argv[])
{
   ...
   TH1D HNewSDMassGoodDR("HNewSDMassGoodDR", "New SD Mass, New DR > 0.1; New SD Mass", 100, 0, 100);
   TH1D HNewSDMassBadDR("HNewSDMassBadDR", "New SD Mass, New DR > 0.1; New SD Mass", 100, 0, 100);
   ...
   for(int iEntry = 0; iEntry < EntryCount; iEntry++)
   {
      ...
      for(int iJ = 0; iJ < MSDJet.JetCount; iJ++)
      {
         ...
         double NewDR = 0;
         if(Groomed->N > 1)
            NewDR = GetDR(Groomed->Child1->P, Groomed->Child2->P);

         if(HSDJet.RecoSubJetDR[iJ] > 0.1 && MSDJet.JetPT[iJ] > 80)
            HSDMass.Fill(MSDJet.JetM[iJ]);
         if(Groomed->N > 1 && NewDR > 0.1 && Groomed->P.GetPT() > 80)
         ...
         if(Groomed->N > 1 && NewDR > 0.1 && Groomed->P.GetPT() > 80
            && NewDR - HSDJet.RecoSubJetDR[iJ] < 0.05)
            HNewSDMassGoodDR.Fill(Groomed->P.GetMass());
         if(Groomed->N > 1 && NewDR > 0.1 && Groomed->P.GetPT() > 80
            && NewDR - HSDJet.RecoSubJetDR[iJ] > 0.05)
            HNewSDMassBadDR.Fill(Groomed->P.GetMass());
   ...
   HNewSDMassGoodDR.Write();
   HNewSDMassBadDR.Write();
