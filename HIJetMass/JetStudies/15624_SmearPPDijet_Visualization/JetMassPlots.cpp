...
int main(int argc, char *argv[])
{
   if(argc < 7)
   {
      cerr << "Usage: " << argv[0] << " Input Output Tag PTHatMin PTHatMax Rho" << endl;
      return -1;
   }
   ...
   cout << "Running with rho = " << Rho << endl;
   ...
   TH1D HNewSDMassReducedRandomJet("HNewSDMassReducedRandomJet",
      "New SD Mass, New DR > 0.1; New SD Mass", 100, 0, 100);

   int EntryCount = MHiEvent.Tree->GetEntries();
   ProgressBar Bar(cout, EntryCount);
   Bar.SetStyle(1);

   for(int iEntry = 0; iEntry < EntryCount && iEntry < 1000; iEntry++)
   {
      ...
      for(int iJ = 0; iJ < MSDJet.JetCount; iJ++)
      {
         ...
         TH2D HJetShape(Form("HJetShape_%d_%d", iEntry, iJ), ";eta;phi;PT", 100, -0.4, 0.4, 100, -0.4, 0.4);
         TH2D HJetShapeAdded(Form("HJetShapeAdded_%d_%d", iEntry, iJ), ";eta;phi;PT", 100, -0.4, 0.4, 100, -0.4, 0.4);
         TH2D HJetShapeSubtracted(Form("HJetShapeSubtracted_%d_%d", iEntry, iJ), ";eta;phi;PT", 400, -0.4, 0.4, 400, -0.4, 0.4);

         TH2D HJetShapeTogether(Form("HJetShapeTogether_%d_%d", iEntry, iJ), ";eta;phi;PT",
            100, -0.8, 0.8, 100, -0.8, 0.8);
         ...
         {
            double DEta = Candidates[i].GetEta() - MSDJet.JetEta[iJ];
            double DPhi = Candidates[i].GetPhi() - MSDJet.JetPhi[iJ];
            if(DPhi > +PI)   DPhi = DPhi - 2 * PI;
            if(DPhi < -PI)   DPhi = DPhi + 2 * PI;
            HJetShape.Fill(DEta, DPhi, Candidates[i].GetPT());
            HJetShapeTogether.Fill(DEta - 0.4, DPhi - 0.4, Candidates[i].GetPT());
            // cout << DEta << " " << DPhi << " " << GetDR(Candidates[i], JetP) << endl;
         }
         HJetShape.Write();
         ...
         for(int i = 0; i < (int)Candidates.size(); i++)
         {
            ...
            if(DPhi > +PI)   DPhi = DPhi - 2 * PI;
            if(DPhi < -PI)   DPhi = DPhi + 2 * PI;
            HJetShapeAdded.Fill(DEta, DPhi, Candidates[i].GetPT());
            HJetShapeTogether.Fill(DEta - 0.4, DPhi + 0.4, Candidates[i].GetPT());
         }
         HJetShapeAdded.Write();
         ...
         {
            double DEta = GoodCandidates[i].GetEta() - MSDJet.JetEta[iJ];
            double DPhi = GoodCandidates[i].GetPhi() - MSDJet.JetPhi[iJ];
            if(DPhi > +PI)   DPhi = DPhi - 2 * PI;
            if(DPhi < -PI)   DPhi = DPhi + 2 * PI;
            HJetShapeSubtracted.Fill(DEta, DPhi, GoodCandidates[i].GetPT());
            HJetShapeTogether.Fill(DEta + 0.4, DPhi - 0.4, GoodCandidates[i].GetPT());
         }
         HJetShapeSubtracted.Write();
         HJetShapeTogether.Write();
         ...
         if(Groomed->N > 1 && GetDR(Groomed->Child1->P, Groomed->Child2->P) > 0.1 && Groomed->P.GetPT() > 80
            && MSDJet.JetPT[iJ] > 50)
            HNewSDMassReducedRandomJet.Fill(Groomed->P.GetMass());
   ...
   HSDMass.Write();
   HNewSDMass.Write();
   HNewSDMassReducedRandomJet.Write();
