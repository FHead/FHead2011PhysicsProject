...
int main(int argc, char *argv[])
{
   ...
   TH2D HSDMassComparison("HSDMassComparison", ";Jet SD Mass;New Jet SD Mass",
      100, 0, 100, 100, 0, 100);
   TH2D HRecoDRComparison("HRecoDRComparison", ";Jet Reco DR;New Jet Reco DR",
      100, 0, 0.4, 100, 0, 0.4);

   TH2D HSDMassJetPT("HSDMassJetPT", ";Jet PT;SD Mass", 100, 80, 500, 100, 0, 100);
   TH2D HNewSDMassNewJetPT("HNewSDMassNewJetPT", ";New Jet PT;New SD Mass",
      100, 0, 600, 100, 0, 100);
   ...
   for(int iEntry = 0; iEntry < EntryCount; iEntry++)
   {
      ...
      for(int iJ = 0; iJ < MSDJet.JetCount; iJ++)
      {
         ...
         TH2D HJetShapeTogether(Form("HJetShapeTogether_%d_%d", iEntry, iJ), ";eta;phi;PT",
            100, -2 * Range, 2 * Range, 100, -2 * Range, 2 * Range);
         ...
         for(int i = 0; i < (int)Candidates.size(); i++)
         {
            ...
            if(DPhi < -PI)   DPhi = DPhi + 2 * PI;
            HJetShapeTogether.Fill(DEta - Range, DPhi - Range, Candidates[i].GetPT());
         }
         ...
         for(int i = 0; i < (int)GoodCandidates.size(); i++)
         {
            ...
            if(DPhi > +PI)   DPhi = DPhi - 2 * PI;
            if(DPhi < -PI)   DPhi = DPhi + 2 * PI;
            HJetShapeTogether.Fill(DEta + Range, DPhi - Range, GoodCandidates[i].GetPT());
         }
         ...
         if(MSDJet.JetPT[iJ] > 80 || NewJetP.GetPT() > 80)
         {
            HJetShapeTogether.SetTitle(Form("New Jet Location = (%f,%f).  PTs %f %f", NewJetP.GetEta() - MSDJet.JetEta[iJ], NewJetP.GetPhi() - MSDJet.JetPhi[iJ], MSDJet.JetPT[iJ], NewJetP.GetPT()));
