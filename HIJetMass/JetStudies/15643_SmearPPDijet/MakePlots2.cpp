...
int main()
{
   ...
   ExportPlots(RangeFiles, RangeLabels, "Plots/Range");
...
void ExportPlots(vector<TFile *> Files, vector<string> Labels, string OutputBase)
{
   ...
   {
      TH1D *HTemp = new TH1D(Form("H%d", i), ";SD Mass;a.u.", 80, 0, 80);
      TTree *Tree = (TTree *)Files[i]->Get("OutputTree");
      Tree->Draw(Form("NewJetSDMass2>>H%d", i), "MCWeight * (NewJetDR2 > 0.1 && NewJetPT > 140)");
      H.push_back(HTemp);
   }
   ...
   TH2D HWorld("HWorld", ";SD Mass;a.u.", 100, 0, 80, 100, 0, 0.125);
   HWorld.SetStats(0);
