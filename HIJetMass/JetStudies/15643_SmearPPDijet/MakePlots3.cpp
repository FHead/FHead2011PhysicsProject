...
int main();
void ExportPlots(vector<TFile *> Files, vector<string> Labels, string OutputBase, PdfFileHelper &PdfFile);
void Normalize(TH1D &H);

int main()
{
   PdfFileHelper PdfFile("Systematics.pdf");
   ...
   ExportPlots(MBFiles, MBLabels, "Plots/MBSpectrum", PdfFile);
   ...
   ExportPlots(SmearFiles, SmearLabels, "Plots/Smear", PdfFile);
   ...
   ExportPlots(RhoFiles, RhoLabels, "Plots/Rho", PdfFile);
   ...
   ExportPlots(GhostFiles, GhostLabels, "Plots/Ghost", PdfFile);

   vector<TFile *> RangeFiles(5);
   vector<string> RangeLabels(5);

   RangeFiles[0] = &File1;   RangeLabels[0] = "0.80";
   RangeFiles[1] = &File14;  RangeLabels[1] = "0.60";
   RangeFiles[2] = &File13;  RangeLabels[2] = "1.20";
   RangeFiles[3] = &File17;  RangeLabels[3] = "1.60";
   RangeFiles[4] = &File18;  RangeLabels[4] = "2.00";

   ExportPlots(RangeFiles, RangeLabels, "Plots/Range", PdfFile);
...
void ExportPlots(vector<TFile *> Files, vector<string> Labels, string OutputBase, PdfFileHelper &PdfFile)
{
   ...
   {
      TH1D *HTemp = new TH1D(Form("H%d", i), ";SD Mass / Jet PT;a.u.", 40, 0.0, 0.4);
      TTree *Tree = (TTree *)Files[i]->Get("OutputTree");
      Tree->Draw(Form("NewJetSDMass2/NewJetPT>>H%d", i), "MCWeight * (NewJetDR2 > 0.1 && NewJetPT > 140)");
      H.push_back(HTemp);
   }
   ...
   TH2D HWorld("HWorld", ";SD Mass / Jet PT;a.u.", 100, 0, 0.4, 100, 0, 22);
   HWorld.SetStats(0);
   ...
   TH2D HWorld2("HWorld2", ";SD Mass / Jet PT;Ratio", 100, 0, 0.4, 100, 0, 2);
   HWorld2.SetStats(0);
