...
int main(int argc, char *argv[])
{
   if(argc != 11)
   {
      cerr << "Usage: " << argv[0] << " Input Output Tag PTHatMin PTHatMax RhoMultiplier GhostDistance Smear MBMultiplier Range" << endl;
      return -1;
   }
   ...
   double TreeNewJetPTBefore, TreeNewJetEtaBefore, TreeNewJetPhiBefore;
   ...
   double TreeBestJetPTBefore, TreeBestJetEtaBefore, TreeBestJetPhiBefore;
   ...
   OutputTree.Branch("NewJetPTBefore", &TreeNewJetPTBefore, "NewJetPTBefore/D");
   OutputTree.Branch("NewJetEtaBefore",&TreeNewJetEtaBefore,"NewJetEtaBefore/D");
   OutputTree.Branch("NewJetPhiBefore",&TreeNewJetPhiBefore,"NewJetPhiBefore/D");
   ...
   OutputTree.Branch("BestJetPTBefore", &TreeBestJetPTBefore, "BestJetPTBefore/D");
   OutputTree.Branch("BestJetEtaBefore",&TreeBestJetEtaBefore,"BestJetEtaBefore/D");
   OutputTree.Branch("BestJetPhiBefore",&TreeBestJetPhiBefore,"BestJetPhiBefore/D");
   ...
   for(int iEntry = 0; iEntry < EntryCount; iEntry++)
   {
      ...
      for(int iJ = 0; iJ < MJet.JetCount; iJ++)
      {
         ...
         for(int i = 0; i < (int)AllCandidates.size(); i++)
         {
            if(GetDR(NewJetPBeforeCS, AllCandidates[i]) < 0.4)
               NewConstituent.push_back(AllCandidates[i]);
            if(GetDR(BestJetPBeforeCS, AllCandidates[i]) < 0.4)
               BestConstituent.push_back(AllCandidates[i]);
         }
         ...
         for(int JetIndex = 0; JetIndex < 2; JetIndex++)
         {
            ...
            vector<double> GhostsEta;
            vector<double> GhostsPhi;

            for(double Eta = -0.4; Eta <= 0.4; Eta = Eta + GhostDistance)
            {
               for(double Phi = -0.4; Phi <= 0.4; Phi = Phi + GhostDistance)
               {
                  if(Eta * Eta + Phi * Phi > 0.4 * 0.4)
                     continue;

                  GhostsPT.push_back(1);
                  GhostsEta.push_back(JetEta + Eta);
                  GhostsPhi.push_back(JetPhi + Phi);
               }
            }
            double GhostPT = Rho * 0.4 * 0.4 * PI / GhostsPT.size();
            ...
            for(int i = 0; i < (int)JetCandidates->size(); i++)
            {
               if(CandidatesAlive[i] == true)
               {
                  ...
                  // cout << i << " " << (*JetCandidates)[i].GetEta() - JetEta << endl;
               }
            }
         }
         ...
         if(GoodNewCandidates.size() == 0 && GoodBestCandidates.size() == 0)
         {
            TreeNewJetPT = -1;
            TreeBestJetPT = -1;
            OutputTree.Fill();
         ...
         vector<Node *> Nodes;
         for(int i = 0; i < (int)GoodNewCandidates.size(); i++)
            Nodes.push_back(new Node(GoodNewCandidates[i]));
         vector<Node *> NodesBest;
         for(int i = 0; i < (int)GoodBestCandidates.size(); i++)
            NodesBest.push_back(new Node(GoodBestCandidates[i]));
