...
int main(int argc, char *argv[])
{
   ...
   string FileNameSmear = "PPDataHighPTJet.root";
   if(IsMC == true)
   {
   ...
   TGraph *GSmearRMS = (TGraph *)FCentrality.Get("SmearPPData");
   if(IsMC == true)
   {
      GMBRMS = (TGraph *)FCentrality.Get("HydjetMB");
      GSmearRMS = (TGraph *)FCentrality.Get("SmearPP6Dijet");
   }
   ...
   for(int iE = 0; iE < SmearEntryCount; iE = iE + SmearJump)
   {
      ...
      //    NegativeWeight = 9;
      if(AdjustedExcessPT < 0)
         NegativeWeight = 9 * exp(-AdjustedExcessPT * AdjustedExcessPT / (2 * SmearRMS * SmearRMS))
            / exp(-AdjustedExcessPT * AdjustedExcessPT / (2 * SmearRMS * SmearRMS * 9 * 9));
      double AdjustedSmearWeight = exp(-AdjustedExcessPT * AdjustedExcessPT / (2 * TargetRMS * TargetRMS))
         / exp(-AdjustedExcessPT * AdjustedExcessPT / (2 * SmearRMS * SmearRMS)) * NegativeWeight;
   ...
   for(int iE = 0; iE < SmearEntryCount; iE = iE + SmearJump)
   {
      ...
      SmearCentralityBins[iPT][SmearCentrality] = SmearCentralityBins[iPT][SmearCentrality] + SmearMCWeight * AdjustedSmearWeight * GroomedWeight * JetWeight;
   }   
