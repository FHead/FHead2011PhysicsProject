...
void QuickDraw(double PTMin, double PTMax, double CentralityMin, double CentralityMax, bool UseSmearWeight)
{
   ...
   {
      Baseline2 = Form("(Centrality >= %f && Centrality < %f && BestJetPT >= %f && BestJetPT < %f && BestJetDR > 0.0 && BestJetDR < 1.0 && abs(BestJetEta) < 1.3 && BestJetZG > 0.00 && MatchedDR < 0.10 && JetPT > 0)", CentralityMin * 100, CentralityMax * 100, PTMin, PTMax);
      Baseline72 = Form("(Centrality >= %f && Centrality < %f && BestJetPT >= %f && BestJetPT < %f && BestJetDR2 > 0.00 && BestJetDR2 < 1.0 && abs(BestJetEta) < 1.3 && BestJetZG2 > 0.00 && MatchedDR < 0.10 && JetPT > 0)", CentralityMin * 100, CentralityMax * 100, PTMin, PTMax);
   }
   ...
   TFile F2("Output_0_0.root");
   // TFile F3("Output_HiForest_2.root");
   TFile F4("OutputIV.root");
   */
   
   TFile F1("Output_HiForestAOD_Merged.root");
   TFile F2("Output_100_All.root");
   // TFile F3("Output_HiForestAOD_Merged.root");
   TFile F4("OutputAA6100Priv.root");
   ...
   // T2->SetAlias("SmearWeight", "(exp(-ExcessPT2*ExcessPT2/(2*RMS2*RMS2/3/3*4))/exp(-ExcessPT2*ExcessPT2/(2*RMSA*RMSA)))");
   ...
   TH1D HA1("HA1", ";DR;a.u.", 30, 0, 0.5);   HA1.Sumw2();
   TH1D HB1("HB1", ";DR;a.u.", 30, 0, 0.5);   HB1.Sumw2();
   TH1D HC1("HC1", ";DR;a.u.", 30, 0, 0.5);   HC1.Sumw2();
   TH1D HD1("HD1", ";DR;a.u.", 30, 0, 0.5);   HD1.Sumw2();
