...
int main(int argc, char *argv[])
{
   ...
   string FileNameData = "AAData_15746.root";
   ...
   TGraph *GSmearRMS = (TGraph *)FCentrality.Get("SmearPPDataInJet");
   if(IsMC == true)
   {
   ...
   int DataEntryCount = TData->GetEntries() * 1.0;
   int SmearEntryCount = TSmear->GetEntries() * 1.0;
   ...
   for(int iE = 0; iE < DataEntryCount; iE++)
   {
      ...
      if(DataSDRecoDR > 0.0)
      {
         ...
         Data0TotalMassUp   [iC][iPT][iMassB] = Data0TotalMassUp   [iC][iPT][iMassB] + TotalWeight * pow(10, UpWeight);
         Data0TotalMassDown [iC][iPT][iMassB] = Data0TotalMassDown [iC][iPT][iMassB] + TotalWeight * pow(10, DownWeight);
   ...
   for(int iE = 0; iE < SmearEntryCount * 0.2; iE = iE + SmearJump)
   {
      ...
      double TotalWeight = SmearMCWeight * SmearWeight * JetWeight;
      ...
      if(SmearSDRecoDR > 0.0)
      {
         ...
         Smear0TotalMassUp   [iC][iPT][iMassB] = Smear0TotalMassUp   [iC][iPT][iMassB] + TotalWeight * pow(10, UpWeight);
         Smear0TotalMassDown [iC][iPT][iMassB] = Smear0TotalMassDown [iC][iPT][iMassB] + TotalWeight * pow(10, DownWeight);
