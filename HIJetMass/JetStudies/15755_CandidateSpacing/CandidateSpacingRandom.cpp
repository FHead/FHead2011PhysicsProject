...
int main(int argc, char *argv[])
{
   ...
   TH1D HSpacing("HSpacing", "", 1000, 0, 0.8);
   ...
   for(int iEntry = 0, ReuseCount = 1; iEntry < EntryCount; ReuseCount++)
   {
      ...
      for(int iJ = 0; iJ < MJet.JetCount; iJ++)
      {
         ...
         TH1D HSpacingCopy("HSpacingCopy", "", 1000, 0, 0.8);
         ...
         {
            if(GetDR(Candidates[i1].eta(), Candidates[i1].phi(), MJet.JetEta[iJ], MJet.JetPhi[iJ]) > 0.4)
               continue;

            for(int i2 = i1 + 1; i2 < Size; i2++)
            {
               if(GetDR(Candidates[i2].eta(), Candidates[i2].phi(), MJet.JetEta[iJ], MJet.JetPhi[iJ]) > 0.4)
                  continue;

               double DR = GetDR(Candidates[i1].eta(), Candidates[i1].phi(), Candidates[i2].eta(), Candidates[i2].phi());
               HSpacingCopy.Fill(DR);
            }
         }
