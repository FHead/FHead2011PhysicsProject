...
double GetPresample();
void MergeTree(Node *N, double MergeDR, bool DoMassLess);
void FillCandidates(vector<PseudoJet> &C, Node *N);
...
int main(int argc, char *argv[])
{
   ...
   if(argc != 8)
   {
      cerr << "Usage: " << argv[0] << " Input Output Tag PTHatMin PTHatMax MergeDR DoMassLess" << endl;
      return -1;
   }
   ...
   double MergeDR = atof(argv[6]);
   bool DoMassLess = ((argv[7][0] == 'Y') ? true : false);

   double RhoMultiplier = 1.00;
   double GhostDistance = 0.07;
   double Smear = 0.20;
   double MBMultiplier = 1.00;
   double Range = 1.20;
      ...
      for(int iJ = 0; iJ < MJet.JetCount; iJ++)
      {
         ...
         for(int i = 0; i < MPF.ID->size(); i++)
         {
            ...
            if(GetDR(P, JetP) < Range)
               CandidateNodes.push_back(new Node(P));
            if(GetDR(P, JetP) < 0.4)
            {
         ...
         // Step 2.5 - build CA tree and merging
         // cout << "Number of candidates = " << CandidateNodes.size() << endl;
         BuildCATree2(CandidateNodes);

         // if(First == true)
         // {
         //    PrintTree(CandidateNodes[0], 1);
         //    First = false;
         // }

         MergeTree(CandidateNodes[0], MergeDR, DoMassLess);

         vector<PseudoJet> Candidates;
         FillCandidates(Candidates, CandidateNodes[0]);
...
void MergeTree(Node *N, double MergeDR, bool DoMassLess)
{
   if(N == NULL)
      return;
   if(N->Child1 == NULL || N->Child2 == NULL)   // this is a leaf!  do nothing
      return;

   MergeTree(N->Child1, MergeDR, DoMassLess);
   MergeTree(N->Child2, MergeDR, DoMassLess);

   N->N = N->Child1->N + N->Child2->N + 1;

   if(N->N == 3 && GetDR(N->Child1->P, N->Child2->P) < MergeDR)   // merge only if the children are both leaves
   {
      N->P = N->Child1->P + N->Child2->P;
      delete N->Child1;
      delete N->Child2;
      N->N = 1;
...
void FillCandidates(vector<PseudoJet> &C, Node *N)
{
   ...
   if(N->N == 1)
   {
      C.push_back(PseudoJet(N->P[1], N->P[2], N->P[3], N->P[0]));
      return;
   }

   FillCandidates(C, N->Child1);
   FillCandidates(C, N->Child2);
}
...
void PrintTree(Node *N, int Indent)
{
   ...
   cout << Indentation << N->P.GetPT() << " " << N->P.GetEta() << " " << N->P.GetPhi() << " " << N->N << endl;
   if(N->N > 1)
   {
      cout << Indentation << GetDR(N->Child1->P, N->Child2->P) << endl;
   ...
   if(N->N > 1)
   {
      PrintTree(N->Child1, Indent + 1);
      PrintTree(N->Child2, Indent + 1);
   }
}
