...
int main(int argc, char *argv[])
{
   ...
   TGraph *GMBRMS = (TGraph *)FCentrality.Get(SmearGraphMB.c_str());
   TGraph *GSmearRMS = (TGraph *)FCentrality.Get(SmearGraphSmear.c_str());
   ...
   for(int iE = 0; iE < SmearEntryCount; iE++)
   {
      ...
      SmearPT[CBin][PTBin][JetBin] += MSmear.MCWeight * MSmear.SmearWeight();
   }
   ...
   for(int iE = 0; iE < SmearEntryCount; iE++)
   {
      ...
      SmearCentralityBins[PTBin][Centrality] += MSmear.MCWeight * MSmear.SmearWeight() * JetWeight;
   }   
   BarSmear.Update(SmearEntryCount);
   BarSmear.Print();
   BarSmear.PrintLine();
   ...
   for(int iE = 0; iE < DataEntryCount; iE++)
   {
      ...
      if(MData.SDRecoDR > 0.1 && MData.SDRecoDR < 0.15)
      {
   ...
   for(int iE = 0; iE < SmearEntryCount; iE++)
   {
      ...
      double TotalWeight = MSmear.MCWeight * MSmear.SmearWeight() * JetWeight * CentralityWeight;
