...
{
   cout << "oh yeah" << endl;
   cout << moduleDescription().moduleName() << ": " << moduleDescription().moduleLabel() << endl;
  ...
  for ( std::vector<fastjet::PseudoJet>::const_iterator ijet = tempJets.begin(), ijetEnd = tempJets.end(); ijet != ijetEnd; ++ijet ) {
     ...
     bool Verbose = false;
     if(ijet->eta() < 1.3 && ijet->eta() > -1.3 && ijet->perp() > 120)
        Verbose = true;
     // if(moduleDescription().moduleLabel() != "akCs4PFJets")
     //    Verbose = false;
     
     if(Verbose == true)
        std::cout << "Start processing jet (" << ijet->perp() << ", " << ijet->eta() << ", " << ijet->phi() << ")" << endl;
    ...
    if(Verbose == true)
    {
       cout << "Subtracted candidates:" << endl;
       for(int iP = 0; iP < (int)subtracted_particles.size(); iP++)
       {
          fastjet::PseudoJet &P = subtracted_particles[iP];
          cout << "   " << iP << " (" << P.perp() << " " << P.eta() << " " << P.phi() << ")" << endl;
       }
    }
  }//jet loop
