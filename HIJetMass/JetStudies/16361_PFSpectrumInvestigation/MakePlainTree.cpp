...
int main(int argc, char *argv[])
{
   ...
   for(int iE = 0; iE < EntryCount; iE++)
   {
      ...
      for(int iJ = 0; iJ < (int)Jets.size(); iJ++)
      {
         ...
         for(int iPF = 0; iPF < MPF.ID->size(); iPF++)
         {
            if((*MPF.Eta)[iPF] < -3 || (*MPF.Eta)[iPF] > 3)
               continue;

            if(GetDR2(Jets[iJ].eta(), Jets[iJ].phi(), (*MPF.Eta)[iPF], (*MPF.Phi)[iPF]) < 0.6 * 0.6)
            {
         ...
         {
            double DR = GetDR(Jets[iJ].eta(), Jets[iJ].phi(), MSDJet.JetEta[i], MSDJet.JetPhi[i]);
