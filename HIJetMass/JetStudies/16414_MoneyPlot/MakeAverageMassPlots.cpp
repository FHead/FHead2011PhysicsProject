...
int main(int argc, char *argv[])
{
   ...
   PdfFileHelper PdfFile("AverageMassSanityCheck_" + SD + "_" + Data + ".pdf");
   ...
   for(int i = 0; i < 16; i++)
   {
      ...
      SmearSys = DHFileSmear[States[i]]["All"].GetDouble();
   ...
   double Min = ((SD == "0") ? 0.08 : 0.05);
   double Max = ((SD == "0") ? 0.16 : 0.12);
   ...
   TH2D HWorld1("HWorld1", ";p_{T,jet} (GeV);<M_{g}/p_{T}>", 16, 0, 16, 100, Min, Max);
   HWorld1.SetStats(0);
   ...
   C.SaveAs(("Plots/AverageMassPlot_" + SD + "_" + Data + ".png").c_str());
   C.SaveAs(("Plots/AverageMassPlot_" + SD + "_" + Data + ".C").c_str());
   C.SaveAs(("Plots/AverageMassPlot_" + SD + "_" + Data + ".eps").c_str());
   C.SaveAs(("PDFPlots/AverageMassPlot_" + SD + "_" + Data + ".pdf").c_str());
