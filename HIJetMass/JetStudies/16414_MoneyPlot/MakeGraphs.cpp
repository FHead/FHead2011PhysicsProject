...
int main(int argc, char *argv[])
{
   ...
   // double Mass0BinEdge[MASS0BINCOUNT+1] =
   //    {0.00, 0.02, 0.04, 0.06, 0.08, 0.10, 0.12, 0.14, 0.16, 0.19, 0.20, 0.21};
   ...
   string FileNameData = "SampleBase/PlainTree/16354NoRhoMSubJetInfo/AAData.root";
   string FileNameSmear = "SampleBase/MBSmearTree/16354NoRhoMSubJetInfo/PPDataHighPTJet.root";
   ...
   if(IsMC == true)
   {
      ...
      if(PTHatMin <= 0)
      {
         FileNameData = "SampleBase/PlainTree/16354NoRhoMSubJetInfo/AA6DijetCymbal.root";
         FileNameSmear = "SampleBase/MBSmearTree/16354NoRhoMSubJetInfo//PP6Dijet.root";
         // FileNameSmear = "SampleBase/MBSmearTree/16354NoRhoMDR//PP6DijetHIReco.root";
         // FileNameSmear = "SampleBase/MBSmearTree/16354EventInfoReduced/AA6DijetCymbal.root";
         PTHatMin = 100;
         PTHatMax = 10000000;
      }
   }
   ...
   //////////////////////////////////
   // Optional - get angle weights //
   //////////////////////////////////

   vector<double> DataAngle[5][6];
   InitializeVectors(DataAngle, DRBINCOUNT);
   ...
   if(DoDRReweight == true)
   {
      ...
      for(int iE = 0; iE < DataEntryCount; iE++)
      {
         ...
         DataAngle[CBin][PTBin][AngleBin] += MData.MCWeight;
      }
      BarData.Update(DataEntryCount);
      BarData.Print();
      BarData.PrintLine();
   ...
   vector<double> SmearAngle[5][6];
   InitializeVectors(SmearAngle, DRBINCOUNT);
   ...
   if(DoDRReweight == true)
   {
      ...
      for(int iE = 0; iE < SmearEntryCount; iE++)
      {
         ...
         SmearAngle[CBin][PTBin][AngleBin] += MSmear.MCWeight * JetWeight * CentralityWeight;
      }
      BarSmear.Update(SmearEntryCount);
      BarSmear.Print();
      BarSmear.PrintLine();
   ...
   vector<double> Data0CountMass[5][6];      InitializeVectors(Data0CountMass, MASS0BINCOUNT);
   vector<double> Data0TotalMassX[5][6];     InitializeVectors(Data0TotalMassX, MASS0BINCOUNT);
   vector<double> Data0TotalMass[5][6];      InitializeVectors(Data0TotalMass, MASS0BINCOUNT);
   vector<double> Data0TotalMass2[5][6];     InitializeVectors(Data0TotalMass2, MASS0BINCOUNT);
   vector<double> Data0TotalMassUp[5][6];    InitializeVectors(Data0TotalMassUp, MASS0BINCOUNT);
   vector<double> Data0TotalMassDown[5][6];  InitializeVectors(Data0TotalMassDown, MASS0BINCOUNT);
   ...
   for(int iE = 0; iE < DataEntryCount; iE++)
   {
      ...
      int iMassB0 = GetBin(MData.SDMassRatio, Mass0BinEdge, MASS0BINCOUNT);
   ...
   vector<double> Smear0CountMass[5][6];      InitializeVectors(Smear0CountMass, MASS0BINCOUNT);
   vector<double> Smear0TotalMassX[5][6];     InitializeVectors(Smear0TotalMassX, MASS0BINCOUNT);
   vector<double> Smear0TotalMass[5][6];      InitializeVectors(Smear0TotalMass, MASS0BINCOUNT);
   vector<double> Smear0TotalMass2[5][6];     InitializeVectors(Smear0TotalMass2, MASS0BINCOUNT);
   vector<double> Smear0TotalMassUp[5][6];    InitializeVectors(Smear0TotalMassUp, MASS0BINCOUNT);
   vector<double> Smear0TotalMassDown[5][6];  InitializeVectors(Smear0TotalMassDown, MASS0BINCOUNT);
   ...
   for(int iE = 0; iE < SmearEntryCount; iE++)
   {
      ...
      int iMassB0 = GetBin(MSmear.SDMassRatio, Mass0BinEdge, MASS0BINCOUNT);
      ...
      // OutputMassFile << "Smear " << Centrality << " " << MSmear.JetPT
      //    << " " << MSmear.SDMassRatio << " " << MSmear.SDZG << " " << MSmear.SDRecoDR
      //    << " " << MSmear.PTPT << " " << TotalWeight << endl;
      ...
      if(MSmear.SDRecoDR > 0.1)
      {
         ...
         Smear0TotalMass     [CBin][PTBin][iMassB0] += TotalWeight;
         Smear0TotalMass2    [CBin][PTBin][iMassB0] += TotalWeight * TotalWeight;
         Smear0TotalMassUp   [CBin][PTBin][iMassB0] += TotalWeight * exp(UpWeight);
         Smear0TotalMassDown [CBin][PTBin][iMassB0] += TotalWeight * exp(DownWeight);
   ...
   for(int CBin = 0; CBin < 5; CBin++)
   {
      for(int PTBin = 0; PTBin < 6; PTBin++)
      {
         ...
         for(int i = 0; i < MASS0BINCOUNT; i++)
         {
            ...
            GMassSmear0.SetPointError(i, MassX - Mass0BinEdge[i], Mass0BinEdge[i+1] - MassX, MassRMS, MassRMS);
            GMassSmearSys0.SetPoint(i, MassX, Mass);
            GMassSmearSys0.SetPointError(i, MassX - Mass0BinEdge[i], Mass0BinEdge[i+1] - MassX, Mass - MassDown, MassUp - Mass);
         }
         for(int i = 0; i < ZGBINCOUNT; i++)
         {
            double Factor = 1 / Smear0GrandTotal[CBin][PTBin];
         ...
         GMassData0.SetMarkerColor(kBlack);
         GMassData0.SetLineColor(kBlue);
         GMassSmear0.SetMarkerColor(kRed);
         GMassSmear0.SetLineColor(kMagenta);
         GMassSmearSys0.SetMarkerColor(kRed);
         GMassSmearSys0.SetLineColor(kRed);
         GZGData0.SetMarkerColor(kBlack);
         GZGData0.SetLineColor(kBlue);
         GZGSmear0.SetMarkerColor(kRed);
         GZGSmear0.SetLineColor(kMagenta);
         GDRData0.SetMarkerColor(kBlack);
         GDRData0.SetLineColor(kBlue);
         GDRSmear0.SetMarkerColor(kRed);
         GDRSmear0.SetLineColor(kMagenta);
         GPTPTData0.SetMarkerColor(kBlack);
         GPTPTData0.SetLineColor(kBlue);
         GPTPTSmear0.SetMarkerColor(kRed);
         GPTPTSmear0.SetLineColor(kMagenta);
         ...
         HWorld.SetStats(0);
         HWorld.Draw();
         GMassDataSys0.Draw("p");
         GMassData0.Draw("p");
         GMassSmearSys0.Draw("p");
         GMassSmear0.Draw("p");
         Legend.Draw();
