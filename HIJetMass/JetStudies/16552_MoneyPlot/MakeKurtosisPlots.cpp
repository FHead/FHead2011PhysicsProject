...
int main(int argc, char *argv[])
{
   ...
   PdfFileHelper PdfFile("KurtosisMassSanityCheck_" + SD + "_" + Data + ".pdf");
   PdfFile.AddTextPage("Sanity checks for mass Kurtosis plots");
   ...
   for(int i = 0; i < 16; i++)
   {
      ...
      DataSys = DHFileData[States[i]]["AllWidth"].GetDouble() * 100;
      SmearAverage = GetAverage(File, SmearNames[i]);
      SmearError = GetError(File, SmearNames[i]);
      SmearSys = DHFileSmear[States[i]]["AllWidth"].GetDouble() * 100;
   ...
   double Min = ((SD == "0") ? 0.00 : 0.00);
   double Max = ((SD == "0") ? 5.00 : 10.00);
   ...
   TH2D HWorld1("HWorld1", ";p_{T,jet} (GeV);Kurtosis (M_{g}/p_{T})", 16, 0, 16, 100, Min, Max);
   HWorld1.SetStats(0);
   ...
   if(Data == "Data")
      Legend.AddEntry(&GDataSys, "PbPb", "p");
   else
      Legend.AddEntry(&GDataSys, "Embedded pp", "p");
   Legend.AddEntry(&GSmearSys, "Smeared pp", "p");
   Legend.Draw();

   C.cd(0)->SetTicks(0, 1);

   C.SaveAs(("Plots/KurtosisMassPlot_" + SD + "_" + Data + ".png").c_str());
   C.SaveAs(("Plots/KurtosisMassPlot_" + SD + "_" + Data + ".C").c_str());
   C.SaveAs(("Plots/KurtosisMassPlot_" + SD + "_" + Data + ".eps").c_str());
   C.SaveAs(("PDFPlots/KurtosisMassPlot_" + SD + "_" + Data + ".pdf").c_str());
