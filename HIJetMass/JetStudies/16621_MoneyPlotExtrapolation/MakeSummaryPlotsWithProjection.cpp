...
#define MASSBINCOUNT 34
#define MASS0BINCOUNT 37
   ...
   vector<TheoryRatio> Extra1, vector<TheoryRatio> Extra2, vector<TheoryRatio> Extra3, vector<TheoryRatio> Extra4)
{
   ...
   if(IsMC == false)
      Latex.DrawLatex((BorderWidth + PadWidth * 4) / TotalWidth, (BorderHeight + RatioHeight + PadHeight + 0.000) / TotalHeight, "PbPb 10 nb^{-1} (5.02 TeV), pp 27.4 pb^{-1} (5.02 TeV)");
   else
   ...
   if(Type == TYPE_MASS || Type == TYPE_MASS0)   // let's do lumi extrapolation
   {
      G1Ratio.Print();
      G2Ratio.Print();
      G3Ratio.Print();
      ...
      TF1 F1Ratio("F1Ratio", "[0]+expo(1)");
      TF1 F2Ratio("F2Ratio", "[0]+expo(1)");
      TF1 F3Ratio("F3Ratio", "[0]+expo(1)");
      TF1 F4Ratio("F4Ratio", "[0]+expo(1)");

      F1Ratio.SetParameters(0.95, -20, 0.01);
      F2Ratio.SetParameters(0.95, -20, 0.01);
      F3Ratio.SetParameters(0.95, -20, 0.01);
      F4Ratio.SetParameters(0.95, -20, 0.01);

      F1Ratio.SetParLimits(0, 0.70, 1.00);
      F2Ratio.SetParLimits(0, 0.70, 1.00);
      F3Ratio.SetParLimits(0, 0.70, 1.00);
      F4Ratio.SetParLimits(0, 0.70, 1.00);
      F1Ratio.SetParLimits(1, -1000, -1000);
      F2Ratio.SetParLimits(1, -1000, -1000);
      F3Ratio.SetParLimits(1, -1000, -1000);
      F4Ratio.SetParLimits(1, -1000, -1000);
      F1Ratio.SetParLimits(2, 1.00, 1.00);
      F2Ratio.SetParLimits(2, 1.00, 1.00);
      F3Ratio.SetParLimits(2, 1.00, 1.00);
      F4Ratio.SetParLimits(2, 1.00, 1.00);

      G1Ratio.Fit(&F1Ratio, "N", "", 0.00, 0.15);
      G2Ratio.Fit(&F2Ratio, "N", "", 0.00, 0.15);
      G3Ratio.Fit(&F3Ratio, "N", "", 0.00, 0.15);
      G4Ratio.Fit(&F4Ratio, "N", "", 0.00, 0.15);

      F1Ratio.SetParLimits(0, F1Ratio.GetParameter(0), F1Ratio.GetParameter(0));
      F2Ratio.SetParLimits(0, F1Ratio.GetParameter(0), F1Ratio.GetParameter(0));
      F3Ratio.SetParLimits(0, F1Ratio.GetParameter(0), F1Ratio.GetParameter(0));
      F4Ratio.SetParLimits(0, F1Ratio.GetParameter(0), F1Ratio.GetParameter(0));
      F1Ratio.SetParLimits(1, -100.00, 100.00);
      F2Ratio.SetParLimits(1, -100.00, 100.00);
      F3Ratio.SetParLimits(1, -100.00, 100.00);
      F4Ratio.SetParLimits(1, -100.00, 100.00);
      F1Ratio.SetParLimits(2, 0.00, 10000.00);
      F2Ratio.SetParLimits(2, 0.00, 10000.00);
      F3Ratio.SetParLimits(2, 0.00, 10000.00);
      F4Ratio.SetParLimits(2, 0.00, 10000.00);

      G1Ratio.Fit(&F1Ratio, "N", "", 0.00, 0.27);
      G2Ratio.Fit(&F2Ratio, "N", "", 0.00, 0.27);
      G3Ratio.Fit(&F3Ratio, "N", "", 0.00, 0.27);
      G4Ratio.Fit(&F4Ratio, "N", "", 0.00, 0.27);

      double StatFactor = 5;
      double SysFactor = 2;

      for(int i = 0; i < G1Data->GetN(); i++)
      {
         double x, y, ex1, ex2, ey1, ey2, newy;
         ex1 = G1Data->GetErrorXlow(i);
         ex2 = G1Data->GetErrorXhigh(i);
         ey1 = G1Data->GetErrorYlow(i);
         ey2 = G1Data->GetErrorYhigh(i);
         G1Smear->GetPoint(i, x, y);
         newy = y * F1Ratio.Eval(x) + DrawGaussian(0, (ey1 + ey2) / StatFactor);
         G1Data->SetPoint(i, x, newy);
         G1Data->SetPointError(i, ex1, ex2, ey1 / StatFactor, ey2 / StatFactor);
         ey1 = G1DataSys->GetErrorYlow(i);
         ey2 = G1DataSys->GetErrorYhigh(i);
         G1DataSys->SetPoint(i, x, newy);
         G1DataSys->SetPointError(i, ex1, ex2, ey1 / SysFactor, ey2 / SysFactor);
         ey1 = G1SmearSys->GetErrorYlow(i);
         ey2 = G1SmearSys->GetErrorYhigh(i);
         G1SmearSys->SetPointError(i, ex1, ex2, ey1 / SysFactor, ey2 / SysFactor);
         
         ex1 = G2Data->GetErrorXlow(i);
         ex2 = G2Data->GetErrorXhigh(i);
         ey1 = G2Data->GetErrorYlow(i);
         ey2 = G2Data->GetErrorYhigh(i);
         G2Smear->GetPoint(i, x, y);
         newy = y * F2Ratio.Eval(x) + DrawGaussian(0, (ey1 + ey2) / StatFactor);
         G2Data->SetPoint(i, x, newy);
         G2Data->SetPointError(i, ex1, ex2, ey1 / StatFactor, ey2 / StatFactor);
         ey1 = G2DataSys->GetErrorYlow(i);
         ey2 = G2DataSys->GetErrorYhigh(i);
         G2DataSys->SetPoint(i, x, newy);
         G2DataSys->SetPointError(i, ex1, ex2, ey1 / SysFactor, ey2 / SysFactor);
         ey1 = G2SmearSys->GetErrorYlow(i);
         ey2 = G2SmearSys->GetErrorYhigh(i);
         G2SmearSys->SetPointError(i, ex1, ex2, ey1 / SysFactor, ey2 / SysFactor);
         
         ex1 = G3Data->GetErrorXlow(i);
         ex2 = G3Data->GetErrorXhigh(i);
         ey1 = G3Data->GetErrorYlow(i);
         ey2 = G3Data->GetErrorYhigh(i);
         G3Smear->GetPoint(i, x, y);
         newy = y * F3Ratio.Eval(x) + DrawGaussian(0, (ey1 + ey2) / StatFactor);
         G3Data->SetPoint(i, x, newy);
         G3Data->SetPointError(i, ex1, ex2, ey1 / StatFactor, ey2 / StatFactor);
         ey1 = G3DataSys->GetErrorYlow(i);
         ey2 = G3DataSys->GetErrorYhigh(i);
         G3DataSys->SetPoint(i, x, newy);
         G3DataSys->SetPointError(i, ex1, ex2, ey1 / SysFactor, ey2 / SysFactor);
         ey1 = G3SmearSys->GetErrorYlow(i);
         ey2 = G3SmearSys->GetErrorYhigh(i);
         G3SmearSys->SetPointError(i, ex1, ex2, ey1 / SysFactor, ey2 / SysFactor);
         
         ex1 = G4Data->GetErrorXlow(i);
         ex2 = G4Data->GetErrorXhigh(i);
         ey1 = G4Data->GetErrorYlow(i);
         ey2 = G4Data->GetErrorYhigh(i);
         G4Smear->GetPoint(i, x, y);
         newy = y * F4Ratio.Eval(x) + DrawGaussian(0, (ey1 + ey2) / StatFactor);
         G4Data->SetPoint(i, x, newy);
         G4Data->SetPointError(i, ex1, ex2, ey1 / StatFactor, ey2 / StatFactor);
         ey1 = G4DataSys->GetErrorYlow(i);
         ey2 = G4DataSys->GetErrorYhigh(i);
         G4DataSys->SetPoint(i, x, newy);
         G4DataSys->SetPointError(i, ex1, ex2, ey1 / SysFactor, ey2 / SysFactor);
         ey1 = G4SmearSys->GetErrorYlow(i);
         ey2 = G4SmearSys->GetErrorYhigh(i);
         G4SmearSys->SetPointError(i, ex1, ex2, ey1 / SysFactor, ey2 / SysFactor);
      }
      ...
      for(int i = 0; i < G1RatioSys.GetN(); i++)
      {
         double ey = G1RatioSys.GetErrorYhigh(i);
         if(ey < 1e-3 || ey != ey)
         {
            G1RatioSys.RemovePoint(i);
            G1Ratio.RemovePoint(i);
            i = i - 1;
         }
      }
      for(int i = 0; i < G2RatioSys.GetN(); i++)
      {
         double ey = G2RatioSys.GetErrorYhigh(i);
         if(ey < 1e-3 || ey != ey)
         {
            G2RatioSys.RemovePoint(i);
            G2Ratio.RemovePoint(i);
            i = i - 1;
         }
      }
      for(int i = 0; i < G3RatioSys.GetN(); i++)
      {
         double ey = G3RatioSys.GetErrorYhigh(i);
         if(ey < 1e-3 || ey != ey)
         {
            G3RatioSys.RemovePoint(i);
            G3Ratio.RemovePoint(i);
            i = i - 1;
         }
      }
      for(int i = 0; i < G4RatioSys.GetN(); i++)
      {
         double ey = G4RatioSys.GetErrorYhigh(i);
         if(ey < 1e-3 || ey != ey)
         {
            G4RatioSys.RemovePoint(i);
            G4Ratio.RemovePoint(i);
            i = i - 1;
         }
      }

      Canvas.SaveAs(Form("%s_Extrapolated.png", OutputBase.c_str()));
      Canvas.SaveAs(Form("%s_Extrapolated.C"  , OutputBase.c_str()));
      Canvas.SaveAs(Form("%s_Extrapolated.eps", OutputBase.c_str()));
      Canvas.SaveAs(Form("%s_Extrapolated.pdf", OutputBase.c_str()));
   }
}
