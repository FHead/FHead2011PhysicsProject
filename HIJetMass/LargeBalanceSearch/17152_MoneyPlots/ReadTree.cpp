...
void StraightTreeMessenger::GetEntry(int iE)
{
   ...
   if(JES != NULL)
   {
      CorrectedJetPT = JES->Apply(MatchJetPT, MatchJetEta) * JESFactor;
      if(JERFactor > 0)
         CorrectedJetPT = CorrectedJetPT * DrawGaussian(1, JERFactor);
   }
   else
...
void StraightTreeMessenger::SetJES(JetEnergyScale *jes, double jesfactor, double jerfactor)
{
   JES = jes;
   JESFactor = jesfactor;
   JERFactor = jerfactor;
...
void SmearTreeMessenger::GetEntry(int iE)
{
   ...
   if(JES != NULL)
   {
      JetPT = JES->Apply(JetRawPT, JetEta) * JESFactor;
      if(JERFactor > 0)
         JetPT = JetPT * DrawGaussian(1, JERFactor);
   }
   else
      JetPT = JetRawPT;
...
void SmearTreeMessenger::SetJES(JetEnergyScale *jes, double jesfactor, double jerfactor)
{
   JES = jes;
   JESFactor = jesfactor;
   JERFactor = jerfactor;
