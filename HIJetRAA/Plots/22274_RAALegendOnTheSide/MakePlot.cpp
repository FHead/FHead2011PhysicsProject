...
int main(int argc, char *argv[])
{
   ...
   // Preamble: sizing
   int PanelSize = 500;
   int LegendPanelSize = 300;
   int PaddingWidthL = 100;
   int PaddingWidthR = 50;
   int PaddingHeight = 100;
   int CanvasWidth = PanelSize * 3 + LegendPanelSize + PaddingWidthL + PaddingWidthR;
   ...
   double PadLX = (double)LegendPanelSize / CanvasWidth;
   ...
   HWorld.SetStats(0);
   TH2D HWorldNoTick("HWorldNoTick", ";;", 100, 0, 1, 100, 0, 1);
   HWorldNoTick.SetStats(0);
   HWorldNoTick.GetXaxis()->SetTickLength(0);
   HWorldNoTick.GetYaxis()->SetTickLength(0);
   ...
   TPad P7("P7", "", PadX0 + PadDX * 3, PadY0 + PadDY * 0, PadX0 + PadDX * 3 + PadLX, PadY0 + PadDY * 2, 0);
   ...
   TLegend LegendAll(0.07, 0.05, 0.7, 0.05 + 0.05 * 13);
   LegendAll.SetTextFont(42);
   LegendAll.SetTextSize(0.14);
   LegendAll.SetFillStyle(0);
   LegendAll.SetBorderSize(0);
   LegendAll.AddEntry("", "Data", "");
   LegendAll.AddEntry(&HR02C00RAASys, "0-10%", "plf");
   LegendAll.AddEntry(&HR02C01RAASys, "10-30%", "plf");
   LegendAll.AddEntry(&HR02C02RAASys, "30-50%", "plf");
   LegendAll.AddEntry(&HR02C03RAASys, "50-90%", "plf");
   LegendAll.AddEntry("", "", "");
   LegendAll.AddEntry(&HLumi, "Lumi", "f");
   LegendAll.AddEntry("", "", "");
   LegendAll.AddEntry("", "T_{AA}", "");
   LegendAll.AddEntry(&HR02C00RAA, "0-10%", "f");
   LegendAll.AddEntry(&HR02C01RAA, "10-30%", "f");
   LegendAll.AddEntry(&HR02C02RAA, "30-50%", "f");
   LegendAll.AddEntry(&HR02C03RAA, "50-90%", "f");
   
   TLegend LegendTAll(0.07, 0.05, 0.7, 0.05 + 0.05 * 7);
   LegendTAll.SetTextFont(42);
   LegendTAll.SetTextSize(0.14);
   LegendTAll.SetFillStyle(0);
   LegendTAll.SetBorderSize(0);
   LegendTAll.AddEntry(&HR02C00RAASys, "Data 0-10%", "plf");
   LegendTAll.AddEntry(&HR02C00RAA, "T_{AA}", "f");
   LegendTAll.AddEntry(&HLumi, "Lumi", "f");
   LegendTAll.AddEntry("", "", "");
   LegendTAll.AddEntry(&Felix[0], "Factorization", "f");
   LegendTAll.AddEntry(&Daniel[0], "Hybrid", "f");
   LegendTAll.AddEntry(&Korinna[0], "Jewel", "f");
   ...
   P7.cd();
   HWorldNoTick.Draw("axis");
   Latex.SetTextFont(42);
   Latex.SetTextSize(0.14);
   Latex.SetTextAlign(11);
   Latex.SetTextAngle(0);
   Latex.DrawLatex(0.075, 0.925, "anti-k_{T}");
   Latex.DrawLatex(0.075, 0.875, "|#eta_{jet}| < 2.0");
   if(DoTheory == false)   LegendAll.Draw();
   if(DoTheory == true)    LegendTAll.Draw();
