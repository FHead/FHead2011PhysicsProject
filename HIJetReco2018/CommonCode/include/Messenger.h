...
public:
   ...
   std::vector<std::vector<int>> *DaughterIndex;
...
public:
   TTree *Tree;
   int TrackCount;
   float TrackPT[TRACKCOUNTMAX];
   float TrackEta[TRACKCOUNTMAX];
   float TrackPhi[TRACKCOUNTMAX];
   bool HighPurity[TRACKCOUNTMAX];
   float TrackMVA[TRACKCOUNTMAX];
   unsigned char TrackAlgo[TRACKCOUNTMAX];
public:
