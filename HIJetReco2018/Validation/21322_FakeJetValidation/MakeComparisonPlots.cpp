...
void AddTurnOnPlot(PdfFileHelper &PdfFile, TFile &F1, TFile &F2, string H1Name, string H2Name,
...
int main(int argc, char *argv[])
{
   ...
   Add1DPlot(PdfFile, F1, F2, "HJetPT", "Inclusive jet PT;p_{T};", 0, 200, 1e-7, 1, true);
   ...
   Add1DPlot(PdfFile, F1, F2, "HJetPTEtaN25N15", "Jet PT, -2.5 < #eta < -1.5;p_{T};", 0, 200, 1e-6, 1, true);
   Add1DPlot(PdfFile, F1, F2, "HJetPTEtaN15N00", "Jet PT, -1.5 < #eta < 0.0;p_{T};", 0, 200, 1e-6, 1, true);
   Add1DPlot(PdfFile, F1, F2, "HJetPTEtaP00P15", "Jet PT, 0.0 < #eta < 1.5;p_{T};", 0, 200, 1e-6, 1, true);
   Add1DPlot(PdfFile, F1, F2, "HJetPTEtaP15P25", "Jet PT, 1.5 < #eta < 2.5;p_{T};", 0, 200, 1e-6, 1, true);
   ...
   AddTurnOnPlot(PdfFile, F1, F2, "HLeadingJetPTPassL156", "HLeadingJetPT",
      ";p_{T};Efficiency", 0, 200, 0, 1.1);
   AddTurnOnPlot(PdfFile, F1, F2, "HLeadingJetPTEtaN25P25PassL156", "HLeadingJetPTEtaN25P25",
      "-2.5 < #eta < 2.5;p_{T};Efficiency", 0, 200, 0, 1.1);
   AddTurnOnPlot(PdfFile, F1, F2, "HLeadingJetPTEtaN25N15PassL156", "HLeadingJetPTEtaN25N15",
      "-2.5 < #eta < -1.5;p_{T};Efficiency", 0, 200, 0, 1.1);
   AddTurnOnPlot(PdfFile, F1, F2, "HLeadingJetPTEtaN15N00PassL156", "HLeadingJetPTEtaN15N00",
      "-1.5 < #eta < 0.0;p_{T};Efficiency", 0, 200, 0, 1.1);
   AddTurnOnPlot(PdfFile, F1, F2, "HLeadingJetPTEtaP00P15PassL156", "HLeadingJetPTEtaP00P15",
      "0.0 < #eta < 1.5;p_{T};Efficiency", 0, 200, 0, 1.1);
   AddTurnOnPlot(PdfFile, F1, F2, "HLeadingJetPTEtaP15P25PassL156", "HLeadingJetPTEtaP15P25",
      "1.5 < #eta < 2.5;p_{T};Efficiency", 0, 200, 0, 1.1);
...
void AddTurnOnPlot(PdfFileHelper &PdfFile, TFile &F1, TFile &F2, string H1Name, string H2Name,
   string Title, double XMin, double XMax, double YMin, double YMax)
{
   TH1D *H11 = (TH1D *)F1.Get(H1Name.c_str());
   TH1D *H12 = (TH1D *)F1.Get(H2Name.c_str());
   TH1D *H21 = (TH1D *)F2.Get(H1Name.c_str());
   TH1D *H22 = (TH1D *)F2.Get(H2Name.c_str());

   if(H11 == nullptr || H21 == nullptr)
   {
      PdfFile.AddTextPage("Histogram " + H1Name + " not found!");
      return;
   }
   if(H12 == nullptr || H22 == nullptr)
   {
      PdfFile.AddTextPage("Histogram " + H2Name + " not found!");
      return;
   }
