...
int main(int argc, char *argv[])
{
   ...
   string OutputFileName = "Meow.root";
   ...
   for(int i = 0; i < 92; i++)
   {
      HM2emLoose.push_back(new TH1D(Form("HM2%demLoose", i), Form("M2 (2e2mu), Loose, %d", i), 100, 0, 65));
      HM2eeLoose.push_back(new TH1D(Form("HM2%deeLoose", i), Form("M2 (4e), Loose, %d", i), 100, 0, 65));
      HM2emTight.push_back(new TH1D(Form("HM2%demTight", i), Form("M2 (2e2mu), Tight, %d", i), 100, 0, 65));
      HM2eeTight.push_back(new TH1D(Form("HM2%deeTight", i), Form("M2 (4e), Tight, %d", i), 100, 0, 65));
      HM2emOpt.push_back(new TH1D(Form("HM2%demOpt", i), Form("M2 (2e2mu), Opt, %d", i), 100, 0, 65));
      HM2eeOpt.push_back(new TH1D(Form("HM2%deeOpt", i), Form("M2 (4e), Opt, %d", i), 100, 0, 65));
      HPhiemLoose.push_back(new TH1D(Form("HPhi%demLoose", i), Form("Phi (2e2mu), Loose, %d", i), 100, -3.14159, 3.14159));
      HPhieeLoose.push_back(new TH1D(Form("HPhi%deeLoose", i), Form("Phi (4e), Loose, %d", i), 100, -3.14159, 3.14159));
      HPhiemTight.push_back(new TH1D(Form("HPhi%demTight", i), Form("Phi (2e2mu), Tight, %d", i), 100, -3.14159, 3.14159));
      HPhieeTight.push_back(new TH1D(Form("HPhi%deeTight", i), Form("Phi (4e), Tight, %d", i), 100, -3.14159, 3.14159));
      HPhiemOpt.push_back(new TH1D(Form("HPhi%demOpt", i), Form("Phi (2e2mu), Opt, %d", i), 100, -3.14159, 3.14159));
      HPhieeOpt.push_back(new TH1D(Form("HPhi%deeOpt", i), Form("Phi (4e), Opt, %d", i), 100, -3.14159, 3.14159));
   }
   ...
   for(int iE = 0; iE < EntryCount; iE++)
   {
      ...
      if(Event4e.Phi0 > PI)   Event4e.Phi = Event4e.Phi - 2 * PI;
      if(Event4e.Phi > PI)    Event4e.Phi0 = Event4e.Phi0 - 2 * PI;
      ...
      bool PassTight2e2mu = TightCut2e2mu(Lepton2e2mu);
      bool PassOpt2e2mu = OptCut2e2mu(Lepton2e2mu);
      bool PassLoose4e = LooseCut4e(Lepton4e);
      bool PassTight4e = TightCut4e(Lepton4e);
      bool PassOpt4e = OptCut4e(Lepton4e);
      
      for(int i = 0; i < 92; i++)
      {
         if(PassLoose2e2mu == true)
            HM2emLoose[i]->Fill(Event2e2mu.Z2Mass, Weight[i][0] * Weight2e2mu);
         if(PassTight2e2mu == true)
            HM2emTight[i]->Fill(Event2e2mu.Z2Mass, Weight[i][0] * Weight2e2mu);
         if(PassOpt2e2mu == true)
            HM2emOpt[i]->Fill(Event2e2mu.Z2Mass, Weight[i][0] * Weight2e2mu);
         if(PassLoose4e == true)
            HM2eeLoose[i]->Fill(Event4e.Z2Mass, Weight[i][1] * Weight4e);
         if(PassTight4e == true)
            HM2eeTight[i]->Fill(Event4e.Z2Mass, Weight[i][1] * Weight4e);
         if(PassOpt4e == true)
            HM2eeOpt[i]->Fill(Event4e.Z2Mass, Weight[i][1] * Weight4e);
         if(PassLoose2e2mu == true)
            HPhiemLoose[i]->Fill(Event2e2mu.Phi, Weight[i][0] * Weight2e2mu);
         if(PassTight2e2mu == true)
            HPhiemTight[i]->Fill(Event2e2mu.Phi, Weight[i][0] * Weight2e2mu);
         if(PassOpt2e2mu == true)
            HPhiemOpt[i]->Fill(Event2e2mu.Phi, Weight[i][0] * Weight2e2mu);
         if(PassLoose4e == true)
            HPhieeLoose[i]->Fill(Event4e.Phi, Weight[i][1] * Weight4e);
         if(PassTight4e == true)
            HPhieeTight[i]->Fill(Event4e.Phi, Weight[i][1] * Weight4e);
         if(PassOpt4e == true)
            HPhieeOpt[i]->Fill(Event4e.Phi, Weight[i][1] * Weight4e);
      }
   }
   ...
   */
   ...
   for(int i = 0; i < 92; i++)
   {
      if(HM2emLoose[i] != NULL)    HM2emLoose[i]->Write();
      if(HM2eeLoose[i] != NULL)    HM2eeLoose[i]->Write();
      if(HM2emTight[i] != NULL)    HM2emTight[i]->Write();
      if(HM2eeTight[i] != NULL)    HM2eeTight[i]->Write();
      if(HM2emOpt[i] != NULL)      HM2emOpt[i]->Write();
      if(HM2eeOpt[i] != NULL)      HM2eeOpt[i]->Write();
      if(HPhiemLoose[i] != NULL)   HPhiemLoose[i]->Write();
      if(HPhieeLoose[i] != NULL)   HPhieeLoose[i]->Write();
      if(HPhiemTight[i] != NULL)   HPhiemTight[i]->Write();
      if(HPhieeTight[i] != NULL)   HPhieeTight[i]->Write();
      if(HPhiemOpt[i] != NULL)     HPhiemOpt[i]->Write();
      if(HPhieeOpt[i] != NULL)     HPhieeOpt[i]->Write();
   }
