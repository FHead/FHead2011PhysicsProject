...
{
   std::cerr << "[HybridCalculator] Warning!  Absolute value calcuated in this modified version!" << std::endl;
...
std::vector<double> HybridCalculator::PassThroughMode(const EventParameters &TargetParameters)
{
   ...
   {
      Result[i] = fabs((*CalculateMEValue[i])(Leptons, ZP));
      if(IncludeSwapping == true)
         Result[i] = Result[i] + fabs((*CalculateMEValue[i])(SwappedLeptons, ZP));
