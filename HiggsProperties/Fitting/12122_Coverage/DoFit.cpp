...
int main(int argc, char *argv[])
{
   ...
   for(int iE = 0; iE < EntryCount; iE++)
   {
      ...
      BestParameters[0] = ResultTemp.A3A1;
      BestParameters[1] = ResultTemp.A3Phase;
      BestParameters[2] = ResultTemp.A2A1;
      BestParameters[3] = ResultTemp.A2Phase;
      BestParameters[4] = ResultTemp.F;
      BestParameters[5] = ResultTemp.N1;
      BestParameters[6] = ResultTemp.N2;
      
      double TrueParameters[7] = {0};
      TrueParameters[0] = sqrt(A3R * A3R + A3I * A3I) / A1R;
      TrueParameters[1] = atan(A3I / A3R);
      TrueParameters[2] = sqrt(A2R * A2R + A2I * A2I) / A1R;
      TrueParameters[3] = atan(A2I / A2R);
      TrueParameters[4] = 0;
      TrueParameters[5] = 0;
      TrueParameters[6] = 0;

      if(A3R < 0)  TrueParameters[1] = TrueParameters[1] + PI;
      if(A2R < 0)  TrueParameters[3] = TrueParameters[3] + PI;
      if(A3R < 1e-10)   TrueParameters[1] = PI;
      if(A2R < 1e-10)   TrueParameters[3] = PI;
   ...
   TNtuple OutputTree("ResultTree", "", "A2A1:A2Phase:A3A1:A3Phase:N1:N2:F:A2A1Error:A2PhaseError:A3A1Error:A3PhaseError:N1Error:N2Error:FError:EDM:Status:LLBest:LLTrue:LLGuu");
...
FitResultSummary FitClass::DoFit()
{
   ...
   Worker->SetLimitedVariable(0, "A3A1", 0, 0.5, -1000, 1000);
   Worker->SetLimitedVariable(1, "A3Phase", DrawRandom(-PI, PI), 0.1, -PI * 10, PI * 10);
   Worker->SetLimitedVariable(2, "A2A1", 0, 0.1, -1000, 1000);
