...
{
public:
   double ReA2A1, ReA2A1Error;
   double ImA2A1, ImA2A1Error;
   double ReA3A1, ReA3A1Error;
   double ImA3A1, ImA3A1Error;
   double F, FError;
   double N1, N1Error;
   double N2, N2Error;
   double EDM;
   int FitStatus;
   double LLBest, LLTrue, LLGuu;
};
...
int main(int argc, char *argv[])
{
   ...
   int DatasetSize = 30;
   double A1R, A2R, A2I, A3R, A3I;

   if(argc <= 7)
   {
      cerr << "Usage: " << argv[0] << " RootFile DatasetSize A1R A2R A2I A3R A3I" << endl;
      return -1;
   }
   ...
   for(int iE = 0; iE < EntryCount; iE++)
   {
      ...
      double BestParameters[7] = {0};
      BestParameters[0] = ResultTemp.ReA3A1;
      BestParameters[1] = ResultTemp.ImA3A1;
      BestParameters[2] = ResultTemp.ReA2A1;
      BestParameters[3] = ResultTemp.ImA2A1;
      BestParameters[4] = ResultTemp.F;
      BestParameters[5] = ResultTemp.N1;
      BestParameters[6] = ResultTemp.N2;
      
      double TrueParameters[7] = {0};
      TrueParameters[0] = A3R / A1R;
      TrueParameters[1] = A3I / A1R;
      TrueParameters[2] = A2R / A1R;
      TrueParameters[3] = A2I / A1R;
      TrueParameters[4] = 0;
      TrueParameters[5] = 0;
      TrueParameters[6] = 0;

      double GuuParameters[7] = {0};
      GuuParameters[0] = TrueParameters[0];
      GuuParameters[1] = TrueParameters[1];
      GuuParameters[2] = TrueParameters[2];
      GuuParameters[3] = TrueParameters[3];
      GuuParameters[4] = ResultTemp.F;
      GuuParameters[5] = ResultTemp.N1;
      GuuParameters[6] = ResultTemp.N2;

      double LL1 = Fits.CalculateLogLikelihoodAll(BestParameters);
      double LL2 = Fits.CalculateLogLikelihoodAll(TrueParameters);
      double LL3 = Fits.CalculateLogLikelihoodAll(GuuParameters);

      ResultTemp.LLBest = LL1;
      ResultTemp.LLTrue = LL2;
      ResultTemp.LLGuu = LL3;
   ...
   TNtuple OutputTree("ResultTree", "", "ReA2A1:ImA2A1:ReA3A1:ImA3A1:N1:N2:F:ReA2A1Error:ImA2A1Error:ReA3A1Error:ImA3A1Error:N1Error:N2Error:FError:EDM:Status:LLBest:LLTrue:LLGuu");

   for(int i = 0; i < (int)Results.size(); i++)
   {
      float Array[100];
      Array[0] = Results[i].ReA2A1;    Array[7] = Results[i].ReA2A1Error;
      Array[1] = Results[i].ImA2A1;    Array[8] = Results[i].ImA2A1Error;
      Array[2] = Results[i].ReA3A1;    Array[9] = Results[i].ReA3A1Error;
      Array[3] = Results[i].ImA3A1;    Array[10] = Results[i].ImA3A1Error;
      ...
      Array[16] = Results[i].LLBest;
      Array[17] = Results[i].LLTrue;
      Array[18] = Results[i].LLGuu;
...
{
   double A1R = 1;
   double A3R = Parameters[0];
   double A3I = Parameters[1];
   double A2R = Parameters[2];
   double A2I = Parameters[3];
...
FitResultSummary FitClass::DoFit()
{
   ...
   Worker->SetMaxFunctionCalls(1e10);
   Worker->SetMaxIterations(1e10);
   Worker->SetTolerance(1e-9);

   ROOT::Math::Functor Function(this, &FitClass::CalculateLogLikelihoodAll, 7);
   Worker->SetFunction(Function);

   Worker->SetLimitedVariable(0, "ReA3A1", 0, 0.5, -10000, 10000);
   Worker->SetLimitedVariable(1, "ImA3A1", 0, 0.5, -10000, 10000);
   Worker->SetLimitedVariable(2, "ReA2A1", 0, 0.1, -10000, 10000);
   Worker->SetLimitedVariable(3, "ImA2A1", 0, 0.1, -10000, 10000);
   Worker->SetLimitedVariable(4, "F", 0, 0, 0, 1);
   Worker->SetLimitedVariable(5, "N1", 0, 0.1, -5, 5);
   Worker->SetLimitedVariable(6, "N2", 0, 0.1, -5, 5);
   ...
   Result.ReA3A1 = Parameters[0];
   Result.ReA3A1Error = Errors[0];
   Result.ImA3A1 = Parameters[1];
   Result.ImA3A1Error = Errors[1];
   Result.ReA2A1 = Parameters[2];
   Result.ReA2A1Error = Errors[2];
   Result.ImA2A1 = Parameters[3];
   Result.ImA2A1Error = Errors[3];
...
void FitClass::PlotResult(PsFileHelper &PsFile, FitResultSummary Result)
{
   ...
   Parameters.aZR = Result.ReA2A1;      Parameters.aZI = Result.ImA2A1;
   Parameters.aZdR = Result.ReA3A1;     Parameters.aZdI = Result.ImA3A1;
