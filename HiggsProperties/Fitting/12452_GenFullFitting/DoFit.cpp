...
int main(int argc, char *argv[])
{
   ...
   for(int i = 1; i <= 63; i++)
   {
      ...
      Configurations.push_back(FitConfiguration(As, Phases, Fs, "NN",
         ...
         ListToVector(2, 0, 0)));
      // Configurations.push_back(FitConfiguration(As, Phases, Fs, "YN",
      //    AValues,
      //    ListToVector(6, 0, 0, 0, 0, 0, 0),
      //    ListToVector(4, ((Fs[0] == 'Y') ? 0.5 : 0), 0, 0, 0),
      //    ListToVector(2, 0, 0)));
      // Configurations.push_back(FitConfiguration(As, Phases, Fs, "YY",
      //    AValues,
      //    ListToVector(6, 0, 0, 0, 0, 0, 0),
      //    ListToVector(4, ((Fs[0] == 'Y') ? 0.5 : 0), 0, 0, 0),
      //    ListToVector(2, 0, 0)));
   ...
   for(int iS = 0; iS < (int)Signals.size(); iS++)
   {
      ...
      if(Signals[iS].state == "LooseCutEM")   Fits.SignalStateCount[0] = Fits.SignalStateCount[0] + 1;
      ...
      while(Fits.SampleSize < DatasetSize + BackgroundDatasetSize)
      {
         ...
         if(Backgrounds[BackgroundCounter].state == "LooseCutEM")
...
void ReadEvents(vector<SingleEvent> &Events, string FileName)
{
   ...
   for(int iE = 0; iE < EntryCount; iE++)
   {
      ...
      if(l1id == 11 && l3id == 13)   State = "LooseCutEM";
      // if(l1id == 13 && l3id == 11)   State = "me";
      // if(l1id == 13 && l3id == 13)   State = "mm";
      
      char BranchPrefix = '\0';
      if(State == "LooseCutEM")   BranchPrefix = 'T';
