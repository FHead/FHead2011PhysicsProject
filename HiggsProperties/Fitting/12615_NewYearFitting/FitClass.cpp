...
double FitClass::CalculateLogLikelihoodAll(const double *Parameters) const
{
   ...
   double N3 = Parameters[18];
   double N4 = Parameters[19];
   double N5 = Parameters[20];
   double N6 = Parameters[21];
   double N7 = Parameters[22];
   ...
   for(int iS = 0; iS < SampleSize; iS++)
   {
      ...
      if(N5 > 1)
         LSb5 = (Likelihood[12] - Likelihood[11]) / 2;
      else if(N5 > 0)
         LSb5 = (Likelihood[11] - Likelihood[0]) / 1;
      else if(N5 > 1)
         LSb5 = (Likelihood[0] - Likelihood[10]) / 1;
      else
         LSb5 = (Likelihood[10] - Likelihood[9]) / 2;

      if(N5 > 1)
         LBb5 = (LikelihoodB[12] - LikelihoodB[11]) / 2;
      else if(N5 > 0)
         LBb5 = (LikelihoodB[11] - LikelihoodB[0]) / 1;
      else if(N5 > 1)
         LBb5 = (LikelihoodB[0] - LikelihoodB[10]) / 1;
      else
         LBb5 = (LikelihoodB[10] - LikelihoodB[9]) / 2;
      ...
      LogTotal = LogTotal + log(SignalLikelihood * (1 - F) + BackgroundLikelihood * F)
         - N1 * N1 / 2 - N2 * N2 / 2 - N3 * N3 / 2 - N4 * N4 / 2 - N5 * N5 / 2 - N6 * N6 / 2 - N7 * N7 / 2;
   }
...
void FitClass::AddPoint(double v[92][13], double i[92][13], EventParameters &Event, string State)
{
...
FitResultSummary FitClass::DoFit(FitConfiguration configuration)
{
   ...
   ROOT::Math::Functor Function(this, &FitClass::CalculateLogLikelihoodAll, 23);
   Worker->SetFunction(Function);

   double StepSize[23] = {0};
   ...
   StepSize[18] = (configuration.FloatN3 == true) ? 0.1 : 0;
   StepSize[19] = (configuration.FloatN4 == true) ? 0.1 : 0;
   StepSize[20] = (configuration.FloatN4 == true) ? 0.1 : 0;
   StepSize[21] = (configuration.FloatN4 == true) ? 0.1 : 0;
   StepSize[22] = (configuration.FloatN4 == true) ? 0.1 : 0;
   ...
   Worker->SetLimitedVariable(18, "N3",        configuration.N1InitialValue, StepSize[18], -5, 5);
   Worker->SetLimitedVariable(19, "N4",        configuration.N2InitialValue, StepSize[19], -5, 5);
   Worker->SetLimitedVariable(20, "N5",        configuration.N2InitialValue, StepSize[20], -5, 5);
   Worker->SetLimitedVariable(21, "N6",        configuration.N2InitialValue, StepSize[21], -5, 5);
   Worker->SetLimitedVariable(22, "N7",        configuration.N2InitialValue, StepSize[22], -5, 5);
   ...
   Result.N3 = Parameters[18];
   Result.N3Error = Errors[18];
   Result.N4 = Parameters[19];
   Result.N4Error = Errors[19];
   Result.N5 = Parameters[20];
   Result.N5Error = Errors[20];
   Result.N6 = Parameters[21];
   Result.N6Error = Errors[21];
   Result.N7 = Parameters[22];
   Result.N7Error = Errors[22];
