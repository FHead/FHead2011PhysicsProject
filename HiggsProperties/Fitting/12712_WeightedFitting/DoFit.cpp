...
void ReadEvents(vector<SingleEvent> &Events, string FileName, bool Capital);
...
int main(int argc, char *argv[])
{
   ...
   ReadEvents(Signals, FileName, true);
   ...
   for(int i = 1; i <= 3; i++)
   {
      ...
      Configurations.push_back(FitConfiguration(As, Phases, "NNNN", "NNNNNNN",
         AValues,
         ListToVector(6, 0, 0, 0, 0, 0, 0),
         ListToVector(4, ((Fs[0] == 'Y') ? 0.5 : 0), ((Fs[1] == 'Y') ? 0.5 : 0),
            ((Fs[2] == 'Y') ? 0.5 : 0), ((Fs[3] == 'Y') ? 0.5 : 0)),
   ...
   for(int iS = 0; iS < (int)Signals.size(); iS++)
   {
      ...
      for(int iC = 0; iC < (int)Configurations.size(); iC++)
      {
         ...
         cout << ResultTemp.Fem << " " << ResultTemp.Fme << " " << ResultTemp.Fee << " " << ResultTemp.Fmm << endl;
...
void ReadEvents(vector<SingleEvent> &Events, string FileName, bool Capital)
{
   ...
   double ZZmass;
   float zzmass;
   Tree->SetBranchAddress("MH", &ZZmass);
   ...
   for(int iE = 0; iE < EntryCount; iE++)
   {
      ...
      if(Capital == false && (zzmass > 135 || zzmass < 115))
         continue;
      if(Capital == true && (ZZmass > 135 || ZZmass < 115))
         continue;
