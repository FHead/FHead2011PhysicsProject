...
double FitClass::CalculateLogLikelihoodAll(const double *Parameters) const
{
   ...
   double F[4] = {Parameters[12], Parameters[13], Parameters[14], Parameters[15]};
   double FZJ[4] = {Parameters[16], Parameters[17], Parameters[18], Parameters[19]};
   double FggZZ[4] = {Parameters[20], Parameters[21], Parameters[22], Parameters[23]};
   double N[7] = {Parameters[24], Parameters[25], Parameters[26], Parameters[27],
   ...
   for(int iS = 0; iS < SampleSize; iS++)
   {
      ...
      // if(N[4] > 0)        LSb5 = (Likelihood[11] - Likelihood[0]);
      // else                LSb5 = (Likelihood[0] - Likelihood[10]);
      
      /*
      if(N[4] > 1)        LSb5 = (Likelihood[12] - Likelihood[11]) / 2;
      else if(N[4] > 0)   LSb5 = (Likelihood[11] - Likelihood[0]) / 1;
      else if(N[4] > -1)  LSb5 = (Likelihood[0] - Likelihood[10]) / 1;
      else                LSb5 = (Likelihood[10] - Likelihood[9]) / 2;

      if(N[4] > 1)        LBb5 = (LikelihoodB[12] - LikelihoodB[11]) / 2;
      else if(N[4] > 0)   LBb5 = (LikelihoodB[11] - LikelihoodB[0]) / 1;
      else if(N[4] > -1)  LBb5 = (LikelihoodB[0] - LikelihoodB[10]) / 1;
      else                LBb5 = (LikelihoodB[10] - LikelihoodB[9]) / 2;

      if(N[4] > 1)        LZJb5 = (LikelihoodZJ[12] - LikelihoodZJ[11]) / 2;
      else if(N[4] > 0)   LZJb5 = (LikelihoodZJ[11] - LikelihoodZJ[0]) / 1;
      else if(N[4] > -1)  LZJb5 = (LikelihoodZJ[0] - LikelihoodZJ[10]) / 1;
      else                LZJb5 = (LikelihoodZJ[10] - LikelihoodZJ[9]) / 2;

      if(N[4] > 1)        LBggb5 = (LikelihoodBgg[12] - LikelihoodBgg[11]) / 2;
      else if(N[4] > 0)   LBggb5 = (LikelihoodBgg[11] - LikelihoodBgg[0]) / 1;
      else if(N[4] > -1)  LBggb5 = (LikelihoodBgg[0] - LikelihoodBgg[10]) / 1;
      ...
      */
      ...
      double SignalLikelihood = LSa + LSb1 * N[0] + LSb5 * N[4];
      double BackgroundLikelihood = LBa + LBb1 * N[0];
      double BackgroundZJLikelihood = LZJa + LZJb1 * N[0];
      double BackgroundggLikelihood = LBgga + LBggb1 * N[0];
   ...
   for(int i = 0; i < 4; i++)
   {
      double NS = (SignalStateCount[i] + BackgroundStateCount[i]) * (1 - F[i]);
      ...
      double NB = (SignalStateCount[i] + BackgroundStateCount[i]) * F[i];
      Mu = NBConstrainMu[i];
   ...
   double Factor[7] = {0.01, 1, 1, 1, 1000, 1, 1};
   for(int i = 0; i < 7; i++)
...
FitResultSummary FitClass::DoFit(FitConfiguration configuration)
{
   ...
   StepSize[12] = (configuration.FloatF[0] == true) ? 0.1 : 0;
   StepSize[13] = (configuration.FloatF[1] == true) ? 0.1 : 0;
   StepSize[14] = (configuration.FloatF[2] == true) ? 0.1 : 0;
   StepSize[15] = (configuration.FloatF[3] == true) ? 0.1 : 0;
   StepSize[16] = (configuration.FloatFZJ[0] == true) ? 0.1 : 0;
   StepSize[17] = (configuration.FloatFZJ[1] == true) ? 0.1 : 0;
   StepSize[18] = (configuration.FloatFZJ[2] == true) ? 0.1 : 0;
   StepSize[19] = (configuration.FloatFZJ[3] == true) ? 0.1 : 0;
   StepSize[20] = (configuration.FloatFggZZ[0] == true) ? 0.1 : 0;
   StepSize[21] = (configuration.FloatFggZZ[1] == true) ? 0.1 : 0;
   StepSize[22] = (configuration.FloatFggZZ[2] == true) ? 0.1 : 0;
   StepSize[23] = (configuration.FloatFggZZ[3] == true) ? 0.1 : 0;
   StepSize[24] = (configuration.FloatN[0] == true) ? 0.1 : 0;
   StepSize[25] = (configuration.FloatN[1] == true) ? 0.1 : 0;
   StepSize[26] = (configuration.FloatN[2] == true) ? 0.1 : 0;
   StepSize[27] = (configuration.FloatN[3] == true) ? 0.1 : 0;
   StepSize[28] = (configuration.FloatN[4] == true) ? 0.1 : 0;
   StepSize[29] = (configuration.FloatN[5] == true) ? 0.1 : 0;
   StepSize[30] = (configuration.FloatN[6] == true) ? 0.1 : 0;
   ...
   Worker->SetLimitedVariable(12, "Fem",       configuration.FInitialValue[0], StepSize[12], 0, 1);
   Worker->SetLimitedVariable(13, "Fme",       configuration.FInitialValue[1], StepSize[13], 0, 1);
   Worker->SetLimitedVariable(14, "Fee",       configuration.FInitialValue[2], StepSize[14], 0, 1);
   Worker->SetLimitedVariable(15, "Fmm",       configuration.FInitialValue[3], StepSize[15], 0, 1);
   Worker->SetLimitedVariable(16, "FZJem",     configuration.FZJInitialValue[0], StepSize[16], 0, 1);
   Worker->SetLimitedVariable(17, "FZJme",     configuration.FZJInitialValue[1], StepSize[17], 0, 1);
   Worker->SetLimitedVariable(18, "FZJee",     configuration.FZJInitialValue[2], StepSize[18], 0, 1);
   Worker->SetLimitedVariable(19, "FZJmm",     configuration.FZJInitialValue[3], StepSize[19], 0, 1);
   Worker->SetLimitedVariable(20, "FggZZem",   configuration.FggZZInitialValue[0], StepSize[20], 0, 1);
   Worker->SetLimitedVariable(21, "FggZZme",   configuration.FggZZInitialValue[1], StepSize[21], 0, 1);
   Worker->SetLimitedVariable(22, "FggZZee",   configuration.FggZZInitialValue[2], StepSize[22], 0, 1);
   Worker->SetLimitedVariable(23, "FggZZmm",   configuration.FggZZInitialValue[3], StepSize[23], 0, 1);
   Worker->SetLimitedVariable(24, "N1",        configuration.NInitialValue[0], StepSize[24], -5, 5);
   Worker->SetLimitedVariable(25, "N2",        configuration.NInitialValue[1], StepSize[25], -5, 5);
   Worker->SetLimitedVariable(26, "N3",        configuration.NInitialValue[2], StepSize[26], -5, 5);
   Worker->SetLimitedVariable(27, "N4",        configuration.NInitialValue[3], StepSize[27], -5, 5);
   Worker->SetLimitedVariable(28, "N5",        configuration.NInitialValue[4], StepSize[28], -5, 5);
   Worker->SetLimitedVariable(29, "N6",        configuration.NInitialValue[5], StepSize[29], -5, 5);
   Worker->SetLimitedVariable(30, "N7",        configuration.NInitialValue[6], StepSize[30], -5, 5);
   ...
   Result.F[0] = Parameters[12];       Result.FError[0] = Errors[12];
   Result.F[1] = Parameters[13];       Result.FError[1] = Errors[13];
   Result.F[2] = Parameters[14];       Result.FError[2] = Errors[14];
   Result.F[3] = Parameters[15];       Result.FError[3] = Errors[15];
   Result.FZJ[0] = Parameters[16];     Result.FZJError[0] = Errors[16];
   Result.FZJ[1] = Parameters[17];     Result.FZJError[1] = Errors[17];
   Result.FZJ[2] = Parameters[18];     Result.FZJError[2] = Errors[18];
   Result.FZJ[3] = Parameters[19];     Result.FZJError[3] = Errors[19];
   Result.FggZZ[0] = Parameters[20];   Result.FggZZError[0] = Errors[20];
   Result.FggZZ[1] = Parameters[21];   Result.FggZZError[1] = Errors[21];
   Result.FggZZ[2] = Parameters[22];   Result.FggZZError[2] = Errors[22];
   Result.FggZZ[3] = Parameters[23];   Result.FggZZError[3] = Errors[23];
   Result.N[0] = Parameters[24];       Result.NError[0] = Errors[24];
   Result.N[1] = Parameters[25];       Result.NError[1] = Errors[25];
   Result.N[2] = Parameters[26];       Result.NError[2] = Errors[26];
   Result.N[3] = Parameters[27];       Result.NError[3] = Errors[27];
   Result.N[4] = Parameters[28];       Result.NError[4] = Errors[28];
   Result.N[5] = Parameters[29];       Result.NError[5] = Errors[29];
   Result.N[6] = Parameters[30];       Result.NError[6] = Errors[30];
