...
int main(int argc, char *argv[])
{
   ...
   string FileNameA =  "EventF.root";
   int DatasetSizeA = 30;
   string FileNameB = "EventF.root";
   int DatasetSizeB = 30;

   if(argc <= 10)
   {
      cerr << "Usage: " << argv[0]
         << " SignalRootFileA SignalDatasetSizeA SignalRootFileB SignalDatasetSizeB"
   ...
   FileNameA = argv[1];
   DatasetSizeA = atoi(argv[2]);
   FileNameB = argv[3];
   DatasetSizeB = atoi(argv[4]);
   ...
   vector<SingleEvent> SignalsA, SignalsB;
   ReadEvents(SignalsA, FileNameA);
   if(DatasetSizeB > 0)
      ReadEvents(SignalsB, FileNameB);
   else
      SignalsB = SignalsA;
   ...
   for(int i = 0; i <= 63; i++)
   {
      // if(i != 63 && i != 48 && i != 60 && i != 51 && i != 15 && i != 3 && i != 12)
      //    continue;
      if(i != 63 && i != 48 && i != 12 && i != 3)
         continue;
   ...
   int SignalCounterB = 0;

   ProgressBar Bar(cout, SignalsA.size());
   Bar.SetStyle(5);

   for(int iS = 0; iS < (int)SignalsA.size(); iS++)
   {
      if((iS % (SignalsA.size() / 300)) == 0)
      {
         Bar.Update(iS);
         Bar.Print();
      }

      Fits.AddPoint(SignalsA[iS]);

      if(SignalsA[iS].state == "em")   Fits.SignalStateCount[0] = Fits.SignalStateCount[0] + 1;
      if(SignalsA[iS].state == "me")   Fits.SignalStateCount[1] = Fits.SignalStateCount[1] + 1;
      if(SignalsA[iS].state == "ee")   Fits.SignalStateCount[2] = Fits.SignalStateCount[2] + 1;
      if(SignalsA[iS].state == "mm")   Fits.SignalStateCount[3] = Fits.SignalStateCount[3] + 1;
      
      if(Fits.SampleSize < DatasetSizeA)
         continue;

      while(Fits.SampleSize < DatasetSizeA + DatasetSizeB)
      {
         Fits.AddPoint(SignalsB[SignalCounterB]);

         if(SignalsB[SignalCounterB].state == "em")
            Fits.SignalStateCount[0] = Fits.SignalStateCount[0] + 1;
         if(SignalsB[SignalCounterB].state == "me")
            Fits.SignalStateCount[1] = Fits.SignalStateCount[1] + 1;
         if(SignalsB[SignalCounterB].state == "ee")
            Fits.SignalStateCount[2] = Fits.SignalStateCount[2] + 1;
         if(SignalsB[SignalCounterB].state == "mm")
            Fits.SignalStateCount[3] = Fits.SignalStateCount[3] + 1;

         SignalCounterB = SignalCounterB + 1;
         if(SignalCounterB >= SignalsB.size())
            SignalCounterB = 0;
      }
