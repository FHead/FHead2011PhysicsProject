...
int main(int argc, char *argv[])
{
   ...
   {
      cerr << "Usage: " << argv[0]
         << " SignalRootFile BackgroundRootFile"
   ...
   double A2ZZ = atof(argv[3]);
   double A3ZZ = atof(argv[4]);
   ...
   ReadEvents(SignalEM, SignalME, SignalEE, SignalMM, SignalFileName, true);
   ReadEvents(BackgroundEM, BackgroundME, BackgroundEE, BackgroundMM, BackgroundFileName, true);
   ...
   for(int i = 1; i <= 63; i++)
   {
      ...
      Configurations.push_back(FitConfiguration(As, Phases, Counts, Fs, "YNNNNNN",
         AValues,
         ListToVector(6, 0, 0, 0, 0, 0, 0),
         ListToVector(4, NS[0] + NB[0], NS[1] + NB[1], NS[2] + NB[2], NS[3] + NB[3]),
   ...
   while(Good == true)
   {
      ...
      for(int iC = 0; iC < (int)Configurations.size(); iC++)
      {
         ...
         TrueParameters[12] = ((Fits.SignalStateCount[0] + Fits.BackgroundStateCount[0] > 0) ?
            (Fits.BackgroundStateCount[0] / (Fits.SignalStateCount[0] + Fits.BackgroundStateCount[0])) : 0);
         TrueParameters[13] = ((Fits.SignalStateCount[1] + Fits.BackgroundStateCount[1] > 0) ?
            (Fits.BackgroundStateCount[1] / (Fits.SignalStateCount[1] + Fits.BackgroundStateCount[1])) : 0);
         TrueParameters[14] = ((Fits.SignalStateCount[2] + Fits.BackgroundStateCount[2] > 0) ?
            (Fits.BackgroundStateCount[2] / (Fits.SignalStateCount[2] + Fits.BackgroundStateCount[2])) : 0);
         TrueParameters[15] = ((Fits.SignalStateCount[3] + Fits.BackgroundStateCount[3] > 0) ?
            (Fits.BackgroundStateCount[3] / (Fits.SignalStateCount[3] + Fits.BackgroundStateCount[3])) : 0);
         for(int i = 16; i < 31; i++)
            TrueParameters[i] = 0;
         // for(int i = 0; i < 31; i++)
