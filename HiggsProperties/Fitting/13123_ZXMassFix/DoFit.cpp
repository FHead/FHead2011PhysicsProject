   ...
   string FileName, bool Capital, double MaxWeight)
{
   ...
   for(int iE = 0; iE < EntryCount; iE++)
   {
      ...
      if(State == "em")   ZX_S_Offset   = log(GetZX8TeV2e2muSpectrum(S));
      if(State == "me")   ZX_S_Offset   = log(GetZX8TeV2mu2eSpectrum(S));
      if(State == "ee")   ZX_S_Offset   = log(GetZX8TeV4eSpectrum(S));
      if(State == "mm")   ZX_S_Offset   = log(GetZX8TeV4muSpectrum(S));
      ...
      // TODO ggZZ has different binning compared to ZJet, but let's fix that later
      double MapIntegralBase = 1 / (2 * PI) / (2 * PI) / 2 / 2 / 2 / (2 * PI) / (135 * 135 - 115 * 115)
         / (120 * 120 - 40 * 40) / (120 * 120 - 12 * 12) / (500) / (5) / (2 * PI);
      double M1Low = 60, M1High = 120, M2Low = 12, M2High = 70;
      int M1Bin = 20, M2Bin = 10;
      double M1Up, M1Down, M2Up, M2Down;

      if(Capital == false)
      {
         M1Down = (int)((z1mass - M1Low) / (M1High - M1Low) * M1Bin) * ((M1High - M1Low) / M1Bin) + M1Low;
         M1Up = M1Down + (M1High - M1Low) / M1Bin;
         M2Down = (int)((z2mass - M2Low) / (M2High - M2Low) * M2Bin) * ((M2High - M2Low) / M2Bin) + M2Low;
         M2Up = M2Down + (M2High - M2Low) / M2Bin;
      }
      else
      {
         M1Down = (int)((M1 - M1Low) / (M1High - M1Low) * M1Bin) * ((M1High - M1Low) / M1Bin) + M1Low;
         M1Up = M1Down + (M1High - M1Low) / M1Bin;
         M2Down = (int)((M2 - M2Low) / (M2High - M2Low) * M2Bin) * ((M2High - M2Low) / M2Bin) + M2Low;
         M2Up = M2Down + (M2High - M2Low) / M2Bin;
      }
      
      for(int i = 0; i < 2; i++)   // Factor in cell size
         for(int j = 0; j < 13; j++)
            Integral2[i][j] = MapIntegralBase * (M1Up * M1Up - M1Down * M1Down) * (M2Up * M2Up - M2Down * M2Down)
               / (M1Up - M1Down) / (M2Up - M2Down);
