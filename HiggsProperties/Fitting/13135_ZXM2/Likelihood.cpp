...
int main(int argc, char *argv[])
{
   ...
   */
   ...
   while(Good == true)
   {
      ...
      Fits.SignalStateCount[0]     = PullEvents(Events, SignalEM,     100 * NS[0],    0.01 * NSSigma[0]);
      Fits.SignalStateCount[1]     = PullEvents(Events, SignalME,     100 * NS[1],    0.01 * NSSigma[1]);
      Fits.SignalStateCount[2]     = PullEvents(Events, SignalEE,     100 * NS[2],    0.01 * NSSigma[2]);
      Fits.SignalStateCount[3]     = PullEvents(Events, SignalMM,     100 * NS[3],    0.01 * NSSigma[3]);
      Fits.BackgroundStateCount[0] = PullEvents(Events, BackgroundEM, 100 * NB[0],    0.01 * NBSigma[0]);
      Fits.BackgroundStateCount[1] = PullEvents(Events, BackgroundME, 100 * NB[1],    0.01 * NBSigma[1]);
      Fits.BackgroundStateCount[2] = PullEvents(Events, BackgroundEE, 100 * NB[2],    0.01 * NBSigma[2]);
      Fits.BackgroundStateCount[3] = PullEvents(Events, BackgroundMM, 100 * NB[3],    0.01 * NBSigma[3]);
      Fits.ZJStateCount[0]         = PullEvents(Events, ZJEM,         100 * NZJ[0],   0.01 * NZJSigma[0]);
      Fits.ZJStateCount[1]         = PullEvents(Events, ZJME,         100 * NZJ[1],   0.01 * NZJSigma[1]);
      Fits.ZJStateCount[2]         = PullEvents(Events, ZJEE,         100 * NZJ[2],   0.01 * NZJSigma[2]);
      Fits.ZJStateCount[3]         = PullEvents(Events, ZJMM,         100 * NZJ[3],   0.01 * NZJSigma[3]);
      Fits.ggZZStateCount[0]       = PullEvents(Events, ggZZEM,       100 * NggZZ[0], 0.01 * NggZZSigma[0]);
      Fits.ggZZStateCount[1]       = PullEvents(Events, ggZZME,       100 * NggZZ[1], 0.01 * NggZZSigma[1]);
      Fits.ggZZStateCount[2]       = PullEvents(Events, ggZZEE,       100 * NggZZ[2], 0.01 * NggZZSigma[2]);
      Fits.ggZZStateCount[3]       = PullEvents(Events, ggZZMM,       100 * NggZZ[3], 0.01 * NggZZSigma[3]);
   ...
   string FileName, bool Capital, double MaxWeight, bool AlternativeWeight)
{
   ...
   for(int iE = 0; iE < EntryCount; iE++)
   {
      ...
      if(State == "em")   ggZZ_S_Offset = log(GetggZZ8TeV2e2muSpectrum(S));
      if(State == "me")   ggZZ_S_Offset = log(GetggZZ8TeV2mu2eSpectrum(S));
      if(State == "ee")   ggZZ_S_Offset = log(GetggZZ8TeV4eSpectrum(S));
      if(State == "mm")   ggZZ_S_Offset = log(GetggZZ8TeV4muSpectrum(S));
