...
int main(int argc, char *argv[])
{
   ...
   */
   ...
   while(Good == true)
   {
      ...
      Fits.SignalStateCount[0]     = PullEvents(Events, SignalEM,     NS[0],    NSSigma[0],    false);
      Fits.SignalStateCount[1]     = PullEvents(Events, SignalME,     NS[1],    NSSigma[1],    false);
      Fits.SignalStateCount[2]     = PullEvents(Events, SignalEE,     NS[2],    NSSigma[2],    false);
      Fits.SignalStateCount[3]     = PullEvents(Events, SignalMM,     NS[3],    NSSigma[3],    false);
      Fits.BackgroundStateCount[0] = PullEvents(Events, BackgroundEM, NB[0],    NBSigma[0],    false);
      Fits.BackgroundStateCount[1] = PullEvents(Events, BackgroundME, NB[1],    NBSigma[1],    false);
      Fits.BackgroundStateCount[2] = PullEvents(Events, BackgroundEE, NB[2],    NBSigma[2],    false);
      Fits.BackgroundStateCount[3] = PullEvents(Events, BackgroundMM, NB[3],    NBSigma[3],    false);
      Fits.ZJStateCount[0]         = PullEvents(Events, ZJEM,         NZJ[0],   NZJSigma[0],   true);
      Fits.ZJStateCount[1]         = PullEvents(Events, ZJME,         NZJ[1],   NZJSigma[1],   true);
      Fits.ZJStateCount[2]         = PullEvents(Events, ZJEE,         NZJ[2],   NZJSigma[2],   true);
      Fits.ZJStateCount[3]         = PullEvents(Events, ZJMM,         NZJ[3],   NZJSigma[3],   true);
      Fits.ggZZStateCount[0]       = PullEvents(Events, ggZZEM,       NggZZ[0], NggZZSigma[0], true);
      Fits.ggZZStateCount[1]       = PullEvents(Events, ggZZME,       NggZZ[1], NggZZSigma[1], true);
      Fits.ggZZStateCount[2]       = PullEvents(Events, ggZZEE,       NggZZ[2], NggZZSigma[2], true);
      Fits.ggZZStateCount[3]       = PullEvents(Events, ggZZMM,       NggZZ[3], NggZZSigma[3], true);
      ...
      cout << "G = " << Fits.ggZZStateCount[0] << " " << Fits.ggZZStateCount[1]
         << " " << Fits.ggZZStateCount[2] << " " << Fits.ggZZStateCount[3] << endl;
