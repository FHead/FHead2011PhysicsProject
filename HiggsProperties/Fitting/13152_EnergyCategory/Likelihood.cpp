...
int main(int argc, char *argv[])
{
   ...
   ReadEvents(SignalEM, SignalME, SignalEE, SignalMM, SignalFileName, true, 1, true);
   ...
   */
   ...
   while(Good == true)
   {
      ...
      Fits.SignalStateCount[0]     = PullEvents(Events, SignalEM,     80 * NS[0],    0.01 * NSSigma[0],    false);
      Fits.SignalStateCount[1]     = PullEvents(Events, SignalME,     80 * NS[1],    0.01 * NSSigma[1],    false);
      Fits.SignalStateCount[2]     = PullEvents(Events, SignalEE,     80 * NS[2],    0.01 * NSSigma[2],    false);
      Fits.SignalStateCount[3]     = PullEvents(Events, SignalMM,     80 * NS[3],    0.01 * NSSigma[3],    false);
      Fits.BackgroundStateCount[0] = PullEvents(Events, BackgroundEM, 80 * NB[0],    0.01 * NBSigma[0],    false);
      Fits.BackgroundStateCount[1] = PullEvents(Events, BackgroundME, 80 * NB[1],    0.01 * NBSigma[1],    false);
      Fits.BackgroundStateCount[2] = PullEvents(Events, BackgroundEE, 80 * NB[2],    0.01 * NBSigma[2],    false);
      Fits.BackgroundStateCount[3] = PullEvents(Events, BackgroundMM, 80 * NB[3],    0.01 * NBSigma[3],    false);
      Fits.ZJStateCount[0]         = PullEvents(Events, ZJEM,         40 * NZJ[0],   0.01 * NZJSigma[0],   true);
      Fits.ZJStateCount[1]         = PullEvents(Events, ZJME,         40 * NZJ[1],   0.01 * NZJSigma[1],   true);
      Fits.ZJStateCount[2]         = PullEvents(Events, ZJEE,         40 * NZJ[2],   0.01 * NZJSigma[2],   true);
      Fits.ZJStateCount[3]         = PullEvents(Events, ZJMM,         40 * NZJ[3],   0.01 * NZJSigma[3],   true);
      Fits.ggZZStateCount[0]       = PullEvents(Events, ggZZEM,       80 * NggZZ[0], 0.01 * NggZZSigma[0], true);
      Fits.ggZZStateCount[1]       = PullEvents(Events, ggZZME,       80 * NggZZ[1], 0.01 * NggZZSigma[1], true);
      Fits.ggZZStateCount[2]       = PullEvents(Events, ggZZEE,       80 * NggZZ[2], 0.01 * NggZZSigma[2], true);
      Fits.ggZZStateCount[3]       = PullEvents(Events, ggZZMM,       80 * NggZZ[3], 0.01 * NggZZSigma[3], true);
