...
int main(int argc, char *argv[])
{
   ...
   ReadEvents(SignalEM[1],     SignalME[1],     SignalEE[1],     SignalMM[1],     SignalFileName[1],     false, -2, 1, true,  false);
   ReadEvents(BackgroundEM[1], BackgroundME[1], BackgroundEE[1], BackgroundMM[1], BackgroundFileName[1], false,  0, 1, true,  false);
   ReadEvents(ggZZEM[1],       ggZZME[1],       ggZZEE[1],       ggZZMM[1],       ggZZFileName[1],       false,  1, 1, true,  false);
   ReadEvents(ZJEM[1],         ZJME[1],         ZJEE[1],         ZJMM[1],         ZJFileName[1],         false,  2, 1, true,  false);
   ReadEvents(SignalEM[0],     SignalME[0],     SignalEE[0],     SignalMM[0],     SignalFileName[0],     false, -2, 1, false, false);
   ReadEvents(BackgroundEM[0], BackgroundME[0], BackgroundEE[0], BackgroundMM[0], BackgroundFileName[0], false,  0, 1, false, false);
   ReadEvents(ggZZEM[0],       ggZZME[0],       ggZZEE[0],       ggZZMM[0],       ggZZFileName[0],       false,  1, 1, false, false);
   ReadEvents(ZJEM[0],         ZJME[0],         ZJEE[0],         ZJMM[0],         ZJFileName[0],         false,  2, 1, false, false);

   // 8 TeV Yield
   // vector<double> NS8 = ListToVector(4, 4.43783616325471, 3.44582607240994, 2.82352993495287, 5.52214303721489);
   vector<double> NS8 = ListToVector(4, 0, 0, 2.82352993495287, 5.52214303721489);
   ...
   vector<double> NB8 = ListToVector(4, 0, 0, 1.88891626468769, 4.72368839592275);
   ...
   vector<double> NZJ8 = ListToVector(4, 0, 0, 1.32305122153358, 0.755234938724669);
   ...
   vector<double> NggZZ8 = ListToVector(4, 0, 0, 0.047627447237569, 0.094525245283019);
   ...
   vector<double> NS7 = ListToVector(4, 0, 0, 0.601314214151534, 1.07576194776327);
   ...
   vector<double> NB7 = ListToVector(4, 0, 0, 0.514518707748627, 1.00979696393626);
   ...
   vector<double> NZJ7 = ListToVector(4, 0, 0, 0.470767996640258, 0.27072274946456);
   ...
   vector<double> NggZZ7 = ListToVector(4, 0, 0, 0.009456115406562, 0.016325980040323);
   ...
   for(int i = 1; i <= 63; i++)
   {
      ...
      string Counts = "NNYY";
      string Fs = "NNYYNNYYNNYY";

      Configurations.push_back(FitConfiguration(As, Phases, Counts, Counts,
   ...
   int SignalEMSize = SignalEM[0].size();
   ...
   while(Good == true)
   {
      Bar.Update(SignalEMSize - SignalEM[0].size());
      Bar.Print();

      int Count = -1;
      vector<SingleEvent> Events;

      Fits.SignalStateCount[1][0]      = PullEvents(Events, SignalEM[1],     10 * NS7[0],    NSSigma7[0],    false);
      Fits.SignalStateCount[1][0]     += PullEvents(Events, SignalEM[1],     10 * NS7[1],    NSSigma7[1],    false);
      Fits.SignalStateCount[1][1]      = 0;
      Fits.SignalStateCount[1][2]      = PullEvents(Events, SignalEE[1],     10 * NS7[2],    NSSigma7[2],    false);
      Fits.SignalStateCount[1][3]      = PullEvents(Events, SignalMM[1],     10 * NS7[3],    NSSigma7[3],    false);
      Fits.SignalStateCount[0][0]      = PullEvents(Events, SignalEM[0],     10 * NS8[0],    NSSigma8[0],    false);
      Fits.SignalStateCount[0][0]     += PullEvents(Events, SignalEM[0],     10 * NS8[1],    NSSigma8[1],    false);
      Fits.SignalStateCount[0][1]      = 0;
      Fits.SignalStateCount[0][2]      = PullEvents(Events, SignalEE[0],     10 * NS8[2],    NSSigma8[2],    false);
      Fits.SignalStateCount[0][3]      = PullEvents(Events, SignalMM[0],     10 * NS8[3],    NSSigma8[3],    false);
      Fits.BackgroundStateCount[1][0]  = PullEvents(Events, BackgroundEM[1], 10 * NB7[0],    NBSigma7[0],    false);
      Fits.BackgroundStateCount[1][0] += PullEvents(Events, BackgroundEM[1], 10 * NB7[1],    NBSigma7[1],    false);
      Fits.BackgroundStateCount[1][1]  = 0;
      Fits.BackgroundStateCount[1][2]  = PullEvents(Events, BackgroundEE[1], 10 * NB7[2],    NBSigma7[2],    false);
      Fits.BackgroundStateCount[1][3]  = PullEvents(Events, BackgroundMM[1], 10 * NB7[3],    NBSigma7[3],    false);
      Fits.BackgroundStateCount[0][0]  = PullEvents(Events, BackgroundEM[0], 10 * NB8[0],    NBSigma8[0],    false);
      Fits.BackgroundStateCount[0][0] += PullEvents(Events, BackgroundEM[0], 10 * NB8[1],    NBSigma8[1],    false);
      Fits.BackgroundStateCount[0][1]  = 0;
      Fits.BackgroundStateCount[0][2]  = PullEvents(Events, BackgroundEE[0], 10 * NB8[2],    NBSigma8[2],    false);
      Fits.BackgroundStateCount[0][3]  = PullEvents(Events, BackgroundMM[0], 10 * NB8[3],    NBSigma8[3],    false);
      Fits.ZJStateCount[1][0]          = PullEvents(Events, ZJEM[1],         10 * NZJ7[0],   NZJSigma7[0],   true);
      Fits.ZJStateCount[1][0]         += PullEvents(Events, ZJEM[1],         10 * NZJ7[1],   NZJSigma7[1],   true);
      Fits.ZJStateCount[1][1]          = 0;
      Fits.ZJStateCount[1][2]          = PullEvents(Events, ZJEE[1],         10 * NZJ7[2],   NZJSigma7[2],   true);
      Fits.ZJStateCount[1][3]          = PullEvents(Events, ZJMM[1],         10 * NZJ7[3],   NZJSigma7[3],   true);
      Fits.ZJStateCount[0][0]          = PullEvents(Events, ZJEM[0],         10 * NZJ8[0],   NZJSigma8[0],   true);
      Fits.ZJStateCount[0][0]         += PullEvents(Events, ZJEM[0],         10 * NZJ8[1],   NZJSigma8[1],   true);
      Fits.ZJStateCount[0][1]          = 0;
      Fits.ZJStateCount[0][2]          = PullEvents(Events, ZJEE[0],         10 * NZJ8[2],   NZJSigma8[2],   true);
      Fits.ZJStateCount[0][3]          = PullEvents(Events, ZJMM[0],         10 * NZJ8[3],   NZJSigma8[3],   true);
      ...
      for(int iC = 0; iC < (int)Configurations.size(); iC++)
      {
         ...
         TrueParameters[12] = Fits.SignalStateCount[0][0] + Fits.BackgroundStateCount[0][0] + Fits.ZJStateCount[0][0] + Fits.ggZZStateCount[0][0];
         TrueParameters[13] = Fits.SignalStateCount[0][1] + Fits.BackgroundStateCount[0][1] + Fits.ZJStateCount[0][1] + Fits.ggZZStateCount[0][1];
         TrueParameters[14] = Fits.SignalStateCount[0][2] + Fits.BackgroundStateCount[0][2] + Fits.ZJStateCount[0][2] + Fits.ggZZStateCount[0][2];
         TrueParameters[15] = Fits.SignalStateCount[0][3] + Fits.BackgroundStateCount[0][3] + Fits.ZJStateCount[0][3] + Fits.ggZZStateCount[0][3];
         TrueParameters[16] = Fits.SignalStateCount[1][0] + Fits.BackgroundStateCount[1][0] + Fits.ZJStateCount[1][0] + Fits.ggZZStateCount[1][0];
         TrueParameters[17] = Fits.SignalStateCount[1][1] + Fits.BackgroundStateCount[1][1] + Fits.ZJStateCount[1][1] + Fits.ggZZStateCount[1][1];
         TrueParameters[18] = Fits.SignalStateCount[1][2] + Fits.BackgroundStateCount[1][2] + Fits.ZJStateCount[1][2] + Fits.ggZZStateCount[1][2];
         TrueParameters[19] = Fits.SignalStateCount[1][3] + Fits.BackgroundStateCount[1][3] + Fits.ZJStateCount[1][3] + Fits.ggZZStateCount[1][3];
         TrueParameters[20] = ((TrueParameters[12] > 0) ? (Fits.BackgroundStateCount[0][0] / TrueParameters[12]) : 0);
         TrueParameters[21] = ((TrueParameters[13] > 0) ? (Fits.BackgroundStateCount[0][1] / TrueParameters[13]) : 0);
         TrueParameters[22] = ((TrueParameters[14] > 0) ? (Fits.BackgroundStateCount[0][2] / TrueParameters[14]) : 0);
         TrueParameters[23] = ((TrueParameters[15] > 0) ? (Fits.BackgroundStateCount[0][3] / TrueParameters[15]) : 0);
         TrueParameters[24] = ((TrueParameters[16] > 0) ? (Fits.BackgroundStateCount[1][0] / TrueParameters[16]) : 0);
         TrueParameters[25] = ((TrueParameters[17] > 0) ? (Fits.BackgroundStateCount[1][1] / TrueParameters[17]) : 0);
         TrueParameters[26] = ((TrueParameters[18] > 0) ? (Fits.BackgroundStateCount[1][2] / TrueParameters[18]) : 0);
         TrueParameters[27] = ((TrueParameters[19] > 0) ? (Fits.BackgroundStateCount[1][3] / TrueParameters[19]) : 0);
         TrueParameters[28] = ((TrueParameters[12] > 0) ? (Fits.ZJStateCount[0][0] / TrueParameters[12]) : 0);
         TrueParameters[29] = ((TrueParameters[13] > 0) ? (Fits.ZJStateCount[0][1] / TrueParameters[13]) : 0);
         TrueParameters[30] = ((TrueParameters[14] > 0) ? (Fits.ZJStateCount[0][2] / TrueParameters[14]) : 0);
         TrueParameters[31] = ((TrueParameters[15] > 0) ? (Fits.ZJStateCount[0][3] / TrueParameters[15]) : 0);
         TrueParameters[32] = ((TrueParameters[16] > 0) ? (Fits.ZJStateCount[1][0] / TrueParameters[16]) : 0);
         TrueParameters[33] = ((TrueParameters[17] > 0) ? (Fits.ZJStateCount[1][1] / TrueParameters[17]) : 0);
         TrueParameters[34] = ((TrueParameters[18] > 0) ? (Fits.ZJStateCount[1][2] / TrueParameters[18]) : 0);
         TrueParameters[35] = ((TrueParameters[19] > 0) ? (Fits.ZJStateCount[1][3] / TrueParameters[19]) : 0);
         TrueParameters[36] = ((TrueParameters[12] > 0) ? (Fits.ggZZStateCount[0][0] / TrueParameters[12]) : 0);
         TrueParameters[37] = ((TrueParameters[13] > 0) ? (Fits.ggZZStateCount[0][1] / TrueParameters[13]) : 0);
         TrueParameters[38] = ((TrueParameters[14] > 0) ? (Fits.ggZZStateCount[0][2] / TrueParameters[14]) : 0);
         TrueParameters[39] = ((TrueParameters[15] > 0) ? (Fits.ggZZStateCount[0][3] / TrueParameters[15]) : 0);
         TrueParameters[40] = ((TrueParameters[16] > 0) ? (Fits.ggZZStateCount[1][0] / TrueParameters[16]) : 0);
         TrueParameters[41] = ((TrueParameters[17] > 0) ? (Fits.ggZZStateCount[1][1] / TrueParameters[17]) : 0);
         TrueParameters[42] = ((TrueParameters[18] > 0) ? (Fits.ggZZStateCount[1][2] / TrueParameters[18]) : 0);
         TrueParameters[43] = ((TrueParameters[19] > 0) ? (Fits.ggZZStateCount[1][3] / TrueParameters[19]) : 0);
         TrueParameters[44] = 0;
         TrueParameters[45] = 0;
         TrueParameters[46] = 0;
         TrueParameters[47] = 0;
         ...
         if(iC == 0)
         {
            cout << "Start running fa2 scan" << endl;
            ...
            cout << "Start running fa3 scan" << endl;
            ...
            /*
            cout << "Start running mu-fa2 scan" << endl;
            TH2D HScanMuFA2(Form("HScanMuFA2_%d_%d", iC, FitCount), "FA2 vs. mu scan;FA2;Mu", 100, -1, 1, 100, 0, 1.5);
            for(int iBX = 1; iBX <= HScanMuFA2.GetNbinsX(); iBX++)
            {
               for(int iBY = 1; iBY <= HScanMuFA2.GetNbinsY(); iBY++)
               {
                  double FA2 = HScanMuFA2.GetXaxis()->GetBinCenter(iBX);
                  double A3A1 = Configurations[iC].AAInitialValue[1];
                  double S3 = A3A1 * A3A1 / 5.042 / 5.042;
                  double A2A1 = sqrt((1 + S3) / (1 - fabs(FA2)) * fabs(FA2) * 3.276 * 3.276);
                  if(FA2 > 0)
                  ...
                  double Mu = HScanMuFA2.GetYaxis()->GetBinCenter(iBY);
                  TrueParameters[51] = Mu;

                  double Likelihood = Fits.CalculateLogLikelihoodAll(TrueParameters);
                  if(Likelihood == Likelihood)
                     HScanMuFA2.SetBinContent(iBX, iBY, Likelihood);

                  TrueParameters[2] = Configurations[iC].AAInitialValue[0];
                  TrueParameters[51] = 1;
               }
            }
            HScanMuFA2.Write();

            cout << "Start running mu-fa3 scan" << endl;
            TH2D HScanMuFA3(Form("HScanMuFA3_%d_%d", iC, FitCount), "FA3 vs. mu scan;FA3;Mu", 100, -1, 1, 100, 0, 1.5);
            for(int iBX = 1; iBX <= HScanMuFA3.GetNbinsX(); iBX++)
            {
               for(int iBY = 1; iBY <= HScanMuFA3.GetNbinsY(); iBY++)
               {
                  double FA3 = HScanMuFA3.GetXaxis()->GetBinCenter(iBX);
                  double A2A1 = Configurations[iC].AAInitialValue[0];
                  double S2 = A2A1 * A2A1 / 3.276 / 3.276;
                  double A3A1 = sqrt((1 + S2) / (1 - fabs(FA3)) * fabs(FA3) * 5.042 * 5.042);
                  if(FA3 > 0)
                  ...
                  double Mu = HScanMuFA3.GetYaxis()->GetBinCenter(iBY);
                  TrueParameters[51] = Mu;

                  double Likelihood = Fits.CalculateLogLikelihoodAll(TrueParameters);
                  if(Likelihood == Likelihood)
                     HScanMuFA3.SetBinContent(iBX, iBY, Likelihood);
            ...
            */
            ...
            cout << "Start running fa2-fa3 scan" << endl;
            TH2D HScanFA2FA3(Form("HScanFA2FA3_%d_%d", iC, FitCount), "FA2 vs. FA3 scan;FA2;FA3", 100, -1, 1, 100, -1, 1);
            for(int iBX = 1; iBX <= HScanFA2FA3.GetNbinsX(); iBX++)
            {
               for(int iBY = 1; iBY <= HScanFA2FA3.GetNbinsY(); iBY++)
               {
                  double FA2 = HScanFA2FA3.GetXaxis()->GetBinCenter(iBX);
                  double FA3 = HScanFA2FA3.GetYaxis()->GetBinCenter(iBY);

                  if(fabs(FA2) + fabs(FA3) >= 1)
                  {
                     HScanFA2FA3.SetBinContent(iBX, iBY, 9999999);
                     continue;
                  }

                  double S2 = fabs(FA2) / (1 - fabs(FA2) - fabs(FA3));
                  double S3 = fabs(FA3) / (1 - fabs(FA2) - fabs(FA3));
                  ...
                  double Likelihood = Fits.CalculateLogLikelihoodAll(TrueParameters);
                  if(Likelihood == Likelihood)
                     HScanFA2FA3.SetBinContent(iBX, iBY, Likelihood);
...
void ReadEvents(vector<SingleEvent> &EventsEM, vector<SingleEvent> &EventsME,
   ...
   double MaxWeight, bool Is7TeV, bool AlternativeWeight)
{
