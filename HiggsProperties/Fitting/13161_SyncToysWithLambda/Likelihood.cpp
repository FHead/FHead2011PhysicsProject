...
int main(int argc, char *argv[])
{
   ...
   ReadEvents(SignalEM[1],     SignalME[1],     SignalEE[1],     SignalMM[1],     SignalFileName[1],     false, -1, 1, true,  false);
   ...
   ReadEvents(SignalEM[0],     SignalME[0],     SignalEE[0],     SignalMM[0],     SignalFileName[0],     false, -1, 1, false, false);
   ...
   for(int i = 1; i <= 127; i++)
   {
      ...
      if((i / 32) % 2 == 1)   As[5] = 'Y';
      if((i / 64) % 2 == 1)   As[6] = 'Y';
      if((i / 128) % 2 == 1)  As[7] = 'Y';   // LQZZ - not really applicable
   ...
   TFile OutputFile("LikelihoodFile2.root", "RECREATE");
   ...
   while(Good == true)
   {
      ...
      Fits.SignalStateCount[1][0]      = PullEvents(Events, SignalEM[1],     50 * NS7[0],    NSSigma7[0],    false);
      Fits.SignalStateCount[1][0]     += PullEvents(Events, SignalEM[1],     50 * NS7[1],    NSSigma7[1],    false);
      Fits.SignalStateCount[1][1]      = 0;
      Fits.SignalStateCount[1][2]      = PullEvents(Events, SignalEE[1],     50 * NS7[2],    NSSigma7[2],    false);
      Fits.SignalStateCount[1][3]      = PullEvents(Events, SignalMM[1],     50 * NS7[3],    NSSigma7[3],    false);
      Fits.SignalStateCount[0][0]      = PullEvents(Events, SignalEM[0],     50 * NS8[0],    NSSigma8[0],    false);
      Fits.SignalStateCount[0][0]     += PullEvents(Events, SignalEM[0],     50 * NS8[1],    NSSigma8[1],    false);
      Fits.SignalStateCount[0][1]      = 0;
      Fits.SignalStateCount[0][2]      = PullEvents(Events, SignalEE[0],     50 * NS8[2],    NSSigma8[2],    false);
      Fits.SignalStateCount[0][3]      = PullEvents(Events, SignalMM[0],     50 * NS8[3],    NSSigma8[3],    false);
      Fits.BackgroundStateCount[1][0]  = PullEvents(Events, BackgroundEM[1], 50 * NB7[0],    NBSigma7[0],    false);
      Fits.BackgroundStateCount[1][0] += PullEvents(Events, BackgroundEM[1], 50 * NB7[1],    NBSigma7[1],    false);
      Fits.BackgroundStateCount[1][1]  = 0;
      Fits.BackgroundStateCount[1][2]  = PullEvents(Events, BackgroundEE[1], 50 * NB7[2],    NBSigma7[2],    false);
      Fits.BackgroundStateCount[1][3]  = PullEvents(Events, BackgroundMM[1], 50 * NB7[3],    NBSigma7[3],    false);
      Fits.BackgroundStateCount[0][0]  = PullEvents(Events, BackgroundEM[0], 50 * NB8[0],    NBSigma8[0],    false);
      Fits.BackgroundStateCount[0][0] += PullEvents(Events, BackgroundEM[0], 50 * NB8[1],    NBSigma8[1],    false);
      Fits.BackgroundStateCount[0][1]  = 0;
      Fits.BackgroundStateCount[0][2]  = PullEvents(Events, BackgroundEE[0], 50 * NB8[2],    NBSigma8[2],    false);
      Fits.BackgroundStateCount[0][3]  = PullEvents(Events, BackgroundMM[0], 50 * NB8[3],    NBSigma8[3],    false);
      Fits.ZJStateCount[1][0]          = PullEvents(Events, ZJEM[1],         50 * NZJ7[0],   NZJSigma7[0],   true);
      Fits.ZJStateCount[1][0]         += PullEvents(Events, ZJEM[1],         50 * NZJ7[1],   NZJSigma7[1],   true);
      Fits.ZJStateCount[1][1]          = 0;
      Fits.ZJStateCount[1][2]          = PullEvents(Events, ZJEE[1],         50 * NZJ7[2],   NZJSigma7[2],   true);
      Fits.ZJStateCount[1][3]          = PullEvents(Events, ZJMM[1],         50 * NZJ7[3],   NZJSigma7[3],   true);
      Fits.ZJStateCount[0][0]          = PullEvents(Events, ZJEM[0],         50 * NZJ8[0],   NZJSigma8[0],   true);
      Fits.ZJStateCount[0][0]         += PullEvents(Events, ZJEM[0],         50 * NZJ8[1],   NZJSigma8[1],   true);
      Fits.ZJStateCount[0][1]          = 0;
      Fits.ZJStateCount[0][2]          = PullEvents(Events, ZJEE[0],         50 * NZJ8[2],   NZJSigma8[2],   true);
      Fits.ZJStateCount[0][3]          = PullEvents(Events, ZJMM[0],         50 * NZJ8[3],   NZJSigma8[3],   true);
      Fits.ggZZStateCount[1][0]        = PullEvents(Events, ggZZEM[1],       50 * NggZZ7[0], NggZZSigma7[0], true);
      Fits.ggZZStateCount[1][0]       += PullEvents(Events, ggZZEM[1],       50 * NggZZ7[1], NggZZSigma7[1], true);
      Fits.ggZZStateCount[1][1]        = 0;
      Fits.ggZZStateCount[1][2]        = PullEvents(Events, ggZZEE[1],       50 * NggZZ7[2], NggZZSigma7[2], true);
      Fits.ggZZStateCount[1][3]        = PullEvents(Events, ggZZMM[1],       50 * NggZZ7[3], NggZZSigma7[3], true);
      Fits.ggZZStateCount[0][0]        = PullEvents(Events, ggZZEM[0],       50 * NggZZ8[0], NggZZSigma8[0], true);
      Fits.ggZZStateCount[0][0]       += PullEvents(Events, ggZZEM[0],       50 * NggZZ8[1], NggZZSigma8[1], true);
      Fits.ggZZStateCount[0][1]        = 0;
      Fits.ggZZStateCount[0][2]        = PullEvents(Events, ggZZEE[0],       50 * NggZZ8[2], NggZZSigma8[2], true);
      Fits.ggZZStateCount[0][3]        = PullEvents(Events, ggZZMM[0],       50 * NggZZ8[3], NggZZSigma8[3], true);
      ...
      for(int iC = 0; iC < (int)Configurations.size(); iC++)
      {
         ...
         TrueParameters[12] = Configurations[iC].AAInitialValue[6];
         TrueParameters[13] = Configurations[iC].PhaseInitialValue[6];
         TrueParameters[14] = Configurations[iC].AAInitialValue[7];
         TrueParameters[15] = Configurations[iC].PhaseInitialValue[7];
         ...
         if(iC == 0)
         {
            ...
            cout << "Start running fA1L1 scan" << endl;
            TH1D HScanFA1L1(Form("HScanFA1L1_%d_%d", iC, FitCount), "FA1L1Scan", 100, -1, 1);
            for(int iB = 1; iB <= HScanFA3.GetNbinsX(); iB++)
            {
               double FA1L1 = HScanFA1L1.GetXaxis()->GetBinCenter(iB);
               double A2A1 = Configurations[iC].AAInitialValue[0];
               double A3A1 = Configurations[iC].AAInitialValue[1];
               double S2 = A2A1 * A2A1 / 3.276 / 3.276;
               double S3 = A3A1 * A3A1 / 5.042 / 5.042;
               double A1L1A1 = sqrt((1 + S2 + S3) / (1 - fabs(FA1L1)) * fabs(FA1L1) * 0.000140071 * 0.000140071);
               if(FA1L1 < 0)
                  A1L1A1 = -A1L1A1;
               TrueParameters[12] = A1L1A1;

               double Likelihood = Fits.CalculateLogLikelihoodAll(TrueParameters);
               if(Likelihood == Likelihood)
                  HScanFA1L1.SetBinContent(iB, Likelihood);
            ...
            cout << "Start running fa2-fa3 scan with phase" << endl;
            TH2D HScanFA2FA3Phased(Form("HScanFA2FA3Phased_%d_%d", iC, FitCount), "FA2 vs. FA3 scan (profile phase);FA2;FA3", 20, 0, 1, 20, 0, 1);
            for(int iBX = 1; iBX <= HScanFA2FA3Phased.GetNbinsX(); iBX++)
            {
               for(int iBY = 1; iBY <= HScanFA2FA3Phased.GetNbinsY(); iBY++)
               {
                  double FA2 = HScanFA2FA3Phased.GetXaxis()->GetBinCenter(iBX);
                  double FA3 = HScanFA2FA3Phased.GetYaxis()->GetBinCenter(iBY);

                  if(fabs(FA2) + fabs(FA3) >= 1)
                  {
                     HScanFA2FA3Phased.SetBinContent(iBX, iBY, 9999999);
                     continue;
                  }
                  ...
                  FitConfiguration Settings = Configurations[iC];
                  Settings.AAInitialValue[0] = A2A1;
                  Settings.AAInitialValue[1] = A3A1;

                  Settings.SetFloatAs("NNNNNNNN");
                  Settings.SetFloatPhases("YYNNNNNN");

                  FitResultSummary Result = Fits.DoFit(Settings);
                  HScanFA2FA3Phased.SetBinContent(iBX, iBY, Result.BestNLL);
               }
            }
            HScanFA2FA3Phased.Write();
            */
         }
      }
      FitCount = FitCount + 1;
