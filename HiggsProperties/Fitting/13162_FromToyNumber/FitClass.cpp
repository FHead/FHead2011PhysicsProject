...
double FitClass::CalculateLogLikelihoodAll(const double *Parameters) const
{
   ...
   for(int i = 0; i < 4; i++)
   {
      ...
      for(int j = 0; j < 2; j++)
      {
         ...
         // double R = GetYieldFractionA1ZZA3ZZ(A1ZZR, A3ZZR, A3ZZI, j, i);
         // double R = 1;
         double NS = Counts[j][i] * (1 - F[j][i] - FZJ[j][i] - FggZZ[j][i]);
         double Mu = NSConstrainMu[j][i] + log(R) + log(SigmaMu);
         ...
         if(EventCount[j][i] > 0)
         {
            double PoissonMean = NS * R * SigmaMu + NB + NZJ + NggZZ;
   ...
   double Factor[7] = {0.01, 0.1, 1, 1, 1, 1, 1};
   for(int i = 0; i < 7; i++)
