...
int main(int argc, char *argv[])
{
   ...
   TFile OutputFile("LikelihoodFileA2AA.root", "RECREATE");
   ...
   */
   ...
   for(int i = 0; i < Bin; i++)
   {
      double FA2AA = Min + (Max - Min) * (i + 0.5) / Bin;
      double SA2AA = fabs(FA2AA) / (1 - fabs(FA2AA));
      double A2AA = sqrt(SA2AA) * 0.108;
      if(FA2AA < 0)
         A2AA = -A2AA;

      Configuration.AAInitialValue[4] = A2AA;
