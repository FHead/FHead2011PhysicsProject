...
int main(int argc, char *argv[])
{
   ...
   TFile OutputFile("LikelihoodFileA3AA.root", "RECREATE");
   ...
   */
   ...
   for(int i = 0; i < Bin; i++)
   {
      double FA3AA = Min + (Max - Min) * (i + 0.5) / Bin;
      double SA3AA = fabs(FA3AA) / (1 - fabs(FA3AA));
      double A3AA = sqrt(SA3AA) * 0.109;
      if(FA3AA < 0)
         A3AA = -A3AA;

      Configuration.AAInitialValue[5] = A3AA;
