...
int main(int argc, char *argv[])
{
   ...
   TFile OutputFile("LikelihoodFileA3ZA.root", "RECREATE");
   ...
   */
   ...
   for(int i = 0; i < Bin; i++)
   {
      double FA3ZA = Min + (Max - Min) * (i + 0.5) / Bin;
      double SA3ZA = fabs(FA3ZA) / (1 - fabs(FA3ZA));
      double A3ZA = sqrt(SA3ZA) * 0.11;
      if(FA3ZA < 0)
         A3ZA = -A3ZA;

      Configuration.AAInitialValue[3] = A3ZA;
