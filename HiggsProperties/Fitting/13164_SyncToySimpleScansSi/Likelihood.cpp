...
int main(int argc, char *argv[])
{
   ...
   if(argc < 21)
   {
      cerr << "Usage: " << argv[0]
         ...
         << " A2ZZ A3ZZ A2ZA A3ZA A2AA A3AA A1L1ZZ ToyNumber FloatParameters FloatPhases FloatFs LumiFactor"
         << endl;
      return -1;
   }
   ...
   string FloatFs = argv[19];
   double LumiFactor = atof(argv[20]);
...
//    string Fs = "YNYYYNYYYNYY";
   ...
   double lumiMultiple = LumiFactor;

   int Count = -1;
   vector<SingleEvent> Events;

      Fits.SignalStateCount[1][0]      = PullEvents(Events, SignalEM[1],     lumiMultiple * NS7[0],    NSSigma7[0],    false);
      Fits.SignalStateCount[1][0]     += PullEvents(Events, SignalEM[1],     lumiMultiple * NS7[1],    NSSigma7[1],    false);
      Fits.SignalStateCount[1][1]      = 0;
      Fits.SignalStateCount[1][2]      = PullEvents(Events, SignalEE[1],     lumiMultiple * NS7[2],    NSSigma7[2],    false);
      Fits.SignalStateCount[1][3]      = PullEvents(Events, SignalMM[1],     lumiMultiple * NS7[3],    NSSigma7[3],    false);
      Fits.SignalStateCount[0][0]      = PullEvents(Events, SignalEM[0],     lumiMultiple * NS8[0],    NSSigma8[0],    false);
      Fits.SignalStateCount[0][0]     += PullEvents(Events, SignalEM[0],     lumiMultiple * NS8[1],    NSSigma8[1],    false);
      Fits.SignalStateCount[0][1]      = 0;
      Fits.SignalStateCount[0][2]      = PullEvents(Events, SignalEE[0],     lumiMultiple * NS8[2],    NSSigma8[2],    false);
      Fits.SignalStateCount[0][3]      = PullEvents(Events, SignalMM[0],     lumiMultiple * NS8[3],    NSSigma8[3],    false);
      Fits.BackgroundStateCount[1][0]  = PullEvents(Events, BackgroundEM[1], lumiMultiple * NB7[0],    NBSigma7[0],    false);
      Fits.BackgroundStateCount[1][0] += PullEvents(Events, BackgroundEM[1], lumiMultiple * NB7[1],    NBSigma7[1],    false);
      Fits.BackgroundStateCount[1][1]  = 0;
      Fits.BackgroundStateCount[1][2]  = PullEvents(Events, BackgroundEE[1], lumiMultiple * NB7[2],    NBSigma7[2],    false);
      Fits.BackgroundStateCount[1][3]  = PullEvents(Events, BackgroundMM[1], lumiMultiple * NB7[3],    NBSigma7[3],    false);
      Fits.BackgroundStateCount[0][0]  = PullEvents(Events, BackgroundEM[0], lumiMultiple * NB8[0],    NBSigma8[0],    false);
      Fits.BackgroundStateCount[0][0] += PullEvents(Events, BackgroundEM[0], lumiMultiple * NB8[1],    NBSigma8[1],    false);
      Fits.BackgroundStateCount[0][1]  = 0;
      Fits.BackgroundStateCount[0][2]  = PullEvents(Events, BackgroundEE[0], lumiMultiple * NB8[2],    NBSigma8[2],    false);
      Fits.BackgroundStateCount[0][3]  = PullEvents(Events, BackgroundMM[0], lumiMultiple * NB8[3],    NBSigma8[3],    false);
      Fits.ZJStateCount[1][0]          = PullEvents(Events, ZJEM[1],         lumiMultiple * NZJ7[0],   NZJSigma7[0],   true);
      Fits.ZJStateCount[1][0]         += PullEvents(Events, ZJEM[1],         lumiMultiple * NZJ7[1],   NZJSigma7[1],   true);
      Fits.ZJStateCount[1][1]          = 0;
      Fits.ZJStateCount[1][2]          = PullEvents(Events, ZJEE[1],         lumiMultiple * NZJ7[2],   NZJSigma7[2],   true);
      Fits.ZJStateCount[1][3]          = PullEvents(Events, ZJMM[1],         lumiMultiple * NZJ7[3],   NZJSigma7[3],   true);
      Fits.ZJStateCount[0][0]          = PullEvents(Events, ZJEM[0],         lumiMultiple * NZJ8[0],   NZJSigma8[0],   true);
      Fits.ZJStateCount[0][0]         += PullEvents(Events, ZJEM[0],         lumiMultiple * NZJ8[1],   NZJSigma8[1],   true);
      Fits.ZJStateCount[0][1]          = 0;
      Fits.ZJStateCount[0][2]          = PullEvents(Events, ZJEE[0],         lumiMultiple * NZJ8[2],   NZJSigma8[2],   true);
      Fits.ZJStateCount[0][3]          = PullEvents(Events, ZJMM[0],         lumiMultiple * NZJ8[3],   NZJSigma8[3],   true);
      Fits.ggZZStateCount[1][0]        = PullEvents(Events, ggZZEM[1],       lumiMultiple * NggZZ7[0], NggZZSigma7[0], true);
      Fits.ggZZStateCount[1][0]       += PullEvents(Events, ggZZEM[1],       lumiMultiple * NggZZ7[1], NggZZSigma7[1], true);
      Fits.ggZZStateCount[1][1]        = 0;
      Fits.ggZZStateCount[1][2]        = PullEvents(Events, ggZZEE[1],       lumiMultiple * NggZZ7[2], NggZZSigma7[2], true);
      Fits.ggZZStateCount[1][3]        = PullEvents(Events, ggZZMM[1],       lumiMultiple * NggZZ7[3], NggZZSigma7[3], true);
      Fits.ggZZStateCount[0][0]        = PullEvents(Events, ggZZEM[0],       lumiMultiple * NggZZ8[0], NggZZSigma8[0], true);
      Fits.ggZZStateCount[0][0]       += PullEvents(Events, ggZZEM[0],       lumiMultiple * NggZZ8[1], NggZZSigma8[1], true);
      Fits.ggZZStateCount[0][1]        = 0;
      Fits.ggZZStateCount[0][2]        = PullEvents(Events, ggZZEE[0],       lumiMultiple * NggZZ8[2], NggZZSigma8[2], true);
      Fits.ggZZStateCount[0][3]        = PullEvents(Events, ggZZMM[0],       lumiMultiple * NggZZ8[3], NggZZSigma8[3], true);
   ...
   for (int i=1; i < 100; ++i) {
     ...
     FitConfiguration Settings = Configuration;
   ...
   for(int i = 0; i < (int)Results.size(); i++)
   {
     ...
     cout << "**********************************************************\n";
     cout << "Fit Result Report\n";
     cout << "A2ZZ : " <<  Results[i].AA[0] << " +- " << Results[i].AAError[0] << "\n";
     cout << "A2ZZ Phase : " <<  Results[i].Phase[0] << " +- " << Results[i].PhaseError[0] << "\n";
     cout << "A3ZZ : " <<  Results[i].AA[1] << " +- " << Results[i].AAError[1] << "\n";
     cout << "A3ZZ Phase : " <<  Results[i].Phase[1] << " +- " << Results[i].PhaseError[1] << "\n";
     cout << "A2ZA : " <<  Results[i].AA[2] << " +- " << Results[i].AAError[2] << "\n";
     cout << "A2ZA Phase : " <<  Results[i].Phase[2] << " +- " << Results[i].PhaseError[2] << "\n";
     cout << "A3ZA : " <<  Results[i].AA[3] << " +- " << Results[i].AAError[3] << "\n";
     cout << "A3ZA Phase : " <<  Results[i].Phase[3] << " +- " << Results[i].PhaseError[3] << "\n";
     cout << "A2AA : " <<  Results[i].AA[4] << " +- " << Results[i].AAError[4] << "\n";
     cout << "A2AA Phase : " <<  Results[i].Phase[4] << " +- " << Results[i].PhaseError[4] << "\n";
     cout << "A3AA : " <<  Results[i].AA[5] << " +- " << Results[i].AAError[5] << "\n";
     cout << "A3AA Phase : " <<  Results[i].Phase[5] << " +- " << Results[i].PhaseError[5] << "\n";
