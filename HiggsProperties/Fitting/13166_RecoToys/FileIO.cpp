...
bool ReadEvents(vector<SingleEvent> &EventsEM, vector<SingleEvent> &EventsME,
   ...
   bool EMMETogether, bool IgnoreWeight)
{
   ...
   if(Capital == false)
   {
      Tree->SetBranchStatus("l1id", true);
      Tree->SetBranchStatus("l3id", true);
   ...
   for(int i = 0; i < 153; i++)
   {
      Tree->SetBranchStatus(Form("B1_%s", Suffix[i].c_str()), true);
      Tree->SetBranchAddress(Form("B1_%s", Suffix[i].c_str()), &Value[i+1][0]);
   }
   Tree->SetBranchStatus("B2", true);
   ...
   Tree->SetBranchStatus("B19", true);
   Tree->SetBranchStatus("B20", true);
   ...
   if(Capital == false)
   {
      Tree->SetBranchStatus("z1mass", true);
      Tree->SetBranchStatus("z2mass", true);
      Tree->SetBranchStatus("zzmass", true);
   ...
   {
      Tree->SetBranchStatus("M1", true);
      Tree->SetBranchStatus("M2", true);
      Tree->SetBranchStatus("MH", true);
   ...
   if(Capital == true)
   {
      Tree->SetBranchStatus("PassEM", true);
      Tree->SetBranchStatus("PassME", true);
      Tree->SetBranchStatus("PassEE", true);
      Tree->SetBranchStatus("PassMM", true);
...
int PullEvents(vector<SingleEvent> &Events, vector<SingleEvent> &Source, double Mean, double Sigma, bool Recycle)
{
   ...
   while(NewEvents.size() < EventCount && Source.size() > 0)
   {
      ...
      Source.pop_back();
      if(Recycle == true)
         Source.insert(Source.begin(), Event);
