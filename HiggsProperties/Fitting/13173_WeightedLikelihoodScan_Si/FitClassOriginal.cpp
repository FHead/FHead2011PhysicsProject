...
double FitClass::CalculateLogLikelihoodAll(const double *Parameters) const
{
   ...
   for(int iS = 0; iS < SampleSize; iS++)
   {
      ...
      for(int i = 0; i < (doSystematics?13:1); i++)
      {
        Value[i] = 0;
        Integral[i] = 0;
...
{
   AddPoint(event.v, event.i, event.v2, event.i2, event.weight, event.event, event.state, event.energy);
}

void FitClass::AddPoint(double v[154][13], double i[154][13], double v2[2][13], double i2[2][13],
   double weight, EventParameters &Event, string State, string SqrtS)
{
