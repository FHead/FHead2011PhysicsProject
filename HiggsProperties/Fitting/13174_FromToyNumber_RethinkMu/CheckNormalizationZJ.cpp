...
bool ReadEvents(vector<SingleEvent> &EventsEM, vector<SingleEvent> &EventsME,
   ...
   int TargetToy = -1, double *FudgeFactors = NULL);
...
int main(int argc, char *argv[])
{
   cout << "REMEMBER TO DISABLE CONSTRAINTS FOR NORMALIZATION CHECKING!!!" << endl;
   cout << "AND TO RE-ENABLE THEM BACK AFTER YOU'RE DONE!!!" << endl;
   ...
   if(argc < 17)
   {
      cerr << "Usage: " << argv[0]
         ...
         << " A2ZZ A3ZZ A2ZA A3ZA A2AA A3AA A1L1ZZ SEED"
         << endl;
      return -1;
   }
   ...
   int TargetToyNumber = -1;

   srand(atoi(argv[16]));
   double FudgeFactors[8] = {0};
   FudgeFactors[0] = DrawRandom(-5, 5);
   FudgeFactors[1] = DrawRandom(-5, 5);
   FudgeFactors[2] = DrawRandom(-5, 5);
   FudgeFactors[3] = DrawRandom(-5, 5);
   FudgeFactors[4] = DrawRandom(-5, 5);
   FudgeFactors[5] = DrawRandom(-5, 5);
   FudgeFactors[6] = DrawRandom(-5, 5);
   FudgeFactors[7] = DrawRandom(-5, 5);
   ...
   HasBadEvent[0] = ReadEvents(SignalEM[1],     SignalME[1],     SignalEE[1],     SignalMM[1],     SignalFileName[1],     false, DEFAULT_PROCESS, 1, true,  false, TargetToyNumber, FudgeFactors);
   HasBadEvent[1] = ReadEvents(BackgroundEM[1], BackgroundME[1], BackgroundEE[1], BackgroundMM[1], BackgroundFileName[1], false,  0, 1, true,  false, TargetToyNumber, FudgeFactors);
   HasBadEvent[2] = ReadEvents(ggZZEM[1],       ggZZME[1],       ggZZEE[1],       ggZZMM[1],       ggZZFileName[1],       false,  1, 1, true,  false, TargetToyNumber, FudgeFactors);
   HasBadEvent[3] = ReadEvents(ZJEM[1],         ZJME[1],         ZJEE[1],         ZJMM[1],         ZJFileName[1],         false,  2, 1, true,  false, TargetToyNumber, FudgeFactors);
   HasBadEvent[4] = ReadEvents(SignalEM[0],     SignalME[0],     SignalEE[0],     SignalMM[0],     SignalFileName[0],     false, DEFAULT_PROCESS, 1, false, false, TargetToyNumber, FudgeFactors);
   HasBadEvent[5] = ReadEvents(BackgroundEM[0], BackgroundME[0], BackgroundEE[0], BackgroundMM[0], BackgroundFileName[0], false,  0, 1, false, false, TargetToyNumber, FudgeFactors);
   HasBadEvent[6] = ReadEvents(ggZZEM[0],       ggZZME[0],       ggZZEE[0],       ggZZMM[0],       ggZZFileName[0],       false,  1, 1, false, false, TargetToyNumber, FudgeFactors);
   HasBadEvent[7] = ReadEvents(ZJEM[0],         ZJME[0],         ZJEE[0],         ZJMM[0],         ZJFileName[0],         false,  2, 1, false, false, TargetToyNumber, FudgeFactors);
   ...
   for(int i = 0; i <= 127; i++)
   {
      ...
      string Fs = "YNYYYNYYNNNN";   // FB(EM, ME, EE, MM), FZJ(EM, ME, EE, MM), FggZZ(EM, ME, EE, MM)
   ...
   TFile OutputFile("NormalizationChecker.root", "RECREATE");
   ...
   while(Good == true)
   {
      ...
      for(int iC = 0; iC < (int)Configurations.size(); iC++)
      {
         ...
         for(int i = 0; i < 2; i++)
            for(int j = 0; j < 4; j++)
               Configurations[iC].CountInitialValue[i][j] = Fits.SignalStateCount[i][j] + Fits.BackgroundStateCount[i][j];
         ...
         int BestNLLIndex = 0;
         // for(int iT = 1; iT < Tries; iT++)
         //    if(ResultTemp[iT].BestNLL < ResultTemp[BestNLLIndex].BestNLL)
         //       BestNLLIndex = iT;
   ...
   BranchList = BranchList + ":rFZJem7:rFZJme7:rFZJee7:rFZJmm7:rFZJem8:rFZJme8:rFZJee8:rFZJmm8";               // 144
   ...
   for(int i = 0; i < (int)Results.size(); i++)
   {
      ...
      Array[136] = rFZJem7 + FudgeFactors[0];   Array[140] = rFZJem8 + FudgeFactors[4];
      Array[137] = rFZJme7 + FudgeFactors[1];   Array[141] = rFZJme8 + FudgeFactors[5];
      Array[138] = rFZJee7 + FudgeFactors[2];   Array[142] = rFZJee8 + FudgeFactors[6];
      Array[139] = rFZJmm7 + FudgeFactors[3];   Array[143] = rFZJmm8 + FudgeFactors[7];
   ...
   double *FudgeFactors)
{
   ...
   for(int iE = 0; iE < EntryCount; iE++)
   {
      ...
      if(Is7TeV == false && State == "em")   ZX_S_Offset   = log(GetZX8TeV2e2muSpectrum(S))   + rFZJem8;
      if(Is7TeV == false && State == "me")   ZX_S_Offset   = log(GetZX8TeV2mu2eSpectrum(S))   + rFZJme8;
      if(Is7TeV == false && State == "ee")   ZX_S_Offset   = log(GetZX8TeV4eSpectrum(S))      + rFZJee8;
      if(Is7TeV == false && State == "mm")   ZX_S_Offset   = log(GetZX8TeV4muSpectrum(S))     + rFZJmm8;
      if(Is7TeV == true  && State == "em")   ZX_S_Offset   = log(GetZX7TeV2e2muSpectrum(S))   + rFZJem7;
      if(Is7TeV == true  && State == "me")   ZX_S_Offset   = log(GetZX7TeV2mu2eSpectrum(S))   + rFZJme7;
      if(Is7TeV == true  && State == "ee")   ZX_S_Offset   = log(GetZX7TeV4eSpectrum(S))      + rFZJee7;
      if(Is7TeV == true  && State == "mm")   ZX_S_Offset   = log(GetZX7TeV4muSpectrum(S))     + rFZJmm7;
      
      if(FudgeFactors != NULL && Is7TeV == false && State == "em")   ZX_S_Offset   = ZX_S_Offset + FudgeFactors[4];
      if(FudgeFactors != NULL && Is7TeV == false && State == "me")   ZX_S_Offset   = ZX_S_Offset + FudgeFactors[5];
      if(FudgeFactors != NULL && Is7TeV == false && State == "ee")   ZX_S_Offset   = ZX_S_Offset + FudgeFactors[6];
      if(FudgeFactors != NULL && Is7TeV == false && State == "mm")   ZX_S_Offset   = ZX_S_Offset + FudgeFactors[7];
      if(FudgeFactors != NULL && Is7TeV == true  && State == "em")   ZX_S_Offset   = ZX_S_Offset + FudgeFactors[0];
      if(FudgeFactors != NULL && Is7TeV == true  && State == "me")   ZX_S_Offset   = ZX_S_Offset + FudgeFactors[1];
      if(FudgeFactors != NULL && Is7TeV == true  && State == "ee")   ZX_S_Offset   = ZX_S_Offset + FudgeFactors[2];
      if(FudgeFactors != NULL && Is7TeV == true  && State == "mm")   ZX_S_Offset   = ZX_S_Offset + FudgeFactors[3];
      ...
      else
      {
         for(int j = 0; j < 13; j++)
         {
            ...
            if(Is7TeV == false && State == "em")   Integral[0][j] *= rFem8;
            if(Is7TeV == false && State == "me")   Integral[0][j] *= rFme8;
            if(Is7TeV == false && State == "ee")   Integral[0][j] *= rFee8;
            if(Is7TeV == false && State == "mm")   Integral[0][j] *= rFmm8;
            if(Is7TeV == true  && State == "em")   Integral[0][j] *= rFem7;
            if(Is7TeV == true  && State == "me")   Integral[0][j] *= rFme7;
            if(Is7TeV == true  && State == "ee")   Integral[0][j] *= rFee7;
            if(Is7TeV == true  && State == "mm")   Integral[0][j] *= rFmm7;
...
int PullToyEvents(vector<SingleEvent> &Events, vector<SingleEvent> &Source, int ToyNumber)
{
   ...
   {
      if(ToyNumber < 0 || Source[i].ToyNumber == ToyNumber)
      {
