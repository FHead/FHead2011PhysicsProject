...
int ReadEvents(vector<SingleEvent> &EventsEM, vector<SingleEvent> &EventsME,
...
int main(int argc, char *argv[])
{
   ...
   int TotalBadEvent = 0;
   for(int i = 0; i < 8; i++)
      TotalBadEvent = TotalBadEvent + HasBadEvent[i];
   ...
   for(int i = 1; i <= 127; i++)
   {
      ...
      string Counts = "YNYY";   // EM, ME, EE, MM
      string Fs = "YNYYYNYYYNYY";   // FB(EM, ME, EE, MM), FZJ(EM, ME, EE, MM), FggZZ(EM, ME, EE, MM)

      Configurations.push_back(FitConfiguration(As, Phases, Counts, Counts,
            Fs, Fs, "NNNNNNN", "Y",                                                  /* What to float */
            true,                                                                    /* Guess F */
   ...
   while(Good == true)
   {
      ...
      // Fits.ggZZStateCount[1][0]        = PullEvents(Events, ggZZEM[1],       10 * NggZZ7[0], NggZZSigma7[0], true);
      // Fits.ggZZStateCount[1][0]       += PullEvents(Events, ggZZEM[1],       10 * NggZZ7[1], NggZZSigma7[1], true);
      // Fits.ggZZStateCount[1][1]        = 0;
      // Fits.ggZZStateCount[1][2]        = PullEvents(Events, ggZZEE[1],       10 * NggZZ7[2], NggZZSigma7[2], true);
      // Fits.ggZZStateCount[1][3]        = PullEvents(Events, ggZZMM[1],       10 * NggZZ7[3], NggZZSigma7[3], true);
      // Fits.ggZZStateCount[0][0]        = PullEvents(Events, ggZZEM[0],       10 * NggZZ8[0], NggZZSigma8[0], true);
      // Fits.ggZZStateCount[0][0]       += PullEvents(Events, ggZZEM[0],       10 * NggZZ8[1], NggZZSigma8[1], true);
      // Fits.ggZZStateCount[0][1]        = 0;
      // Fits.ggZZStateCount[0][2]        = PullEvents(Events, ggZZEE[0],       10 * NggZZ8[2], NggZZSigma8[2], true);
      // Fits.ggZZStateCount[0][3]        = PullEvents(Events, ggZZMM[0],       10 * NggZZ8[3], NggZZSigma8[3], true);
      ...
      Fits.SignalStateCount[1][0] = NS7Prime[0] * GetYieldFractionA1ZZA2ZZ(1, -3.276, 0, 1, 0);
      Fits.SignalStateCount[1][1] = NS7Prime[1] * GetYieldFractionA1ZZA2ZZ(1, -3.276, 0, 1, 1);
      Fits.SignalStateCount[1][2] = NS7Prime[2] * GetYieldFractionA1ZZA2ZZ(1, -3.276, 0, 1, 2);
      Fits.SignalStateCount[1][3] = NS7Prime[3] * GetYieldFractionA1ZZA2ZZ(1, -3.276, 0, 1, 3);
      Fits.SignalStateCount[0][0] = NS8Prime[0] * GetYieldFractionA1ZZA2ZZ(1, -3.276, 0, 0, 0);
      Fits.SignalStateCount[0][1] = NS8Prime[1] * GetYieldFractionA1ZZA2ZZ(1, -3.276, 0, 0, 1);
      Fits.SignalStateCount[0][2] = NS8Prime[2] * GetYieldFractionA1ZZA2ZZ(1, -3.276, 0, 0, 2);
      Fits.SignalStateCount[0][3] = NS8Prime[3] * GetYieldFractionA1ZZA2ZZ(1, -3.276, 0, 0, 3);
      ...
      {
         map<double, int> AllSorter;
         map<double, int> Status0Sorter;
         map<double, int> Status1Sorter;
         ...
         for(int i = 0; i < 2; i++)
            for(int j = 0; j < 4; j++)
               Configurations[iC].CountInitialValue[i][j]
                  = Fits.SignalStateCount[i][j]
                  + Fits.BackgroundStateCount[i][j]
                  + Fits.ZJStateCount[i][j]
                  + Fits.ggZZStateCount[i][j];
         
         ResultTemp[0] = Fits.DoFit(Configurations[iC]);
         AllSorter.insert(pair<double, int>(ResultTemp[0].BestNLLNoPenalty, 0));
         if(ResultTemp[0].FitStatus == 0)
            Status0Sorter.insert(pair<double, int>(ResultTemp[0].BestNLLNoPenalty, 0));
         else
            Status1Sorter.insert(pair<double, int>(ResultTemp[0].BestNLLNoPenalty, 0));
         
         for(int iT = 1; iT < Tries; iT++)
         {
            FitConfiguration Current = Configurations[iC];
            if(Current.FloatAA[0] == true)   Current.AAInitialValue[0] = DrawRandom(-50, 50);
            if(Current.FloatAA[1] == true)   Current.AAInitialValue[1] = DrawRandom(-15, 15);
            if(Current.FloatMu == true)      Current.MuInitialValue = exp(DrawRandom(-log(5), log(15)));
            ResultTemp[iT] = Fits.DoFit(Current);

            cout << "FIT STATUS = " << ResultTemp[iT].FitStatus << endl;

            AllSorter.insert(pair<double, int>(ResultTemp[iT].BestNLLNoPenalty, iT));
            if(ResultTemp[iT].FitStatus == 0)
               Status0Sorter.insert(pair<double, int>(ResultTemp[iT].BestNLLNoPenalty, iT));
            else
               Status1Sorter.insert(pair<double, int>(ResultTemp[iT].BestNLLNoPenalty, iT));
         }

         int BestNLLIndex = AllSorter.begin()->second;
         // if(Status0Sorter.size() > 0)
         //    BestNLLIndex = Status0Sorter.begin()->second;
         // else
         //    BestNLLIndex = Status1Sorter.begin()->second;

         for(int iT = 0; iT < Tries; iT++)
            cout << iT << " " << ResultTemp[iT].BestNLL << endl;
         cout << "iC = " << iC << ", BestNLLIndex = " << BestNLLIndex
            << ", (A2, A3) = (" << ResultTemp[BestNLLIndex].AA[0]
            << ", " << ResultTemp[BestNLLIndex].AA[1] << ")" << endl;

         Results.push_back(ResultTemp[BestNLLIndex]);

         /**/
         FitResultSummary Best = ResultTemp[BestNLLIndex];

         FitConfiguration Current = Configurations[iC];
         Current.SetFloatAs("NNNNNNNN");
         Current.SetFloatPhases("NNNNNNNN");
         Current.SetFloatCounts(1, "NNNN");
         Current.SetFloatCounts(0, "NNNN");
         Current.SetFloatFractions(1, "NNNNNNNNNNNN");
         Current.SetFloatFractions(0, "NNNNNNNNNNNN");
         Current.SetFloatNs("NNNNNNN");
         Current.SetFloatMu("N");

         Current.SetInitialRatios(vector<double>(Best.AA, Best.AA + 8));
         Current.SetInitialPhases(vector<double>(Best.Phase, Best.Phase + 8));
         Current.SetInitialCounts(1, ListToVector(4, Best.Count[1][0], Best.Count[1][1], Best.Count[1][2], Best.Count[1][3]));
         Current.SetInitialCounts(0, ListToVector(4, Best.Count[0][0], Best.Count[0][1], Best.Count[0][2], Best.Count[0][3]));
         Current.SetInitialFractions(1, ListToVector(12, Best.F[1][0], Best.F[1][1], Best.F[1][2], Best.F[1][3],
            Best.FZJ[1][0], Best.FZJ[1][1], Best.FZJ[1][2], Best.FZJ[1][3],
            Best.FggZZ[1][0], Best.FggZZ[1][1], Best.FggZZ[1][2], Best.FggZZ[1][3]));
         Current.SetInitialFractions(0, ListToVector(12, Best.F[0][0], Best.F[0][1], Best.F[0][2], Best.F[0][3],
            Best.FZJ[0][0], Best.FZJ[0][1], Best.FZJ[0][2], Best.FZJ[0][3],
            Best.FggZZ[0][0], Best.FggZZ[0][1], Best.FggZZ[0][2], Best.FggZZ[0][3]));
         Current.SetInitialNs(vector<double>(Best.N, Best.N + 7));
         Current.SetInitialMu(Best.Mu);

         TH2D HScanFA2LogMu(Form("HScanFA2LogMu_%d_%d", TargetToyNumber, iC), "Scan FA2 vs. LogMu;FA2;LogMu", 100, -1, 1, 100, -log(3), log(60));

         for(int iX = 1; iX <= HScanFA2LogMu.GetNbinsX(); iX++)
         {
            for(int iY = 1; iY <= HScanFA2LogMu.GetNbinsY(); iY++)
            {
               double FA2 = HScanFA2LogMu.GetXaxis()->GetBinCenter(iX);
               double Mu = exp(HScanFA2LogMu.GetYaxis()->GetBinCenter(iY));

               double S2 = abs(FA2) / (1 - abs(FA2));
               double A2 = sqrt(S2) * 3.276;
               if(FA2 > 0)
                  A2 = -A2;

               Current.AAInitialValue[0] = A2;
               Current.AAInitialValue[1] = 0;
               Current.SetFloatMu("N");
               Current.MuInitialValue = Mu;

               FitResultSummary Temp = Fits.DoFit(Current);

               HScanFA2LogMu.SetBinContent(iX, iY, Temp.BestNLLNoPenalty);
            }
         }

         HScanFA2LogMu.Write();
         
         TH1D HScanFA2(Form("HScanFA2_%d_%d", TargetToyNumber, iC), "Scan FA2 vs. LogMu;FA2", 100, -1, 1);

         for(int iX = 1; iX <= HScanFA2.GetNbinsX(); iX++)
         {
            double FA2 = HScanFA2.GetXaxis()->GetBinCenter(iX);
            ...
            Current.AAInitialValue[0] = A2;
            Current.AAInitialValue[1] = 0;
            Current.SetFloatMu("Y");
            Current.SetFloatNs("YYNNNNN");
            Current.SetInitialMu(1);

            FitResultSummary Temp = Fits.DoFit(Current);
            Current.SetFloatNs("NNNNNNN");

            HScanFA2.SetBinContent(iX, Temp.BestNLLNoPenalty);
         }

         HScanFA2.Write();
 
         TH1D HScanFA3(Form("HScanFA3_%d_%d", TargetToyNumber, iC), "Scan FA3 vs. LogMu;FA3", 100, -1, 1);

         for(int iX = 1; iX <= HScanFA3.GetNbinsX(); iX++)
         {
            double FA3 = HScanFA3.GetXaxis()->GetBinCenter(iX);

            double S3 = abs(FA3) / (1 - abs(FA3));
            double A3 = sqrt(S3) * 5.042;
            if(FA3 > 0)
               A3 = -A3;

            Current.AAInitialValue[0] = 0;
            Current.AAInitialValue[1] = A3;
            Current.SetFloatMu("Y");
            Current.SetInitialMu(1);

            FitResultSummary Temp = Fits.DoFit(Current);

            HScanFA3.SetBinContent(iX, Temp.BestNLLNoPenalty);
         }

         HScanFA3.Write();

         TH2D HScanFA3LogMu(Form("HScanFA3LogMu_%d_%d", TargetToyNumber, iC), "Scan FA3 vs. LogMu;FA3;LogMu", 100, -1, 1, 100, -log(3), log(60));

         for(int iX = 1; iX <= HScanFA3LogMu.GetNbinsX(); iX++)
         {
            for(int iY = 1; iY <= HScanFA3LogMu.GetNbinsY(); iY++)
            {
               double FA3 = HScanFA3LogMu.GetXaxis()->GetBinCenter(iX);
               double Mu = exp(HScanFA3LogMu.GetYaxis()->GetBinCenter(iY));
               ...
               HScanFA3LogMu.SetBinContent(iX, iY, Temp.BestNLLNoPenalty);
            }
         }
   ...
   BranchList = BranchList + ":Toy:BadCount";                                                  // 136
