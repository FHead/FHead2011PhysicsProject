...
double FitClass::CalculateLogLikelihoodAll(const double *Parameters) const
{
   ...
   // cout << SampleSize << " " << V[1][0].size() << endl;
   for(int iS = 0; iS < SampleSize; iS++)
   {
      ...
      if(LTotal != LTotal || LTotal <= 0 || F[Is7TeV][StateIndex] + FZJ[Is7TeV][StateIndex] + FggZZ[Is7TeV][StateIndex] > 1)
      {
         ...
         {
            LogTotal = LogTotal - 99999;
            LogTotal = LogTotal - (F[Is7TeV][StateIndex] + FZJ[Is7TeV][StateIndex] + FggZZ[Is7TeV][StateIndex] - 1) * 1000000;
         }
      }
      else
   ...
   for(int i = 0; i < 4; i++)
   {
      ...
      for(int j = 0; j < 2; j++)
      {
         ...
         // double R = GetYieldFractionA1ZZA2ZZReal(A1ZZR, A2ZZR, j, i);
         double R = GetYieldFractionA1ZZA3ZZ(A1ZZR, A3ZZR, A3ZZI, j, i) * GetYieldFractionA1ZZA2ZZ(A1ZZR, A2ZZR, A2ZZI, j, i);
         // double R = 1;
         // double R = GetYieldFractionA1ZZA2ZZ(A1ZZR, A2ZZR, A2ZZI, j, i) / GetYieldFractionA1ZZA2ZZ(1, -3.276, 0, j, i);
   ...
   for(int i = 0; i < 7; i++)
      LogTotal = LogTotal - N[i] * N[i] / 2 / Factor[i] * 1000;
...
FitResultSummary FitClass::DoFit(FitConfiguration configuration)
{
   ...
   if(configuration.FloatMu == true)             Worker->SetLimitedVariable(55, "Mu",          configuration.MuInitialValue, StepSize[55], 0.01, 1000);
   ...
   for(int i = 0; i < 3; i++)
   {
      if(configuration.TotalNumberOfFloats() > 0)
         Worker->Minimize();
