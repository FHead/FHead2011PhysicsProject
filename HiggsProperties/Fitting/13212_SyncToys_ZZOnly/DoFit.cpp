...
bool ReadEvents(FitClass &Fits, string FileName, bool Capital, int Process = DEFAULT_PROCESS,
...
vector<double> ScaleVector(vector<double> V1, double X);
...
int main(int argc, char *argv[])
{
   ...
   HasBadEvent[0] = ReadEvents(Fits, SignalFileName[1],     false, DEFAULT_PROCESS, 1, true,  false, TargetToyNumber);
   HasBadEvent[1] = ReadEvents(Fits, BackgroundFileName[1], false,  0, 1, true,  false, TargetToyNumber);
   HasBadEvent[2] = ReadEvents(Fits, ggZZFileName[1],       false,  1, 1, true,  false, TargetToyNumber);
   HasBadEvent[3] = ReadEvents(Fits, ZJFileName[1],         false,  2, 1, true,  false, TargetToyNumber);
   HasBadEvent[4] = ReadEvents(Fits, SignalFileName[0],     false, DEFAULT_PROCESS, 1, false, false, TargetToyNumber);
   HasBadEvent[5] = ReadEvents(Fits, BackgroundFileName[0], false,  0, 1, false, false, TargetToyNumber);
   HasBadEvent[6] = ReadEvents(Fits, ggZZFileName[0],       false,  1, 1, false, false, TargetToyNumber);
   HasBadEvent[7] = ReadEvents(Fits, ZJFileName[0],         false,  2, 1, false, false, TargetToyNumber);
   ...
   for(int i = 0; i <= 127; i++)
   {
      ...
      Configurations.push_back(FitConfiguration(As, Phases, Counts, Counts,
            ...
            true,                                                                    /* Guess initial fractions in the fit code */
   ...
   while(Good == true)
   {
      ...
      for(int iC = 0; iC < (int)Configurations.size(); iC++)
      {
         ...
         for(int i = 0; i < 2; i++)
            for(int j = 0; j < 4; j++)
               Configurations[iC].CountInitialValue[i][j] = Fits.SignalStateCount[i][j] + Fits.BackgroundStateCount[i][j] + Fits.ZJStateCount[i][j];
         
         ResultTemp[0] = Fits.DoFit(Configurations[iC]);
         if(ResultTemp[0].EDM < 1)
         ...
         for(int iT = 1; iT < Tries; iT++)
         {
            ...
            if(Current.FloatAA[6] == true)   Current.AAInitialValue[6] = DrawRandom(-0.0003, 0.0003);
            if(Current.FloatPhase[0] == true)   Current.PhaseInitialValue[0] = DrawRandom(-1, 1);
            if(Current.FloatPhase[1] == true)   Current.PhaseInitialValue[1] = DrawRandom(-1, 1);
            Current.MuInitialValue = 0.4 + 0.4 * iT;
            ResultTemp[iT] = Fits.DoFit(Current);

            cout << "FIT STATUS = " << ResultTemp[iT].FitStatus << endl;

            if(ResultTemp[iT].EDM < 1)
               AllSorter.insert(pair<double, int>(ResultTemp[iT].BestNLLNoPenalty, iT));
         }

         int BestNLLIndex = 0;
         if(AllSorter.size() > 0)
            BestNLLIndex = AllSorter.begin()->second;
         for(int iT = 0; iT < Tries; iT++)
            cout << iT << " " << ResultTemp[iT].BestNLLNoPenalty << endl;
...
//---------------------------------------------------------------------------
bool ReadEvents(FitClass &Fits, string FileName, bool Capital, int Process,
   double MaxWeight, bool Is7TeV, bool AlternativeWeight, int TargetToyNumber)
{
   ...
   for(int iE = 0; iE < EntryCount; iE++)
   {
      ...
      int ChannelID = 0;
      if(State == "em")   ChannelID = 0;
      if(State == "me")   ChannelID = 1;
      if(State == "ee")   ChannelID = 2;
      if(State == "mm")   ChannelID = 3;

      Events[ChannelID] = Events[ChannelID] + 1;

      if(Process < 0 || Process == DEFAULT_PROCESS)
         Fits.SignalStateCount[Is7TeV][ChannelID] = Fits.SignalStateCount[Is7TeV][ChannelID] + 1;
      if(Process == 0)
         Fits.BackgroundStateCount[Is7TeV][ChannelID] = Fits.BackgroundStateCount[Is7TeV][ChannelID] + 1;
      if(Process == 1)
         Fits.ggZZStateCount[Is7TeV][ChannelID] = Fits.ggZZStateCount[Is7TeV][ChannelID] + 1;
      if(Process == 2)
         Fits.ZJStateCount[Is7TeV][ChannelID] = Fits.ZJStateCount[Is7TeV][ChannelID] + 1;
   }
   ...
   cout << "Events read: " << Events[0] << ", " << Events[1] << ", " << Events[2] << ", " << Events[3] << endl;
...
void SetAlias(TTree &ResultTree)
{
   ...
   ResultTree.SetAlias("A1L1", "A1L1ZZ");
   ...
   ResultTree.SetAlias("SL", "(A1L1*A1L1/0.0001204601/0.0001204601)");

   ResultTree.SetAlias("FA1", "(1/(1+S2+S3+SL))");
   ResultTree.SetAlias("FA2", "(-S2/(1+S2+S3+SL)*sign(A2ZZ))");
   ResultTree.SetAlias("FA3", "(-S3/(1+S2+S3+SL)*sign(A3ZZ))");
   ResultTree.SetAlias("FAL", "(-SL/(1+S2+S3+SL)*sign(A1L1))");
