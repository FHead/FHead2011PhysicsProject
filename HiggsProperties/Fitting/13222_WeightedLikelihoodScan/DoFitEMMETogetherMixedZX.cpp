...
int main(int argc, char *argv[])
{
   ...
   cout << "DoFitEMMETogetherMixedZX.cpp!" << endl;
   ...
   */
   ...
   // 8 TeV Yield from sync toys
   vector<double> NS8 = ListToVector(4, 7.710, 0, 2.827, 5.632);
   vector<double> NSSigma8 = ListToVector(4, 0.11, 0.11, 0.11, 0.11);
   vector<double> NB8 = ListToVector(4, 5.420, 0, 1.652, 4.297);
   vector<double> NBSigma8 = ListToVector(4, 0.048, 0.048, 0.047, 0.047);
   vector<double> NZJ8 = ListToVector(4, 2.673, 0, 1.614, 0.740);
   vector<double> NZJSigma8 = ListToVector(4, 0.40, 0.40, 0.40, 0.40);
   vector<double> NggZZ8 = ListToVector(4, 0.306, 0, 0.084, 0.205);
   vector<double> NggZZSigma8 = ListToVector(4, 0.25, 0.25, 0.25, 0.25);
   ...
   */
   ...
   // NZJ8 = ListToVector(4, 0, 0, 0, 0);
   // NggZZ8 = ListToVector(4, 0, 0, 0, 0);
   ...
   */
   ...
   // 7 TeV Yield from sync toys
   vector<double> NS7 = ListToVector(4, 1.654, 0, 0.635, 1.209);
   vector<double> NSSigma7 = ListToVector(4, 0.11, 0.11, 0.11, 0.11);
   vector<double> NB7 = ListToVector(4, 1.355, 0, 0.446, 0.991);
   vector<double> NBSigma7 = ListToVector(4, 0.048, 0.048, 0.047, 0.047);
   vector<double> NZJ7 = ListToVector(4, 0.696, 0, 0.366, 0.107);
   vector<double> NZJSigma7 = ListToVector(4, 0.40, 0.40, 0.40, 0.40);
   vector<double> NggZZ7 = ListToVector(4, 0.036, 0, 0.015, 0.038);
   vector<double> NggZZSigma7 = ListToVector(4, 0.25, 0.25, 0.25, 0.25);
   ...
   */
   ...
   // NZJ7 = ListToVector(4, 0, 0, 0, 0);
   // NggZZ7 = ListToVector(4, 0, 0, 0, 0);
   ...
   while(Good == true)
   {
      ...
      // NS7Prime[0] = NS7Prime[0] * GetYieldFractionA1ZZA2ZZ(1, -3.276, 0, true, 0);
      // NS7Prime[1] = NS7Prime[1] * GetYieldFractionA1ZZA2ZZ(1, -3.276, 0, true, 1);
      // NS7Prime[2] = NS7Prime[2] * GetYieldFractionA1ZZA2ZZ(1, -3.276, 0, true, 2);
      // NS7Prime[3] = NS7Prime[3] * GetYieldFractionA1ZZA2ZZ(1, -3.276, 0, true, 3);
      // NS8Prime[0] = NS8Prime[0] * GetYieldFractionA1ZZA2ZZ(1, -3.276, 0, false, 0);
      // NS8Prime[1] = NS8Prime[1] * GetYieldFractionA1ZZA2ZZ(1, -3.276, 0, false, 1);
      // NS8Prime[2] = NS8Prime[2] * GetYieldFractionA1ZZA2ZZ(1, -3.276, 0, false, 2);
      // NS8Prime[3] = NS8Prime[3] * GetYieldFractionA1ZZA2ZZ(1, -3.276, 0, false, 3);
      ...
      Fits.SignalStateCount[1][0]      = PullAndReweightEvents(Events, SignalEM[1],     NS7Prime[0]);
      Fits.SignalStateCount[1][1]      = PullAndReweightEvents(Events, SignalME[1],     NS7Prime[1]);
      Fits.SignalStateCount[1][2]      = PullAndReweightEvents(Events, SignalEE[1],     NS7Prime[2]);
      Fits.SignalStateCount[1][3]      = PullAndReweightEvents(Events, SignalMM[1],     NS7Prime[3]);
      for(int i = 0; i < (int)Events.size(); i++)
         Fits.AddPoint(Events[i]);
      // ExportSimpleTree("SyncSignal7TeV.root", Events);
      Events.clear();
      
      HasBadEvent[1] = ReadEvents(BackgroundEM[1], BackgroundME[1], BackgroundEE[1], BackgroundMM[1], BackgroundFileName[1],  false, 0, 1, true,  false, TargetToyNumber, true, false, true);
      Fits.BackgroundStateCount[1][0]  = PullAndReweightEvents(Events, BackgroundEM[1], NB7Prime[0]);
      Fits.BackgroundStateCount[1][1]  = PullAndReweightEvents(Events, BackgroundME[1], NB7Prime[1]);
      Fits.BackgroundStateCount[1][2]  = PullAndReweightEvents(Events, BackgroundEE[1], NB7Prime[2]);
      Fits.BackgroundStateCount[1][3]  = PullAndReweightEvents(Events, BackgroundMM[1], NB7Prime[3]);
      for(int i = 0; i < (int)Events.size(); i++)
         Fits.AddPoint(Events[i]);
      // ExportSimpleTree("SyncBackground7TeV.root", Events);
      Events.clear();
      // return 0;
      
      HasBadEvent[2] = ReadEvents(ggZZEM[1],       ggZZME[1],       ggZZEE[1],       ggZZMM[1],       ggZZFileName[1],       false, 1, 1, true,  false, TargetToyNumber, true, false, true);
      Fits.ggZZStateCount[1][0]        = PullAndReweightEvents(Events, ggZZEM[1],       NggZZ7Prime[0]);
      Fits.ggZZStateCount[1][1]        = PullAndReweightEvents(Events, ggZZME[1],       NggZZ7Prime[1]);
      Fits.ggZZStateCount[1][2]        = PullAndReweightEvents(Events, ggZZEE[1],       NggZZ7Prime[2]);
      Fits.ggZZStateCount[1][3]        = PullAndReweightEvents(Events, ggZZMM[1],       NggZZ7Prime[3]);
      for(int i = 0; i < (int)Events.size(); i++)
         Fits.AddPoint(Events[i]);
      Events.clear();
      ...
      HasBadEvent[3] = ReadEvents(ZJEM[1],         ZJME[1],         ZJEE[1],         ZJMM[1],         ZJFileName[1],         false, 2, 1, true,  false, TargetToyNumber, true, false, true);
      Fits.ZJStateCount[1][0]          = PullAndReweightEvents(Events, ZJEM[1],         NZJ7Prime[0]);
      Fits.ZJStateCount[1][1]          = PullAndReweightEvents(Events, ZJME[1],         NZJ7Prime[1]);
      Fits.ZJStateCount[1][2]          = PullAndReweightEvents(Events, ZJEE[1],         NZJ7Prime[2]);
      Fits.ZJStateCount[1][3]          = PullAndReweightEvents(Events, ZJMM[1],         NZJ7Prime[3]);
      for(int i = 0; i < (int)Events.size(); i++)
         Fits.AddPoint(Events[i]);
      Events.clear();
      ...
      Fits.SignalStateCount[0][0]      = PullAndReweightEvents(Events, SignalEM[0],     NS8Prime[0]);
      Fits.SignalStateCount[0][1]      = PullAndReweightEvents(Events, SignalME[0],     NS8Prime[1]);
      Fits.SignalStateCount[0][2]      = PullAndReweightEvents(Events, SignalEE[0],     NS8Prime[2]);
      Fits.SignalStateCount[0][3]      = PullAndReweightEvents(Events, SignalMM[0],     NS8Prime[3]);
      for(int i = 0; i < (int)Events.size(); i++)
         Fits.AddPoint(Events[i]);
      Events.clear();
      ...
      Fits.BackgroundStateCount[0][0]  = PullAndReweightEvents(Events, BackgroundEM[0], NB8Prime[0]);
      Fits.BackgroundStateCount[0][1]  = PullAndReweightEvents(Events, BackgroundME[0], NB8Prime[1]);
      Fits.BackgroundStateCount[0][2]  = PullAndReweightEvents(Events, BackgroundEE[0], NB8Prime[2]);
      Fits.BackgroundStateCount[0][3]  = PullAndReweightEvents(Events, BackgroundMM[0], NB8Prime[3]);
      for(int i = 0; i < (int)Events.size(); i++)
         Fits.AddPoint(Events[i]);
      Events.clear();
     
      HasBadEvent[6] = ReadEvents(ggZZEM[0],       ggZZME[0],       ggZZEE[0],       ggZZMM[0],       ggZZFileName[0],       false, 1, 1, false, false, TargetToyNumber, true, false, true);
      Fits.ggZZStateCount[0][0]        = PullAndReweightEvents(Events, ggZZEM[0],       NggZZ8Prime[0]);
      Fits.ggZZStateCount[0][1]        = PullAndReweightEvents(Events, ggZZME[0],       NggZZ8Prime[1]);
      Fits.ggZZStateCount[0][2]        = PullAndReweightEvents(Events, ggZZEE[0],       NggZZ8Prime[2]);
      Fits.ggZZStateCount[0][3]        = PullAndReweightEvents(Events, ggZZMM[0],       NggZZ8Prime[3]);
      for(int i = 0; i < (int)Events.size(); i++)
         Fits.AddPoint(Events[i]);
      Events.clear();
      ...
      HasBadEvent[7] = ReadEvents(ZJEM[0],         ZJME[0],         ZJEE[0],         ZJMM[0],         ZJFileName[0],         false, 2, 1, false, false, TargetToyNumber, true, false, true);
      Fits.ZJStateCount[0][0]          = PullAndReweightEvents(Events, ZJEM[0],         NZJ8Prime[0]);
      Fits.ZJStateCount[0][1]          = PullAndReweightEvents(Events, ZJME[0],         NZJ8Prime[1]);
      Fits.ZJStateCount[0][2]          = PullAndReweightEvents(Events, ZJEE[0],         NZJ8Prime[2]);
      Fits.ZJStateCount[0][3]          = PullAndReweightEvents(Events, ZJMM[0],         NZJ8Prime[3]);
      for(int i = 0; i < (int)Events.size(); i++)
         Fits.AddPoint(Events[i]);
      Events.clear();
      ...
      for(int iC = 0; iC < (int)Configurations.size(); iC++)
      {
         ...
         Configurations[iC].SetFloatNs("NNNNNNN");
         Configurations[iC].SetFloatFractions(0, "YNYYYNYYNNNN");
         Configurations[iC].SetFloatFractions(1, "YNYYYNYYNNNN");
