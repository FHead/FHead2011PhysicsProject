...
int main()
{
   TFile FE("A2Expected.root");
   TFile FO("MeowFA2.root");
   ...
   TE->SetBranchAddress("A2ZZA1ZZ", &A2ZZ);
   TO->SetBranchAddress("BestNLLNoPenalty", &NLL);
   TO->SetBranchAddress("A2ZZA1ZZ", &A2ZZ);
   ...
   {
      TE->GetEntry(i);
      Sorter.push_back(pair<double, double>(A2ZZ, NLL / 1000 + 408.0725));
   }
   ...
   {
      TO->GetEntry(i);
      Sorter.push_back(pair<double, double>(A2ZZ, NLL - 24.4));
   }
   sort(Sorter.begin(), Sorter.end());
   for(int i = 0; i < (int)Sorter.size(); i++)
   ...
   TH1D HWorld("HWorld", ";A_{2}^{ZZ} / A_{1}^{ZZ};-2 #Delta ln(L)", 100, -20, 20);
   HWorld.SetMinimum(0);
   ...
   TLegend Legend(0.15, 0.8, 0.45, 0.6);
