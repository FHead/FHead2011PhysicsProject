...
bool ReadEvents(vector<SingleEvent> &EventsEM, vector<SingleEvent> &EventsME,
   ...
   bool Is7TeV, bool AlternativeWeight, int TargetToyNumber,
   ...
   int FillInJacobian)
{
   ...
   float z1mass, z2mass, zzmass, theta0, theta1, theta2;
   ...
   for(int iE = 0; iE < EntryCount; iE++)
   {
      ...
      {
         // cout << EventsEE.size() << " " << EventsEM.size()
         //    << " " << EventsME.size() << " " << EventsMM.size() << endl;

         if(l1id == 11 && l3id == 11 && EventsEE.size() >= BUFFERSIZE)
            ReduceEvents(EventsEE, EVENTLIMIT * EventLimitFactor);
         if(l1id == 11 && l3id == 13 && EventsEM.size() >= BUFFERSIZE)
            ReduceEvents(EventsEM, EVENTLIMIT * EventLimitFactor);
         if(l1id == 13 && l3id == 11 && EventsME.size() >= BUFFERSIZE)
            ReduceEvents(EventsME, EVENTLIMIT * EventLimitFactor);
         if(l1id == 13 && l3id == 13 && EventsMM.size() >= BUFFERSIZE)
            ReduceEvents(EventsMM, EVENTLIMIT * EventLimitFactor);
      }
...
//---------------------------------------------------------------------------
void ExportSimpleTree(string FileName, vector<SingleEvent> &Events)
{
...
//---------------------------------------------------------------------------
void ReduceEvents(std::vector<SingleEvent> &Event, int TargetCount)
{
   // cout << Event.size() << " " << TargetCount << endl;

   if(TargetCount >= Event.size())
      return;

   // cout << "Initial Count = " << Event.size() << endl;

   double T = (double)TargetCount / Event.size();
   ...
   vector<double> Weights(Event.size());
   for(int i = 0; i < (int)Event.size(); i++)
      Weights[i] = Event[i].weight;
   sort(Weights.begin(), Weights.end());

   double Wc = Weights[Weights.size()*F1];

   int Size = Event.size();
   for(int i = 0; i < Size; i++)
   {
      if(Event[i].weight > Wc)
         continue;

      if(DrawRandom(0, 1) < F2)
         Event[i].weight = Event[i].weight / F2;
      else
      {
         swap(Event[Size-1], Event[i]);
         Size = Size - 1;
      }
   }
   if(Size < Event.size())
      Event.erase(Event.begin() + Size, Event.end());

   // cout << "Final Count = " << Event.size() << endl;
   
   if(Final == false)
      ReduceEvents(Event, TargetCount);
