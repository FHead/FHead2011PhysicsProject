...
int main(int argc, char *argv[])
{
   ...
   vector<ReducedSingleEvent> SignalEM[2],     SignalME[2],     SignalEE[2],     SignalMM[2];
   vector<ReducedSingleEvent> BackgroundEM[2], BackgroundME[2], BackgroundEE[2], BackgroundMM[2];
   vector<ReducedSingleEvent> ZJEM[2],         ZJME[2],         ZJEE[2],         ZJMM[2];
   vector<ReducedSingleEvent> ggZZEM[2],       ggZZME[2],       ggZZEE[2],       ggZZMM[2];
   ...
   */
   ...
   NS8[0] = 0,    NS8[1] = 0,    NS8[2] = 0,    NS8[3] != 0;  
   NB8[0] = 0,    NB8[1] = 0,    NB8[2] = 0,    NB8[3] != 0;
   NZJ8[0] = 0,   NZJ8[1] = 0,   NZJ8[2] = 0,   NZJ8[3] != 0;
   NggZZ8[0] = 0, NggZZ8[1] = 0, NggZZ8[2] = 0, NggZZ8[3] != 0;
   ...
   */
   ...
   NS7[0] = 0,    NS7[1] = 0,    NS7[2] = 0,    NS7[3] = 0;  
   NB7[0] = 0,    NB7[1] = 0,    NB7[2] = 0,    NB7[3] = 0;
   NZJ7[0] = 0,   NZJ7[1] = 0,   NZJ7[2] = 0,   NZJ7[3] = 0;
   NggZZ7[0] = 0, NggZZ7[1] = 0, NggZZ7[2] = 0, NggZZ7[3] = 0;
   ...
   for(int i = 0; i <= 127; i++)
   {
      ...
      string Counts = "NNYY";   // EM, ME, EE, MM
      string Fs = "YYNNNNNNNNNN";   // FB(EM, ME, EE, MM), FZJ(EM, ME, EE, MM), FggZZ(EM, ME, EE, MM)

      Configurations.push_back(FitConfiguration("Y",                                 /* Weighted dataset! */
            As, Phases, "NNNN", "NNNY",
   ...
   {
      int Count = -1;
      vector<ReducedSingleEvent> Events;

      // NS7[0] = NS7[0] * GetYieldFractionA1ZZA3ZZ(1, 0, -5.042, 1, 0);
      // NS7[1] = NS7[1] * GetYieldFractionA1ZZA3ZZ(1, 0, -5.042, 1, 1);
      // NS7[2] = NS7[2] * GetYieldFractionA1ZZA3ZZ(1, 0, -5.042, 1, 2);
      // NS7[3] = NS7[3] * GetYieldFractionA1ZZA3ZZ(1, 0, -5.042, 1, 3);
      // NS8[0] = NS8[0] * GetYieldFractionA1ZZA3ZZ(1, 0, -5.042, 0, 0);
      // NS8[1] = NS8[1] * GetYieldFractionA1ZZA3ZZ(1, 0, -5.042, 0, 1);
      // NS8[2] = NS8[2] * GetYieldFractionA1ZZA3ZZ(1, 0, -5.042, 0, 2);
      // NS8[3] = NS8[3] * GetYieldFractionA1ZZA3ZZ(1, 0, -5.042, 0, 3);
      
      // HasBadEvent[0] = ReadEvents(SignalEM[1],     SignalME[1],     SignalEE[1],     SignalMM[1],     SignalFileName[1],      false, DEFAULT_PROCESS, 1, true, false, TargetToyNumber, true, false, true);
      // HasBadEvent[0] = ReadEvents(SignalEM[1],     SignalME[1],     SignalEE[1],     SignalMM[1],     SignalFileName[1],      true, DEFAULT_PROCESS, 1, true, false, TargetToyNumber, true, true, false, 1, JACOBIAN_SIGNAL);
      // Fits.SignalStateCount[1][0]      = PullAndReweightEvents(Events, SignalEM[1],     NS7[0]);
      // Fits.SignalStateCount[1][0]     += PullAndReweightEvents(Events, SignalME[1],     NS7[1]);
      // Fits.SignalStateCount[1][2]      = PullAndReweightEvents(Events, SignalEE[1],     NS7[2]);
      // Fits.SignalStateCount[1][3]      = PullAndReweightEvents(Events, SignalMM[1],     NS7[3]);
      // for(int i = 0; i < (int)Events.size(); i++)
      //    Fits.AddPoint(Events[i]);
      // ExportSimpleTree("Signal7TeV.root", Events);
      // Events.clear();

      /*
      HasBadEvent[1] = ReadEvents(BackgroundEM[1], BackgroundME[1], BackgroundEE[1], BackgroundMM[1], BackgroundFileName[1],  true,  DEFAULT_PROCESS, 1, true,  false, TargetToyNumber, true, true, false, 0.5, JACOBIAN_BACKGROUND);
      // Fits.BackgroundStateCount[1][0]  = PullAndReweightEvents(Events, BackgroundEM[1], NB7[0]);
      // Fits.BackgroundStateCount[1][0] += PullAndReweightEvents(Events, BackgroundME[1], NB7[1]);
      Fits.BackgroundStateCount[1][2]  = PullAndReweightEvents(Events, BackgroundEE[1], NB7[2]);
      Fits.BackgroundStateCount[1][3]  = PullAndReweightEvents(Events, BackgroundMM[1], NB7[3]);
      for(int i = 0; i < (int)Events.size(); i++)
         Fits.AddPoint(Events[i]);
      ExportSimpleTree("Background7TeV.root", Events);
      Events.clear();

      HasBadEvent[2] = ReadEvents(ggZZEM[1],       ggZZME[1],       ggZZEE[1],       ggZZMM[1],       ggZZFileName[1],        false, DEFAULT_PROCESS, 1e-4, true,  false, TargetToyNumber, true);
      // Fits.ggZZStateCount[1][0]        = PullAndReweightEvents(Events, ggZZEM[1],       NggZZ7[0]);
      // Fits.ggZZStateCount[1][0]       += PullAndReweightEvents(Events, ggZZME[1],       NggZZ7[1]);
      Fits.ggZZStateCount[1][2]        = PullAndReweightEvents(Events, ggZZEE[1],       NggZZ7[2]);
      Fits.ggZZStateCount[1][3]        = PullAndReweightEvents(Events, ggZZMM[1],       NggZZ7[3]);
      for(int i = 0; i < (int)Events.size(); i++)
         Fits.AddPoint(Events[i]);
      ExportSimpleTree("GGZZ7TeV.root", Events);
      Events.clear();

      HasBadEvent[3] = ReadEvents(ZJEM[1],         ZJME[1],         ZJEE[1],         ZJMM[1],         ZJFileName[1],          false, DEFAULT_PROCESS, 6e-6, true,  false, TargetToyNumber, true);
      // Fits.ZJStateCount[1][0]          = PullAndReweightEvents(Events, ZJEM[1],         NZJ7[0]);
      // Fits.ZJStateCount[1][0]         += PullAndReweightEvents(Events, ZJME[1],         NZJ7[1]);
      Fits.ZJStateCount[1][2]          = PullAndReweightEvents(Events, ZJEE[1],         NZJ7[2]);
      Fits.ZJStateCount[1][3]          = PullAndReweightEvents(Events, ZJMM[1],         NZJ7[3]);
      for(int i = 0; i < (int)Events.size(); i++)
         Fits.AddPoint(Events[i]);
      ExportSimpleTree("ZJ7TeV.root", Events);
      Events.clear();
      */

      // HasBadEvent[4] = ReadEvents(SignalEM[0],     SignalME[0],     SignalEE[0],     SignalMM[0],     SignalFileName[0],      false, DEFAULT_PROCESS, 1, false, false, TargetToyNumber, true, false, true);
      HasBadEvent[4] = ReadEvents(SignalEM[0],     SignalME[0],     SignalEE[0],     SignalMM[0],     SignalFileName[0],      true, DEFAULT_PROCESS, 0.3, false,  true, TargetToyNumber, false, true, false, 1, JACOBIAN_SIGNAL);
      // Fits.SignalStateCount[0][0]      = PullAndReweightEvents(Events, SignalEM[0],     NS8[0]);
      // Fits.SignalStateCount[0][0]     += PullAndReweightEvents(Events, SignalME[0],     NS8[1]);
      // Fits.SignalStateCount[0][2]      = PullAndReweightEvents(Events, SignalEE[0],     NS8[2]);
      Fits.SignalStateCount[0][3]      = PullAndReweightEvents(Events, SignalMM[0],     NS8[3]);
      for(int i = 0; i < (int)Events.size(); i++)
         Fits.AddPoint(Events[i]);
      ExportSimpleTree("Signal8TeV.root", Events);
      Events.clear();

      // HasBadEvent[5] = ReadEvents(BackgroundEM[0], BackgroundME[0], BackgroundEE[0], BackgroundMM[0], BackgroundFileName[0],  false, 0, 1, false, false, TargetToyNumber, true, false, true);
      HasBadEvent[5] = ReadEvents(BackgroundEM[0], BackgroundME[0], BackgroundEE[0], BackgroundMM[0], BackgroundFileName[0],  true,  DEFAULT_PROCESS, 0.3, false, false, TargetToyNumber, true, true, false, 0.5, JACOBIAN_BACKGROUND);
      // Fits.BackgroundStateCount[0][0]  = PullAndReweightEvents(Events, BackgroundEM[0], NB8[0]);
      // Fits.BackgroundStateCount[0][0] += PullAndReweightEvents(Events, BackgroundME[0], NB8[1]);
      // Fits.BackgroundStateCount[0][2]  = PullAndReweightEvents(Events, BackgroundEE[0], NB8[2]);
      Fits.BackgroundStateCount[0][3]  = PullAndReweightEvents(Events, BackgroundMM[0], NB8[3]);
      for(int i = 0; i < (int)Events.size(); i++)
         Fits.AddPoint(Events[i]);
      ExportSimpleTree("Background8TeV.root", Events);
      Events.clear();

      /*
      HasBadEvent[6] = ReadEvents(ggZZEM[0],       ggZZME[0],       ggZZEE[0],       ggZZMM[0],       ggZZFileName[0],        false, DEFAULT_PROCESS, 1e-4, false, false, TargetToyNumber, true);
      Fits.ggZZStateCount[0][0]        = PullAndReweightEvents(Events, ggZZEM[0],       NggZZ8[0]);
      Fits.ggZZStateCount[0][0]       += PullAndReweightEvents(Events, ggZZME[0],       NggZZ8[1]);
      Fits.ggZZStateCount[0][2]        = PullAndReweightEvents(Events, ggZZEE[0],       NggZZ8[2]);
      Fits.ggZZStateCount[0][3]        = PullAndReweightEvents(Events, ggZZMM[0],       NggZZ8[3]);
      for(int i = 0; i < (int)Events.size(); i++)
         Fits.AddPoint(Events[i]);
      ExportSimpleTree("GGZZ8TeV.root", Events);
      Events.clear();

      HasBadEvent[7] = ReadEvents(ZJEM[0],         ZJME[0],         ZJEE[0],         ZJMM[0],         ZJFileName[0],          false, DEFAULT_PROCESS, 6e-6, false, false, TargetToyNumber, true);
      Fits.ZJStateCount[0][0]          = PullAndReweightEvents(Events, ZJEM[0],         NZJ8[0]);
      Fits.ZJStateCount[0][0]         += PullAndReweightEvents(Events, ZJME[0],         NZJ8[1]);
      Fits.ZJStateCount[0][2]          = PullAndReweightEvents(Events, ZJEE[0],         NZJ8[2]);
      Fits.ZJStateCount[0][3]          = PullAndReweightEvents(Events, ZJMM[0],         NZJ8[3]);
      for(int i = 0; i < (int)Events.size(); i++)
         Fits.AddPoint(Events[i]);
      ExportSimpleTree("ZJ8TeV.root", Events);
      Events.clear();
      */
      ...
      for(int i = 0; i < 8; i++)
         if(HasBadEvent[i] == true)
            AnyHasBadEvent = true;
      cout << Fits.SignalStateCount[0][0] << endl;
      ...
      for(int iC = 0; iC < (int)Configurations.size(); iC++)
      {
         ...
         Configurations[iC].SetFloatNs("NNNNNNN");
         Configurations[iC].SetFloatFractions(0, "NNNYNNNNNNNN");
         Configurations[iC].SetFloatFractions(1, "NNNNNNNNNNNN");
