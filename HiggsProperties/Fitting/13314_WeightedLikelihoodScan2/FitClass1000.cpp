...
double FitClass::CalculateLogLikelihoodAll(const double *Parameters) const
{
   ...
   double ConstraintPart = 0;
   double PoissonPart = 0;
   ...
   for(int i = 0; i < 4; i++)
   {
      ...
      for(int j = 0; j < 2; j++)
      {
         ...
         // double R = GetYieldFractionA1ZZA3ZZ(A1ZZR, A3ZZR, A3ZZI, j, i) / GetYieldFractionA1ZZA3ZZ(1, -5.042, 0, j, i);
         // double R = GetYieldFractionA1ZZA2ZZ(A1ZZR, A2ZZR, A2ZZI, j, i);
         double R = GetYieldFractionA1ZZA3ZZ(A1ZZR, A3ZZR, A3ZZI, j, i) * GetYieldFractionA1ZZA2ZZ(A1ZZR, A2ZZR, A2ZZI, j, i);
         // double R = 1;
         double NS = Counts[j][i] * (1 - F[j][i] - FZJ[j][i] - FggZZ[j][i]) / SigmaMu / R * 1000;
         double Mu = NSConstrainMu[j][i] + log(1000);
         double Sigma = NSConstrainSigma[j][i];
         if(Mu == Mu && Sigma > 0 && NS > 0)
            ConstraintPart = ConstraintPart - (log(NS) - Mu) * (log(NS) - Mu) / (2 * Sigma * Sigma) - log(NS) - log(Sigma);

         double NB = Counts[j][i] * F[j][i] * 1000;
         Mu = NBConstrainMu[j][i] + log(1000);
         Sigma = NBConstrainSigma[j][i];
         if(Mu == Mu && Sigma > 0 && NB > 0)
            ConstraintPart = ConstraintPart - (log(NB) - Mu) * (log(NB) - Mu) / (2 * Sigma * Sigma) - log(NB) - log(Sigma);

         double NZJ = Counts[j][i] * FZJ[j][i] * 1000;
         Mu = NZJConstrainMu[j][i] + log(1000);
         Sigma = NZJConstrainSigma[j][i];
         if(Mu == Mu && Sigma > 0 && NZJ > 0)
            ConstraintPart = ConstraintPart - (log(NZJ) - Mu) * (log(NZJ) - Mu) / (2 * Sigma * Sigma) - log(NZJ) - log(Sigma);

         double NggZZ = Counts[j][i] * FggZZ[j][i] * 1000;
         Mu = NggZZConstrainMu[j][i] + log(1000);
         Sigma = NggZZConstrainSigma[j][i];
         if(Mu == Mu && Sigma > 0 && NggZZ > 0)
            ConstraintPart = ConstraintPart - (log(NggZZ) - Mu) * (log(NggZZ) - Mu) / (2 * Sigma * Sigma) - log(NggZZ) - log(Sigma);

         if(EventCount[j][i] > 0)
         {
            double PoissonMean = Counts[j][i];
            PoissonPart = PoissonPart + 1000 * EventCount[j][i] * log(1000 * PoissonMean) - PoissonMean * 1000;
         }
      }
   }

   LogTotal = LogTotal + ConstraintPart;
   LogTotal = LogTotal + PoissonPart / 1000;
...
{
   cout << "WARNING: USING 1000x VERSION OF FITCLASS" << endl;
