...
double FitClass::PrimaryAngle(double x)
{
   double CosX = cos(x);
   double SinX = sin(x);
...
FitResultSummary FitClass::DoFit(FitConfiguration configuration)
{
   ...
   Result.Phi[0] = PrimaryAngle(Parameters[0]);
   Result.Phi[1] = PrimaryAngle(Parameters[1]);
   Result.Phi[2] = PrimaryAngle(Parameters[2]);
