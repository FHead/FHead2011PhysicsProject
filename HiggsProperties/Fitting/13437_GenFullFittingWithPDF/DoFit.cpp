...
int main(int argc, char *argv[])
{
   ...
   if(SEMSize >= 0)   ReadEvents(SEM, SEMFileName, CutLabel, true);
   if(SEESize >= 0)   ReadEvents(SEE, SEEFileName, CutLabel, true);
   if(BEMSize >= 0)   ReadEvents(BEM, BEMFileName, CutLabel, false);
   if(BEESize >= 0)   ReadEvents(BEE, BEEFileName, CutLabel, false);
   ...
   */
   ...
   for(int iE = 0; iE < EntryCount; iE++)
   {
      ...
      else
      {
         for(int j = 0; j < 5; j++)
         {
            // Integral[0][j] = DHFile[DHState][BackgroundBranch[j]].GetDouble() / 1.04442844774799082;
            // Integral[0][j] = DHFile[DHState][BackgroundBranch[j]].GetDouble() / 1.08;
