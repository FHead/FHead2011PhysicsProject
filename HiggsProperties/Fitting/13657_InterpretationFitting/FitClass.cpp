...
void FitConfiguration::SetFloatAs(string FloatAs)
{
   ...
   if(FloatAs.size() > 5 && FloatAs[5] == 'Y')   FloatYTA = true;
...
void FitConfiguration::SetInitialRatios(const vector<double> Ratios)
{
   ...
   if(Ratios.size() > 5)   YTAInitialValue = Ratios[5];
...
FitResultSummary FitClass::DoFit(FitConfiguration configuration)
{
   ...
   StepSize[9] = (configuration.FloatYTA == true) ? 0.1 : 0;
   ...
   Worker->SetLimitedVariable(9,  "YTA",       configuration.YTAInitialValue,       StepSize[9], -100, 100);
   ...
   TrueParameters[9] = configuration.YTAInitialValue;
   ...
   Result.YTA = Parameters[9];
   Result.YTAError = Errors[9];
