...
FitConfiguration::FitConfiguration()
{
   ...
   SetInitialRatios(ListToVector(9, 0, 0, 0, 0, 0, 0, 0, 0, 0));
...
void FitConfiguration::SetFloatAs(string FloatAs)
{
   ...
   if(FloatAs.size() > 3 && FloatAs[3] == 'Y')   FloatA5ZZA1ZZ = true;
   if(FloatAs.size() > 4 && FloatAs[4] == 'Y')   FloatYT = true;
   if(FloatAs.size() > 5 && FloatAs[5] == 'Y')   FloatYTPhase = true;
   if(FloatAs.size() > 6 && FloatAs[6] == 'Y')   FloatMT = true;
   if(FloatAs.size() > 7 && FloatAs[7] == 'Y')   FloatGWW = true;
   if(FloatAs.size() > 8 && FloatAs[8] == 'Y')   FloatMW = true;
}
...
void FitConfiguration::SetFloatPhases(string FloatPhases)
{
   ...
   if(FloatPhases.size() > 2 && FloatPhases[2] == 'Y')   FloatA4ZZPhase = true;
   if(FloatPhases.size() > 3 && FloatPhases[3] == 'Y')   FloatA5ZZPhase = true;
}
...
void FitConfiguration::SetInitialRatios(const vector<double> Ratios)
{
   ...
   if(Ratios.size() > 3)   A5ZZA1ZZInitialValue = Ratios[3];
   if(Ratios.size() > 4)   YTInitialValue = Ratios[4];
   if(Ratios.size() > 5)   YTPhaseInitialValue = Ratios[5];
   if(Ratios.size() > 6)   MTInitialValue = Ratios[6];
   if(Ratios.size() > 7)   GWWInitialValue = Ratios[7];
   if(Ratios.size() > 8)   MWInitialValue = Ratios[8];
}
...
void FitConfiguration::SetInitialPhases(const vector<double> Phases)
{
   ...
   if(Phases.size() > 2)   A4ZZPhaseInitialValue = Phases[6];
   if(Phases.size() > 3)   A5ZZPhaseInitialValue = Phases[7];
}
...
int FitConfiguration::TotalNumberOfFloats()
{
   ...
   if(FloatYTPhase == true)      Count = Count + 1;
...
double FitClass::CalculateLogLikelihoodAll(const double *Parameters) const
{
   ...
   C.yt = Parameters[8] * cos(Parameters[9]);
   C.yta = Parameters[8] * sin(Parameters[9]);
   C.mt = Parameters[10];
   C.gww = Parameters[11];
   C.mw = Parameters[12];
   ...
   double A3AAI = 0;
   double A4ZZR = Parameters[4] * cos(Parameters[5]) / 91.1876 / 91.1876;
   double A4ZZI = Parameters[4] * sin(Parameters[5]) / 91.1876 / 91.1876;
   double A5ZZR = Parameters[6] * cos(Parameters[7]);
   double A5ZZI = Parameters[6] * sin(Parameters[7]);
   double Fem = Parameters[13];
   double Fme = Parameters[14];
   double Fee = Parameters[15];
   double Fmm = Parameters[16];
   double N1 = Parameters[17];
   double N2 = Parameters[18];

   // C.Print();
   // cout << A2ZAR << " " << A3ZAR << " " << A2AAR << " " << A3AAR << endl;
   ...
   for(int iS = 0; iS < SampleSize; iS++)
   {
      ...
      // cout << SignalLikelihood << " " << BackgroundLikelihood << endl;

      LogTotal = LogTotal + log(SignalLikelihood * (1 - F) + BackgroundLikelihood * F) - N1 * N1 / 2 - N2 * N2 / 2;
   }

   // LogTotal = LogTotal - (C.mt - 174) * (C.mt - 174) / (2 * 5 * 5);
...
FitResultSummary FitClass::DoFit(FitConfiguration configuration)
{
   ...
   ROOT::Math::Functor Function(this, &FitClass::CalculateLogLikelihoodAll, 19);
   Worker->SetFunction(Function);

   double StepSize[19] = {0};
   ...
   StepSize[4] = (configuration.FloatA4ZZA1ZZ == true) ? 0.1 : 0;
   StepSize[5] = (configuration.FloatA4ZZPhase == true) ? 0.1 : 0;
   StepSize[6] = (configuration.FloatA5ZZA1ZZ == true) ? 0.1 : 0;
   StepSize[7] = (configuration.FloatA5ZZPhase == true) ? 0.1 : 0;
   StepSize[8] = (configuration.FloatYT == true) ? 0.1 : 0;
   StepSize[9] = (configuration.FloatYTPhase == true) ? 0.1 : 0;
   StepSize[10] = (configuration.FloatMT == true) ? 0.1 : 0;
   StepSize[11] = (configuration.FloatGWW == true) ? 0.1 : 0;
   StepSize[12] = (configuration.FloatMW == true) ? 0.1 : 0;
   StepSize[13] = (configuration.FloatFem == true) ? 0.1 : 0;
   StepSize[14] = (configuration.FloatFme == true) ? 0.1 : 0;
   StepSize[15] = (configuration.FloatFee == true) ? 0.1 : 0;
   StepSize[16] = (configuration.FloatFmm == true) ? 0.1 : 0;
   StepSize[17] = (configuration.FloatN1 == true) ? 0.1 : 0;
   StepSize[18] = (configuration.FloatN2 == true) ? 0.1 : 0;
   ...
   Worker->SetLimitedVariable(4,  "A4ZZA1ZZ",  configuration.A4ZZA1ZZInitialValue,  StepSize[4], -1000, 1000);
   Worker->SetLimitedVariable(5,  "A4ZZPhase", configuration.A4ZZPhaseInitialValue, StepSize[5], -PI * 10, PI * 10);
   Worker->SetLimitedVariable(6,  "A5ZZA1ZZ",  configuration.A5ZZA1ZZInitialValue,  StepSize[6], -1000, 1000);
   Worker->SetLimitedVariable(7,  "A5ZZPhase", configuration.A5ZZPhaseInitialValue, StepSize[7], -PI * 10, PI * 10);
   Worker->SetLimitedVariable(8,  "YT",        configuration.YTInitialValue,        StepSize[8], -100, 100);
   Worker->SetLimitedVariable(9,  "YTPhase",   configuration.YTPhaseInitialValue,   StepSize[9], -100, 100);
   Worker->SetLimitedVariable(10, "MT",        configuration.MTInitialValue,        StepSize[10], 10, 1000);
   Worker->SetLimitedVariable(11, "GWW",       configuration.GWWInitialValue,       StepSize[11], -100, 100);
   Worker->SetLimitedVariable(12, "MW",        configuration.MWInitialValue,        StepSize[12], 10, 1000);
   Worker->SetLimitedVariable(13, "Fem",       configuration.FemInitialValue,       StepSize[13], 0, 1);
   Worker->SetLimitedVariable(14, "Fme",       configuration.FmeInitialValue,       StepSize[14], 0, 1);
   Worker->SetLimitedVariable(15, "Fee",       configuration.FeeInitialValue,       StepSize[15], 0, 1);
   Worker->SetLimitedVariable(16, "Fmm",       configuration.FmmInitialValue,       StepSize[16], 0, 1);
   Worker->SetLimitedVariable(17, "N1",        configuration.N1InitialValue,        StepSize[17], -5, 5);
   Worker->SetLimitedVariable(18, "N2",        configuration.N2InitialValue,        StepSize[18], -5, 5);
   ...
   TrueParameters[4] = configuration.A4ZZA1ZZInitialValue;
   TrueParameters[5] = configuration.A4ZZPhaseInitialValue;
   TrueParameters[6] = configuration.A5ZZA1ZZInitialValue;
   TrueParameters[7] = configuration.A5ZZPhaseInitialValue;
   TrueParameters[8] = configuration.YTInitialValue;
   TrueParameters[9] = configuration.YTPhaseInitialValue;
   TrueParameters[10] = configuration.MTInitialValue;
   TrueParameters[11] = configuration.GWWInitialValue;
   TrueParameters[12] = configuration.MWInitialValue;
   TrueParameters[13] = ((EventCount[0] > 0) ? (BackgroundStateCount[0] / EventCount[0]) : 0);
   TrueParameters[14] = ((EventCount[1] > 0) ? (BackgroundStateCount[1] / EventCount[1]) : 0);
   TrueParameters[15] = ((EventCount[2] > 0) ? (BackgroundStateCount[2] / EventCount[2]) : 0);
   TrueParameters[16] = ((EventCount[3] > 0) ? (BackgroundStateCount[3] / EventCount[3]) : 0);
   TrueParameters[17] = 0;
   TrueParameters[18] = 0;
   ...
   Result.A4ZZA1ZZ = Parameters[4];
   Result.A4ZZA1ZZError = Errors[4];
   Result.A4ZZPhase = Parameters[5];
   Result.A4ZZPhaseError = Errors[5];
   Result.A5ZZA1ZZ = Parameters[6];
   Result.A5ZZA1ZZError = Errors[6];
   Result.A5ZZPhase = Parameters[7];
   Result.A5ZZPhaseError = Errors[7];
   Result.YT = Parameters[8];
   Result.YTError = Errors[8];
   Result.YTPhase = Parameters[9];
   Result.YTPhaseError = Errors[9];
   Result.MT = Parameters[10];
   Result.MTError = Errors[10];
   Result.GWW = Parameters[11];
   Result.GWWError = Errors[11];
   Result.MW = Parameters[12];
   Result.MWError = Errors[12];
   Result.Fem = Parameters[13];
   Result.FemError = Errors[13];
   Result.Fme = Parameters[14];
   Result.FmeError = Errors[14];
   Result.Fee = Parameters[15];
   Result.FeeError = Errors[15];
   Result.Fmm = Parameters[16];
   Result.FmmError = Errors[16];
   Result.N1 = Parameters[17];
   Result.N1Error = Errors[17];
   Result.N2 = Parameters[18];
   Result.N2Error = Errors[18];
