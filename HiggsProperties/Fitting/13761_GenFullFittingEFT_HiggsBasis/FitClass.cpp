...
void FitConfiguration::SetFloatAs(string FloatAs)
{
   ...
   if(FloatAs.size() > 3 && FloatAs[3] == 'Y')   FloatCZZ = true;
   if(FloatAs.size() > 4 && FloatAs[4] == 'Y')   FloatCZZD = true;
   if(FloatAs.size() > 5 && FloatAs[5] == 'Y')   FloatCZA = true;
   if(FloatAs.size() > 6 && FloatAs[6] == 'Y')   FloatCZAD = true;
   if(FloatAs.size() > 7 && FloatAs[7] == 'Y')   FloatCAA = true;
   if(FloatAs.size() > 8 && FloatAs[8] == 'Y')   FloatCAAD = true;
}
...
void FitConfiguration::SetInitialRatios(const vector<double> Ratios)
{
   ...
   if(Ratios.size() > 3)   CZZInitialValue     = Ratios[3];
   if(Ratios.size() > 4)   CZZDInitialValue    = Ratios[4];
   if(Ratios.size() > 5)   CZAInitialValue     = Ratios[5];
   if(Ratios.size() > 6)   CZADInitialValue    = Ratios[6];
   if(Ratios.size() > 7)   CAAInitialValue     = Ratios[7];
   if(Ratios.size() > 8)   CAADInitialValue    = Ratios[8];
}
...
int FitConfiguration::TotalNumberOfFloats()
{
   ...
   if(FloatCZZ == true)             Count = Count + 1;
   if(FloatCZZD == true)            Count = Count + 1;
   if(FloatCZA == true)             Count = Count + 1;
   if(FloatCZAD == true)            Count = Count + 1;
   if(FloatCAA == true)             Count = Count + 1;
   if(FloatCAAD == true)            Count = Count + 1;
...
double FitClass::CalculateLogLikelihoodAll(const double *Parameters) const
{
   ...
   double CZZ     = Parameters[3];
   double CZZd    = Parameters[4];
   double CZA     = Parameters[5];
   double CZAd    = Parameters[6];
   double CAA     = Parameters[7];
   double CAAd    = Parameters[8];
   ...
   double A2ZZR = 4 * AlphaEM * CZZ * PI / CW2 / SW2;
   double A3ZZR = 4 * AlphaEM * CZZd * PI / CW2 / SW2;
   double A4ZZR = (1 / SW2 * (DeltaCW - DeltaCZ - 4 * DeltaM - 8 * AlphaEM * CZZ * PI + 8 * AlphaEM * CZA * PI * (1 - 2 * SW2)
      + 2 * (-DeltaCW + DeltaCZ + 4 * DeltaM + 4 * AlphaEM * CAA * PI * CW2) * SW2)) / 2;
   double A1ZAR = 0;
   double A2ZAR = 4 * AlphaEM * CZA * PI / CW / SW;
   double A3ZAR = 4 * AlphaEM * CZAd * PI / CW / SW;
   double A4ZAR = 1 / CW / SW * (-4 * AlphaEM * CZZ * PI + 4 * AlphaEM * CZA * PI * (1 - 2 * SW2)
      + CW2 * (DeltaCW - DeltaCZ - 4 * DeltaM + 4 * AlphaEM * CAA * PI * SW2));
   double A1AZR = 0;
   double A2AZR = 4 * AlphaEM * CZA * PI / CW / SW;
   double A3AZR = 4 * AlphaEM * CZAd * PI / CW / SW;
   double A4AZR = 1 / CW / SW * (-4 * AlphaEM * CZZ * PI + 4 * AlphaEM * CZA * PI * (1 - 2 * SW2)
      + CW2 * (DeltaCW - DeltaCZ - 4 * DeltaM + 4 * AlphaEM * CAA * PI * SW2));
   double A1AAR = 0;
   double A2AAR = 4 * AlphaEM * CAA * PI;
   double A3AAR = 4 * AlphaEM * CAAd * PI;
   double A4AAR = 0;
   // THIS BLOCK COPIED FROM ROBERTO TEXT FILE //
...
FitResultSummary FitClass::DoFit(FitConfiguration configuration)
{
   ...
   StepSize[3] = (configuration.FloatCZZ == true) ? 0.1 : 0;
   StepSize[4] = (configuration.FloatCZZD == true) ? 0.1 : 0;
   StepSize[5] = (configuration.FloatCZA == true) ? 0.1 : 0;
   StepSize[6] = (configuration.FloatCZAD == true) ? 0.1 : 0;
   StepSize[7] = (configuration.FloatCAA == true) ? 0.1 : 0;
   StepSize[8] = (configuration.FloatCAAD == true) ? 0.1 : 0;
   ...
   Worker->SetLimitedVariable(3,  "CZZ",       configuration.CZZInitialValue,       StepSize[3], -1000, 1000);
   Worker->SetLimitedVariable(4,  "CZZD",      configuration.CZZDInitialValue,      StepSize[4], -1000, 1000);
   Worker->SetLimitedVariable(5,  "CZA",       configuration.CZAInitialValue,       StepSize[5], -1000, 1000);
   Worker->SetLimitedVariable(6,  "CZAD",      configuration.CZADInitialValue,      StepSize[6], -1000, 1000);
   Worker->SetLimitedVariable(7,  "CAA",       configuration.CAAInitialValue,       StepSize[7], -1000, 1000);
   Worker->SetLimitedVariable(8,  "CAAD",      configuration.CAADInitialValue,      StepSize[8], -1000, 1000);
   ...
   TrueParameters[3] = configuration.CZZInitialValue;
   TrueParameters[4] = configuration.CZZDInitialValue;
   TrueParameters[5] = configuration.CZAInitialValue;
   TrueParameters[6] = configuration.CZADInitialValue;
   TrueParameters[7] = configuration.CAAInitialValue;
   TrueParameters[8] = configuration.CAADInitialValue;
   ...
   Result.CZZ = Parameters[3];       Result.CZZError = Errors[3];
   Result.CZZD = Parameters[4];      Result.CZZDError = Errors[4];
   Result.CZA = Parameters[5];       Result.CZAError = Errors[5];
   Result.CZAD = Parameters[6];      Result.CZADError = Errors[6];
   Result.CAA = Parameters[7];       Result.CAAError = Errors[7];
   Result.CAAD = Parameters[8];      Result.CAADError = Errors[8];
