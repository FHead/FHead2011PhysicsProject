...
int main(int argc, char *argv[])
{
   ...
   if(argc <= 18)
   {
      cerr << "Usage: " << argv[0]
         ...
         << " DeltaCZ DeltaCW DeltaM CWW CWWD CWB CWBD CBB CBBD"
         << endl;
      return -1;
   }
   ...
   double CWW     = atof(argv[13]);
   double CWWD    = atof(argv[14]);
   double CWB     = atof(argv[15]);
   double CWBD    = atof(argv[16]);
   double CBB     = atof(argv[17]);
   double CBBD    = atof(argv[18]);
   ...
   BranchList = BranchList + ":CWW:CWWError";                      // 12    
   BranchList = BranchList + ":CWWD:CWWDError";                    // 14
   BranchList = BranchList + ":CWB:CWBError";                      // 16
   BranchList = BranchList + ":CWBD:CWBDError";                    // 18
   BranchList = BranchList + ":CBB:CBBError";                      // 20
   BranchList = BranchList + ":CBBD:CBBDError";                    // 22
   ...
   for(int i = 0; i < (int)Results.size(); i++)
   {
      ...
      Array[10] = Results[i].CWW;         Array[11] = Results[i].CWWError;
      Array[12] = Results[i].CWWD;        Array[13] = Results[i].CWWDError;
      Array[14] = Results[i].CWB;         Array[15] = Results[i].CWBError;
      Array[16] = Results[i].CWBD;        Array[17] = Results[i].CWBDError;
      Array[18] = Results[i].CBB;         Array[19] = Results[i].CBBError;
      Array[20] = Results[i].CBBD;        Array[21] = Results[i].CBBDError;
