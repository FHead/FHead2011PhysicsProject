...
FitConfiguration::FitConfiguration(string FloatAs, string FloatFractions, string Priors)
{
   SetFloatAs(FloatAs);
...
double FitClass::CalculateLogLikelihoodAll(const double *Parameters) const
{
   ...
   if(UsePriors[0] == true)   // RWWZZ
   {
      ...
      double LogFunction = -(RWWZZ - TrueRWWZZ) * (RWWZZ - TrueRWWZZ) / (2 * TrueSigmaRWWZZ * TrueSigmaRWWZZ);
      LogTotal = LogTotal + LogFunction;
   }
   if(UsePriors[1] == true)   // RZA
   {
      ...
      double LogFunction = -(RZA - TrueRZA) * (RZA - TrueRZA) / (2 * TrueSigmaRZA * TrueSigmaRZA);
      LogTotal = LogTotal + LogFunction;
   }
   if(UsePriors[2] == true)   // RAA
   {
      ...
      double TrueSigmaRAA = TrueRAA * 0.30;

      double LogFunction = -(RAA - TrueRAA) * (RAA - TrueRAA) / (2 * TrueSigmaRAA * TrueSigmaRAA);
      LogTotal = LogTotal + LogFunction + log(abs(A.A2AA));
   }
   ...
   if(UsePriors[8] == true)   // prior on magnitude of top yukawa (CP even & CP odd)
   {
      LoopBasis L;
      L.A2ZZ = As[1];
      L.A3ZZ = As[2];
      ...
      if(Basis != BASIS_LOOP)   cerr << "Warning!  Prior[8] requires loop basis!  Do nothing..." << endl;
      else                      Chi2 = GetChi2_TopYukawaMagnitude_15243(L);
