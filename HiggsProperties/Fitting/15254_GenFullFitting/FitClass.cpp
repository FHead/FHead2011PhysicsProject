...
double FitClass::CalculateLogLikelihoodAll(const double *Parameters) const
{
   ...
   for(int iS = 0; iS < SampleSize; iS++)
   {
      ...
      // LikelihoodB = (VB[0][iS] * 2 + VB[1][iS]) / (IB[0][iS] * 2 + IB[1][iS]);
      LikelihoodB = VB[0][iS] / IB[0][iS] * 2 / 3 + VB[1][iS] / IB[1][iS] / 3;
      // LikelihoodB = VB[0][iS] / IB[0][iS];   // uubar-only
      // LikelihoodB = VB[1][iS] / IB[1][iS];   // ddbar-only
