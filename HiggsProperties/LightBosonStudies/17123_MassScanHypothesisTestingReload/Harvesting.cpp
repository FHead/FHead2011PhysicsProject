...
int main(int argc, char *argv[])
{
   ...
   if(argc != 6)
   {
      cerr << "Usage: " << argv[0] << " Model1 Mdoel2 Mass File1 File2" << endl;
      return -1;
   }
         ...
         for(int iS = 0; iS < (int)Scenarios1.size(); iS++)
         {
            ...
            for(int i = 0; i < EntryCount1; i++)
            {
               ...
               for(int j = 0; j < MODELCOUNT; j++)
                  if(Likelihood[j] > 1e-5 || Likelihood[j] < -1e-5)
                     AllZero = false;
               if(AllZero == false)
                  L1.push_back(vector<double>(Likelihood, Likelihood + MODELCOUNT));
            }
            for(int i = 0; i < EntryCount2; i++)
            {
               T2->GetEntry(i);
               ...
               for(int j = 0; j < MODELCOUNT; j++)
                  if(Likelihood[j] > 1e-5 || Likelihood[j] < -1e-5)
                     AllZero = false;
               if(AllZero == false)
                  L2.push_back(vector<double>(Likelihood, Likelihood + MODELCOUNT));
            }
            ...
            for(int iM1 = 0; iM1 < MODELCOUNT; iM1++)
            {
               for(int iM2 = iM1 + 1; iM2 < MODELCOUNT; iM2++)
               {
                  ...
                  */
                  ...
                  string State = Mass + " " + Model1 + " " + Model2 + " Cut" + Cuts[iC]
