...
vector<Likelihood> ReadTree(string FileName, string DHFileName, char Cut, bool IsEM);
...
int main(int argc, char *argv[])
{
   ...
   if(argc != 8)
   {
      cerr << "Usage: " << argv[0] << "SignalEM SignalEE BackgroundEM BackgroundEE DHFileName Cut Tag" << endl;
      return -1;
   }
   ...
   Scenarios.push_back(EventCount(-1, 5, -1, -1));
   Scenarios.push_back(EventCount(-1, 10, -1, -1));
   Scenarios.push_back(EventCount(-1, 20, -1, -1));
   Scenarios.push_back(EventCount(-1, 50, -1, -1));
   Scenarios.push_back(EventCount(-1, 100, -1, -1));
   Scenarios.push_back(EventCount(-1, 200, -1, -1));
   Scenarios.push_back(EventCount(-1, 500, -1, -1));
   Scenarios.push_back(EventCount(-1, 1000, -1, -1));
   Scenarios.push_back(EventCount(-1, 2000, -1, -1));
   Scenarios.push_back(EventCount(-1, 5000, -1, -1));
   ...
   SEM = ReadTree(SEMFileName, DHFileName, Cut, true);
   SEE = ReadTree(SEEFileName, DHFileName, Cut, false);
   BEM = ReadTree(BEMFileName, DHFileName, Cut, true);
   BEE = ReadTree(BEEFileName, DHFileName, Cut, false);
...
vector<Likelihood> ReadTree(string FileName, string DHFileName, char Cut, bool IsEM)
{
   ...
   double A1UUA1UU = 0, A2UUA2UU = 0;
   if(T->GetBranch("B1_A1VVR_A1VVR"))
      T->SetBranchAddress("B1_A1VVR_A1VVR", &A1UUA1UU);
   if(T->GetBranch("B1_A2VVR_A2VVR"))
      T->SetBranchAddress("B1_A2VVR_A2VVR", &A2UUA2UU);
   ...
   if(DHFile["0TeV"].Exist(Prefix + "2_DDbar"))
   ...
   for(int iE = 0; iE < EntryCount; iE++)
   {
      ...
      // cout << Messenger.VS[6][6] << " " << A1UUA1UU << " " << Messenger.VS[6][6] / A1UUA1UU << endl;
      // cout << Messenger.VS[7][7] << " " << A2UUA2UU << " " << Messenger.VS[7][7] / A2UUA2UU << endl;
...
vector<FullAVVBasis> GetModels()
{
   ...
   Models[10].AVV.A3VAR = 1;
   Models[10].AVV.A3AVR = 1;

   Models[11].AVV.A3VZR = 1;
   Models[11].AVV.A3ZVR = 1;
