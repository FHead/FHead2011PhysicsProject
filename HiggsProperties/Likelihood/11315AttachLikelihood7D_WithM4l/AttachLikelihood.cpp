...
int main(int argc, char *argv[])
{
   ...
   for(int iEntry = 0; iEntry < EntryCount; iEntry++)
   {
      ...
      else
      {
         ...
         double HMass = index.HiggsMass;
         index.HiggsMass = 0;
         vector<double> &LikelihoodVector = Likelihoods[index];

         // cout << HMass << endl;

         if(HMass < HiggsMass[0])
            logLikelihood = 0;
         else if(HMass >= HiggsMass[NBinsHiggs-1])
            logLikelihood = 0;
         else
         {
            for(int i = 0; i < NBinsHiggs - 1; i++)
            {
               if(HMass < HiggsMass[i] || HMass >= HiggsMass[i+1])
                  continue;

               double likelihood = LikelihoodVector[i] / Integral[i]
                  * (HiggsMass[i+1] * HiggsMass[i+1] - HMass * HMass)
                  + LikelihoodVector[i+1] / Integral[i+1]
                  * (HMass * HMass - HiggsMass[i] * HiggsMass[i]);
