...
int main()
{
   ...
   */
   ScalarIntegral.push_back(1.16326e14);
   ScalarIntegral.push_back(8.72926e14);
   ScalarIntegral.push_back(2.11677e15);
   ScalarIntegral.push_back(4.72246e15);
   ScalarIntegral.push_back(1.95567e16);
   PseudoscalarIntegral.push_back(1.22008e12);
   PseudoscalarIntegral.push_back(8.18179e12);
   PseudoscalarIntegral.push_back(2.11693e13);
   PseudoscalarIntegral.push_back(5.14443e13);
   PseudoscalarIntegral.push_back(2.50260e14);
   
   for(int i = 0; i < (int)ScalarMaps.size(); i++)
   {
      if(ScalarIntegral[i] < 0)
      {
         cerr << "Integral error, exit now..." << endl;
         return -1;
      }
   }
   for(int i = 0; i < (int)PseudoscalarMaps.size(); i++)
   ...
   vector<double> Integral1, vector<double> Integral2, string ResultLocation, int ToyCount)
{
      ...
      i < NBinsPhi0 * NBinsTheta0 * NBinsPhi * NBinsTheta1 * NBinsTheta2 * NBinsMass1 * NBinsMass2; i++)
   {
      ...
      vector<double> Value1(MapCount);   // These are bin values
   ...
   const double Mean, const double Gamma)
{
   ...
   for(int i = 0; i < BinCount; i++)
   {
      ...
      double Value = Values[Index+1] * (Mass * Mass - HiggsMasses[Index] * HiggsMasses[Index])
         + Values[Index] * (HiggsMasses[Index+1] * HiggsMasses[Index+1] - Mass * Mass);
      Value = Value / (HiggsMasses[Index+1] * HiggsMasses[Index+1] - HiggsMasses[Index] * HiggsMasses[Index]);
...
double Interpolate(const vector<double> &HiggsMasses, const vector<double> &Values, const double Mass)
{
   ...
   double Result = Values[Min] * (HiggsMasses[Max] * HiggsMasses[Max] - Mass * Mass)
      + Values[Max] * (Mass * Mass - HiggsMasses[Min] * HiggsMasses[Min]);
   Result = Result / (HiggsMasses[Max] * HiggsMasses[Max] - HiggsMasses[Min] * HiggsMasses[Min]);
