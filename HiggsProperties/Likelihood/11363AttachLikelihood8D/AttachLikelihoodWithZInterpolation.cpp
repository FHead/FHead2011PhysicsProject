...
bool Index::operator <(const Index &other) const
{
   ...
   if(Z1Mass < other.Z1Mass)   return true;
   if(Z1Mass > other.Z1Mass)   return false;

   if(Z2Mass < other.Z2Mass)   return true;
   if(Z2Mass > other.Z2Mass)   return false;
...
bool Index::operator ==(const Index &other) const
{
   ...
   if(HiggsMass != other.HiggsMass)
      return false;
   if(Z1Mass != other.Z1Mass)
      return false;
   if(Z2Mass != other.Z2Mass)
      return false;
...
int main(int argc, char *argv[])
{
   ...
   cout << NBinsMass1 << " " << Mass1Min << " " << Mass1Max << endl;
   ...
   map<Index, PartialMap> Likelihoods;
   ...
   for(int iEvent = 0; iEvent < EntryCount; iEvent++)
   {
      ...
      EventParameters Angles = ConvertVectorsToAngles(Leptons, false);
      // cout << Angles << endl;
      ...
      index.Z1Mass = Angles.ZMass / Angles.HMass;
      index.Z2Mass = Angles.Z2Mass / Angles.HMass;
         ...
         for(int iPhi = 0; iPhi < NBinsPhi; iPhi++)
         {
            for(int iTheta1 = 0; iTheta1 < NBinsTheta1; iTheta1++)
            {
               for(int iTheta2 = 0; iTheta2 < NBinsTheta2; iTheta2++)
               {
                     ...
                     for(int iZ2Mass = 0; iZ2Mass < NBinsMass2; iZ2Mass++)
                     {
                        ...
                        for(int i = 0; i < NBinsHiggs; i++)
                        {
                           ...
                           *(in[i]) >> TempString;
                           ...
                           Temp.M[iZ1Mass][iZ2Mass][i] = TempValue;

                           // cout << iZ1Mass << "-" << iZ2Mass << "-" << i << " " << TempValue << endl;
                        }
                        if(BadBin == true)
                        {
                           Keep = false;
                        ...
                        for(int i = 0; i < NBinsHiggs; i++)
                           Integral[i] = Integral[i] + Temp.M[iZ1Mass][iZ2Mass][i] * HiggsMass[i] * HiggsMass[i];
                     }
                  }
                  ...
                  /*
                  for(int i = 0; i < NBinsMass1; i++)
                  {
                     for(int j = 0; j < NBinsMass2; j++)
                     {
                        for(int k = 0; k < NBinsHiggs; k++)
                           cout << " " << Temp.M[i][j][k];
                        cout << endl;
                     }
                     cout << endl;
                  ...
                  */
                  ...
                  {
                     Likelihoods.insert(pair<Index, PartialMap>(ThisEntry, Temp));
   ...
   for(int iEntry = 0; iEntry < EntryCount; iEntry++)
   {
      ...
      else
      {
         ...
         double EventZ1Mass = index.Z1Mass;
         double EventZ2Mass = index.Z2Mass;
         index.HiggsMass = 0;
         index.Z1Mass = 0;
         index.Z2Mass = 0;
         PartialMap &LikelihoodVector = Likelihoods[index];
         ...
         else
         {
            for(int i = 0; i < NBinsHiggs - 1; i++)
            {
               ...
               for(int j = 0; j < NBinsMass1 - 1; j++)
               {
                  if(EventZ1Mass < (Mass1Max - Mass1Min) / NBinsMass1 * j + Mass1Min
                     || EventZ1Mass >= (Mass1Max - Mass1Min) / NBinsMass1 * (j + 1) + Mass1Min)
                     continue;

                  for(int k = 0; k < NBinsMass2 - 1; k++)
                  {
                     if(EventZ2Mass < (Mass2Max - Mass2Min) / NBinsMass2 * k + Mass2Min
                        || EventZ2Mass >= (Mass2Max - Mass2Min) / NBinsMass2 * (k + 1) + Mass2Min)
                        continue;

                     int IL = j, IR = j + 1, IT = k, IB = k + 1;

                     // cout << IL << " " << IR << " " << IT << " " << IB << endl;

                     if(EventZ1Mass < (Mass1Max - Mass1Min) / NBinsMass1 * (j + 0.5) + Mass1Min)
                     {
                        IL = j - 1;
                        IR = j;
                     }
                     if(EventZ2Mass < (Mass2Max - Mass2Min) / NBinsMass2 * (k + 0.5) + Mass2Min)
                     {
                        IT = k - 1;
                        IB = k;
                     }

                     double CellL = (Mass1Max - Mass1Min) / NBinsMass1 * (IL + 0.5) + Mass1Min;
                     double CellT = (Mass2Max - Mass2Min) / NBinsMass2 * (IT + 0.5) + Mass2Min;
                     double CellR = (Mass1Max - Mass1Min) / NBinsMass1 * (IR + 0.5) + Mass1Min;
                     double CellB = (Mass2Max - Mass2Min) / NBinsMass2 * (IB + 0.5) + Mass2Min;

                     if(IL < 0)             IL = 0;
                     if(IR >= NBinsMass1)   IR = NBinsMass1 - 1;
                     if(IT < 0)             IT = 0;
                     if(IB >= NBinsMass2)   IB = NBinsMass2 - 1;

                     double TL1 = LikelihoodVector.M[IL][IT][i];
                     double TR1 = LikelihoodVector.M[IR][IT][i];
                     double BL1 = LikelihoodVector.M[IL][IB][i];
                     double BR1 = LikelihoodVector.M[IR][IB][i];
                     double TL2 = LikelihoodVector.M[IL][IT][i+1];
                     double TR2 = LikelihoodVector.M[IR][IT][i+1];
                     double BL2 = LikelihoodVector.M[IL][IB][i+1];
                     double BR2 = LikelihoodVector.M[IR][IB][i+1];

                     /*
                     double Likelihood1 = TL1 * (CellR - EventZ1Mass) * (CellB - EventZ2Mass)
                        + TR1 * (EventZ1Mass - CellL) * (CellB - EventZ2Mass)
                        + BL1 * (CellR - EventZ1Mass) * (EventZ2Mass - CellT)
                        + BR1 * (EventZ1Mass - CellL) * (EventZ2Mass - CellT);
                     Likelihood1 = Likelihood1 / (CellR - CellL) / (CellB - CellT);
                     
                     double Likelihood2 = TL2 * (CellR - EventZ1Mass) * (CellB - EventZ2Mass)
                        + TR2 * (EventZ1Mass - CellL) * (CellB - EventZ2Mass)
                        + BL2 * (CellR - EventZ1Mass) * (EventZ2Mass - CellT)
                        + BR2 * (EventZ1Mass - CellL) * (EventZ2Mass - CellT);
                     Likelihood2 = Likelihood2 / (CellR - CellL) / (CellB - CellT);
                     */
                     ...
                     double EventZ1Mass_2 = EventZ1Mass * EventZ1Mass;
                     double EventZ2Mass_2 = EventZ2Mass * EventZ2Mass;

                     double Likelihood1 = TL1 * (CellR - EventZ1Mass_2) * (CellB - EventZ2Mass_2)
                        + TR1 * (EventZ1Mass_2 - CellL) * (CellB - EventZ2Mass_2)
                        + BL1 * (CellR - EventZ1Mass_2) * (EventZ2Mass_2 - CellT)
                        + BR1 * (EventZ1Mass_2 - CellL) * (EventZ2Mass_2 - CellT);
                     Likelihood1 = Likelihood1 / (CellR - CellL) / (CellB - CellT);
                     
                     double Likelihood2 = TL2 * (CellR - EventZ1Mass_2) * (CellB - EventZ2Mass_2)
                        + TR2 * (EventZ1Mass_2 - CellL) * (CellB - EventZ2Mass_2)
                        + BL2 * (CellR - EventZ1Mass_2) * (EventZ2Mass_2 - CellT)
                        + BR2 * (EventZ1Mass_2 - CellL) * (EventZ2Mass_2 - CellT);
                     Likelihood2 = Likelihood2 / (CellR - CellL) / (CellB - CellT);

                     double likelihood = Likelihood1 / Integral[i]
                        * (HiggsMass[i+1] * HiggsMass[i+1] - EventHMass * EventHMass)
                        + Likelihood2 / Integral[i+1]
