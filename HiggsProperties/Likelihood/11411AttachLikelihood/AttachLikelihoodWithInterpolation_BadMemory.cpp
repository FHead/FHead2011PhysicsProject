...
int main(int argc, char *argv[])
{
   ...
   for(int iEvent = 0; iEvent < EntryCount; iEvent++)
   {
      ...
      for(int i = 0; i < 2187; i++)
      {
         ...
         for(int j = 0; j < 7; j++)
         {
            newindex.Indices[j] = index.Indices[j] + Neighbors[j][i];

            if(Cyclic[j] == true)
            {
               if(newindex.Indices[j] < 0)            newindex.Indices[j] = BinMax[j] - 1;
               if(newindex.Indices[j] >= BinMax[j])   newindex.Indices[j] = 0;
            }
            else
            {
               if(newindex.Indices[j] < 0)            Bad = true;
               if(newindex.Indices[j] >= BinMax[j])   Bad = true;
            }
         }
         if(Bad == true)
            continue;
