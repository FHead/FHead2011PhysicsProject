...
bool CheckInRange(Index &index, int BinMax[]);
//---------------------------------------------------------------------------
class Index
{
public:
...
int main(int argc, char *argv[])
{
   ...
   for(int iEvent = 0; iEvent < EntryCount; iEvent++)
   {
      ...
      if(Angles.ZMass + Angles.Z2Mass < Angles.HMass && CheckInRange(index, BinMax) == true)
      {
         for(int i = 0; i < 128; i++)
         {
            Index newindex;
...
{
   for(int i = 0; i < 7; i++)
      if(index.Indices[i] < 0 || index.Indices[i] >= BinMax[i])
         return false;
