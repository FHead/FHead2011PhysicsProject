...
int main(int argc, char *argv[])
{
   ...
   for(int iEvent = 0; iEvent < EntryCount; iEvent++)
   {
      ...
      Leptons.Lepton11 = Leptons.Lepton11 * (1 + 0.20 * GaussianBoxMuller());
      Leptons.Lepton12 = Leptons.Lepton12 * (1 + 0.20 * GaussianBoxMuller());
      Leptons.Lepton21 = Leptons.Lepton21 * (1 + 0.20 * GaussianBoxMuller());
      Leptons.Lepton22 = Leptons.Lepton22 * (1 + 0.20 * GaussianBoxMuller());

      // cout << 1 + 0.03 * GaussianBoxMuller() << endl;
   ...
   double treeLikelihood = 0, treeIntegral = 0;
   TBranch *bLikelihood1 = Tree->Branch(BranchName.c_str(), &treeLikelihood, (BranchName + "/D").c_str());
   TBranch *bLikelihood2 = Tree->Branch((BranchName + "_Integral").c_str(), &treeIntegral, (BranchName + "_Integral/D").c_str());
   ...
   for(int iEntry = 0; iEntry < EntryCount; iEntry++)
   {
      ...
      else
      {
         ...
         if(NBinsHiggs == 1)   // no interpolation, just look up the f**king number
         {
            double X[7] = {0};
            X[0] = Angles.Phi0;
            X[1] = cos(Angles.Theta0);
            ...
            for(int i = 0; i < 128; i++)
            {
               ...
               IndividualLikelihoods[i] = LikelihoodVector[0] / Integral[0];
            }
            ...
            treeLikelihood = FinalLikelihood;
            treeIntegral = Integral[0];
         }
         else if(Angles.HMass >= HiggsMass[0] && Angles.HMass < HiggsMass[NBinsHiggs-1])
         {
            int IHL = 0;
            int IHR = 0;
            ...
            treeLikelihood = FinalLikelihood;
            treeIntegral = -1;   // not implemented yet due to laziness
         }
      }
...
double GetNextRandomNumber()
{
   static double X = 0.732;
   static double Mu = 3.9994;
...
double GaussianBoxMuller()
{
   double x1 = GetNextRandomNumber();
   double x2 = GetNextRandomNumber();
