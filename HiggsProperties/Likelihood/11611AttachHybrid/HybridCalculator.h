...
private:
   ...
   double (*CalculateMEValue)(LeptonVectors &);
public:
   double SimpleMassIntegral(const EventParameters &TargetParameters);
private:
   double CalculateIntegrand(const EventParameters &TargetParameters, const EventParameters &CentralParameters);
   void CalculateMEs(const EventParameters &CentralValue, double &ME0, double &B1, double &B3);
