   ...
   for(int iM1 = 0; iM1 <= MassStep; iM1++)
   {
      ...
      for(int iM2 = 0; iM2 <= MassStep; iM2++)
      {
         ...
         */
         ...
         CalculateIntegrand(TargetParameters, CentralParameters, Contribution);
...
double HybridCalculator::CalculateF(double M, double M0, int Order, bool FirstZ)
{
   ...
   for(int i = 0; i < InitialStepSize; i++)
   {
      ...
      Integral = Integral + AdaptCalculateF(sab, fa, fc, fb, a, b, ToleranceF / InitialStepSize, TargetC1C2, Order, FirstZ, 0);
   }
