...
int main(int argc, char *argv[])
{
   ...
   CalculatorEM.SetToleranceF(1e-8);
   CalculatorEM.SetSimpleMassIntegralStep(120);
   CalculatorEM.SetSimpleMassIntegralWindow(1.2);
   CalculatorEM.SetExpansionOrder(1);
   CalculatorEM.SetDoDeltaMH(true, 125);
   CalculatorEM.SetIgnorePhi(true);

   CalculatorEM.SetMECalculator(CalculateA1RA1RSignal);
   CalculatorEM.SetMECalculator(CalculateA2RA2RSignal);
   CalculatorEM.SetMECalculator(CalculateA2IA2ISignal);
   CalculatorEM.SetMECalculator(CalculateA3RA3RSignal);
   CalculatorEM.SetMECalculator(CalculateA3IA3ISignal);
   CalculatorEM.SetMECalculator(CalculateA1RA2RSignal);
   CalculatorEM.SetMECalculator(CalculateA1RA2ISignal);
   CalculatorEM.SetMECalculator(CalculateA1RA3RSignal);
   CalculatorEM.SetMECalculator(CalculateA1RA3ISignal);
   CalculatorEM.SetMECalculator(CalculateA2RA2ISignal);
   CalculatorEM.SetMECalculator(CalculateA2RA3RSignal);
   CalculatorEM.SetMECalculator(CalculateA2RA3ISignal);
   CalculatorEM.SetMECalculator(CalculateA2IA3RSignal);
   CalculatorEM.SetMECalculator(CalculateA2IA3ISignal);
   CalculatorEM.SetMECalculator(CalculateA3RA3ISignal);
   ...
   CalculatorEE.SetToleranceF(1e-8);
   CalculatorEE.SetSimpleMassIntegralStep(120);
   CalculatorEE.SetSimpleMassIntegralWindow(1.2);
   ...
   for(int iEvent = StartEvent; iEvent < EventCount && iEvent <= EndEvent; iEvent++)
   {
      ...
      if(PassBaselineCut(Angles) == true)
      {
         if(l1id == 11 && l3id == 11)
         {
            ...
            cout << Integral[0] << " " << AlternativeIntegral[0] << endl;
            cout << Integral[1] << " " << AlternativeIntegral[1] << endl;
            cout << Integral[2] << " " << AlternativeIntegral[2] << endl;
            cout << Integral[3] << " " << AlternativeIntegral[3] << endl;
            cout << Integral[4] << " " << AlternativeIntegral[4] << endl;
            cout << Integral[5] << " " << AlternativeIntegral[5] << endl;
...
double CalculateBackground4e(LeptonVectors &Leptons)
{
   return 0;
   // double M4l = (Leptons.Lepton11 + Leptons.Lepton12 + Leptons.Lepton21 + Leptons.Lepton22).GetMass();
   // return (GetZZZAAABackground4e(Leptons, false, true) + GetZZZAAABackground4e(Leptons, true, true)
   //    + GetZZZAAABackground4e(Leptons, false, false) + GetZZZAAABackground4e(Leptons, true, false)) * M4l;
}
