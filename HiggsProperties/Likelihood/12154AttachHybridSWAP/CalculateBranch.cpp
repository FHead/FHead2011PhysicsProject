...
int main(int argc, char *argv[])
{
   ...
   CalculatorEM.SetToleranceF(1e-10);
   ...
   CalculatorEE.SetToleranceF(1e-10);
   ...
   for(int iEvent = StartEvent; iEvent < EventCount && iEvent <= EndEvent; iEvent++)
   {
      ...
      LeptonVectors AlternativeLeptons = Leptons;
      if((Leptons.Lepton11 + Leptons.Lepton22).GetMass() > (Leptons.Lepton21 + Leptons.Lepton12).GetMass())
         swap(AlternativeLeptons.Lepton12, AlternativeLeptons.Lepton22);
      else
         swap(AlternativeLeptons.Lepton11, AlternativeLeptons.Lepton21);

      LeptonVectors SwapLeptons = Leptons;
      LeptonVectors SwapAlternativeLeptons = AlternativeLeptons;

      swap(SwapLeptons.Lepton11, SwapLeptons.Lepton21);
      swap(SwapLeptons.Lepton12, SwapLeptons.Lepton22);
      swap(SwapAlternativeLeptons.Lepton11, SwapAlternativeLeptons.Lepton21);
      swap(SwapAlternativeLeptons.Lepton12, SwapAlternativeLeptons.Lepton22);

      EventParameters AlternativeAngles = ConvertVectorsToAnglesRoberto(AlternativeLeptons);
      EventParameters SwapAngles = ConvertVectorsToAnglesRoberto(SwapLeptons);
      EventParameters SwapAlternativeAngles = ConvertVectorsToAnglesRoberto(SwapAlternativeLeptons);
      ...
      vector<double> SwapIntegral, SwapIntegralUp, SwapIntegralDown,
         SwapIntegralWide, SwapIntegralNarrow;
      vector<double> SwapAlternativeIntegral, SwapAlternativeIntegralUp, SwapAlternativeIntegralDown,
         SwapAlternativeIntegralWide, SwapAlternativeIntegralNarrow;
      ...
      if(PassBaselineCut(Angles) == true)
      {
         if(l1id == 11 && l3id == 11)
         {
            ...
            SwapIntegral = CalculatorEE.SimpleMassIntegral(SwapAngles);
            SwapIntegralUp = CalculatorUpEE.SimpleMassIntegral(SwapAngles);
            SwapIntegralDown = CalculatorDownEE.SimpleMassIntegral(SwapAngles);
            SwapIntegralWide = CalculatorWideEE.SimpleMassIntegral(SwapAngles);
            SwapIntegralNarrow = CalculatorNarrowEE.SimpleMassIntegral(SwapAngles);

            SwapAlternativeIntegral = CalculatorEE.SimpleMassIntegral(SwapAlternativeAngles);
            SwapAlternativeIntegralUp = CalculatorUpEE.SimpleMassIntegral(SwapAlternativeAngles);
            SwapAlternativeIntegralDown = CalculatorDownEE.SimpleMassIntegral(SwapAlternativeAngles);
            SwapAlternativeIntegralWide = CalculatorWideEE.SimpleMassIntegral(SwapAlternativeAngles);
            SwapAlternativeIntegralNarrow = CalculatorNarrowEE.SimpleMassIntegral(SwapAlternativeAngles);
            ...
            for(int i = 0; i < (int)Integral.size(); i++)
            {
               ...
               Integral[i] = Integral[i] + SwapIntegral[i];
               IntegralUp[i] = IntegralUp[i] + SwapIntegralUp[i];
               IntegralDown[i] = IntegralDown[i] + SwapIntegralDown[i];
               IntegralWide[i] = IntegralWide[i] + SwapIntegralWide[i];
               IntegralNarrow[i] = IntegralNarrow[i] + SwapIntegralNarrow[i];

               Integral[i] = Integral[i] + SwapAlternativeIntegral[i];
               IntegralUp[i] = IntegralUp[i] + SwapAlternativeIntegralUp[i];
               IntegralDown[i] = IntegralDown[i] + SwapAlternativeIntegralDown[i];
               IntegralWide[i] = IntegralWide[i] + SwapAlternativeIntegralWide[i];
               IntegralNarrow[i] = IntegralNarrow[i] + SwapAlternativeIntegralNarrow[i];
            }
         }
         if(l1id == 11 && l3id == 13)
         {
            ...
            SwapIntegral = CalculatorEM.SimpleMassIntegral(SwapAngles);
            SwapIntegralUp = CalculatorUpEM.SimpleMassIntegral(SwapAngles);
            SwapIntegralDown = CalculatorDownEM.SimpleMassIntegral(SwapAngles);
            SwapIntegralWide = CalculatorWideEM.SimpleMassIntegral(SwapAngles);
            SwapIntegralNarrow = CalculatorNarrowEM.SimpleMassIntegral(SwapAngles);
         ...
         if(l1id == 13 && l3id == 11)
         {
            ...
            SwapIntegral = CalculatorME.SimpleMassIntegral(SwapAngles);
            SwapIntegralUp = CalculatorUpME.SimpleMassIntegral(SwapAngles);
            SwapIntegralDown = CalculatorDownME.SimpleMassIntegral(SwapAngles);
            SwapIntegralWide = CalculatorWideME.SimpleMassIntegral(SwapAngles);
            SwapIntegralNarrow = CalculatorNarrowME.SimpleMassIntegral(SwapAngles);
         ...
         if(l1id == 13 && l3id == 13)
         {
            ...
            SwapIntegral = CalculatorMM.SimpleMassIntegral(SwapAngles);
            SwapIntegralUp = CalculatorUpMM.SimpleMassIntegral(SwapAngles);
            SwapIntegralDown = CalculatorDownMM.SimpleMassIntegral(SwapAngles);
            SwapIntegralWide = CalculatorWideMM.SimpleMassIntegral(SwapAngles);
            SwapIntegralNarrow = CalculatorNarrowMM.SimpleMassIntegral(SwapAngles);

            SwapAlternativeIntegral = CalculatorMM.SimpleMassIntegral(SwapAlternativeAngles);
            SwapAlternativeIntegralUp = CalculatorUpMM.SimpleMassIntegral(SwapAlternativeAngles);
            SwapAlternativeIntegralDown = CalculatorDownMM.SimpleMassIntegral(SwapAlternativeAngles);
            SwapAlternativeIntegralWide = CalculatorWideMM.SimpleMassIntegral(SwapAlternativeAngles);
            SwapAlternativeIntegralNarrow = CalculatorNarrowMM.SimpleMassIntegral(SwapAlternativeAngles);
