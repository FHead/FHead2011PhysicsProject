...
double CalculateAverage(const vector<double> &Min, const vector<double> &Max, double (*F)(LeptonVectors &Leptons));
//---------------------------------------------------------------------------
class Index
{
public:
...
int main(int argc, char *argv[])
{
   ...
   double HiggsDivider[100] = {0};
   for(int i = 1; i < NBinsHiggs; i++)
      HiggsDivider[i] = (double)(HiggsMass[i] + HiggsMass[i-1]) / 2;
   HiggsDivider[0] = HiggsMass[0] * 2 - HiggsDivider[1];
   HiggsDivider[NBinsHiggs] = HiggsMass[NBinsHiggs-1] * 2 - HiggsDivider[NBinsHiggs-1];
   ...
   vector<double> RangeMin(8);
   vector<double> RangeMax(8);

   for(int iPhi0 = 0; iPhi0 < NBinsPhi0; iPhi0++)
   {
      RangeMin[3] = iPhi0 * BinSize[0] + BinStart[0];
      RangeMax[3] = (iPhi0 + 1) * BinSize[0] + BinStart[0];

      for(int iTheta0 = 0; iTheta0 < NBinsTheta0; iTheta0++)
      {
         RangeMin[4] = iTheta0 * BinSize[1] + BinStart[1];
         RangeMax[4] = (iTheta0 + 1) * BinSize[1] + BinStart[1];
         ...
         {
            RangeMin[5] = iPhi * BinSize[2] + BinStart[2];
            RangeMax[5] = (iPhi + 1) * BinSize[2] + BinStart[2];

            for(int iTheta1 = 0; iTheta1 < NBinsTheta1; iTheta1++)
            {
               RangeMin[6] = iTheta1 * BinSize[3] + BinStart[3];
               RangeMax[6] = (iTheta1 + 1) * BinSize[3] + BinStart[3];

               for(int iTheta2 = 0; iTheta2 < NBinsTheta2; iTheta2++)
               {
                  RangeMin[7] = iTheta2 * BinSize[4] + BinStart[4];
                  RangeMax[7] = (iTheta2 + 1) * BinSize[4] + BinStart[4];

                  for(int iZ1Mass = 0; iZ1Mass < NBinsMass1; iZ1Mass++)
                  {
                     RangeMin[1] = iZ1Mass * BinSize[5] + BinStart[5];
                     RangeMax[1] = (iZ1Mass + 1) * BinSize[5] + BinStart[5];
                     
                     for(int iZ2Mass = 0; iZ2Mass < NBinsMass2; iZ2Mass++)
                     {
                        RangeMin[2] = iZ2Mass * BinSize[6] + BinStart[6];
                        RangeMax[2] = (iZ2Mass + 1) * BinSize[6] + BinStart[6];
                        
                        vector<double> Temp(NBinsHiggs);
                        for(int i = 0; i < NBinsHiggs; i++)
                        {
                           RangeMin[0] = HiggsDivider[i];
                           RangeMax[0] = HiggsDivider[i+1];

                           Temp[i] = CalculateAverage(RangeMin, RangeMax, CalculateBackground2e2mu_Roberto12253);
                        }
...
double CalculateAverage(const vector<double> &Min, const vector<double> &Max, double (*F)(LeptonVectors &Leptons))
{
   ...
   for(int i = 0; i < SampleCount; i++)
   {
      ...
      Event.HMass = DrawRandom(Min[0], Max[0]);
      Event.ZMass = DrawRandom(Min[1], Max[1]);
      Event.Z2Mass = DrawRandom(Min[2], Max[2]);
      Event.Phi0 = DrawRandom(Min[3], Max[3]);
      Event.Theta0 = acos(DrawRandom(Min[4], Max[4]));
      Event.Phi = DrawRandom(Min[5], Max[5]);
      Event.Theta1 = acos(DrawRandom(Min[6], Max[6]));
      Event.Theta2 = acos(DrawRandom(Min[7], Max[7]));

      LeptonVectors Leptons = ConvertAnglesToVectorsRoberto(Event);

      Total = Total + F(Leptons) * Event.HMass * Event.ZMass * Event.Z2Mass;
   }
