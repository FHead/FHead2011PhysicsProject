...
int main(int argc, char *argv[])
{
   ...
   float zzmass, z1mass, z2mass, theta0, phi0,
      theta1, theta2, phi;
   ...
   for(int iEvent = StartEvent; iEvent < EventCount && iEvent <= EndEvent; iEvent++)
   {
      ...
      vector<double> Integral[4], IntegralUp[4], IntegralDown[4], IntegralWide[4], IntegralNarrow[4],
         IntegralHard[4], IntegralSoft[4], IntegralCentral[4], IntegralForward[4];
      vector<double> AlternativeIntegral[4], AlternativeIntegralUp[4], AlternativeIntegralDown[4],
         AlternativeIntegralWide[4], AlternativeIntegralNarrow[4], AlternativeIntegralHard[4],
         AlternativeIntegralSoft[4], AlternativeIntegralCentral[4], AlternativeIntegralForward[4];
      ...
      if(PassBaselineCut(Angles) == true)
      {
         for(int iInitial = 0; iInitial < 4; iInitial++)
         {
            if(abs(l1id) == 11 && abs(l3id) == 11)
            {
               ...
               IntegralHard[iInitial] = CalculatorHardEE[iInitial].PassThroughMode(Angles);
               IntegralSoft[iInitial] = CalculatorSoftEE[iInitial].PassThroughMode(Angles);
               IntegralForward[iInitial] = CalculatorForwardEE[iInitial].PassThroughMode(Angles);
               IntegralCentral[iInitial] = CalculatorCentralEE[iInitial].PassThroughMode(Angles);
               ...
               AlternativeIntegralHard[iInitial] = CalculatorHardEE[iInitial].PassThroughMode(AlternativeAngles);
               AlternativeIntegralSoft[iInitial] = CalculatorSoftEE[iInitial].PassThroughMode(AlternativeAngles);
               AlternativeIntegralForward[iInitial] = CalculatorForwardEE[iInitial].PassThroughMode(AlternativeAngles);
               AlternativeIntegralCentral[iInitial] = CalculatorCentralEE[iInitial].PassThroughMode(AlternativeAngles);
               ...
               for(int i = 0; i < (int)Integral[iInitial].size(); i++)
               {
                  ...
                  IntegralHard[iInitial][i] = (IntegralHard[iInitial][i] + AlternativeIntegralHard[iInitial][i]);
                  IntegralSoft[iInitial][i] = (IntegralSoft[iInitial][i] + AlternativeIntegralSoft[iInitial][i]);
                  IntegralForward[iInitial][i] = (IntegralForward[iInitial][i] + AlternativeIntegralForward[iInitial][i]);
                  IntegralCentral[iInitial][i] = (IntegralCentral[iInitial][i] + AlternativeIntegralCentral[iInitial][i]);
               }
            }
            else if(abs(l1id) == 11 && abs(l3id) == 13)
            {
               ...
               IntegralHard[iInitial] = CalculatorHardEM[iInitial].PassThroughMode(Angles);
               IntegralSoft[iInitial] = CalculatorSoftEM[iInitial].PassThroughMode(Angles);
               IntegralForward[iInitial] = CalculatorForwardEM[iInitial].PassThroughMode(Angles);
               IntegralCentral[iInitial] = CalculatorCentralEM[iInitial].PassThroughMode(Angles);
            }
            else if(abs(l1id) == 13 && abs(l3id) == 11)
            {
               ...
               IntegralHard[iInitial] = CalculatorHardME[iInitial].PassThroughMode(Angles);
               IntegralSoft[iInitial] = CalculatorSoftME[iInitial].PassThroughMode(Angles);
               IntegralForward[iInitial] = CalculatorForwardME[iInitial].PassThroughMode(Angles);
               IntegralCentral[iInitial] = CalculatorCentralME[iInitial].PassThroughMode(Angles);
            }
            else if(abs(l1id) == 13 && abs(l3id) == 13)
            {
               ...
               IntegralHard[iInitial] = CalculatorHardMM[iInitial].PassThroughMode(Angles);
               IntegralSoft[iInitial] = CalculatorSoftMM[iInitial].PassThroughMode(Angles);
               IntegralForward[iInitial] = CalculatorForwardMM[iInitial].PassThroughMode(Angles);
               IntegralCentral[iInitial] = CalculatorCentralMM[iInitial].PassThroughMode(Angles);
               ...
               AlternativeIntegralHard[iInitial] = CalculatorHardMM[iInitial].PassThroughMode(AlternativeAngles);
               AlternativeIntegralSoft[iInitial] = CalculatorSoftMM[iInitial].PassThroughMode(AlternativeAngles);
               AlternativeIntegralForward[iInitial] = CalculatorForwardMM[iInitial].PassThroughMode(AlternativeAngles);
               AlternativeIntegralCentral[iInitial] = CalculatorCentralMM[iInitial].PassThroughMode(AlternativeAngles);
      ...
      //    double Total = 0;
      //    for(int iInitial = 0; iInitial < 4; iInitial++)
      //       Total = Total + IntegralUp[iInitial][i];
      //    cout << " " << Total;
      // }
      // cout << endl;
      ...
      //    double Total = 0;
      //    for(int iInitial = 0; iInitial < 4; iInitial++)
      //       Total = Total + IntegralDown[iInitial][i];
      //    cout << " " << Total;
      // }
      // cout << endl;
      ...
      //    double Total = 0;
      //    for(int iInitial = 0; iInitial < 4; iInitial++)
      //       Total = Total + IntegralWide[iInitial][i];
      //    cout << " " << Total;
      // }
      // cout << endl;
      ...
      //    double Total = 0;
      //    for(int iInitial = 0; iInitial < 4; iInitial++)
      //       Total = Total + IntegralNarrow[iInitial][i];
      //    cout << " " << Total;
      // }
      // cout << endl;
      // cout << FileName << " BSpectrumHard " << iEvent;
      // for(int i = 0; i < (int)IntegralHard[0].size(); i++)
      // {
      //    double Total = 0;
      //    for(int iInitial = 0; iInitial < 4; iInitial++)
      //       Total = Total + IntegralHard[iInitial][i];
      //    cout << " " << Total;
      // }
      // cout << endl;
      // cout << FileName << " BSpectrumSoft " << iEvent;
      // for(int i = 0; i < (int)IntegralSoft[0].size(); i++)
      // {
      //    double Total = 0;
      //    for(int iInitial = 0; iInitial < 4; iInitial++)
      //       Total = Total + IntegralSoft[iInitial][i];
      //    cout << " " << Total;
      // }
      // cout << endl;
      // cout << FileName << " BSpectrumCentral " << iEvent;
      // for(int i = 0; i < (int)IntegralCentral[0].size(); i++)
      // {
      //    double Total = 0;
      //    for(int iInitial = 0; iInitial < 4; iInitial++)
      //       Total = Total + IntegralCentral[iInitial][i];
      //    cout << " " << Total;
      // }
      // cout << endl;
      // cout << FileName << " BSpectrumForward " << iEvent;
      // for(int i = 0; i < (int)IntegralForward[0].size(); i++)
      // {
      //    double Total = 0;
      //    for(int iInitial = 0; iInitial < 4; iInitial++)
      //       Total = Total + IntegralForward[iInitial][i];
      //    cout << " " << Total;
      // }
      // cout << endl;
   }
