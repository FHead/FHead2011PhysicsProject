...
int main(int argc, char *argv[])
{
   ...
   string MappingFile = "DuplicateEventMapping_8TeV.txt";

   if(argc < 5)
   {
      cerr << "Usage: " << argv[0] << " BranchNames Values FileName MappingFile" << endl;
      return -1;
   }
   ...
   // Read Mapping
   ifstream in3(MappingFile.c_str());
   vector<int> Mappings;
   while(in3)
   {
      int A = -1, B = -1;
      ...
      if(Mappings.size() <= A)   // not really ideal, but fine for data
      {
         Mappings.insert(Mappings.end(), A + 1 - Mappings.size(), -1);
         Mappings.resize(A + 1);
      }
   ...
   for(int iEntry = 0; iEntry < EntryCount; iEntry++)
   {
      ...
      if(BranchValues.find(iEntry) != BranchValues.end())
         for(int i = 0; i < (int)BranchNames.size(); i++)
            TreeVector[i] = BranchValues[Mappings[iEntry]][i];
