...
do
   ...
   do
      ...
      do
         ...
         Component1=`echo $j | cut -d '_' -f 3 | sed "s/.txt//g"`
         Component2=`echo $j | cut -d '_' -f 4 | sed "s/.txt//g"`

         Source=Cleaned/Function_${k}_${Component1}_${Component2}

         FunctionName=GetSpin2XVV_${k}_${Component1}_${Component2}_15721

         echo "double $FunctionName(LeptonVectors &leptons, ZPrimeParameters &P);" >> Header
         ...
         if [ "`cat $j | wc -c`" -gt 2 ]; then
            ...
            echo "double $FunctionName(LeptonVectors &leptons, ZPrimeParameters &P)" >> $Source
            echo "{" >> $Source
