...
int main(int argc, char *argv[])
{
   ...
   string Name3[3][10] =
      ...
      {"DeltaCZ", "DeltaCW", "CZZ", "CZZD", "CZA", "CZAD", "CAA", "CAAD", "Fem", "Fee"},
   ...
   for(int i = 0; i < 3; i++)
   {
      for(int j = 0; j < 2; j++)
      {
         for(int k = 0; k < 10; k++)
         {
            for(int l = 0; l < 256; l++)
            {
               string ID = Name1[i] + "_" + Name3[i][k] + "_" + Type + "_" + Cut + "_" + "Count" + "_" + Name4[l] + "_" + Name2[j] + "_" + "Point" + Point;
               G[i][j][k][l].SetNameTitle(ID.c_str(), ID.c_str());
            }
         }
      }
   }
   ...
   for(int iN = 0; iN < 10; iN++)
   {
      ...
      double Total[3][2][10][256] = {{{0}}};
      int Count[3][2][10][256] = {{{0}}};
      ...
      for(int iE = 0; iE < EntryCount; iE++)
      {
         ...
         {
            TrueValue[0] = TrueH.DeltaCZ;         TrueValue[1] = TrueH.DeltaCW;
         ...
         for(int i = 0; i < 10; i++)
         {
            Total[IntBasis][IntNoSquareMode][i][FloatIndex] = Total[IntBasis][IntNoSquareMode][i][FloatIndex] + fabs(P[i] - TrueValue[i]);
            Count[IntBasis][IntNoSquareMode][i][FloatIndex] = Count[IntBasis][IntNoSquareMode][i][FloatIndex] + 1;
         }
      }
         ...
         for(int j = 0; j < 2; j++)
         {
            for(int k = 0; k < 10; k++)
            {
               for(int l = 0; l < 256; l++)
               {
                  ...
                  int Index = G[i][j][k][l].GetN();

                  G[i][j][k][l].SetPoint(Index, N[iN] * Factor, Total[i][j][k][l] / Count[i][j][k][l] * sqrt(PI / 2));
               }
            }
         }
      }
