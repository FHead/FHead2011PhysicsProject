...
int main(int argc, char *argv[])
{
   ...
   for(int iE = 0; iE < EntryCount; iE++)
   {
      ...
      if(Sigmas.find(IntL) == Sigmas.end())
         Sigmas.insert(pair<int, vector<double> >(IntL, vector<double>()));

      Sigmas[IntL].push_back(S);
   }
   ...
   {
      if(Sigmas.find(Ls[iL]) == Sigmas.end())
         continue;

      vector<double> &Data = Sigmas[Ls[iL]];
