...
int main(int argc, char *argv[])
{
   ...
   if(argc != 7)
   {
      cerr << "Usage: " << argv[0] << " NullFile AltFile Output Tag Fixed Cut" << endl;
      return -1;
   }
   ...
   TGraphAsymmErrors GGG;
   GGG.SetNameTitle(("ERR_" + Tag + "_AverageNS").c_str(), "");

   DataHelper DHFile("Yields.dh");
   double Yield = 0;
   Yield = Yield + DHFile["SMVV_2e2mu"][Cut].GetDouble() / 2;
   Yield = Yield + DHFile["SMVV_4e"][Cut].GetDouble() / 2;
   Yield = Yield + DHFile["5plet_2e2mu"][Cut].GetDouble() / 2;
   Yield = Yield + DHFile["5plet_4e"][Cut].GetDouble() / 2;
   ...
   for(map<int, vector<double> >::iterator iter = Data1.begin(); iter != Data1.end(); iter++)
   {
      ...
      if(Lumi >= 200)
      {
         ...
         GG.SetPointError(GG.GetN() - 1, 0, 0,
            P - PValues[PValues.size()*0.1585],
            PValues[PValues.size()*0.8415] - P);
         GGG.SetPointError(GGG.GetN() - 1, 0, 0,
            P - PValues[PValues.size()*0.1585],
            PValues[PValues.size()*0.8415] - P);
      }
   }
