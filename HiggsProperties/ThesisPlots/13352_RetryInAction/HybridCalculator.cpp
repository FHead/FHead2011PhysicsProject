...
//----------------------------------------------------------------------------
void HybridCalculator::SetDoReboot(bool Decision)
{
   if(Verbosity >= VerboseLevel_Normal)
      std::cerr << "[HybridCalculator] Smart center set to " << TextBool(Decision) << std::endl;
...
void HybridCalculator::PrintSettings(std::ostream &out)
{
   ...
   out << "Rebooting: " << TextBool(DoReboot) << std::endl;
...
std::vector<double> HybridCalculator::SimpleMassIntegral(const EventParameters &TargetParameters)
{
   ...
   double RM = (RecoMM.MAA_2 + RecoMM.MAB_2 + RecoMM.MBA_2 + RecoMM.MBB_2) / sqrt(RecoMM.M12_2 * RecoMM.M34_2);
   ...
   // std::cout << "Efficiency = " << RecoEfficiency << std::endl;
   // std::cout << "E11 " << L11 << " " << Efficiency12(L11.GetPT(), L11[3] / L11.GetP(), 0) << std::endl;
   // std::cout << "E12 " << L12 << " " << Efficiency12(L12.GetPT(), L12[3] / L12.GetP(), 0) << std::endl;
   // std::cout << "E21 " << L21 << " " << Efficiency34(L21.GetPT(), L21[3] / L21.GetP(), 0) << std::endl;
   // std::cout << "E22 " << L22 << " " << Efficiency34(L22.GetPT(), L22[3] / L22.GetP(), 0) << std::endl;
   ...
   double MassWindowPlusFactor = 1, MassWindowMinusFactor = 1;
   ...
   if(UseSmartCenter == false)
   {
      if(DoDiagonal == true)
      {
         MassCenterPlus = TargetParameters.HMass * TargetParameters.HMass;
      ...
      if(DoDeltaMH == true)
      {
         MassCenterPlus = MassCenterPlus / TargetParameters.HMass * DeltaMH / TargetParameters.HMass * DeltaMH;
   ...
   else
   {
      ...
      {
         double M1 = CenterResult.first;
         double M2 = CenterResult.second;
         MassCenterPlus = M1 * M1 + M2 * M2 + M1 * M2 * RM;
         MassCenterMinus = M1 - M2;
      }
   }
   ...
   for(int iTry = 0; iTry < 8 && DoIntegral == true; iTry++)
   {
      ...
      if(DoDiagonal == false)
      {
         FillMassPosition(MassStepPlus, MassCenterPlus, MassWindowPlus * MassWindowPlusFactor,
            SimpleMassIntegralAttractorStrengthPlus, MassPlus);
         FillMassPosition(MassStepMinus, MassCenterMinus, MassWindowMinus * MassWindowMinusFactor,
            SimpleMassIntegralAttractorStrengthMinus, MassMinus);
      }
      else
      {
         FillMassPosition(MassStepPlus, MassCenterPlus, MassWindowPlus * DeltaMH * MassWindowPlusFactor,
            SimpleMassIntegralAttractorStrengthPlus, MassPlus);
         FillMassPosition(MassStepMinus, MassCenterMinus, MassWindowMinus * MassWindowMinusFactor,
            SimpleMassIntegralAttractorStrengthMinus, MassMinus);
      }
      ...
      double Plus1 = 0, PlusN = 0, PlusNN = 0;
      double Minus1 = 0, MinusN = 0, MinusNN = 0;
      ...
      for(int iMPlus = 0; iMPlus <= MassStepPlus; iMPlus++)
      {
         ...
         for(int iMMinus = 0; iMMinus <= MassStepMinus; iMMinus++)
         {
            ...
            if(DoDiagonal == true)
            {
               double Delta = ZMassMinus / 2;
               double MBar = sqrt((ZMassPlus - (2 - RM) * Delta * Delta) / (2 + RM));
            ...
            if(DebugPrintIntegrand == true)
               std::cout << iTry << " " << iMPlus << " " << iMMinus << " "
                  << ZMass << " " << Z2Mass << " " << Contribution[0] << std::endl;

            // Update best point
            if(fabs(Contribution[0]) > BestContribution)
            {
               BestIMPlus = iMPlus;
               BestIMMinus = iMMinus;
               BestContribution = fabs(Contribution[0]);
            }

            // Update counts
            Plus1 = Plus1 + Contribution[0];
            PlusN = PlusN + Contribution[0] * iMPlus;
            PlusNN = PlusNN + Contribution[0] * iMPlus * iMPlus;
            Minus1 = Minus1 + Contribution[0];
            MinusN = MinusN + Contribution[0] * iMMinus;
            MinusNN = MinusNN + Contribution[0] * iMMinus * iMMinus;
      ...
      if(DoReboot == true)
      {
         ...
         {
            double GoodRange = 0.05;   // within middle 5%

            if(fabs((double)BestIMPlus / MassStepPlus * 2 - 1) > GoodRange
               || fabs((double)BestIMMinus / MassStepMinus * 2 - 1) > GoodRange)
            {
               // std::cerr << "Rebooting!" << std::endl;
               // std::cerr << "(" << MassCenterPlus << ", " << MassCenterMinus << ") => "
               //    << "(" << MassPlus[BestIMPlus] << ", " << MassMinus[BestIMMinus] << ")" << std::endl;
               // std::cerr << "Contribution = " << BestContribution << std::endl;

               DoIntegral = true;
            }
            else
            {
               double RMSPlus = sqrt(PlusNN - PlusN * PlusN / Plus1) / sqrt(Plus1);
               double RMSMinus = sqrt(MinusNN - MinusN * MinusN / Minus1) / sqrt(Minus1);

               // std::cerr << "RMSs = " << RMSPlus << " " << RMSMinus << std::endl;
               ...
               {
                  DoIntegral = true;
                  MassWindowPlusFactor = MassWindowPlusFactor * pow(RMSPlus / MinRMS, 2.5) * 0.9;
               }
               if(RMSMinus < MinRMS)
               {
                  DoIntegral = true;
                  MassWindowMinusFactor = MassWindowMinusFactor * pow(RMSMinus / MinRMS, 2.5) * 0.9;
               }
            }
         }
         
         MassCenterPlus = MassPlus[BestIMPlus];
         MassCenterMinus = MassMinus[BestIMMinus];
      }
      else
         DoIntegral = false;
   }
