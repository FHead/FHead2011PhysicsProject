...
int main()
{
   ...
   double PVZ = -0.15;
   ThinLines.push_back(pair<V4, V4>(V4(0, 0, -1.25, 1), V4(0, 0, 20, 1)));
   Dots.push_back(V4(0, 0, PVZ, 1));
   ...
   double Front = -1;
   double Back = 1;
   Arrows.push_back(pair<V4, V4>(V4(DX * Front, Y0, DZ * Front, 1), V4(DX * Back, Y0, DZ * Back, 1)));

   double Delta = 0.1;
   double L = fabs(Y0 * Delta);
   ThinLines.push_back(pair<V4, V4>(V4(0, 0, PVZ, 1), V4(0, Y0, 0, 1)));
   ThinLines.push_back(pair<V4, V4>(V4(0, Y0 + L, PVZ * Delta, 1), V4(DX * L, Y0 + L, DZ * L + PVZ * Delta, 1)));
   ThinLines.push_back(pair<V4, V4>(V4(DX * L, Y0 + L, DZ * L + PVZ * Delta, 1), V4(DX * L, Y0, DZ * L, 1)));

   Texts.push_back(pair<V4, string>(V4(0, Y0 * 1.5, 1.25, 1), "Closest point"));
   Texts.push_back(pair<V4, string>(V4(DX * Back * 1.1, Y0, DZ * Back * 1.1, 1), "#splitline{Lepton}{track}"));
   Texts.push_back(pair<V4, string>(V4(0, Y0 * -0.4, 0, 1), "#splitline{Primary}{vertex}"));
   Texts.push_back(pair<V4, string>(V4(0, Y0 * -0.15, -1, 1), "Beamline"));

   MakePlot(Lines, Arrows, ThinLines, Dots, Texts, Screen1, "HiggsDiscovery-Selection-Leptons-ImpactParameter", false);
