...
int main()
{
   ...
   for(int i = 0; i < 8; i++)
   {
      RooFormulaVar R(Form("MethodT_%d_0_R", PTValue[i]), "R",
         Form("@0+@1*%d+@2*%d*%d", PTValue[i], PTValue[i], PTValue[i]), RooArgList(P0, P1, P2));
      ...
      RooRealVar SigmaData(Form("MethodT_%d_0_Data_Sigma", PTValue[i]), "data sigma", 1.0, 0.0, 2.0);
   ...
   for(int i = 0; i < 8; i++)
   {
      RooRealVar BackgroundMC(Form("MethodT_%d_0_MC_Background", PTValue[i]), "Background level", 1, 1, 1);
      RooRealVar BackgroundData(Form("MethodT_%d_0_Data_Background", PTValue[i]), "Background level", 1, 1, 1);
