...
string GetG(CommandLine &CL, char T);
...
void SplitUp(vector<Record> &R1, vector<Record> &R2, double MinEta, double MaxEta);
...
int main(int argc, char *argv[])
{
   ...
   SplitUp(LinesInside, LinesOutside, MinEta - R0, MaxEta + R0);
...
string ModifyFormula(string F1, string F2)
{
   ...
   for(int i = 0; i < F1.size(); i++)
   {
      ...
      int j = i + 1;
      while(j < F1.size() && F1[j] != ']')
         j = j + 1;

      int NFormula = atoi(F1.substr(i + 1, j - i - 1).c_str());
      if(NFormula + 1 > N)
         N = NFormula + 1;
   }
...
string GetG(CommandLine &CL, char T)
{
   ...
   if(XR > 0)   sprintf(R1, "(%.4f-%c)/%.4f", XL, T, R0 / sqrt(2));
   else         sprintf(R1, "(%.4f-%c)/%.4f", XL, T, R0 / sqrt(2));
   sprintf(G1, "(TMath::Erf(%s)+1)/2", R1);
   if(XL > 0)   sprintf(R2, "(%c-%.4f)/%.4f", T, XR, R0 / sqrt(2));
   else         sprintf(R2, "(%c+%.4f)/%.4f", T, -XR, R0 / sqrt(2));
