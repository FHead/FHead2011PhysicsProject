...
void JetCorrector::Initialize(std::string FileName)
{
   ...
   while(in)
   {
      ...
      else
      {
         ...
         if(CurrentFormula == "None")
            Formulas.push_back("[0]");
         else
...
double JetCorrector::GetCorrection()
{
   ...
   for(int iE = 0; iE < N; iE++)
   {
      ...
      if(Dependencies[iE].size() <= 1)
         Function = new TF1("Function", (Formulas[iE] + "+0*x").c_str());
      if(Dependencies[iE].size() == 2)
         Function = new TF2("Function", (Formulas[iE] + "+0*x+0*y").c_str());
      if(Dependencies[iE].size() == 3)
         Function = new TF3("Function", (Formulas[iE] + "+0*x+0*y+0*z").c_str());
