...
void Fill(vector<vector<TProfile *>> &P, vector<double> &PTBins, vector<double> &RhoBins, string FileName, string TreeName, double EtaMin, double EtaMax, string JECName = "");
...
int main(int argc, char *argv[])
{
   ...
   string GoodJECFileName   = CL.Get("goodjec", "");
   ...
   for(string FileName : FileNames)
      Fill(POffsetVsPhi, PTBins, RhoBins, FileName, TreeName, EtaMin, EtaMax, JECFileName);
   for(string FileName : GoodNames)
      Fill(GOffsetVsPhi, PTBins, RhoBins, FileName, TreeName, EtaMin, EtaMax, GoodJECFileName);
...
void Fill(vector<vector<TProfile *>> &P, vector<double> &PTBins, vector<double> &RhoBins, string FileName, string TreeName, double EtaMin, double EtaMax, string JECFileName)
{
   int NP = PTBins.size() - 1;
   int NR = RhoBins.size() - 1;
   ...
   double PUPT, PUEta, PUPhi, PUArea;
   ...
   Tree->SetBranchAddress("PUArea",  &PUArea);
