...
parser.add_argument("--SmoothingCheck", help = "Do we export plots for smoothing check", action = 'store_true')
...
if args.SmoothingCheck == True:
    with PdfPages('plots/SmoothingPlots.pdf') as pdf:
        for i in range(DesignCount):
            print(i)

            Y1 = savgol_filter(RawPrediction1L['Prediction'][i, :], Window, Order)
            Y2 = savgol_filter(RawPrediction2L['Prediction'][i, :], Window, Order)
            Y3 = savgol_filter(RawPrediction3L['Prediction'][i, :], Window, Order)
            Y4 = savgol_filter(RawPrediction4L['Prediction'][i, :], Window, Order)
            Y5 = savgol_filter(RawPrediction5L['Prediction'][i, :], Window, Order)
            Y6 = savgol_filter(RawPrediction6L['Prediction'][i, :], Window, Order)
            ...
            axes[0][0].plot(RawPrediction1L['Prediction'][i, :], 'bo')
            axes[0][1].plot(RawPrediction2L['Prediction'][i, :], 'bo')
            axes[1][0].plot(RawPrediction3L['Prediction'][i, :], 'bo')
            axes[1][1].plot(RawPrediction4L['Prediction'][i, :], 'bo')
            axes[2][0].plot(RawPrediction5L['Prediction'][i, :], 'bo')
            axes[2][1].plot(RawPrediction6L['Prediction'][i, :], 'bo')
...
for i in range(DesignCount):
    RawPrediction1L['Prediction'][i, :] = savgol_filter(RawPrediction1L['Prediction'][i, :], Window, Order)
    RawPrediction2L['Prediction'][i, :] = savgol_filter(RawPrediction2L['Prediction'][i, :], Window, Order)
    RawPrediction3L['Prediction'][i, :] = savgol_filter(RawPrediction3L['Prediction'][i, :], Window, Order)
    RawPrediction4L['Prediction'][i, :] = savgol_filter(RawPrediction4L['Prediction'][i, :], Window, Order)
    RawPrediction5L['Prediction'][i, :] = savgol_filter(RawPrediction5L['Prediction'][i, :], Window, Order)
    RawPrediction6L['Prediction'][i, :] = savgol_filter(RawPrediction6L['Prediction'][i, :], Window, Order)
...
if args.N >= 0:
    ...
    HoldoutDesignL = RawDesignL['Design'][0]
