...
for i in `seq 0 69`
do
   python3 RunOne.py --N $i --Model MatterLBT1 --Nu 2.5 --KernelChoice MaternNoise --Alpha 0.20 --CovType Separated
   bzip2 txt/*txt
done

for i in `seq 0 63`
do
   python3 RunOne.py --N $i --Model Matter --Nu 2.5 --KernelChoice MaternNoise --Alpha 0.20 --CovType Separated
   bzip2 txt/*txt
done
