...
if args.N >= 0:   # this is holdout test.  We don't need gazillions of samples
    np.random.shuffle(MCMCSamples)
    MCMCSamples = MCMCSamples[0:20000,:]
...
chain.likelihood_type = 0
chain.model_cov_factor = 1
chain.model_cov_modifier = 1
PosteriorValues = chain.log_posterior(MCMCSamples[0:25000,:])
print(PosteriorValues)

np.savetxt(FileName + "_PosteriorValues.txt", PosteriorValues)

chain.likelihood_type = 3
chain.model_cov_factor = 0
chain.model_cov_modifier = 0
NoModelChi2Values = chain.log_posterior(MCMCSamples[0:25000,:])

np.savetxt(FileName + "_NoModelChi2Values.txt", NoModelChi2Values)

chain.likelihood_type = 3
chain.model_cov_factor = 1
chain.model_cov_modifier = 1
YesModelChi2Values = chain.log_posterior(MCMCSamples[0:25000,:])

np.savetxt(FileName + "_YesModelChi2Values.txt", YesModelChi2Values)

HoldoutPosteriorValue = chain.log_posterior(AllData["holdoutdesign"])
print(HoldoutPosteriorValue)

np.savetxt(FileName + "_HoldoutDesign.txt", AllData["holdoutdesign"])
np.savetxt(FileName + "_HoldoutPosteriorValue.txt", HoldoutPosteriorValue)
...
# import subprocess
# subprocess.call(["bzip2", FileName + "*.txt"])
