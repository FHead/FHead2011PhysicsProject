...
int main(int argc, char *argv[])
{
   ...
   TDirectory *TkJetChargedDirectory   = OutputFile.mkdir("TkJetCharged",   "Histograms for TkJet (Charged jet)");
   ...
   TDirectory *BarrelPuppiJetDirectory = OutputFile.mkdir("BarrelPuppiJet", "Histograms for PuppiJet (eta 2.4)");
   ...
   vector<double> BarrelPuppiJetThresholds = {0, 25, 50, 75, 100, 150, 200, 250, 300};
   Histograms HBarrelPuppiJet          (200, 0, 1000, 50, -3, 3,  2000, 0, 5, BarrelPuppiJetThresholds, "BarrelPuppiJet");
   Histograms HBarrelPuppiJetNoMatching(200, 0, 1000, 50, -3, 3,  2000, 0, 5, BarrelPuppiJetThresholds, "BarrelPuppiJetNoMatching");
   ...
   for(int iE = 0; iE < EntryCount; iE++)
   {
      ...
      // Cluster charged gen jets
      vector<fastjet::PseudoJet> ChargedCandidates;
      for(int i = 0; i < (int)ChargedParticles.size(); i++)
         ChargedCandidates.push_back(fastjet::PseudoJet(ChargedParticles[i][1], ChargedParticles[i][2], ChargedParticles[i][3], ChargedParticles[i][0]));
      fastjet::ClusterSequence ChargedSequence(ChargedCandidates, Definition);
      vector<fastjet::PseudoJet> ChargedJets = ChargedSequence.inclusive_jets();
      vector<FourVector> ChargedGenJets;
      for(int i = 0; i < (int)ChargedJets.size(); i++)
         ChargedGenJets.push_back(FourVector(ChargedJets[i].e(), ChargedJets[i].px(), ChargedJets[i].py(), ChargedJets[i].pz()));


      // Best reference objects
      FourVector BestGenJet        = BestInRange(GenJets,           4.7);
      FourVector BestBarrelGenJet  = BestInRange(GenJets,           2.4);
      FourVector BestCenGenJet     = BestInRange(ChargedGenJets,    2.2);
      ...
      GenericObject BestL1Muon           = BestInRange(MPhaseII.Muon,        BestMuonWide,     2.4);
      GenericObject BestL1MuonKF         = BestInRange(MPhaseII.MuonKF,      BestMuonBarrel,   1.5);
      ...
      GenericObject BestL1TkMuon         = BestInRange(MPhaseII.TkMuon,      BestMuonWide,     2.4);
      GenericObject BestL1PuppiJet       = BestInRange(MPhaseII.PuppiJet,    BestGenJet,       4.7);
      GenericObject BestL1PuppiJetBarrel = BestInRange(MPhaseII.PuppiJet,    BestBarrelGenJet, 2.4);
      ...
      if(BestMuonWide[0] > 0 && BestL1Muon.P[0] > 0)
         HMuon.Fill(BestL1Muon.P, BestMuonWide, -1);
      if(BestMuonWide[0] > 0)
         HMuonNoMatching.Fill(BestL1Muon.P, BestMuonWide, -1);
      
      if(BestMuonBarrel[0] > 0 && BestL1MuonKF.P[0] > 0)
         HMuonKF.Fill(BestL1MuonKF.P, BestMuonBarrel, -1);
      if(BestMuonBarrel[0] > 0)
         HMuonKFNoMatching.Fill(BestL1MuonKF.P, BestMuonBarrel, -1);
      ...
      if(BestMuonWide[0] > 0 && BestL1TkMuon.P[0] > 0)
         HTkMuon.Fill(BestL1TkMuon.P, BestMuonWide, -1);
      if(BestMuonWide[0] > 0)
         HTkMuonNoMatching.Fill(BestL1TkMuon.P, BestMuonWide, -1);
      ...
      if(BestBarrelGenJet[0] > 0 && BestL1PuppiJetBarrel.P[0] > 0)
         HBarrelPuppiJet.Fill(BestL1PuppiJetBarrel.P, BestBarrelGenJet, -1);
      if(BestBarrelGenJet[0] > 0)
         HBarrelPuppiJetNoMatching.Fill(BestL1PuppiJetBarrel.P, BestBarrelGenJet, -1);
   ...
   HBarrelPuppiJet.Write(BarrelPuppiJetDirectory);
   HBarrelPuppiJetNoMatching.Write(BarrelPuppiJetDirectory);
