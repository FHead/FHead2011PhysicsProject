...
void Histograms::Fill(double check, double pt, double Eta, double iso, double dr, bool Mode)
{
...
{
   if(P[0] > 0 && Check[0] > 0)
      Fill(Check.GetPT(), P.GetPT(), P.GetEta(), iso, GetDR(Check, P), MODE_NORMAL);
   if(P[0] > 0)
      Fill(Check.GetPT(), P.GetPT(), P.GetEta(), iso, GetDR(Check, P), MODE_NOMATCH);
}
