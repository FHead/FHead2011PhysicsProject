...
int main(int argc, char *argv[])
{
   ...
   for(string InputFileName : InputFileNames)
   {
      ...
      L1PhaseIITreeV7p2Messenger MPhaseII(File, "l1PhaseIITree/L1PhaseIITree");
      ...
      for(int iE = 0; iE < EntryCount; iE++)
      {
         ...
         for(int i = 0; i < N; i++)
         {
            ...
            if(C.Quality == "None")
               ...
               Quality = QUAL_12;
            else if(C.Quality == "BarrelOddEndcap2")
            ...
            if(C.InputObject == "Jet")
               Best = BestInRange(MPhaseII.Jet, Ref, C.AbsEtaMax, C.DRCut, Quality, Range);
            else if(C.InputObject == "TrackerJet")
               Best = BestInRange(MPhaseII.TrackerJet, Ref, C.AbsEtaMax, C.DRCut, Quality, Range);
            else if(C.InputObject == "TkCaloJet")
               Best = BestInRange(MPhaseII.TkCaloJet, Ref, C.AbsEtaMax, C.DRCut, Quality, Range);
            else if(C.InputObject == "PuppiJet")
               Best = BestInRange(MPhaseII.PuppiJet, Ref, C.AbsEtaMax, C.DRCut, Quality, Range);
            else if(C.InputObject == "GlobalMuon")
               Best = BestInRange(MPhaseII.GlobalMuon, Ref, C.AbsEtaMax, C.DRCut, Quality, Range);
            else if(C.InputObject == "StandaloneMuon")
               Best = BestInRange(MPhaseII.StandaloneMuon, Ref, C.AbsEtaMax, C.DRCut, Quality, Range);
            else if(C.InputObject == "TkMuon")
               Best = BestInRange(MPhaseII.TkMuon, Ref, C.AbsEtaMax, C.DRCut, Quality, Range);
            else if(C.InputObject == "TkMuonStub")
               Best = BestInRange(MPhaseII.TkMuonStub, Ref, C.AbsEtaMax, C.DRCut, Quality, Range);
            else if(C.InputObject == "TkGlbMuon")
               Best = BestInRange(MPhaseII.TkGlbMuon, Ref, C.AbsEtaMax, C.DRCut, Quality, Range);
            else if(C.InputObject == "PFMuon")
               Best = BestInRange(MPhaseII.PFMuon, Ref, C.AbsEtaMax, C.DRCut, Quality, Range);
            else if(C.InputObject == "EG")
            {
               Best = BestInRange(MPhaseII.EG, Ref, C.AbsEtaMax, C.DRCut, Quality, Range);
               if(Best.P[0] < 0)
                  Best.Type = ((Ref.GetAbsEta() < 1.5) ? 0 : 1);
            }
            else if(C.InputObject == "TkElectron")
            {
               Best = BestInRange(MPhaseII.TkElectron, Ref, C.AbsEtaMax, C.DRCut, Quality, Range);
               if(Best.P[0] < 0)
                  Best.Type = ((Ref.GetAbsEta() < 1.5) ? 0 : 1);
            }
            else if(C.InputObject == "TkElectronLoose")
            {
               Best = BestInRange(MPhaseII.TkElectronLoose, Ref, C.AbsEtaMax, C.DRCut, Quality, Range);
               if(Best.P[0] < 0)
                  Best.Type = ((Ref.GetAbsEta() < 1.5) ? 0 : 1);
            }
            else if(C.InputObject == "TkPhoton")
               Best = BestInRange(MPhaseII.TkPhoton, Ref, C.AbsEtaMax, C.DRCut, Quality, Range);
            else if(C.InputObject == "Tau")
               Best = BestInRange(MPhaseII.Tau, Ref, C.AbsEtaMax, C.DRCut, Quality, Range);
            else if(C.InputObject == "TkTau")
               Best = BestInRange(MPhaseII.TkTau, Ref, C.AbsEtaMax, C.DRCut, Quality, Range);
         ...
         for(int i = 0; i < N; i++)
         {
            if(Configurations[i].IsolationBB > 0 || Configurations[i].IsolationEC > 0)
            {
               bool PassIsolation = true;

               if(L1Objects[i].P.GetAbsEta() < 1.5 && (L1Objects[i].Iso > Configurations[i].IsolationBB))
                  PassIsolation = false;
               if(L1Objects[i].P.GetAbsEta() > 1.5 && (L1Objects[i].Iso > Configurations[i].IsolationEC))
                  PassIsolation = false;

               if(PassIsolation == false)
                  continue;
            }

            FillHistograms(ObjectHistograms[i], L1Objects[i].P, ReferenceObjects[i], L1Objects[i].Iso, L1Objects[i].Type);
         }
      }
