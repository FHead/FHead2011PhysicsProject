...
int main(int argc, char *argv[])
{
   ...
   for(string InputFileName : InputFileNames)
   {
      ...
      L1PhaseIITreeV7p3Messenger MPhaseII(File, "l1PhaseIITree/L1PhaseIITree");
      ...
      for(int iE = 0; iE < EntryCount; iE++)
      {
         ...
         vector<GenericObject> Electrons;
         vector<GenericObject> Photons;
         vector<GenericObject> Muons;
         vector<GenericObject> Taus;
         ...
         for(int i = 0; i < (int)MGen.GenP.size(); i++)
         {
            ...
            GenericObject O;
            O.P = MGen.GenP[i];
            O.Charge = MGen.GenCharge[i];
            O.ID = MGen.GenID[i];

            if(AbsID == 11)
               Electrons.push_back(O);
            if(AbsID == 22)
               Photons.push_back(O);
            if(AbsID == 13)
               Muons.push_back(O);
         ...
         // Cluster jets
         vector<FourVector> GenJetsMomentum        = (UseStoredGen ? MGen.GenJet : ClusterJets(Particles));
         vector<FourVector> ChargedGenJetsMomentum = ClusterJets(ChargedParticles);
         
         vector<GenericObject> GenJets, ChargedGenJets;
         for(auto P : GenJetsMomentum)
            GenJets.push_back(P);
         for(auto P : ChargedGenJetsMomentum)
            ChargedGenJets.push_back(P);
         ...
         vector<GenericObject> ReferenceObjects;
         for(int i = 0; i < N; i++)
         {
            GenericObject Best;
            ...
            {
               double Isolation = CalculateGenIsolation(Best.P, Particles, 0.3);
         ...
         for(int i = 0; i < N; i++)
         {
            ...
            FourVector &Ref = ReferenceObjects[i].P;
            ...
            if(C.InputObject == "Gen")
               Best = ReferenceObjects[i];
            else if(C.InputObject == "Jet")
               Best = BestInRange(MPhaseII.Jet, Ref, C.AbsEtaMax, C.DRCut, Quality, Range, C.IsolationBB, C.IsolationEC);
            else if(C.InputObject == "TrackerJet")
               Best = BestInRange(MPhaseII.TrackerJet, Ref, C.AbsEtaMax, C.DRCut, Quality, Range, C.IsolationBB, C.IsolationEC);
            else if(C.InputObject == "TkCaloJet")
               Best = BestInRange(MPhaseII.TkCaloJet, Ref, C.AbsEtaMax, C.DRCut, Quality, Range, C.IsolationBB, C.IsolationEC);
            else if(C.InputObject == "PuppiJet")
               Best = BestInRange(MPhaseII.PuppiJet, Ref, C.AbsEtaMax, C.DRCut, Quality, Range, C.IsolationBB, C.IsolationEC);
            else if(C.InputObject == "PuppiJetForMET")
               Best = BestInRange(MPhaseII.PuppiJetForMET, Ref, C.AbsEtaMax, C.DRCut, Quality, Range, C.IsolationBB, C.IsolationEC);
            else if(C.InputObject == "CaloJet")
               Best = BestInRange(MPhaseII.CaloJet, Ref, C.AbsEtaMax, C.DRCut, Quality, Range, C.IsolationBB, C.IsolationEC);
            else if(C.InputObject == "GlobalMuon")
               Best = BestInRange(MPhaseII.GlobalMuon, Ref, C.AbsEtaMax, C.DRCut, Quality, Range, C.IsolationBB, C.IsolationEC);
            else if(C.InputObject == "StandaloneMuon")
               Best = BestInRange(MPhaseII.StandaloneMuon, Ref, C.AbsEtaMax, C.DRCut, Quality, Range, C.IsolationBB, C.IsolationEC);
            else if(C.InputObject == "StandaloneDisplacedMuon")
               Best = BestInRange(MPhaseII.StandaloneDisplacedMuon, Ref, C.AbsEtaMax, C.DRCut, Quality, Range, C.IsolationBB, C.IsolationEC);
            else if(C.InputObject == "TkMuon")
               Best = BestInRange(MPhaseII.TkMuon, Ref, C.AbsEtaMax, C.DRCut, Quality, Range, C.IsolationBB, C.IsolationEC);
            else if(C.InputObject == "TkMuonStub")
               Best = BestInRange(MPhaseII.TkMuonStub, Ref, C.AbsEtaMax, C.DRCut, Quality, Range, C.IsolationBB, C.IsolationEC);
            else if(C.InputObject == "TkGlbMuon")
               Best = BestInRange(MPhaseII.TkGlbMuon, Ref, C.AbsEtaMax, C.DRCut, Quality, Range, C.IsolationBB, C.IsolationEC);
            else if(C.InputObject == "PFMuon")
               Best = BestInRange(MPhaseII.PFMuon, Ref, C.AbsEtaMax, C.DRCut, Quality, Range, C.IsolationBB, C.IsolationEC);
            else if(C.InputObject == "EG")
            {
               Best = BestInRange(MPhaseII.EG, Ref, C.AbsEtaMax, C.DRCut, Quality, Range, C.IsolationBB, C.IsolationEC);
               if(Best.P[0] < 0)
                  Best.Type = ((Ref.GetAbsEta() < 1.5) ? 0 : 1);
            }
            else if(C.InputObject == "EGTrackID")
            {
               Best = BestInRange(MPhaseII.EGTrackID, Ref, C.AbsEtaMax, C.DRCut, Quality, Range, C.IsolationBB, C.IsolationEC);
               if(Best.P[0] < 0)
                  Best.Type = ((Ref.GetAbsEta() < 1.5) ? 0 : 1);
            }
            else if(C.InputObject == "TkElectron")
            {
               Best = BestInRange(MPhaseII.TkElectron, Ref, C.AbsEtaMax, C.DRCut, Quality, Range, C.IsolationBB, C.IsolationEC);
               if(Best.P[0] < 0)
                  Best.Type = ((Ref.GetAbsEta() < 1.5) ? 0 : 1);
            }
            else if(C.InputObject == "TkElectronTrackID")
            {
               Best = BestInRange(MPhaseII.TkElectronTrackID, Ref, C.AbsEtaMax, C.DRCut, Quality, Range, C.IsolationBB, C.IsolationEC);
               if(Best.P[0] < 0)
                  Best.Type = ((Ref.GetAbsEta() < 1.5) ? 0 : 1);
            }
            else if(C.InputObject == "TkElectronLoose")
            {
               Best = BestInRange(MPhaseII.TkElectronLoose, Ref, C.AbsEtaMax, C.DRCut, Quality, Range, C.IsolationBB, C.IsolationEC);
               if(Best.P[0] < 0)
                  Best.Type = ((Ref.GetAbsEta() < 1.5) ? 0 : 1);
            }
            else if(C.InputObject == "TkElectronLooseTrackID")
            {
               Best = BestInRange(MPhaseII.TkElectronLooseTrackID, Ref, C.AbsEtaMax, C.DRCut, Quality, Range, C.IsolationBB, C.IsolationEC);
               if(Best.P[0] < 0)
                  Best.Type = ((Ref.GetAbsEta() < 1.5) ? 0 : 1);
            }
            else if(C.InputObject == "TkPhoton")
            {
               Best = BestInRange(MPhaseII.TkPhoton, Ref, C.AbsEtaMax, C.DRCut, Quality, Range, C.IsolationBB, C.IsolationEC);
               if(Best.P[0] < 0)
                  Best.Type = ((Ref.GetAbsEta() < 1.5) ? 0 : 1);
            }
            else if(C.InputObject == "TkPhotonTrackID")
            {
               Best = BestInRange(MPhaseII.TkPhotonTrackID, Ref, C.AbsEtaMax, C.DRCut, Quality, Range, C.IsolationBB, C.IsolationEC);
               if(Best.P[0] < 0)
                  Best.Type = ((Ref.GetAbsEta() < 1.5) ? 0 : 1);
            }
            else if(C.InputObject == "TkPhotonPVTrackID")
            {
               Best = BestInRange(MPhaseII.TkPhotonPVTrackID, Ref, C.AbsEtaMax, C.DRCut, Quality, Range, C.IsolationBB, C.IsolationEC);
               if(Best.P[0] < 0)
                  Best.Type = ((Ref.GetAbsEta() < 1.5) ? 0 : 1);
            }
            else if(C.InputObject == "Tau")
               Best = BestInRange(MPhaseII.Tau, Ref, C.AbsEtaMax, C.DRCut, Quality, Range, C.IsolationBB, C.IsolationEC);
            else if(C.InputObject == "TkTau")
               Best = BestInRange(MPhaseII.TkTau, Ref, C.AbsEtaMax, C.DRCut, Quality, Range, C.IsolationBB, C.IsolationEC);
            else if(C.InputObject == "PFTau")
               Best = BestInRange(MPhaseII.PFTau, Ref, C.AbsEtaMax, C.DRCut, Quality, Range, C.IsolationBB, C.IsolationEC);
         ...
         // Fill Histograms
         for(int i = 0; i < N; i++)
            FillHistograms(ObjectHistograms[i], L1Objects[i].P, ReferenceObjects[i].P, L1Objects[i].Iso, L1Objects[i].Type, L1Objects[i].DXY);
      }
