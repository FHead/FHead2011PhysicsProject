...
{
   GenericObject Best(FourVector(-1, 0, 0, 0));
   for(int i = 0; i < (int)List.size(); i++)
      if(List[i].P.GetAbsEta() < AbsEtaMax && List[i].P.GetAbsEta() > AbsEtaMin)
         if(Best.P.GetPT() < List[i].P.GetPT())
            Best = List[i];
   return Best;
}

GenericObject BestInRange(vector<GenericObject> &List, FourVector &Reference, double MaxEta, double DRMax, int Qual, int Type, double IsoBB, double IsoEC)
{
   GenericObject Best;
