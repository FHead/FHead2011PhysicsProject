...
int BestIndexInRange(vector<FourVector> &List, double AbsEta)
{
   return BestIndexInRange(List, -1, AbsEta);
}

FourVector BestInRange(vector<FourVector> &List, double AbsEta)
{
   return BestInRange(List, -1, AbsEta);
}

int BestIndexInRange(vector<FourVector> &List, double AbsEtaMin, double AbsEtaMax)
{
   int Best = -1;
   for(int i = 0; i < (int)List.size(); i++)
      if(List[i].GetAbsEta() < AbsEtaMax && List[i].GetAbsEta() > AbsEtaMin)
         if(Best < 0 || List[Best].GetPT() < List[i].GetPT())
            Best = i;
   return Best;
}

FourVector BestInRange(vector<FourVector> &List, double AbsEtaMin, double AbsEtaMax)
{
...
int BestIndexInRange(vector<GenericObject> &List, FourVector &Reference, double MinEta, double MaxEta, double DRMax, int Qual, int Type)
{
   GenericObject Best;
   int BestIndex = -1;
...
GenericObject BestInRange(vector<GenericObject> &List, FourVector &Reference, double MinEta, double MaxEta, double DRMax, int Qual, int Type, double IsoBB, double IsoEC)
{
   GenericObject Best;
   Best.P = FourVector(-1, 0, 0, 0);
   Best.Iso = -1;
