      ...
      for(int iE = 0; iE < EntryCount; iE++)
      {
         ...
         for(int i = 0; i < N; i++)
         {
            ...
            else if(C.InputObject == "TkElectronPV")
            {
               ...
               Best = BestInRange(MPhaseII.CaloTkTau, Ref, C.AbsEtaMin, C.AbsEtaMax, C.DRCut, Quality, Range, C.IsolationBB, C.IsolationEC);
            else if(C.InputObject == "TkEGTau")
               Best = BestInRange(MPhaseII.TkEGTau, Ref, C.AbsEtaMin, C.AbsEtaMax, C.DRCut, Quality, Range, C.IsolationBB, C.IsolationEC);
