...
int main(int argc, char *argv[])
{
   ...
   bool Hack                   = CL.GetBool("hack", false);
   ...
   for(int i = 0; i < N; i++)
   {
      ...
      if(Hack == true)
      {
         TH1D *H = (TH1D *)File.Get("TkElectronV2TrackIDZIso/TkElectronV2TrackIDZIso_PTEta15_001500");
         cout << "Hacking..."  << " " << H << endl;
         if(H != nullptr)
            H->SetBinContent(5, 0);
         
         H = (TH1D *)File.Get("TkElectronV2TrackIDZIso/TkElectronV2TrackIDZIso_PTEta15_003000");
         cout << "Hacking..."  << " " << H << endl;
         if(H != nullptr)
            H->SetBinContent(5, 0);
      }
