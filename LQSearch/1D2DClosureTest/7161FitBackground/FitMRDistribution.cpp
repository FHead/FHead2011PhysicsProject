...
int main(int argc, char *argv[])
{
   ...
   FitWithRCut(PsFile, "Samples/ttbar.root", CutsToTry);
...
SingleFitResult FitWithRCut(PsFileHelper &PsFile, string Filename, double RCut, int Strategy)
{
   ...
   RooDataSet Dataset("Dataset", "MR Dataset", Tree, TreeVarSet, Form("RSQ > %f", RCut));
...
void FitWithRCut(PsFileHelper &PsFile, string Filename, vector<double> RCuts)
{
   ...
   RooDataSet Dataset("Dataset", "MR Dataset", Tree, TreeVarSet, Form("RSQ > %f", RCuts[0]));
