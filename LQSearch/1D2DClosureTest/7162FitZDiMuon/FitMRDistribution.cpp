...
#define MRLOWERBOUND 150
#define MRUPPERBOUND 500
...
int main(int argc, char *argv[])
{
   ...
   CutsToTry.push_back(0.195);
   CutsToTry.push_back(0.23);
   CutsToTry.push_back(0.26);

   PsFileHelper PsFile("Result.ps");

   FitWithRCut(PsFile, "Samples/Zll.root", CutsToTry);
...
SingleFitResult FitWithRCut(PsFileHelper &PsFile, string Filename, double RCut, int Strategy)
{
   ...
   double MRLowerBound = MRLOWERBOUND;
   double MRUpperBound = MRUPPERBOUND;
   ...
   RooRealVar BoxNum("BOX_NUM", "box!!!", -10, 10);
   RooArgSet TreeVarSet(MR, R, BoxNum);

   RooDataSet Dataset("Dataset", "MR Dataset", Tree, TreeVarSet,
      Form("RSQ > %f && BOX_NUM > 1.9 && BOX_NUM < 3.1", RCut));
   ...
   RooRealVar S1("S1", "exponent of the tail", 0.01, 0, 0.5);
   RooRealVar S2("S2", "exponent of the tail", 0.01, 0, 0.5);
   RooFormulaVar NegativeS1("NegativeS1", "Negative S1", "-S1", RooArgList(S1));
   ...
   RooAbsPdf *TempModel1 = NULL;
   RooAbsPdf *TempModel2 = NULL;
   if(Strategy == Strategy_Normal)
      Model = new RooTwoSideGaussianWithTwoExponentialTails("Model", "Model", MR, X0, SigmaL, SigmaR, S1, S2, Fraction);
   else if(Strategy == Strategy_IgnoreLeft)
      Model = new RooTwoSideGaussianWithTwoExponentialTails("Model", "Model", MR, X0, SigmaR, SigmaR, S1, S2, Fraction);
   else if(Strategy == Strategy_IgnoreGaussian)
   {
      TempModel1 = new RooExponential("Model1", "Model1", MR, NegativeS1);
      TempModel2 = new RooExponential("Model2", "Model2", MR, NegativeS2);
      Model = new RooAddPdf("Model", "Model", *TempModel1, *TempModel2, Fraction);
   }
   ...
   if(Strategy == Strategy_Normal)
   {
      double XC = (S1.getVal() + Fraction.getVal() * S2.getVal()) / (1 + Fraction.getVal())
         * SigmaR.getVal() * SigmaR.getVal() + X0.getVal();
...
void FitWithRCut(PsFileHelper &PsFile, string Filename, vector<double> RCuts)
{
   ...
   SingleCentralValues1.SetNameTitle("SingleCentralValues1", "Central values from single fits (first component)");
   TGraphErrors SingleCentralValues2;
   SingleCentralValues2.SetNameTitle("SingleCentralValues2", "Central values from single fits (second component)");
   for(int i = 0; i < (int)RCuts.size(); i++)
   {
   ...
   SingleCentralValues1.SetLineWidth(1);
   SingleCentralValues1.SetMarkerStyle(8);
   PsFile.AddPlot(SingleCentralValues1, "apx");
   
   SingleCentralValues1.SetLineWidth(2);
   SingleCentralValues1.SetMarkerStyle(1);
   PsFile.AddPlot(SingleCentralValues1, "ap");
   
   SingleCentralValues2.SetLineWidth(1);
   SingleCentralValues2.SetMarkerStyle(8);
   PsFile.AddPlot(SingleCentralValues2, "apx");
   ...
   RooRealVar MR("MR", "Chris' kinematic variable", MRLowerBound, MRUpperBound, "GeV");
   RooRealVar R("RSQ", "Chris' another kinematic variable", -200, 200);
   RooRealVar BoxNum("BOX_NUM", "box number", -10, 10);
   RooArgSet TreeVarSet(MR, R, BoxNum);

   RooDataSet Dataset("Dataset", "MR Dataset", Tree, TreeVarSet,
      Form("RSQ > %f && BOX_NUM > 1.9 && BOX_NUM < 3.1", RCuts[0]));

   RooRealVar ParameterA1("ParameterA1", "s1 = \"a1\" + b1 R^2", ParameterA1Value, -0.1, 0.1);
   RooRealVar ParameterB1("ParameterB1", "s1 = a1 + \"b1\" R^2", ParameterB1Value, 0, 1);
   RooRealVar ParameterA2("ParameterA2", "s2 = \"a2\" + b2 R^2", ParameterA2Value, -0.1, 0.1);
   RooRealVar ParameterB2("ParameterB2", "s2 = a2 + \"b2\" R^2", ParameterB2Value, 0, 1);
   ...
   vector<RooAbsPdf *> TempModels1;
   vector<RooAbsPdf *> TempModels2;
   ...
   for(int i = 0; i < (int)RCuts.size(); i++)
   {
      if(SingleFitResults[i].Strategy == Strategy_Normal)
      {
         X0.push_back(new RooRealVar(Form("X0_%d", i),
            Form("gaussian mean, R > %f", RCuts[i]), SingleFitResults[i].X0, "GeV"));
         // X0[i]->setError(SingleFitResults[i].X0Error * 5);
         SigmaL.push_back(new RooRealVar(Form("SigmaL_%d", i),
            Form("left-hand side gaussian width, R > %f", RCuts[i]), SingleFitResults[i].SigmaL, "GeV"));
         // SigmaL[i]->setError(SingleFitResults[i].SigmaLError * 5);
         SigmaR.push_back(new RooRealVar(Form("SigmaR_%d", i),
            Form("right-hand side gaussian width, R > %f", RCuts[i]), SingleFitResults[i].SigmaR, "GeV"));
         // SigmaR[i]->setError(SingleFitResults[i].SigmaRError * 5);
         S1.push_back(new RooFormulaVar(Form("S1_%d", i),
            Form("first exponent of the tail, R > %f", RCuts[i]),
            Form("@0 + %f * @1", RCuts[i]), RooArgList(ParameterA1, ParameterB1)));
         S2.push_back(new RooFormulaVar(Form("S2_%d", i),
            Form("second exponent of the tail, R > %f", RCuts[i]),
            Form("@0 + %f * @1", RCuts[i]), RooArgList(ParameterA2, ParameterB2)));
         Fraction.push_back(new RooRealVar(Form("Fraction_%d", i),
            Form("Fraction modelling R > %f part", RCuts[i]), SingleFitResults[i].Fraction, 0, 1));

         TempModels1.push_back(NULL);
         TempModels2.push_back(NULL);
         Models.push_back(new RooTwoSideGaussianWithTwoExponentialTails(Form("Model_%d", i),
            Form("Model for R > %f", RCuts[i]), MR, *X0[i], *SigmaL[i], *SigmaR[i], *S1[i], *S2[i], *Fraction[i]));
      }
      else if(SingleFitResults[i].Strategy == Strategy_IgnoreLeft)
      {
         ...
         TempModels2.push_back(NULL);
         Models.push_back(new RooTwoSideGaussianWithTwoExponentialTails(Form("Model_%d", i),
            Form("Model for R > %f", RCuts[i]), MR, *X0[i], *SigmaR[i], *SigmaR[i], *S1[i], *S2[i], *Fraction[i]));
      }
      else   // if SingleFitResults[i].Strategy == Strategy_IgnoreGaussian
      {
         ...
         TempModels1.push_back(new RooExponential(Form("Model_%d_1", i), Form("Model for R > %f", RCuts[i]),
            MR, *S1[i]));
         TempModels2.push_back(new RooExponential(Form("Model_%d_2", i), Form("Model for R > %f", RCuts[i]),
            MR, *S2[i]));
         Models.push_back(new RooAddPdf(Form("Model_%d", i), Form("Model for R > %f", RCuts[i],
            (RooExponential *)TempModels1[i], (RooExponential *)TempModels2[i], (RooRealVar *)Fraction[i])));
      }
   }
   ...
   RooPlot *FinalResultPlot2 = MR.frame(Bins(50), Name("FitResults2"), Title("MR, Simultaneous fit"), Range(100, 600));
   ...
   */
   ...
   for(int i = 0; i < (int)RCuts.size(); i++)
   {
      ...
      if(TempModels1[i] != NULL)   delete TempModels1[i];
      if(TempModels2[i] != NULL)   delete TempModels2[i];
