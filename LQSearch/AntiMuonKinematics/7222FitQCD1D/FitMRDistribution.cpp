...
int main(int argc, char *argv[])
{
   ...
   FitWithRCut(PsFile, "QCDAll.root", CutsToTry);
...
SingleFitResult FitWithRCut(PsFileHelper &PsFile, string Filename, double RCut, int Strategy)
{
   ...
   RooRealVar W("W", "Weight", 0, 1000);
   RooArgSet TreeVarSet(MR, R, W);

   RooDataSet Dataset("Dataset", "MR Dataset", Tree, TreeVarSet, Form("Rsq > %f", RCut), "W");
   ...
   RooFitResult *FitResult = Model->fitTo(Dataset, Save(true), SumW2Error(true));
   ...
   if(Strategy == Strategy_Normal)
   {
      ...
      if((MRLowerBound + SafetyMargin < XC && MRLowerBound + SafetyMargin >= X0.getVal())
         || S.getError() < fabs(S.getVal()) * 0.001)
   ...
   if(Strategy == Strategy_IgnoreLeft)
   {
      ...
      if(MRLowerBound + SafetyMargin >= XC || S.getError() < fabs(S.getVal()) * 0.001)
         return FitWithRCut(PsFile, Filename, RCut, Strategy_IgnoreGaussian);
   }
...
void FitWithRCut(PsFileHelper &PsFile, string Filename, vector<double> RCuts)
{
   ...
   for(int i = 0; i < (int)RCuts.size(); i++)
   {
      SingleCentralValues.SetPoint(i, RCuts[i], SingleFitResults[i].S);
   ...
   RooDataSet Dataset("Dataset", "MR Dataset", Tree, TreeVarSet, Form("Rsq > %f", RCuts[0]), "W");
   ...
   RooRealVar ParameterA("ParameterA", "s = \"a\" + b R^2", ParameterAValue, 0, 0.1);
   // RooRealVar ParameterB("ParameterB", "s = a + \"b\" R^2", ParameterBValue, 0, 1);
   RooRealVar ParameterB("ParameterB", "s = a + \"b\" R^2", 0);
   ...
   for(int i = 0; i < (int)RCuts.size(); i++)
   {
      ...
      SingleBinYields.push_back(new RooRealVar(Form("SingleBinYield_%d", i),
         Form("Yield with R in bin %d", i), GuessYield, 0, GuessYield * 5));
   }
   ...
   for(int i = 0; i < (int)RCuts.size(); i++)
   {
      if(SingleFitResults[i].Strategy == Strategy_Normal)
      {
         ...
         S.push_back(new RooFormulaVar(Form("S_%d", i),
            ...
            Form("@0 + %f * @1", RCuts[i]), RooArgList(ParameterA, ParameterB)));
      ...
      else   // if SingleFitResults[i].Strategy == Strategy_IgnoreGaussian
      {
         ...
         S.push_back(new RooFormulaVar(Form("S_%d", i),
            Form("exponent of the tail, R > %f", RCuts[i]),
            Form("-1 * @0 - %f * @1", RCuts[i]), RooArgList(ParameterA, ParameterB)));
   ...
   */
   ...
   RooPlot *FinalResultPlot2 = MR.frame(Bins(50), Name("FitResults2"), Title("MR, Simultaneous fit"),
      Range(MRLowerBound, MRUpperBound));
   ...
   for(int i = 0; i < (int)RCuts.size(); i++)
   {
      ...
      legend.AddEntry(DummyHistograms[i], Form("Rsq > %f", RCuts[i]), "l");
   }
   legend.Draw();
   ...
   for(int i = 0; i < (int)RCuts.size(); i++)
   {
      double FitCentral = ParameterAValue + ParameterBValue * RCuts[i];

      SingleRatioPoint.SetPoint(i, RCuts[i], SingleFitResults[i].S / FitCentral);
