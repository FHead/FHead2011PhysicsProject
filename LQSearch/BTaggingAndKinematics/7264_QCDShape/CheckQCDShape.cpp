...
int main(int argc, char *argv[])
{
   ...
   const double Luminosity = 1000;   // fixed - doesn't really matter....
   string Tag = "QCD";

   if(argc < 3)
   {
      cout << "Usage: " << argv[0] << " InputFile CrossSection [Tag]" << endl;
      return -1;
   }

   InputFileName = argv[1];
   CrossSection = atof(argv[2]);
   ...
   TH1D *H = (TH1D *)InputFile.Get("HProcessedEventsPU");
   for(int i = 1; i <= H->GetNbinsX(); i++)
      TotalWeight = TotalWeight + M.GetWeight(i - 1) * H->GetBinContent(i);
   InputFile.Close();

   TFile OutputFile((Tag + ".root").c_str(), "recreate");

   TH1D HMRNewAll("HMRNewAll", "MR (new)", 40, 0, 2000);
   TH1D HMRNewNoTag("HMRNewAllNoTag", "MR (new), b-tag count == 0", 40, 0, 2000);
   TH1D HMRNewTag("HMRNewAllTag", "MR (new) b-tag count > 0", 40, 0, 2000);
   ...
   TH1D HMRNewAll_R2New010("HMRNewAll_R2New010", "MR (new), R^{2} (new) > 0.10", 40, 0, 2000);
   TH1D HMRNewNoTag_R2New010("HMRNewAllNoTag_R2New010", "MR (new), R^{2} (new) > 0.10, b-tag count == 0", 40, 0, 2000);
   TH1D HMRNewTag_R2New010("HMRNewAllTag_R2New010", "MR (new), R^{2} (new) > 0.10, b-tag count > 0", 40, 0, 2000);
   
   TH1D HMRNewAll_R2New015("HMRNewAll_R2New015", "MR (new), R^{2} (new) > 0.15", 40, 0, 2000);
   TH1D HMRNewNoTag_R2New015("HMRNewAllNoTag_R2New015", "MR (new), R^{2} (new) > 0.15, b-tag count == 0", 40, 0, 2000);
   TH1D HMRNewTag_R2New015("HMRNewAllTag_R2New015", "MR (new), R^{2} (new) > 0.15, b-tag count > 0", 40, 0, 2000);
   ...
   TH1D HR2NewAll_MRNew500("HR2NewAll_MRNew500", "R^{2} (new), MR (new) > 500", 40, 0, 0.5);
   TH1D HR2NewNoTag_MRNew500("HR2NewAllNoTag_MRNew500", "R^{2} (new), MR (new) > 500, b-tag count == 0", 40, 0, 0.5);
   TH1D HR2NewTag_MRNew500("HR2NewAllTag_MRNew500", "R^{2} (new), MR (new) > 500, b-tag count > 0", 40, 0, 0.5);

   cout << Tree.GetEntries() << " " << TotalWeight << endl;
   ...
   for(int iEntry = 0; iEntry < EntryCount; iEntry++)
   {
      ...
      double WPU = Luminosity * CrossSection / TotalWeight;
      WPU = WPU * M.GetCurrentWeight();
      ...
      if(R2New > 0.15)
      {
         HMRNewAll_R2New015.Fill(MRNew, WPU);
         if(JetPassBTagLoose == 0)
            HMRNewNoTag_R2New015.Fill(MRNew, WPU);
         else
            HMRNewTag_R2New015.Fill(MRNew, WPU);
      }
      ...
      if(MRNew > 500)
      {
         HR2NewAll_MRNew500.Fill(R2New, WPU);
         if(JetPassBTagLoose == 0)
            HR2NewNoTag_MRNew500.Fill(R2New, WPU);
         else
            HR2NewTag_MRNew500.Fill(R2New, WPU);
      }
   }

   PsFileHelper PsFile(Tag + ".ps");
   PsFile.AddTextPage("QCD shape check [" + Tag + "]");
   ...
   HMRNewAll_R2New010.Write();
   HMRNewNoTag_R2New010.Write();
   HMRNewTag_R2New010.Write();

   HMRNewAll_R2New015.Write();
   HMRNewNoTag_R2New015.Write();
   HMRNewTag_R2New015.Write();

   HR2NewAll_MRNew400.Write();
   HR2NewNoTag_MRNew400.Write();
   HR2NewTag_MRNew400.Write();

   HR2NewAll_MRNew500.Write();
   HR2NewNoTag_MRNew500.Write();
   HR2NewTag_MRNew500.Write();
