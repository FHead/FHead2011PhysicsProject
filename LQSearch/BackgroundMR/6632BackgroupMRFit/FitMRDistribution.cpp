...
int main(int argc, char *argv[])
{
   ...
   if(argc >= 3)
   {
      ...
      InputFile = "Samples/" + Tag + "_MRTree.root";
   }
   else
   {
      cerr << "Usage: " << argv[0] << " Tag MinimumMR" << endl;
      return -1;
   }
   ...
   if(argc >= 4)
   {
      stringstream CategoryStream(argv[3]);
      CategoryStream >> Category;
      CategoryString = argv[3];
   }

   bool DoubleExponential = false;
   if(argc >= 5 && string(argv[4]) == "--double")
      DoubleExponential = true;

   PsFileHelper PsFile("FitBackground_" + Tag + "_" + CategoryString + ".ps");
   ...
   DataHelper DHFile("AllFitResults_" + CategoryString + ".dh");

   for(double RCut = 0.20; RCut <= 0.60; RCut = RCut + 0.025)
   {
      MRFitSingleFitResult result = FitWithRCut(&PsFile, InputFile, RCut, "MRTree", "MR", "R",
         MRMin, 1500, MRFit_Strategy_Normal, DoubleExponential, true, Category);
   ...
   DHFile.SaveToFile("AllFitResults_" + CategoryString + ".dh");
}
