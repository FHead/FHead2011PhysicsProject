...
SingleFitResult FitWithRCut(PsFileHelper &PsFile, string Filename, double RCut, int Strategy)
{
   ...
   RooRealVar R("GroupPFR", "Chris' another kinematic variable", 0, 2);
...
void FitWithRCut(PsFileHelper &PsFile, string Filename, vector<double> RCuts)
{
   ...
   for(int i = 0; i < (int)RCuts.size(); i++)   // ps. last bin is special
   {
      ...
      if(i == RCuts.size() - 1)
         ...
         //    Form("Constraint R = %f - inf.", RCuts[i]), R, RCuts[i] + 10, 10));
      else
         Constraint.push_back(new RooSameAs(Form("Constraint_%d", i),
      ...
      TopLevelYields.push_back(SingleBinYields[i]);
   ...
   // generate toys?
   /*
   TFile ToyFile("ToyFile.root", "RECREATE");
   for(int i = 1; i <= 1000; i++)
   {
      RooDataSet *NewToyDataset = (RooDataSet *)FinalModel.generate(RooArgSet(R, MR), 17594);
      NewToyDataset->tree()->Clone(Form("Sample_%d", i))->Write();
   }
   ToyFile.Close();
   */
   ...
   */
   ...
   RooPlot *FinalResultPlot3 = R.frame(Bins(50), Name("FitResults3"), Title("R, Simultaneous fit"));
   Dataset.plotOn(FinalResultPlot3, MarkerSize(0.9), MarkerColor(1));
   for(int i = 0; i < (int)RCuts.size(); i++)
   {
      RooArgList ComponentsToAdd;
      for(int j = i; j < (int)RCuts.size(); j++)
   ...
   for(int i = 1; i < (int)RCuts.size(); i++)
      Dataset.reduce(Cut(Form("GroupPFR > %f", RCuts[i])))->plotOn(FinalResultPlot3, MarkerSize(0.9), MarkerColor(i + 1));

   PsFile.AddPlot(FinalResultPlot3, "", true);
