...
SingleFitResult FitWithRCut(PsFileHelper &PsFile, string Filename, double RCut, int Strategy = Strategy_Normal,
   int RunNumber = 0);
void FitWithRCut(PsFileHelper &PsFile, string Filename, vector<double> RCuts, DataHelper &DHFile, string State, int RunNumber = 0);
...
int main(int argc, char *argv[])
{
   ...
   { 
      ...
      // FitWithRCut(PsFile, "Samples/ToyOnMRScalingFit_" + NumberToString.str() + ".root", CutsToTry, DHFile, State.str());
      FitWithRCut(PsFile, "ToyFile.root", CutsToTry, DHFile, State.str(), i);
...
SingleFitResult FitWithRCut(PsFileHelper &PsFile, string Filename, double RCut, int Strategy, int RunNumber)
{
   TFile F(Filename.c_str());
   TTree *Tree = (TTree *)F.Get(Form("Sample_%d", RunNumber));
   if(Tree == NULL)
   ...
   if(Strategy == Strategy_Normal)
   {
      ...
      if(MRLowerBound + SafetyMargin < XC && MRLowerBound + SafetyMargin >= X0.getVal())
         return FitWithRCut(PsFile, Filename, RCut, Strategy_IgnoreLeft, RunNumber);
      if(MRLowerBound + SafetyMargin >= XC)
         return FitWithRCut(PsFile, Filename, RCut, Strategy_IgnoreGaussian, RunNumber);
   }
   ...
   int RunNumber)
{
   ...
   for(int i = 0; i < (int)RCuts.size(); i++)
      SingleFitResults.push_back(FitWithRCut(PsFile, Filename, RCuts[i], Strategy_IgnoreGaussian, RunNumber));
   ...
   */
   ...
   DHFile[State]["SingleA"] = InitialParameterValues.GetParameter(0);
   DHFile[State]["SingleAError"] = InitialParameterValues.GetParError(0);
   DHFile[State]["SingleB"] = InitialParameterValues.GetParameter(1);
   DHFile[State]["SingleBError"] = InitialParameterValues.GetParError(1);
   DHFile[State]["SimultaneousA"] = ParameterA.getVal();
   DHFile[State]["SimultaneousAError"] = ParameterA.getError();
   DHFile[State]["SimultaneousB"] = ParameterB.getVal();
   DHFile[State]["SimultaneousBError"] = ParameterB.getError();
   DHFile[State]["SimultaneousCorrelation"] = FitResult->correlation("ParameterA", "ParameterB");
   for(int i = 0; i < (int)RCuts.size(); i++)
   {
      stringstream NumberToString;
      NumberToString << i + 1;
      DHFile[State]["SingleBin" + NumberToString.str() + "S"] = SingleFitResults[i].S;
      DHFile[State]["SingleBin" + NumberToString.str() + "SError"] = SingleFitResults[i].SError;
   }
