...
void BackgroundMRDistribution(string InputList, string OutputBase)
{
   ...
   for(int iEntry = 0; iEntry < EntryCount; iEntry++)
   {
      ...
      bool ExistLeptonicW = CheckLeptonicW(M);
      ...
      if(M.nAK5PFJet >= 2)
      {
         ...
         for(int i = 0; i < 100 && i < M.nAK5PFJet; i++)
         {
         ...
         if(PFJets.size() >= 2)
         {
            ...
            double GroupPFMR = GetMRStar(Groups[0], Groups[1]);
            double GroupPFMRT = GetMRT(Groups[0], Groups[1], PFMETP);
            double GroupPFR = GetRStar(Groups[0], Groups[1], PFMETP);
