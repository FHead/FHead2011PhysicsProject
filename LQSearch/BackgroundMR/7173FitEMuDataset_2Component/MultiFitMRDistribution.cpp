...
SingleFitResult FitWithRCut(PsFileHelper &PsFile, string FileName, double RCut, bool Single = false);
RooFitResult *FitWithRCut(PsFileHelper &PsFile, string FileName, vector<double> RCuts, bool Single = false);
...
int main(int argc, char *argv[])
{
   ...
   for(int i = 0; i < NumberOfTries; i++)
   {
      ...
      RooFitResult *Result = FitWithRCut(PsFile, "TTbar.root", CutsToTry, false);
      PsFile.AddTimeStampPage();
      PsFile.Close();
   ...
   RooPlot ParameterABPlot(0.003, 0.014, 0, 0.04);
   ...
   DataHorizontalLineAverage.SetLineStyle(10);
   DataVerticalLineAverage.SetLineStyle(10);
   TTbarHorizontalLineAverage.SetLineStyle(10);
   TTbarVerticalLineAverage.SetLineStyle(10);

   TLegend legend(0.15, 0.75, 0.4, 0.85);
   legend.SetFillColor(0);
   legend.SetFillStyle(0);
   legend.SetBorderSize(0);
   legend.AddEntry(&DataHorizontalLine, "Data", "l");
   legend.AddEntry(&TTbarHorizontalLine, "TTbar", "l");
   ...
   TLegend legend2(0.3, 0.75, 0.55, 0.85);
...
SingleFitResult FitWithRCut(PsFileHelper &PsFile, string FileName, double RCut, bool Single)
{
   ...
   RooRealVar S1("S1", "first exponent of the tail", -0.1, -1.0, 0.0);
   RooExponential Component1("Component1", "First component", MR, S1);
   
   RooRealVar S2("S2", "second exponent of the tail", -0.025, -1.0, 0.0);
...
RooFitResult *FitWithRCut(PsFileHelper &PsFile, string FileName, vector<double> RCuts, bool Single)
{
   if(RCuts.size() == 0)
      return NULL;
   if(RCuts.size() == 1)
   ...
   for(int i = 0; i < (int)RCuts.size(); i++)
      SingleFitResults[i] = FitWithRCut(PsFile, FileName, RCuts[i], Single);
	...
	*/
   double ParameterA1Value = 0.012;
   double ParameterB1Value = 0.020;
   double ParameterA2Value = 0.009;
   double ParameterB2Value = 0.001;
   ...
   double MRLowerBound = DrawRandom(250, 275);
   double MRUpperBound = 1500;
