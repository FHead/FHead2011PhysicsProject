...
int main(int argc, char *argv[])
{
   ...
   */
   ...
   ParameterABPlot.SetTitle("Summary of fits to ttbar in single mu selection (Chris MC)");
   for(int i = 0; i < NumberOfTries; i++)
   {
      /*
...
SingleFitResult FitWithRCut(PsFileHelper &PsFile, string FileName, double RCut)
{
   ...
   RooArgSet TreeVarSet(MR, R, BoxNumber, WPU);

   RooDataSet Dataset("Dataset", "MR Dataset", Tree, TreeVarSet,
      Form("%s > %f && BOX_NUM == 3", RString.c_str(), RCut), "WPU");
...
RooFitResult *FitWithRCut(PsFileHelper &PsFile, string FileName, vector<double> RCuts)
{
   ...
   RooDataSet Dataset("Dataset", "MR Dataset", Tree, TreeVarSet,
      Form("%s > %f && BOX_NUM == 3", RString.c_str(), RCuts[0]), "WPU");
