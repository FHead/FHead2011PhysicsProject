...
int main(int argc, char *argv[])
{
   ...
   for(int i = 0; i < NumberOfTries; i++)
   {
      ...
      RooFitResult *Result = FitWithRCut(PsFile, "DataAll.root", CutsToTry);
   ...
   */
   ...
   {
      vector<double> CutsToTry;
      CutsToTry.push_back(DrawRandom(0.30, 0.31));   CutsToTry[0] = CutsToTry[0] * CutsToTry[0];
      CutsToTry.push_back(DrawRandom(0.33, 0.34));   CutsToTry[1] = CutsToTry[1] * CutsToTry[1];
      CutsToTry.push_back(DrawRandom(0.37, 0.38));   CutsToTry[2] = CutsToTry[2] * CutsToTry[2];
      CutsToTry.push_back(DrawRandom(0.40, 0.41));   CutsToTry[3] = CutsToTry[3] * CutsToTry[3];
      // CutsToTry.push_back(DrawRandom(0.44, 0.46));
   
      PsFileHelper PsFile(Form("TTbarMultiFit%d_NoWeight.ps", i));
      PsFile.AddTextPage("Fit ttbar in the muon box (weightless)");
   ...
   PsFileHelper PsFile("TotalPlot_NoWeight.ps");

   TFile F("YAYAYA_NoWeight.root", "recreate");
   ...
   RooPlot ParameterABPlot(0.003, 0.01, 0, 0.04);
   ParameterABPlot.GetXaxis()->SetTitle("Parameter A");
   ParameterABPlot.GetYaxis()->SetTitle("Parameter B");
   ParameterABPlot.SetTitle("Summary of fits ttbar in single mu selection (Chris MC)");
   for(int i = 0; i < NumberOfTries; i++)
   {
      /*
      ...
      */
      ...
      double x = ((RooRealVar *)FitResultsTTbar[i]->floatParsFinal().find("ParameterA1"))->getVal();
      double y = ((RooRealVar *)FitResultsTTbar[i]->floatParsFinal().find("ParameterB1"))->getVal();
      double ex = ((RooRealVar *)FitResultsTTbar[i]->floatParsFinal().find("ParameterA1"))->getError();
      double ey = ((RooRealVar *)FitResultsTTbar[i]->floatParsFinal().find("ParameterB1"))->getError();
      double rho = FitResultsTTbar[i]->correlation("ParameterA1", "ParameterB1");
      RooEllipse *Contour = new RooEllipse(Form("TTbarEllipse%d", i), x, y, ex, ey, rho);
      Contour->SetLineWidth(2);
   ...
   */
   ...
   {
      if(FitResultsTTbar[i] != NULL)
         FitResultsTTbar[i]->plotOn(&ParameterABPlot3, "ParameterA1", "ParameterB1", "M");
   }
...
SingleFitResult FitWithRCut(PsFileHelper &PsFile, string FileName, double RCut)
{
   ...
   RooDataSet Dataset("Dataset", "MR Dataset", Tree, TreeVarSet,
      Form("%s > %f && BOX_NUM == 3", RString.c_str(), RCut));
   ...
   Component1.fitTo(Dataset, SumW2Error(false));
   // Model->fitTo(Dataset);
...
RooFitResult *FitWithRCut(PsFileHelper &PsFile, string FileName, vector<double> RCuts)
{
   ...
   double MRLowerBound = DrawRandom(350, 450);
   double MRUpperBound = 1500;
   ...
   RooDataSet Dataset("Dataset", "MR Dataset", Tree, TreeVarSet,
      Form("%s > %f && BOX_NUM == 3", RString.c_str(), RCuts[0]));
   ...
   RooFitResult *FitResult = FinalModel.fitTo(Dataset, Save(true), PrintEvalErrors(-1), NumCPU(3), SumW2Error(false));
