...
int main(int argc, char *argv[])
{
   ...
   FitWithRCut(PsFile, "TTbar.root", CutsToTry);
...
SingleFitResult FitWithRCut(PsFileHelper &PsFile, string Filename, double RCut, int Strategy)
{
   ...
   RooRealVar X0("X0", "gaussian mean", 225, 0, 5000, "GeV");
   RooRealVar SigmaL("SigmaL", "left-hand side gaussian width", 50, 30, 5000, "GeV");
   RooRealVar SigmaR("SigmaR", "right-hand side gaussian width", 100, 30, 5000, "GeV");
   RooRealVar S1("S", "exponent1 of the tail", 0.008, 0, 0.5);
   RooRealVar S2("S2", "exponent2 of the tail", 0.005, 0, 0.5);
   RooRealVar Fraction("Fraction", "fraction of the tail", 0.219, 0, 1);
   ...
   if(Strategy == Strategy_Normal)
      ...
      Model = new RooExponential("Model", "Model", MR, NegativeS1);   // TODO!
   ...
   double SafetyMargin = 25;

   if(Strategy == Strategy_Normal)
   {
      double XC = S1.getVal() * SigmaR.getVal() * SigmaR.getVal() + X0.getVal();
      ...
      if((MRLowerBound + SafetyMargin < XC && MRLowerBound + SafetyMargin >= X0.getVal())
         || S1.getError() < fabs(S1.getVal()) * 0.001)
   ...
   if(Strategy == Strategy_IgnoreLeft)
   {
      ...
      if(MRLowerBound + SafetyMargin >= XC || S1.getError() < fabs(S1.getVal()) * 0.001)
         return FitWithRCut(PsFile, Filename, RCut, Strategy_IgnoreGaussian);
   }
...
void FitWithRCut(PsFileHelper &PsFile, string Filename, vector<double> RCuts)
{
   ...
   for(int i = 0; i < (int)RCuts.size(); i++)
   {
      SingleCentralValues.SetPoint(i, RCuts[i] * RCuts[i], SingleFitResults[i].S1);
      SingleCentralValues.SetPointError(i, 0, SingleFitResults[i].S1Error);
   }
   ...
   RooRealVar ParameterA("ParameterA", "s = \"a\" + b R^2", ParameterAValue);
   RooRealVar ParameterB("ParameterB", "s = a + \"b\" R^2", ParameterBValue);
   ...
   for(int i = 0; i < (int)RCuts.size(); i++)
   {
      double GuessYield = 0;
      if(i != RCuts.size() - 1)
         GuessYield = Dataset.reduce(Cut(Form("R > %f && R <= %f", RCuts[i], RCuts[i+1])))->sumEntries();
      else
         GuessYield = Dataset.reduce(Cut(Form("R > %f", RCuts[i])))->sumEntries();

      cout << "Guess yield for bin " << i << " is " << GuessYield << endl;
         
      SingleBinYields.push_back(new RooRealVar(Form("SingleBinYield_%d", i),
         Form("Yield with R in bin %d", i), GuessYield));
   }
