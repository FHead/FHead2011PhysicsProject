...
int main(int argc, char *argv[])
{
   ...
   for(int i = 0; i < NumberOfTries; i++)
   {
      ...
      if(i > 1)
      {
         CutsToTry.push_back(DrawRandom(0.25, 0.27));
         CutsToTry.push_back(DrawRandom(0.35, 0.37));
         CutsToTry.push_back(DrawRandom(0.45, 0.47));
         CutsToTry.push_back(DrawRandom(0.55, 0.57));
      }
      else if(i == 0)
      {
         CutsToTry.push_back(0.2);
         CutsToTry.push_back(0.3);
         CutsToTry.push_back(0.4);
         CutsToTry.push_back(0.5);
      ...
      for(int j = 0; j < (int)CutsToTry.size(); j++)
         CutsToTry[j] = CutsToTry[j] * CutsToTry[j];
   
      PsFileHelper PsFile(Form("%sMultiFit%d.ps", OutputBase.c_str(), i));
      PsFile.AddTextPage("Fit!!");
      RooFitResult *Result = FitWithRCut(PsFile, InputFile, CutsToTry, BoxNumber, BTagRequirement, MRMin, OneComponent, Base);
   ...
   for(int i = 0; i < NumberOfTries; i++)
   {
      ...
      if(ex1 > 10 * x1 || ey1 > 10 * y1 || ey1 > 0.5)   // potentially bad fit
         continue;
   ...
   double MRMin, bool OneComponent)
{
   ...
   */
   ...
   double sr = 0.02095;
   double a = b * r;
   double sa = sqrt(b * b * sr * sr + r * r * sb * sb);
   RooRealVar S1("S1", "first exponent of the tail", -0.022, -1.0, 0.0);
   RooExponential Component1BeforePenalty("Component1BeforePenalty", "First component", MR, S1);
   RooRealVar S1ValueVar("S1ValueVar", "S1ValueVar", -(b * RCut + a));
   RooRealVar S1ErrorVar("S1ErrorVar", "S1ErrorVar", sqrt(sb * sb * RCut * RCut + sa * sa));
   ...
   sr = 0.03604;
   a = b * r;
   sa = sqrt(b * b * sr * sr + r * r * sb * sb);
   RooRealVar S2("S2", "second exponent of the tail", -0.007, -1.0, 0.0);
   RooExponential Component2BeforePenalty("Component2BeforePenalty", "Second component", MR, S2);
   RooRealVar S2ValueVar("S2ValueVar", "S2ValueVar", -(b * RCut + a));
   RooRealVar S2ErrorVar("S2ErrorVar", "S2ErrorVar", sqrt(sb * sb * RCut * RCut + sa * sa));
   ...
   double MRMin, bool OneComponent, string OutputBase)
{
   ...
   if(OutputBase != "")
   {
      ...
      SingleCentralValues1.GetYaxis()->SetTitle("1^{st} Slope Parameter [1/GeV]");
      ...
      SingleCentralValues2.GetXaxis()->SetTitle("(R Cut)^{2}");
      SingleCentralValues2.GetYaxis()->SetTitle("2^{nd} Slope Parameter [1/GeV]");
   ...
   */
   ...
   // RooRealVar ParameterA1("ParameterA1", "s1 = \"a1\" + b1 R^2", ParameterA1Value, -1, 1);
   // RooRealVar ParameterB1("ParameterB1", "s1 = a1 + \"b1\" R^2", ParameterB1Value, -1, 1);
   // RooRealVar ParameterA2("ParameterA2", "s2 = \"a2\" + b2 R^2", ParameterA2Value, -1, 1);
   // RooRealVar ParameterB2("ParameterB2", "s2 = a2 + \"b2\" R^2", ParameterB2Value, -1, 1);
   RooRealVar ParameterA1("ParameterA1", "s1 = \"a1\" + b1 R^2", 0.07976 * 0.12386, -1, 1);
   RooRealVar ParameterB1("ParameterB1", "s1 = a1 + \"b1\" R^2", 0.07976, -1, 1);
   RooRealVar ParameterA2("ParameterA2", "s2 = \"a2\" + b2 R^2", 0.01587 * 0.45939, -1, 1);
   RooRealVar ParameterB2("ParameterB2", "s2 = a2 + \"b2\" R^2", 0.01587, -1, 1);
