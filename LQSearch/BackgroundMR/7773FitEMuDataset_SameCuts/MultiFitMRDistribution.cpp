...
int main(int argc, char *argv[])
{
   ...
   vector<RooFitResult *> FitResultsTTbar;
   ...
   for(int i = 0; i < NumberOfTries; i++)
   {
      ...
      CutsToTry.push_back(0.305);
      CutsToTry.push_back(0.335);
      CutsToTry.push_back(0.375);
      CutsToTry.push_back(0.405);
      CutsToTry.push_back(0.450);
   ...
   for(int i = 0; i < NumberOfTries; i++)
   {
      ...
      PsFileHelper PsFile(Form("TTbarMultiFit%d.ps", i));
      PsFile.AddTextPage("Fit ttbar in the muon box");
      RooFitResult *Result = FitWithRCut(PsFile, "TTbar.root", CutsToTry);
      PsFile.AddTimeStampPage();
      PsFile.Close();

      FitResultsTTbar.push_back(Result);
   }
   ...
   vector<double> DataAValues;
   vector<double> DataBValues;
   vector<double> TTbarAValues;
   vector<double> TTbarBValues;
   ...
   RooPlot ParameterABPlot(0.001, 0.012, 0, 0.05);
   ParameterABPlot.GetXaxis()->SetTitle("a_{1}");
   ParameterABPlot.GetYaxis()->SetTitle("b_{1}");
   ParameterABPlot.SetTitle("Summary of fits to data and ttbar in EMu selection (no tag)");
   for(int i = 0; i < NumberOfTries; i++)
   {
      ...
      DataAValues.push_back(x);
      DataBValues.push_back(y);
      DataA = DataA + x / NumberOfTries;
      DataB = DataB + y / NumberOfTries;
      
      x = ((RooRealVar *)FitResultsTTbar[i]->floatParsFinal().find("ParameterA1"))->getVal();
      y = ((RooRealVar *)FitResultsTTbar[i]->floatParsFinal().find("ParameterB1"))->getVal();
      ex = ((RooRealVar *)FitResultsTTbar[i]->floatParsFinal().find("ParameterA1"))->getError();
      ey = ((RooRealVar *)FitResultsTTbar[i]->floatParsFinal().find("ParameterB1"))->getError();
      rho = FitResultsTTbar[i]->correlation("ParameterA1", "ParameterB1");
      Contour = new RooEllipse(Form("TTbarEllipse%d", i), x, y, ex, ey, rho);
      Contour->SetLineWidth(2);
      Contour->SetLineColor(kRed);
      ParameterABPlot.addPlotable(Contour);
      
      TTbarAValues.push_back(x);
      TTbarBValues.push_back(y);
      TTbarA = TTbarA + x / NumberOfTries;
      TTbarB = TTbarB + y / NumberOfTries;
   }
   ParameterABPlot.Write();

   sort(DataAValues.begin(), DataAValues.end());
   sort(DataBValues.begin(), DataBValues.end());
   sort(TTbarAValues.begin(), TTbarAValues.end());
   sort(TTbarBValues.begin(), TTbarBValues.end());
   
   TGraph TTbarCenterGraph, DataCenterGraph;
   TTbarCenterGraph.SetMarkerStyle(11);
   DataCenterGraph.SetMarkerStyle(11);
   TTbarCenterGraph.SetMarkerColor(kBlack);
   DataCenterGraph.SetMarkerColor(kBlack);
   for(int i = 0; i < NumberOfTries; i++)
   {
      TTbarCenterGraph.SetPoint(i, TTbarAValues[i], TTbarBValues[i]);
      DataCenterGraph.SetPoint(i, DataAValues[i], DataBValues[i]);
   }

   double DataAMedian = DataAValues[DataAValues.size()/2];
   double DataBMedian = DataBValues[DataBValues.size()/2];
   double TTbarAMedian = TTbarAValues[TTbarAValues.size()/2];
   double TTbarBMedian = TTbarBValues[TTbarBValues.size()/2];

   TGraph DataHorizontalLine, DataVerticalLine;
   DataHorizontalLine.SetPoint(0, -100, DataBMedian);
   DataHorizontalLine.SetPoint(1, 100, DataBMedian);
   DataVerticalLine.SetPoint(0, DataAMedian, -100);
   DataVerticalLine.SetPoint(1, DataAMedian, 100);

   TGraph DataHorizontalLineAverage, DataVerticalLineAverage;
   DataHorizontalLineAverage.SetPoint(0, -100, DataB);
   DataHorizontalLineAverage.SetPoint(1, 100, DataB);
   DataVerticalLineAverage.SetPoint(0, DataA, -100);
   DataVerticalLineAverage.SetPoint(1, DataA, 100);

   TGraph TTbarHorizontalLine, TTbarVerticalLine;
   TTbarHorizontalLine.SetPoint(0, -100, TTbarBMedian);
   TTbarHorizontalLine.SetPoint(1, 100, TTbarBMedian);
   TTbarVerticalLine.SetPoint(0, TTbarAMedian, -100);
   TTbarVerticalLine.SetPoint(1, TTbarAMedian, 100);

   TGraph TTbarHorizontalLineAverage, TTbarVerticalLineAverage;
   TTbarHorizontalLineAverage.SetPoint(0, -100, TTbarB);
   TTbarHorizontalLineAverage.SetPoint(1, 100, TTbarB);
   TTbarVerticalLineAverage.SetPoint(0, TTbarA, -100);
   TTbarVerticalLineAverage.SetPoint(1, TTbarA, 100);

   DataHorizontalLine.SetLineColor(kBlue);
   DataVerticalLine.SetLineColor(kBlue);
   TTbarHorizontalLine.SetLineColor(kRed);
   TTbarVerticalLine.SetLineColor(kRed);
   
   DataHorizontalLineAverage.SetLineColor(kBlue);
   DataVerticalLineAverage.SetLineColor(kBlue);
   TTbarHorizontalLineAverage.SetLineColor(kRed);
   TTbarVerticalLineAverage.SetLineColor(kRed);
   
   DataHorizontalLine.SetLineWidth(2);
   DataVerticalLine.SetLineWidth(2);
   TTbarHorizontalLine.SetLineWidth(2);
   TTbarVerticalLine.SetLineWidth(2);
   
   DataHorizontalLineAverage.SetLineWidth(2);
   DataVerticalLineAverage.SetLineWidth(2);
   TTbarHorizontalLineAverage.SetLineWidth(2);
   TTbarVerticalLineAverage.SetLineWidth(2);

   DataHorizontalLineAverage.SetLineStyle(7);
   DataVerticalLineAverage.SetLineStyle(7);
   TTbarHorizontalLineAverage.SetLineStyle(7);
   TTbarVerticalLineAverage.SetLineStyle(7);

   TLegend legend(0.15, 0.75, 0.8, 0.85);
   legend.SetFillStyle(0);
   legend.SetBorderSize(0);
   legend.SetTextFont(42);
   legend.AddEntry(&DataHorizontalLine, "Data", "l");
   legend.AddEntry(&TTbarHorizontalLine, "t#bar{t}", "l");
   
   TGraph ExampleMedian, ExampleAverage;
   ExampleMedian.SetLineWidth(2);
   ExampleAverage.SetLineWidth(2);
   ExampleAverage.SetLineStyle(7);

   TLegend legend2(0.15, 0.6, 0.8, 0.7);
   ...
   DataHorizontalLine.Draw("l");
   DataVerticalLine.Draw("l");
   TTbarHorizontalLine.Draw("l");
   TTbarVerticalLine.Draw("l");
   DataHorizontalLineAverage.Draw("l");
   DataVerticalLineAverage.Draw("l");
   TTbarHorizontalLineAverage.Draw("l");
   TTbarVerticalLineAverage.Draw("l");
   legend.Draw();
   legend2.Draw();
   PsFile.AddCanvas(ParameterABCanvas);
   
   TLegend legend3(0.15, 0.6, 0.8, 0.85);
   legend3.SetFillStyle(0);
   legend3.SetBorderSize(0);
   legend3.SetTextFont(42);
   legend3.AddEntry(&DataHorizontalLine, "Data", "l");
   legend3.AddEntry(&TTbarHorizontalLine, "t#bar{t}+jet", "l");
   legend3.AddEntry((TObject *)NULL, "", "");
   legend3.AddEntry(&ExampleMedian, "Median", "l");
   legend3.AddEntry(&ExampleAverage, "Average", "l");
   
   TCanvas ParameterABCanvas2("C2", "C2", 1024, 1024);
   ParameterABPlot.SetTitle("");
   ParameterABPlot.GetXaxis()->SetTitle("a_{1}");
   ParameterABPlot.GetYaxis()->SetTitle("b_{1}");
   ParameterABPlot.Draw();
   DataHorizontalLine.Draw("l");
   ...
   TTbarVerticalLineAverage.Draw("l");
   legend3.Draw();
   DataCenterGraph.Draw("p");
   TTbarCenterGraph.Draw("p");
   AddCMSPreliminary(0.1, 0.925, -1, 0.025);
   ParameterABCanvas2.SaveAs("EMuFit.png");
   ParameterABCanvas2.SaveAs("EMuFit.C");
   ParameterABCanvas2.SaveAs("EMuFit.eps");
   ParameterABCanvas2.SaveAs("EMuFit.pdf");
   ...
   for(int i = 0; i < NumberOfTries; i++)
   {
      ...
      if(FitResultsTTbar[i] != NULL)
         FitResultsTTbar[i]->plotOn(&ParameterABPlot2, "ParameterA1", "ParameterB1", "M");
   }
...
RooFitResult *FitWithRCut(PsFileHelper &PsFile, string FileName, vector<double> RCuts)
{
   ...
   ParameterA1Value = DrawRandom(0.5, 1.5) * ParameterA1Value;
   ParameterB1Value = DrawRandom(0.5, 1.5) * ParameterB1Value;
   ParameterA2Value = DrawRandom(0.5, 1.5) * ParameterA2Value;
   ParameterB2Value = DrawRandom(0.5, 1.5) * ParameterB2Value;
