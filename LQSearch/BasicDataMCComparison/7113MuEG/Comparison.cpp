...
void ReadSample(string Input, string OutputBase, double CrossSection, double Luminosity)
{
   ...
   for(int iEntry = 0; iEntry < EntryCount; iEntry++)
   {
      ...
      if(M.Muon1PassLoose == true && M.GoodMuonCount >= 1)
      {
         FourVector MuP;
         MuP.SetPtEtaPhi(M.Muon1PT, M.Muon1Eta, M.Muon1Phi);
         GoodJets.push_back(MuP);
      }
      if(M.Muon2PassLoose == true && M.GoodMuonCount >= 2)
      {
         FourVector MuP;
         MuP.SetPtEtaPhi(M.Muon2PT, M.Muon2Eta, M.Muon2Phi);
         GoodJets.push_back(MuP);
      }
      if(M.Muon3PassLoose == true && M.GoodMuonCount >= 3)
      {
         FourVector MuP;
         MuP.SetPtEtaPhi(M.Muon3PT, M.Muon3Eta, M.Muon3Phi);
         GoodJets.push_back(MuP);
      }
