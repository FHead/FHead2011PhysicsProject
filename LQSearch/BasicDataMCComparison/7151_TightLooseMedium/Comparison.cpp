...
int main()
{
   ...
   ReadSample("SamplesNew/HTRun2011Av4_UpTo167746.root", "DataPlots", -1, 747);
   ...
   ReadSample("Samples/ZZtoAnything_TuneZ2_7TeV-pythia6-tauola_All.root", "ZZ", 4.2, 747);   // Production
   ReadSample("Samples/WZtoAnything_TuneZ2_7TeV-pythia6-tauola_All.root", "WZ", 10.4, 747);   // Production
   ReadSample("Samples/WWtoAnything_TuneZ2_7TeV-pythia6-tauola_All.root", "WW", 27.79, 747);   // Production
   ReadSample("Samples/PhotonVJets_7TeV-madgraph_All.root", "GammaVJet", 173, 747);   // Production
   // ReadSample("Samples/DYJetsToLL_TuneD6T_M-10To50_7TeV-madgraph-tauola_All.root", "DY10", 310, 747);   // Production
   ReadSample("Samples/DYJetsToLL_TuneD6T_M-10To50_7TeV-madgraph-tauola_All.root", "DY10", 3100, 747);   // Guess
   ReadSample("Samples/DYJetsToLL_TuneD6T_M-50_7TeV-madgraph-tauola_All.root", "DY50", 3048, 747);   // NNLO
}
...
void ReadSample(string Input, string OutputBase, double CrossSection, double Luminosity)
{
   ...
   TH1D HPFJetMR_R02("HPFJetMR_R02", "MR from calo system (R > 0.2);MR (GeV/c^2)", 40, 0, 2000);
   TH1D HPFJetMR_R03("HPFJetMR_R03", "MR from calo system (R > 0.3);MR (GeV/c^2)", 40, 0, 2000);
   TH1D HPFJetMR_R04("HPFJetMR_R04", "MR from calo system (R > 0.4);MR (GeV/c^2)", 40, 0, 2000);
   TH1D HPFJetMR_R05("HPFJetMR_R05", "MR from calo system (R > 0.5);MR (GeV/c^2)", 40, 0, 2000);
   TH1D HPFJetMR_R06("HPFJetMR_R06", "MR from calo system (R > 0.6);MR (GeV/c^2)", 40, 0, 2000);
   ...
   TH1D HPFJetGammaMRStar_RStar06("HPFJetGammaMRStar_RStar06",
      "MR*** from calo system (RStar > 0.6);MR*** (GeV/c^2)", 40, 0, 2000);
   ...
   for(int iEntry = 0; iEntry < EntryCount; iEntry++)
   {
      ...
      for(int i = 0; i < M.PFJetCount && i < 100; i++)
         if(M.PFJetPT[i] > 60 && M.PFJetEta[i] < 3 && M.PFJetEta[i] > -3)
            PFJetCount60 = PFJetCount60 + 1;
      if(PFJetCount60 < 2)
         continue;

      int JetPassBTagTight = 0;
      int JetPassBTagLoose = 0;
      for(int i = 0; i < M.CaloJetCount; i++)
      {
         if(M.CaloJetTCHETag[i] > 10.2 && M.CaloJetPT[i] > 60 && M.CaloJetEta[i] < 3 && M.CaloJetEta[i] > -3)
            JetPassBTagTight = JetPassBTagTight + 1;
         if(M.CaloJetTCHETag[i] > 1.7 && M.CaloJetPT[i] > 60 && M.CaloJetEta[i] < 3 && M.CaloJetEta[i] > -3)
            JetPassBTagLoose = JetPassBTagLoose + 1;
      }
      if(JetPassBTagLoose < 2)
         continue;
      if(JetPassBTagTight < 1)
         continue;
      ...
      */
      ...
      if(fabs(GetDPhi(Hemispheres[0], Hemispheres[1])) > 2.8)
         continue;
      ...
      if(R > 0.2)
         HPFJetMR_R02.Fill(MR, Factor);
      if(R > 0.3)
         HPFJetMR_R03.Fill(MR, Factor);
      if(R > 0.4)
         HPFJetMR_R04.Fill(MR, Factor);
      if(R > 0.5)
         HPFJetMR_R05.Fill(MR, Factor);
      if(R > 0.6)
         HPFJetMR_R06.Fill(MR, Factor);
      ...
      if(RStar > 0.6)
         HPFJetGammaMRStar_RStar06.Fill(MRStar * GammaRStar, Factor);
   }
   ...
   HPFJetGammaMRStar_RStar06.Write();
   HPFMET.Write();
   ...
   PsFile.AddPlot(HPFJetMR_R02, "", true);
   PsFile.AddPlot(HPFJetMR_R03, "", true);
   PsFile.AddPlot(HPFJetMR_R04, "", true);
   PsFile.AddPlot(HPFJetMR_R05, "", true);
   PsFile.AddPlot(HPFJetMR_R06, "", true);
   ...
   PsFile.AddPlot(HPFJetGammaMRStar_RStar06, "", true);
