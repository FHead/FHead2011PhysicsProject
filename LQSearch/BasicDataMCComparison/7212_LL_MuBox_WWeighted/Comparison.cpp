...
int main();
void ReadSample(string Input, string OutputBase, double CrossSection, double Luminosity, bool SpecialTreatment = false);

int main()
{
   SetStyle();

   // ReadSample("DataSamples/MuHadRun2011Av4_All.root", "DataPlots", -1, 718);
   // ReadSample("Samples/TTJets_TuneZ2_7TeV-madgraph-tauola_All.root", "TTbar", 165, 718);   // NNLL
   // ReadSample("Samples/WJetsToLNu_TuneZ2_7TeV-madgraph-tauola_All.root", "WNormal", 31314, 718);   // NNLO
   ReadSample("Samples/WJetsToLNu_TuneZ2_7TeV-madgraph-tauola_All.root", "W", 31314, 718, true);   // NNLO
   /*
   ReadSample("Samples/VVJetsTo4L_TuneD6T_7TeV-madgraph-tauola_All.root", "VV", 4.8, 718);   // Production
   ReadSample("Samples/PhotonVJets_7TeV-madgraph_All.root", "GammaVJet", 173, 718);   // Production
   ReadSample("Samples/DYJetsToLL_TuneZ2_M-50_7TeV-madgraph-tauola_All.root", "DY50", 3048, 718);   // NNLO
   ReadSample("Samples/ZinvisibleJets_7TeV-madgraph_All.root", "Znunu", 3048 * 6, 718);   // guessed
   ReadSample("Samples/QCD_TuneD6T_HT-50To100_7TeV-madgraph_All.root", "QCD1", 30000000, 718);   // production
   ReadSample("Samples/QCD_TuneD6T_HT-100To250_7TeV-madgraph_All.root", "QCD2", 7000000, 718);   // production
   ReadSample("Samples/QCD_TuneD6T_HT-250To500_7TeV-madgraph_All.root", "QCD3", 171000, 718);   // production
   ReadSample("Samples/QCD_TuneD6T_HT-500To1000_7TeV-madgraph_All.root", "QCD4", 5200, 718);   // production
   ReadSample("Samples/QCD_TuneD6T_HT-1000_7TeV-madgraph_All.root", "QCD5", 83, 718);   // production
   */
}

void ReadSample(string Input, string OutputBase, double CrossSection, double Luminosity, bool SpecialTreatment)
{
   ...
   TaggingEfficiencyHelper NonBHelper;
   ...
   for(int iEntry = 0; iEntry < EntryCount; iEntry++)
   {
      ...
      if(M.GoodMuonCountTight != 1)   // one good muon!!!!!
         continue;
      if(M.GoodElectronCount80 != 0)   // no good electron!!!
         continue;
      if(M.Muon1PT < 20)
         continue;
      ...
      for(int i = 0; i < M.CaloJetCount && i < 100; i++)
      {
         ...
         if(M.CaloJetTCHETag[i] > 1.7)
            BTagLooseCount = BTagLooseCount + 1;

         if(SpecialTreatment == true)
         {
            double Efficiency = NonBHelper.GetEfficiency(M.CaloJetPT[i], M.CaloJetEta[i]);
            Efficiencies.push_back(Efficiency);
         }
      }
      if(BTagLooseCount < 2 && SpecialTreatment == false)   // double loose
         continue;
      if(SpecialTreatment == true)
         WPU = WPU * EventTaggedChance(Efficiencies, 2);
      ...
      for(int i = 0; i < M.CaloJetCount && i < 100; i++)
      {
         ...
         if(GetDR(NewJet, ElectronP) < 0.3)   // Das ist das Elektron!  Ueberspringen!!!
            continue;
