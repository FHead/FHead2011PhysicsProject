   ...
   vector<bool> Bundle)
{
   ...
   for(int i = (int)Files.size() - 1; i >= 1; i--)
   {
      ...
      StackedHistograms[i]->Draw("same hist");
   }
   ...
   for(int i = (int)StackedHistograms2.size() - 1; i >= 1; i--)
   {
      ...
      StackedHistograms2[i]->Draw("same hist");
   }
   Histograms[0]->Draw("error point same");
   C4.SetLogy();
   legend2.Draw();
   ...
   TGraph LineAtOne;
   LineAtOne.SetNameTitle("LineAtOne", "A horizontal line at 1");
   LineAtOne.SetPoint(0, 0, 1);
   LineAtOne.SetPoint(1, 99999, 1);

   TCanvas C5("C5", "C5", 1024, 512);
   TH1D *RatioHistogram = (TH1D *)Histograms[0]->Clone("RatioHistogram");
   RatioHistogram->SetMinimum(0.1);
   RatioHistogram->SetMaximum(10.0);
   RatioHistogram->Divide(StackedHistograms2[StackedHistograms2.size()-1]);
   RatioHistogram->GetYaxis()->SetTitle("Data / MC");
   RatioHistogram->Draw("error");
   LineAtOne.Draw("l");
   C5.SetLogy();
   ...
   cout << Histograms[0]->Integral() << endl;
   cout << StackedHistograms2[StackedHistograms2.size()-1]->Integral() << endl;

   TCanvas C6("C6", "C6", 1024, 1024 + 512);
   ...
   C6_2.SetLogy();
   C6_2.Modified();
   C6.SaveAs((HistogramName + "_Together.png").c_str());
   C6.SaveAs((HistogramName + "_Together.C").c_str());
   C6.SaveAs((HistogramName + "_Together.eps").c_str());
