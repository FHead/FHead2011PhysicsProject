...
int main()
{
   ...
   ReadSample("Samples/TTJets_TuneZ2_7TeV-madgraph-tauola_All.root", "TTbar", 165, 1752.853);   // NNLL
   ReadSample("Samples/AllWJetsToLNu_Added.root", "W", 31314, 1752.853);   // NNLO
   ReadSample("Samples/VVJetsTo4L_TuneD6T_7TeV-madgraph-tauola_All.root", "VV", 4.8, 1752.853);   // Production
   ReadSample("Samples/PhotonVJets_7TeV-madgraph_All.root", "GammaVJet", 173, 1752.853);   // Production
   ReadSample("Samples/DYJetsToLL_TuneZ2_M-50_7TeV-madgraph-tauola_All.root", "DY50", 3048, 1752.853);   // NNLO
   ReadSample("Samples/ZinvisibleJets_7TeV-madgraph_All.root", "Znunu", 3048 * 6, 1752.853);   // guessed
   ReadSample("Samples/QCD_TuneD6T_HT-50To100_7TeV-madgraph_All.root", "QCD1", 30000000, 1752.853);   // production
   ReadSample("Samples/QCD_TuneD6T_HT-100To250_7TeV-madgraph_All.root", "QCD2", 7000000, 1752.853);   // production
   ReadSample("Samples/QCD_TuneD6T_HT-250To500_7TeV-madgraph_All.root", "QCD3", 171000, 1752.853);   // production
   ReadSample("Samples/QCD_TuneD6T_HT-500To1000_7TeV-madgraph_All.root", "QCD4", 5200, 1752.853);   // production
   ReadSample("Samples/QCD_TuneD6T_HT-1000_7TeV-madgraph_All.root", "QCD5", 83, 1752.853);   // production
   */
   ReadSample("Samples/LQ200.root", "LQ200", 11.9, 1752.853);   // production
   ReadSample("Samples/LQ250.root", "LQ250", 3.47, 1752.853);   // production
   ReadSample("Samples/LQ350.root", "LQ350", 0.477, 1752.853);   // production
   ReadSample("Samples/LQ450.root", "LQ450", 0.0949, 1752.853);   // production
}
...
void ReadSample(string Input, string OutputBase, double CrossSection, double Luminosity)
{
   ...
   TH1D HR2New_MRNew400("HR2New_MRNew400", "R^{2} (new), MR (new) > 400;R^{2} (new)", 50, 0, 1.0);
   TH1D HMRNew_RNew02("HMRNew_RNew02", "MR (new) (R^{2} (new) > 0.04;MR (new) (GeV/c^2)", 50, 0, 1500);
   TH1D HMRNew_RNew03("HMRNew_RNew03", "MR (new) (R^{2} (new) > 0.09;MR (new) (GeV/c^2)", 50, 0, 1500);
   TH1D HMRNew_RNew04("HMRNew_RNew04", "MR (new) (R^{2} (new) > 0.16;MR (new) (GeV/c^2)", 50, 0, 1500);
   TH1D HMRNew_RNew05("HMRNew_RNew05", "MR (new) (R^{2} (new) > 0.25;MR (new) (GeV/c^2)", 50, 0, 1500);
   TH1D HMRNew_RNew05_2("HMRNew_RNew05_2", "MR (new) (R^{2} (new) > 0.25;MR (new) (GeV/c^2)", 25, 400, 1400);
   TH1D HMRNew_RNew06("HMRNew_RNew06", "MR (new) (R^{2} (new) > 0.36;MR (new) (GeV/c^2)", 50, 0, 1500);
   ...
   for(int iEntry = 0; iEntry < EntryCount; iEntry++)
   {
      ...
      for(int i = 0; i < M.CaloJetCount && i < 100; i++)
      {
         ...
         if(M.CaloJetPT[i] < 80)
            continue;
         double Tag = -1;
         for(int j = 0; j < M.PFJetCount && j < 100; j++)
         {
         }
         if(Tag < 1.7)
            continue;
         JetTagCount = JetTagCount + 1;
      }
      if(JetTagCount < 2)
         continue;
      ...
      if(MRNew > 300 && R2New > 0.09 && MegaJetDeltaPhi > 0.25)
      {
         ...
         if(MRNew > 400)
            HR2New_MRNew400.Fill(R2New, Factor * WPU);
         ...
         if(RNew > 0.5)
            HMRNew_RNew05_2.Fill(MRNew, Factor * WPU);
         if(RNew > 0.6)
      ...
      if(MegaJetDeltaPhi > 0.25)
      {
         ...
         if(MRNew > 400 && R2New > 0.25)
            cout << "[Passed Event] " << (int)M.RunNumber
               << " " << (int)M.LumiSection << " " << (long long)M.EventNumber << endl;
      }
   }
   ...
   PsFile.AddPlot(HMRNew_RNew05_2, "", true);
