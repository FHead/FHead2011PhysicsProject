...
int main()
{
   ...
   // for(int i = 0; i < 4; i++)
   //    SignalFiles.push_back(NULL);

   string Category[8] = {"Had", "TightMuon", "LooseMuon", "TightElectron", "LooseElectron",
   ...
   for(int k = 0; k < 3; k++)   // LQ 550 is weird
   {
      for(int i = 0; i < 8; i++)
      {
         TFile *DataFile = NULL;
         ...
         if(i == 0)   Luminosity = 4.625;
         if(i == 1)   Luminosity = 2.826;
         if(i == 2)   Luminosity = 2.826;
         if(i == 3)   Luminosity = 4.700;
         if(i == 4)   Luminosity = 4.700;
         if(i == 5)   Luminosity = 4.698;
         if(i == 6)   Luminosity = 4.698;
         if(i == 7)   Luminosity = 4.716;
         ...
         for(int j = 0; j < 8; j++)
         {
            ...
            MakeOverlay(PsFile, MCFiles, DataFile, SignalFiles[k], "HPhotonIsolation_" + Category[i] + "_" + BTag[j], Labels, Luminosity, LQMasses[k]);
            MakeOverlay(PsFile, MCFiles, DataFile, SignalFiles[k], "HPhotonHOverE_" + Category[i] + "_" + BTag[j], Labels, Luminosity, LQMasses[k]);
            MakeOverlay(PsFile, MCFiles, DataFile, SignalFiles[k], "HPhotonR9_" + Category[i] + "_" + BTag[j], Labels, Luminosity, LQMasses[k]);
            MakeOverlay(PsFile, MCFiles, DataFile, SignalFiles[k], "HPhotonSigmaIEtaIEta_" + Category[i] + "_" + BTag[j], Labels, Luminosity, LQMasses[k]);

            MakeOverlay(PsFile, MCFiles, DataFile, SignalFiles[k], "HAllPhotonPT_" + Category[i] + "_" + BTag[j], Labels, Luminosity, LQMasses[k]);
            MakeOverlay(PsFile, MCFiles, DataFile, SignalFiles[k], "HAllPhotonIsolation_" + Category[i] + "_" + BTag[j], Labels, Luminosity, LQMasses[k]);
            MakeOverlay(PsFile, MCFiles, DataFile, SignalFiles[k], "HAllPhotonHOverE_" + Category[i] + "_" + BTag[j], Labels, Luminosity, LQMasses[k]);
            MakeOverlay(PsFile, MCFiles, DataFile, SignalFiles[k], "HAllPhotonR9_" + Category[i] + "_" + BTag[j], Labels, Luminosity, LQMasses[k]);
            MakeOverlay(PsFile, MCFiles, DataFile, SignalFiles[k], "HAllPhotonSigmaIEtaIEta_" + Category[i] + "_" + BTag[j], Labels, Luminosity, LQMasses[k]);
...
void MakeOverlay(PsFileHelper &PsFile, vector<TFile *> Files, TFile *DataFile, TFile *SignalFile, string HistogramName, vector<string> Labels, double Luminosity, int LQMass)
{
   ...
   // cout << "? " << DataHist->GetBinContent(1) << endl;
   ...
   // C.SaveAs(("IndividualPlots/" + HistogramName + ".C").c_str());
   // C.SaveAs(("IndividualPlots/" + HistogramName + ".eps").c_str());
   // C.SaveAs(("IndividualPlots/" + HistogramName + ".pdf").c_str());
