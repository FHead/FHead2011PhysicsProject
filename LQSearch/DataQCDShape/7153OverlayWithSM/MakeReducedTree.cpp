...
int main()
{
   ...
   OutputTree.Branch("PassDiJetAve30Int", &PassDiJetAve30Int, "PassDiJetAve30/I");
   ...
   for(int iEntry = 0; iEntry < EntryCount; iEntry++)
   {
      ...
      if(M.PrimaryVertexCount == 0)
         continue;
      // if(M.PassCaloJetID == false)
      //    continue;

      vector<FourVector> InputJets;
      for(int i = 0; i < M.CaloJetCount; i++)
      {
         if(M.CaloJetEta[i] < -2.4 || M.CaloJetEta[i] > 2.4)
            continue;
         if(M.CaloJetPT[i] < 30)
            continue;
      ...
      R = GetR(Groups[0], Groups[1], CaloMET);
      MRStar = GetMRStar(Groups[0], Groups[1]);
      RStar = GetRStar(Groups[0], Groups[1], CaloMET);
      ...
      PassDiJetAve30Int = (int)PassDiJetAve30;
