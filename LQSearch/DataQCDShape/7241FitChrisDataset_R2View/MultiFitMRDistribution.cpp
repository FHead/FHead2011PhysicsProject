...
int main(int argc, char *argv[])
{
   ...
   for(int i = 0; i < NumberOfTries; i++)
   {
      ...
      CutsToTry.push_back(DrawRandom(200, 220));
      CutsToTry.push_back(DrawRandom(250, 270));
      CutsToTry.push_back(DrawRandom(300, 320));

      PsFileHelper PsFile(Form("%sMultiFit%d.ps", OutputBase.c_str(), i));
      PsFile.AddTextPage("Fit!!");
      RooFitResult *Result = FitWithRCut(PsFile, InputFile, CutsToTry, BoxNumber, BTagRequirement, R2Min, OneComponent);
   ...
   vector<double> CutsToTry;
   CutsToTry.push_back(200);
   CutsToTry.push_back(250);
   CutsToTry.push_back(300);
   CutsToTry.push_back(225);
   CutsToTry.push_back(275);
   CutsToTry.push_back(325);
   PsFileHelper PsFileNominal(Form("%sMultiFitNominal.ps", OutputBase.c_str()));
   PsFileNominal.AddTextPage("Fit!!");
   RooFitResult *NominalResult = FitWithRCut(PsFileNominal, InputFile, CutsToTry, BoxNumber, BTagRequirement, R2Min, OneComponent, "NominalFitResult");
   ...
   RooFitResult *NominalResult2 = FitWithRCut(PsFileNominal2, InputFile, CutsToTry, BoxNumber, BTagRequirement, R2Min, OneComponent, "NominalFitResult2");
   ...
   for(int i = 0; i < NumberOfTries; i++)
   {
      ...
      if(ey1 > 0.1)   // potentially bad fit
         continue;
   ...
   double R2Min, bool OneComponent)
{
   ...
   */
   ...
   RooExponential Component1("Component1", "First component", R, S1);
   
   RooRealVar S2("S2", "second exponent of the tail", -8, -500.0, 0.0);
   RooExponential Component2("Component2", "Second component", R, S2);
   ...
   RooPlot *FinalResultPlot = R.frame(Bins(50), Name("FitResults"), Title("R^{2}"), Range(R2Min, 0.25));
   ...
   if(OneComponent == false && fabs(S2.getVal()) > fabs(S1.getVal()))
   {
      result.S1 = -S2.getVal();
   ...
   double R2Min, bool OneComponent, string OutputBase)
{
   ...
   if(OutputBase != "")
   {
      ...
      SingleCentralValues1.GetXaxis()->SetTitle("MR Cut [GeV]");
      SingleCentralValues1.GetYaxis()->SetTitle("Slope Parameter");
   ...
   */
   ...
   RooPlot *FinalResultPlot = R.frame(Bins(50), Name("FitResults"), Title("R^{2}, Simultaneous fit, total model"), Range(R2Min, 0.2));
   Dataset.plotOn(FinalResultPlot, MarkerSize(0.9), DataError(RooAbsData::SumW2));
   FinalModel.plotOn(FinalResultPlot, LineColor(kBlue));
   PsFile.AddPlot(FinalResultPlot, "", true);

   // Each bin, model vs. data
   RooPlot *FinalResultPlot2 = R.frame(Bins(50), Name("FitResults2"), Title("   "), Range(R2Min, 0.2));
   ...
   TLegend legend(0.65, 0.8, 0.8, 0.5);
   legend.SetTextFont(42);
   legend.SetFillStyle(0);
   legend.SetBorderSize(0);
   for(int i = 0; i < (int)MRCuts.size(); i++)
