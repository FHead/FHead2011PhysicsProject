   ...
   double Total, bool LogNormal)
{
   ...
   const int NumberOfTries = 1000;
   const int BaseSampleCount = 1000;
   ...
   TH1D HLogLikelihoodRatio("HLogLikelihoodRatio", "Log likelihood ratio (S+B on S+B)", 100, 0, 50);
   ...
   TH1D HBLikelihoodRatio("HBLikelihoodRatio", "B likelihood ratio", 100, 0, 50);
   ...
   for(int i = 0; i < BaseSampleCount; i++)
   {
      ...
      NLLVar->createProfile(RooArgSet())->getVal();
