...
int main();
map<double, pair<double, double> > ReadCrossSection(string FileName = "squark_crosssection.txt");
pair<double, double> CrossSection(const map<double, pair<double, double> > &ReferencePoints, double Mass);
...
int main()
{
   ...
   TH2D HExpectedMinus2Copy("HExpectedMinus2Copy",
      "Expected (-2) limit - cross section;m_{sb};m_{#chi};#sigma (pb)", 50, -12.5, 1237.5, 50, -12.5, 1237.5);
   TH2D HObservedPlusCopy("HObservedPlusCopy",
      "Observed (+1) limit - cross section;m_{sb};m_{#chi};#sigma (pb)", 50, -12.5, 1237.5, 50, -12.5, 1237.5);
   TH2D HObservedMinusCopy("HObservedMinusCopy",
      "Observed (-1) limit - cross section;m_{sb};m_{#chi};#sigma (pb)", 50, -12.5, 1237.5, 50, -12.5, 1237.5);

   TH2D HSimpleCount("HSimpleCount",
      "count", 50, -12.5, 1237.5, 50, -12.5, 1237.5);

   TH2D HWorld("HWorld", "Limit curves;Sbottom;Neutralino;#sigma (pb)", 50, 0, 600, 50, 0, 600);
   HWorld.SetStats(0);

   map<double, pair<double, double> > Sigma = ReadCrossSection("SUSYCrossSectionCentral.txt");

   TH1D HReferenceSigma("HReferenceSigma", "Reference cross section;Squark mass;Sigma", 1000, 100, 900);
   for(int i = 1; i <= 1000; i++)
      HReferenceSigma.SetBinContent(i, CrossSection(Sigma, HReferenceSigma.GetXaxis()->GetBinCenter(i)).first);
   for(int i = 1; i <= 50; i++)
      for(int j = 1; j <= 50; j++)
         if(i >= j)
            HCrossSection.SetBinContent(i, j, CrossSection(Sigma, HCrossSection.GetXaxis()->GetBinCenter(i)).first);
   ...
   while(in)
   {
      ...
      HObservedPlusCopy.Fill(m0, m12, obs);
      HObservedMinusCopy.Fill(m0, m12, obs);

      HSimpleCount.Fill(m0, m12, 1);
   }

   HExpected.Divide(&HSimpleCount);
   HObserved.Divide(&HSimpleCount);
   HExpectedPlus.Divide(&HSimpleCount);
   HExpectedPlus2.Divide(&HSimpleCount);
   HExpectedMinus.Divide(&HSimpleCount);
   HExpectedMinus2.Divide(&HSimpleCount);
   
   HExpectedCopy.Divide(&HSimpleCount);
   HObservedCopy.Divide(&HSimpleCount);
   HExpectedPlusCopy.Divide(&HSimpleCount);
   HExpectedPlus2Copy.Divide(&HSimpleCount);
   HExpectedMinusCopy.Divide(&HSimpleCount);
   HExpectedMinus2Copy.Divide(&HSimpleCount);
   HObservedPlusCopy.Divide(&HSimpleCount);
   HObservedMinusCopy.Divide(&HSimpleCount);
   ...
   HObservedPlusCopy.SetStats(0);
   HObservedMinusCopy.SetStats(0);
   ...
   HObservedPlusCopy.GetZaxis()->SetTitleOffset(0.5);
   HObservedMinusCopy.GetZaxis()->SetTitleOffset(0.5);

   FillInHoles(HExpectedCopy);
   FillInHoles(HObservedCopy);
   FillInHoles(HExpectedPlusCopy);
   FillInHoles(HExpectedMinusCopy);
   FillInHoles(HObservedPlusCopy);
   FillInHoles(HObservedMinusCopy);
   ...
   for(int i = 1; i <= HExpectedCopy.GetNbinsX(); i++)
   {
      for(int j = 1; j <= HExpectedCopy.GetNbinsY(); j++)
      {
         ...
         pair<double, double> TheorySigma = CrossSection(Sigma, HExpectedCopy.GetXaxis()->GetBinCenter(i));
         double Theory = TheorySigma.first;
         double TheoryShift = TheorySigma.second;
         ...
         HObservedPlusCopy.SetBinContent(i, j, HObservedPlusCopy.GetBinContent(i, j) - Theory - TheoryShift);
         HObservedMinusCopy.SetBinContent(i, j, HObservedMinusCopy.GetBinContent(i, j) - Theory + TheoryShift);
      }
   }
   ...
   PsFile.AddPlot(HObservedPlusCopy, "colz", false, false);   HObservedPlusCopy.Write();
   PsFile.AddPlot(HObservedPlusCopy, "colz", false, true);
   PsFile.AddPlot(HObservedMinusCopy, "colz", false, false);   HObservedMinusCopy.Write();
   PsFile.AddPlot(HObservedMinusCopy, "colz", false, true);
   ...
   HObservedPlusCopy.SetContour(1, Contours);
   HObservedMinusCopy.SetContour(1, Contours);
   ...
   HObservedPlusCopy.Draw("cont3 same");
   HObservedMinusCopy.Draw("cont3 same");
   ...
   HObservedPlusCopy.SetLineWidth(2);
   HObservedMinusCopy.SetLineWidth(2);
   ...
   HObservedPlusCopy.SetLineColor(kBlack);
   HObservedMinusCopy.SetLineColor(kBlack);
   ...
   // Legend.AddEntry(&HObservedPlusCopy, "Observed Limit (+1 #sigma)", "l");
   // Legend.AddEntry(&HObservedMinusCopy, "Observed Limit (-1 #sigma)", "l");
   Legend.Draw();
   ...
   HObservedPlusCopy.Draw("cont list");
   C.Update();
   ContourArray = (TObjArray *)(gROOT->GetListOfSpecials()->FindObject("contours"));
   ContourCount = ContourArray->GetSize();
   List = (TList *)ContourArray->At(0);
   TGraph *GR5 = (TGraph *)((TGraph *)List->First())->Clone("ObservedPlusContour");
   
   HObservedMinusCopy.Draw("cont list");
   C.Update();
   ContourArray = (TObjArray *)(gROOT->GetListOfSpecials()->FindObject("contours"));
   ContourCount = ContourArray->GetSize();
   List = (TList *)ContourArray->At(0);
   TGraph *GR6 = (TGraph *)((TGraph *)List->First())->Clone("ObservedMinusContour");
   
   PsFile.AddPlot(GR1, "al");
   PsFile.AddPlot(GR2, "al");
   PsFile.AddPlot(GR3, "al");
   PsFile.AddPlot(GR4, "al");
   PsFile.AddPlot(GR5, "al");
   PsFile.AddPlot(GR6, "al");

   TCanvas CObserved;
   GR4->Draw("al");
   GR5->Draw("l");
   GR6->Draw("l");
   PsFile.AddCanvas(CObserved);
   ...
   ListPoints(*GR5, "ObservedPlusGraph.txt");
   ListPoints(*GR6, "ObservedMinuxGraph.txt");
...
map<double, pair<double, double> > ReadCrossSection(string FileName)
{
   ...
   while(in)
   {
      ...
      if(temp1 < 0 || temp2 < 0 || temp3 < 0)
         continue;
      temp3 = temp3 / 100 * temp2;   // convert to absolute change

      result.insert(pair<double, pair<double, double> >(temp1, pair<double, double>(temp2, temp3)));
   }
...
pair<double, double> CrossSection(const map<double, pair<double, double> > &ReferencePoints, double Mass)
{
   ...
   double logPreviousSigma = ReferencePoints.begin()->second.first;
   double logPreviousError = ReferencePoints.begin()->second.second;
   double NextMass = 0;
   double logNextSigma = 0;
   double logNextError = 0;

   for(map<double, pair<double, double> >::const_iterator iter = ReferencePoints.begin(); iter != ReferencePoints.end(); iter++)
   {
      if(Mass < iter->first)
      {
         NextMass = iter->first;
         logNextSigma = log(iter->second.first);
         logNextError = log(iter->second.second);
         break;
      }

      PreviousMass = iter->first;
      logPreviousSigma = log(iter->second.first);
      logPreviousError = log(iter->second.second);
   }

   if(NextMass > 0)
   {
      double logCrossSection = (NextMass - Mass) / (NextMass - PreviousMass) * (logPreviousSigma - logNextSigma) + logNextSigma;
      double logCrossSectionError = (NextMass - Mass) / (NextMass - PreviousMass) * (logPreviousError - logNextError) + logNextError;
      return pair<double, double>(exp(logCrossSection), exp(logCrossSectionError));
   }
