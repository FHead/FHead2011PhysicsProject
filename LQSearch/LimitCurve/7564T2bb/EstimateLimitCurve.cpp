...
int main()
{
   ...
   TGraph G;
   MapOutContour(HExpected, HExpectedCopy, G);
   MapOutContour(HObserved, HObservedCopy, G);
   MapOutContour(HExpectedPlus, HExpectedPlusCopy, G);
   MapOutContour(HExpectedMinus, HExpectedMinusCopy, G);
...
double CrossSection(const map<double, double> &ReferencePoints, double Mass)
{
   ...
   if(Mass < ReferencePoints.begin()->first)
      return ReferencePoints.begin()->second / 5 / 2;
   ...
   if(NextMass > 0)
   {
      ...
      return exp(logCrossSection) / 5 / 2;
   }
