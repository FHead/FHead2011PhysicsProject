...
int main(int argc, char *argv[])
{
   ...
   BTaggingEfficiency("TTbar.list", Tagger_TCHE, 1.7, "ReproduceBTaggingEfficiencyPU_TCHEL");
   BTaggingEfficiency("TTbar.list", Tagger_TCHE, 1.7, "ReproduceBTaggingEfficiencyPUSmall_TCHEL", 0, 4);
   BTaggingEfficiency("TTbar.list", Tagger_TCHE, 1.7, "ReproduceBTaggingEfficiencyPUMedium_TCHEL", 5, 7);
   BTaggingEfficiency("TTbar.list", Tagger_TCHE, 1.7, "ReproduceBTaggingEfficiencyPULarge_TCHEL", 8, 1000);
...
void BTaggingEfficiency(string InputList, int Tagger, double CutValue, string OutputBase, int PUMin, int PUMax)
{
   ...
   case Tagger_TCHE:
      TaggerString = "TCHE";
      Tag = M.trackCountingHighEffBJetTagsAK5PFPUcorrJet;
      break;
   case Tagger_TCHP:
      TaggerString = "TCHP";
      Tag = M.trackCountingHighPurBJetTagsAK5PFPUcorrJet;
      break;
   case Tagger_JetProbability:
      TaggerString = "JetProbability";
      Tag = M.jetProbabilityBJetTagsAK5PFPUcorrJet;
      break;
   case Tagger_JetBProbability:
      TaggerString = "JetBProbability";
      Tag = M.jetBProbabilityBJetTagsAK5PFPUcorrJet;
      break;
   case Tagger_SSVHE:
      TaggerString = "SSVHE";
      Tag = M.simpleSecondaryVertexHighEffBJetTagsAK5PFPUcorrJet;
      break;
   case Tagger_SSVHP:
      TaggerString = "SSVHP";
      Tag = M.simpleSecondaryVertexHighPurBJetTagsAK5PFPUcorrJet;
      break;
   case Tagger_CSV:
      TaggerString = "CSV";
      Tag = M.combinedSecondaryVertexBJetTagsAK5PFPUcorrJet;
      break;
   case Tagger_CSVMVA:
      TaggerString = "CSVMVA";
      Tag = M.combinedSecondaryVertexMVABJetTagsAK5PFPUcorrJet;
      break;
   default:
   ...
   for(int iEvent = 0; iEvent < NumberOfEvents; iEvent++)
   {
      ...
      if(FinalBHadronFromT >= 0)
      {
         ClosestPFJetToBHadronFromT = FindClosestJet(M.etaAK5PFPUcorrJet, M.phiAK5PFPUcorrJet, M.nAK5PFPUcorrJet,
            M.etaMc[FinalBHadronFromT], M.phiMc[FinalBHadronFromT]);
         if(ClosestPFJetToBHadronFromT >= 0)
            ClosestPFJetToBHadronFromTP = FourVector(M.energyAK5PFPUcorrJet[ClosestPFJetToBHadronFromT],
               M.pxAK5PFPUcorrJet[ClosestPFJetToBHadronFromT],
               M.pyAK5PFPUcorrJet[ClosestPFJetToBHadronFromT],
               M.pzAK5PFPUcorrJet[ClosestPFJetToBHadronFromT]);
      }
      if(FinalBHadronFromTBar >= 0)
      {
         ClosestPFJetToBHadronFromTBar = FindClosestJet(M.etaAK5PFPUcorrJet, M.phiAK5PFPUcorrJet, M.nAK5PFPUcorrJet,
            M.etaMc[FinalBHadronFromTBar], M.phiMc[FinalBHadronFromTBar]);
         if(ClosestPFJetToBHadronFromTBar >= 0)
            ClosestPFJetToBHadronFromTBarP = FourVector(M.energyAK5PFPUcorrJet[ClosestPFJetToBHadronFromTBar],
               M.pxAK5PFPUcorrJet[ClosestPFJetToBHadronFromTBar],
               M.pyAK5PFPUcorrJet[ClosestPFJetToBHadronFromTBar],
               M.pzAK5PFPUcorrJet[ClosestPFJetToBHadronFromTBar]);
      }
      ...
      for(int i = 0; i < M.nAK5PFPUcorrJet; i++)
      {
         ...
         */
         ...
         if(M.pxAK5PFPUcorrJet[i] * M.pxAK5PFPUcorrJet[i] + M.pyAK5PFPUcorrJet[i] * M.pyAK5PFPUcorrJet[i] < 40 * 40)
            continue;
         if(M.etaAK5PFPUcorrJet[i] > 2.4 || M.etaAK5PFPUcorrJet[i] < -2.4)
            continue;

         GoodJets.push_back(FourVector(M.energyAK5PFPUcorrJet[i], M.pxAK5PFPUcorrJet[i], M.pyAK5PFPUcorrJet[i], M.pzAK5PFPUcorrJet[i]));
      }
