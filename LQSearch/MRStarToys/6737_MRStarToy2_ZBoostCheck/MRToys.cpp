...
double DoToyAndAppendPlots(PsFileHelper &PsFile, const double HeavyMass, const double Energy,
   const double VisibleMass, const double InvisibleMass, const double BetaZ);
...
int main(int argc, char *argv[])
{
   ...
   PsFileHelper PsFile("MRToy2_StaticGluon_CheckingZBoost.ps");
   PsFile.AddTextPage("Toys to understand MRStar better - part 2");
   ...
   Explanations.push_back("Check MRStar and RStar distribution changing z boost, GammaCM = 1.05");
   ...
   const double VisibleMass, const double InvisibleMass, const double BetaZ)
{
   ...
   for(int iEntry = 0; iEntry < 1000000; iEntry++)
   {
      ...
      // boost to lab frame
      FourVector ZDirection(1, 0, 0, 1);   // vector in z direction
      FourVector VisibleChild1 = VisibleChild1GluonCM.Boost(ZDirection, BetaZ);
      FourVector InvisibleChild1 = InvisibleChild1GluonCM.Boost(ZDirection, BetaZ);
      FourVector VisibleChild2 = VisibleChild2GluonCM.Boost(ZDirection, BetaZ);
      FourVector InvisibleChild2 = InvisibleChild2GluonCM.Boost(ZDirection, BetaZ);
