...
int main(int argc, char *argv[])
{
   ...
   PsFileHelper PsFile("MRToy24_ISRScaled.ps");
   PsFile.AddTextPage("Toys to understand MR better - part 24");
...
void DoToyAndAppendPlots(PsFileHelper &PsFile, const double LQMass, const string OutputFileName)
{
   ...
   double TreeISRStrength = 0;
   double TreeISRSize = 0;
   ...
   OutputTree.Branch("ISRStrength", &TreeISRStrength, "ISRStrength/D");
   OutputTree.Branch("ISRSize", &TreeISRSize, "ISRSize/D");
   ...
   for(int iEntry = 0; iEntry < EntryCount; iEntry++)
   {
      ...
      // calculate ISR strength
      FourVector ISRTotal;
      for(int i = 0; i < Tree.ParticleCount(); i++)
      {
         if(Tree[i].PDGID != 42 && Tree[i].PDGID != -42)   // we want LQ
            continue;
         if(Tree[i].Mothers.size() == 0)   // ?????
            continue;
         if(Tree[Tree[i].Mothers[0]].PDGID == 42 || Tree[Tree[i].Mothers[0]].PDGID == -42)   // only the first one
            continue;

         ISRTotal = ISRTotal + FourVector(Tree[i].P);
      }
      ISRTotal = AllFinalMomentum - ISRTotal;

      double BetaCCP = AllFinalMomentum[3] / AllFinalMomentum[0];
      ISRTotal = ISRTotal.BoostZ(BetaCCP);
      ...
      TreeISRStrength = ISRTotal.GetP();
      TreeISRSize = fabs(ISRTotal[0]);
