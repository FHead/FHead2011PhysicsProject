...
int main(int argc, char *argv[])
{
   ...
   ifstream in("HcalNoise.list.nolumi");
   while(in)
   {
      int Run = -1;
      long long Event = -1;
   ...
   TH2D HMRNewVsR2New("HMRNewVsR2New", "MR (new) vs. R^{2} (new);MR (new); R^{2} (new)", 100, 200, 1400, 100, 0, 1);
   TH2D HMRNewVsR2NewPassFilter("HMRNewVsR2NewPassFilter",
      "MR (new) vs. R^{2} (new), after applying filter;MR (new); R^{2} (new)", 100, 200, 1400, 100, 0, 1);
   ...
   for(int iEntry = 0; iEntry < EntryCount; iEntry++)
   {
      ...
      */
      ...
      HMRNewVsR2New.Fill(MRNew, R2New);
      if(M.PassNoiseFilter == true)
         HMRNewVsR2NewPassFilter.Fill(MRNew, R2New);

      /*
      if((MRNew > 400 && R2New > 0.2) || (MRNew > 500 && R2New > 0.1))
         cout << (long long)M.RunNumber << " " << (long long)M.EventNumber << " " << (int)M.LumiSection << endl;
      */
   }
   ...
   HMRNewVsR2New.SetStats(0);
   HMRNewVsR2NewPassFilter.SetStats(0);

   PsFile.AddPlot(HMRNewVsR2New, "colz");
   PsFile.AddPlot(HMRNewVsR2NewPassFilter, "colz");
