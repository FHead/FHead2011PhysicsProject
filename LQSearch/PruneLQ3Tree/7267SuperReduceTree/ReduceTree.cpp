...
int main(int argc, char *argv[])
{
   ...
   for(int iEntry = 0; iEntry < EntryCount; iEntry++)
   {
      ...
      double MRNew = GetMRStar(Hemispheres[0], Hemispheres[1]) * GetGammaRStar(Hemispheres[0], Hemispheres[1]);
      double RNew = GetRStar(Hemispheres[0], Hemispheres[1], PFMET);

      if(MRNew < 200 || RNew < 0.2)
         continue;
