...
SingleFitResult FitWithRCut(PsFileHelper &PsFile, string FileName, double RCut, string Threshold);
void FitWithRCut(PsFileHelper &PsFile, string FileName, vector<double> RCuts, string Threshold);
...
int main(int argc, char *argv[])
{
   ...
   Thresholds.push_back("11");
   Thresholds.push_back("12");
   Thresholds.push_back("13");
   Thresholds.push_back("14");
   ...
   {
      PsFileHelper PsFile("TTbar_" + Thresholds[i] + ".ps");
      PsFile.AddTextPage("TTbar, muon pt threshold " + Thresholds[i]);

      FitWithRCut(PsFile, "TTbar.root", CutsToTry, Thresholds[i]);
...
SingleFitResult FitWithRCut(PsFileHelper &PsFile, string FileName, double RCut, string Threshold)
{
   ...
   string MRString = "MR" + Threshold;
   string RString = "R" + Threshold;
...
void FitWithRCut(PsFileHelper &PsFile, string FileName, vector<double> RCuts, string Threshold)
{
   ...
   for(int i = 0; i < (int)RCuts.size(); i++)
      SingleFitResults[i] = FitWithRCut(PsFile, FileName, RCuts[i], Threshold);
