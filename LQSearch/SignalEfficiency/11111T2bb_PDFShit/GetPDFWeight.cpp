...
int main()
{
   CheckSampleEfficiency("T2bbListShort", 1, 0.25, PDFSET_CTEQ66);
   CheckSampleEfficiency("T2bbListShort", 1, 0.25, PDFSET_MRST2006NNLO);
   CheckSampleEfficiency("T2bbListShort", 1, 0.25, PDFSET_NNPDF10100);
...
void CheckSampleEfficiency(string InputFile, double ExpectedYield, double R2Min, int Type)
{
   ...
   for(int iEntry = 0; iEntry < EntryCount; iEntry++)
   {
      ...
      AllEvents2Central[Model] = AllEvents2Central[Model] + WPDF[0] * WPDF[0];
      for(int i = 1; i < *NPDF; i++)   // loop over the eigenvalue variations
      {
         AllEventsPairs[Model][i-1] = AllEventsPairs[Model][i-1] + WPDF[i];
         AllEvents2Pairs[Model][i-1] = AllEvents2Pairs[Model][i-1] + WPDF[i] * WPDF[i];
      }
      ...
      if(M.GoodElectronCount == 0 && M.GoodMuonCount == 0 && CaloJetCount60 >= 2 && JetPassBTagLoose >= 2 && R2 >= R2Min && MR >= 400)
      {
         ...
         PassedEvents2Central[Model] = PassedEvents2Central[Model] + WPDF[0] * WPDF[0];
         for(int i = 1; i < *NPDF; i++)   // loop over the eigenvalue variations
         {
            PassedEventsPairs[Model][i-1] = PassedEventsPairs[Model][i-1] + WPDF[i];
            PassedEvents2Pairs[Model][i-1] = PassedEvents2Pairs[Model][i-1] + WPDF[i] * WPDF[i];
         }
      }
   }
