      ...
      for(int i = 1 + 1; i <= H.GetNbinsX() - 1; i++)
      {
         for(int j = 1 + 1; j <= H.GetNbinsY() - 1; j++)
         {
                  ...
                  H.GetBinContent(i, j) == 0)
            {
               ...
               Sum = H.GetBinError(i, j - 1);
               Sum = Sum + H.GetBinError(i, j + 1);
               Sum = Sum + H.GetBinError(i - 1, j - 1);
               Sum = Sum + H.GetBinError(i - 1, j);
               Sum = Sum + H.GetBinError(i - 1, j + 1);
               Sum = Sum + H.GetBinError(i + 1, j - 1);
               Sum = Sum + H.GetBinError(i + 1, j);
               Sum = Sum + H.GetBinError(i + 1, j + 1);
               H.SetBinError(i, j, Sum / 8);
      ...
      for(int i = 1 + 1; i <= H.GetNbinsX() - 1; i++)
      {
         for(int j = 1 + 1; j <= H.GetNbinsY() - 1; j++)
         {
               ...
               H.GetBinContent(i, j) == 0)
            {
               ...
               Sum = H.GetBinError(i - 1, j - 1);
               Sum = Sum + H.GetBinError(i - 1, j + 1);
               Sum = Sum + H.GetBinError(i + 1, j - 1);
               Sum = Sum + H.GetBinError(i + 1, j + 1);
               H.SetBinError(i, j, Sum / 4);
