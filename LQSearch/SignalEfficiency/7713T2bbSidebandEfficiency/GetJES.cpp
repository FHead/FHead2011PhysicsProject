...
int main()
{
   CheckSampleEfficiency("Samples/T2bb_All.root", 1, 0.20, 0.25);
...
void CheckSampleEfficiency(string InputFile, double ExpectedYield, double R2Min, double R2Max)
{
   ...
   for(int iEntry = 0; iEntry < EntryCount; iEntry++)
   {
      ...
      if(CaloJetCount60 >= 2 && JetPassBTagLoose == 1 && R2New >= R2Min && MRNew >= 400 && R2New < R2Max && MRNew < 600)
         PassedWeight[Model] = PassedWeight[Model] + WPU;
      if(CaloJetCount60Up >= 2 && JetPassBTagLooseUp == 1 && R2NewUp >= R2Min && MRNewUp >= 400 && R2NewUp < R2Max && MRNewUp < 600)
         PassedWeightUp[Model] = PassedWeightUp[Model] + WPU;
      if(CaloJetCount60Down >= 2 && JetPassBTagLooseDown == 1 && R2NewDown >= R2Min && MRNewDown >= 400 && R2NewDown < R2Max && MRNewDown < 600)
         PassedWeightDown[Model] = PassedWeightDown[Model] + WPU;
   }

   cout << "Summary for input file \"" << InputFile << "\" with R2 cut " << R2Min << "-" << R2Max << endl;
   cout << endl;
   ...
   for(map<pair<int, int>, double>::iterator iter = AllWeight.begin(); iter != AllWeight.end(); iter++)
   {
      ...
      cout << "Efficiencies (MR = 400-600):" << endl;
