...
int main()
{
   ...
   TH2D HSigmaBound("HSigmaBound", "5%;m_{#tilde{b}} [GeV];m_{#tilde{#chi}^{0}} [GeV]", 50, 0, 1250, 50, 0, 1250);

   ifstream in("Processed");

   double SideBandCount = 295;
   double Luminosity = 1752;
   while(in)
   {
      double m0 = 0;
      double m12 = 0;
      ...
      HSigmaBound.Fill(m0, m12, (SideBandCount * 0.05) / (eff * Luminosity));
   }
   ...
   PsFile.AddPlot(HSigmaBound, "colz");   HSigmaBound.Write();
   PsFile.AddPlot(HSigmaBound, "colz", false, true);   HSigmaBound.Write();
