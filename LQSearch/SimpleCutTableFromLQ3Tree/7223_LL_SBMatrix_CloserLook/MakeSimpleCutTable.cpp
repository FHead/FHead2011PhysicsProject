...
public:
   ...
   TH2D *HNewMRNewR2MatrixSideBand;
...
public:
   ...
   void CleanHistograms()
   {
      ...
      if(HNewMRNewR2MatrixSideBand != NULL)   delete HNewMRNewR2MatrixSideBand;
   }
};
//---------------------------------------------------------------------------
int main(int argc, char *argv[])
{
   ...
   Samples.push_back(ReadSample(PsFile, "Samples/AllWJetsToLNu_TuneZ2_7TeV-madgraph-tauola.root", "W", 31314));
   ...
   Samples.push_back(ReadSample(PsFile, "Samples/ZinvisibleJets_7TeV-madgraph_All.root", "Znunu", 3048 * 6));   // guessed
   ...
   for(int i = 0; i < (int)Samples.size(); i++)
   {
      ...
      if(Samples[i].HNewMRNewR2MatrixSideBand != NULL)
         Samples[i].HNewMRNewR2MatrixSideBand->Write();
   }
...
SampleCounts ReadSample(PsFileHelper &PsFile, string FileName, string Tag, double CrossSection)
{
   ...
   Result.HNewMRNewR2MatrixSideBand = new TH2D(Form("HNewMRNewR2MatrixSideBand_%s", Tag.c_str()),
      Form("MR (new) vs. R^{2} (new), (%s);MR (new);R^{2} (new)", Tag.c_str()), 18, 400, 1000, 10, 0.05, 0.15);
   ...
   for(int iEntry = 0; iEntry < EntryCount; iEntry++)
   {
      ...
      Result.HNewMRNewR2MatrixSideBand->Fill(MRStar, RStar * RStar, WPU);
      if(R > 0.20)
   ...
   Result.HNewMRNewR2MatrixSideBand->Scale(CrossSection * 1000 / Result.AllEvents);
   ...
   Result.HNewMRNewR2MatrixSideBand->SetStats(0);
   PsFile.AddPlot(Result.HNewMRNewR2MatrixSideBand, "colz text30", false);
