...
Int_t banner(){
  ...
  std::cout << "Banner!  Version 1.15" << std::endl;
#endif
  return 0 ;
}
...
{
   double Lumi = 1753;
   double sLumi = 1753 * 0.045;
   double Efficiency = 1;   // input
   double sEfficiency = 1;   // input too
   double Background = 326.98;   // R2 0.25, MR 400
   double sBackground = 30.98;   // R2 0.25, MR 400
   double Observed = 295;   // data

   LimitResult limit = roostats_limit(Lumi, sLumi, Efficiency, sEfficiency, Background, sBackground, Observed,
      false, 1, "cls", "dummy.root", 31415);
   Double_t obs_limit = limit.GetObservedLimit();
   Double_t exp_limit = limit.GetExpectedLimit();
   Double_t exp_up    = limit.GetOneSigmaHighRange();
   Double_t exp_down  = limit.GetOneSigmaLowRange();
