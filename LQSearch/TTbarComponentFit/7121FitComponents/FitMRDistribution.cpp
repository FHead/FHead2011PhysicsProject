...
SingleFitResult FitWithRCut(PsFileHelper &PsFile, string FileName, double RCut, int MCCategory, int BoxCategory);
void FitWithRCut(PsFileHelper &PsFile, string FileName, vector<double> RCuts, int MCCategory, int BoxCategory);
...
int main(int argc, char *argv[])
{
   ...
   PsFileHelper PsFile1("HadHad_HadBox.ps");
   PsFile1.AddTextPage("Fitting fully-hadronic ttbar in the HAD box");
   FitWithRCut(PsFile1, "TTbarReducedTree.root", CutsToTry, 9, 0);
   ...
   */
   ...
   PsFileHelper PsFile("MuMu_MuMuBox.ps");
   PsFile.AddTextPage("Fitting fully-leptonic (mu-mu) ttbar in the MUMU box");
   FitWithRCut(PsFile, "TTbarReducedTree.root", CutsToTry, 4, 5);
...
SingleFitResult FitWithRCut(PsFileHelper &PsFile, string FileName, double RCut, int MCCategory, int BoxCategory)
{
   ...
   RooRealVar R("RStar", "Chris' another kinematic variable", -200, 200);
   RooRealVar MCType("MCCategory", "MC-level category", MCCategory - 0.5, MCCategory + 0.5);
   RooRealVar BoxType("BoxCategory", "Box", BoxCategory - 0.5, BoxCategory + 0.5);
   RooArgSet TreeVarSet(MR, R, MCType, BoxType);

   RooDataSet Dataset("Dataset", "MR Dataset", Tree, TreeVarSet, Form("RStar > %f", RCut));
...
void FitWithRCut(PsFileHelper &PsFile, string FileName, vector<double> RCuts, int MCCategory, int BoxCategory)
{
   ...
   for(int i = 0; i < (int)RCuts.size(); i++)
      SingleFitResults[i] = FitWithRCut(PsFile, FileName, RCuts[i], MCCategory, BoxCategory);
   ...
   RooDataSet Dataset("Dataset", "MR Dataset", Tree, TreeVarSet, Form("RStar > %f", RCuts[0]));

   RooRealVar ParameterA1("ParameterA1", "s1 = \"a1\" + b1 R^2", ParameterA1Value, -0.1, 0.1);
   RooRealVar ParameterB1("ParameterB1", "s1 = a1 + \"b1\" R^2", fabs(ParameterB1Value), 0, 1);
   RooRealVar ParameterA2("ParameterA2", "s2 = \"a2\" + b2 R^2", ParameterA2Value, -0.1, 0.1);
   RooRealVar ParameterB2("ParameterB2", "s2 = a2 + \"b2\" R^2", fabs(ParameterB2Value), 0, 1);
   ...
   for(int i = 0; i < (int)RCuts.size(); i++)
   {
      double GuessYield = 0;
      if(i != RCuts.size() - 1)
         GuessYield = Dataset.reduce(Cut(Form("RStar > %f && RStar <= %f", RCuts[i], RCuts[i+1])))->sumEntries();
      else
         GuessYield = Dataset.reduce(Cut(Form("RStar > %f", RCuts[i])))->sumEntries();
   ...
   for(int i = 1; i < (int)RCuts.size(); i++)
      Dataset.reduce(Cut(Form("RStar > %f", RCuts[i])))->plotOn(FinalResultPlot2, MarkerSize(0.9), MarkerColor(i + 1));
