...
int main(int argc, char *argv[])
{
   if(argc < 3)
   {
      cerr << "Usage " << argv[0] << " output seed" << endl;
      return -1;
   }
   ...
   TH2D HMRRGoodLeptonTight("HMRRGoodLeptonTight", ";MR;R*R", 50, 0, 1500, 50, 0, 1.0);
   ...
   pythia.readString(Form("Random:seed = %s", argv[2]));   // Set random seed based on input

   pythia.init(2212, 2212, 7000);                          // Proton-proton collision at 7 TeV

   int TotalEntry = 1000000;
   for(int iEntry = 0; iEntry < TotalEntry; iEntry++)
   {
      ...
      for(int i = 0; i < Tree.ParticleCount(); i++)
      {
         ...
         if(FourVector(Tree[i].P).GetAbsEta() < 2.4)
            GoodLeptonCount = GoodLeptonCount + 1;
         if(FourVector(Tree[i].P).GetAbsEta() < 1.8)
            GoodLeptonCountTight = GoodLeptonCountTight + 1;
      }

      HMRR.Fill(MR, R * R);
      if(GoodLeptonCount >= 2)
         HMRRGoodLepton.Fill(MR, R * R);
      if(GoodLeptonCountTight >= 2)
         HMRRGoodLeptonTight.Fill(MR, R * R);
   }

   HMRR.Write();
   HMRRGoodLepton.Write();
   HMRRGoodLeptonTight.Write();
