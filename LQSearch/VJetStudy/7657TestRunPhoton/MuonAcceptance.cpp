...
int main(int argc, char *argv[])
{
   ...
   pythia.readString("PromptPhoton:all = on");             // Switch on prompt photon
   pythia.readString("WeakSingleBoson:ffbar2gmZ = off");   // Switch off Z
   ...
   for(int iEntry = 0; iEntry < TotalEntry; iEntry++)
   {
      ...
      for(int i = 0; i < Tree.ParticleCount(); i++)
      {
         ...
         if(Tree[i].PDGID == 22 && FourVector(Tree[i].P).GetPT() < 20)   // identified photon
            continue;
