...
int main(int argc, char *argv[])
{
   ...
   CutsToTry.push_back(sqrt(0.16));
   CutsToTry.push_back(sqrt(0.18));
   
   PsFileHelper PsFile("TTbarFitResultExportBand.ps");
   PsFile.AddTextPage("Fit ttbar in the signal selection");

   RooFitResult *Result = FitWithRCut(PsFile, "TTbarQCD.root", CutsToTry);
...
SingleFitResult FitWithRCut(PsFileHelper &PsFile, string FileName, double RCut)
{
   ...
   RooDataSet Dataset("Dataset", "MR Dataset", Tree, TreeVarSet, Form("%s > %f", RString.c_str(), RCut), "Factor");

   RooRealVar S1("S1", "first exponent of the tail", -0.00836027 - 0.0185574 * RCut * RCut, -1.0, 0.0);
   RooExponential Component1("Component1", "First component", MR, S1);
   
   RooRealVar S2("S2", "second exponent of the tail", -0.00445463 - 0.00743833 * RCut * RCut, -1.0, 0.0);
...
RooFitResult *FitWithRCut(PsFileHelper &PsFile, string FileName, vector<double> RCuts)
{
   ...
   RooRealVar Factor("Factor", "Factor", 0, 100);
   RooArgSet TreeVarSet(MR, R, Factor);
   ...
   */
   ...
   RooRealVar ParameterB1("ParameterB1", "s1 = a1 + \"b1\" R^2", 0.019, 0, 1);
   RooRealVar ParameterA2("ParameterA2", "s2 = \"a2\" + b2 R^2", 0.004, 0, 0.1);
   RooRealVar ParameterB2("ParameterB2", "s2 = a2 + \"b2\" R^2", 0.009, 0, 1);
   ...
   RooRealVar ParameterA1Var("ParameterA1Var", "ParameterA1Var", 0.008);
   RooRealVar ParameterA2Var("ParameterA2Var", "ParameterA2Var", 0.004);
   RooRealVar ParameterB1Var("ParameterB1Var", "ParameterB1Var", 0.019);
   RooRealVar ParameterB2Var("ParameterB2Var", "ParameterB2Var", 0.009);
   RooRealVar ParameterA1Error("ParameterA1Error", "ParameterA1Error", 0.002);
   RooRealVar ParameterA2Error("ParameterA2Error", "ParameterA2Error", 0.002);
   RooRealVar ParameterB1Error("ParameterB1Error", "ParameterB1Error", 0.023);
   RooRealVar ParameterB2Error("ParameterB2Error", "ParameterB2Error", 0.024);
   ...
   */
   ...
   TFile BandFile("Destination/Bands.root", "recreate");
   ...
   cout << "Weighted yield --- " << WeightedYields << endl;
