...
RooFitResult *FitWithRCut(PsFileHelper &PsFile, string FileName, vector<double> RCuts)
{
   ...
   double WeightedYields = FullDataset.reduce("R * R > 0.07 && MR > 400 && MR < 3500")->sumEntries();

   cout << "W --- " << WeightedYields << endl;

   PsFile.AddTextPage("Fit-band");
   PlotBands(PsFile, 400, 3500, FitResult, FloatParameters, FloatNames,
   ...
   for(int i = 0; i < Tries; i++)
   {
      ...
      for(int j = 0; j < (int)RCuts.size(); j++)
      {
         for(int k = 0; k < BinCount; k++)
         {
            ...
            double MRMax = (k + 1) * (Max - Min) / BinCount + Max;
