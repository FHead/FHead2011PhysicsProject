...
void PlotBands(PsFileHelper &PsFile, double Min, double Maxt, RooFitResult *Result,
   vector<double> ExpectedYields, double WeightedYields, vector<double> RCuts,
...
int main(int argc, char *argv[])
{
   ...
   CutsToTry.push_back(sqrt(0.10));
   CutsToTry.push_back(sqrt(0.12));
   CutsToTry.push_back(sqrt(0.14));
   CutsToTry.push_back(sqrt(0.20));
   
   PsFileHelper PsFile("TTbarFitResultExportBand.ps");
   PsFile.AddTextPage("Fit ttbar in the signal selection");
...
RooFitResult *FitWithRCut(PsFileHelper &PsFile, string FileName, vector<double> RCuts)
{
   ...
   FinalResultPlot->SetMinimum(0.01);
   PsFile.AddPlot(FinalResultPlot, "", true);
   ...
   FinalResultPlot2->SetMinimum(0.01);
   ...
   vector<double> ExpectedYields;
   ExpectedYields.push_back(FullDataset.reduce("R * R > 0.07 && MR > 500 && MR < 3500")->numEntries());
   ExpectedYields.push_back(FullDataset.reduce("R * R > 0.10 && MR > 500 && MR < 3500")->numEntries());
   ExpectedYields.push_back(FullDataset.reduce("R * R > 0.12 && MR > 500 && MR < 3500")->numEntries());
   ExpectedYields.push_back(FullDataset.reduce("R * R > 0.14 && MR > 500 && MR < 3500")->numEntries());
   ExpectedYields.push_back(FullDataset.reduce("R * R > 0.20 && MR > 500 && MR < 3500")->numEntries());
   ExpectedYields.push_back(FullDataset.reduce("R * R > 0.30 && MR > 500 && MR < 3500")->numEntries());

   double WeightedYields = FullDataset.reduce("R * R > 0.07 && MR > 500 && MR < 3500")->sumEntries();

   for(int i = 0; i < RCuts.size(); i++)
      cout << "E" << i << " --- " << ExpectedYields[i] << endl;
   cout << "W --- " << WeightedYields << endl;

   PsFile.AddTextPage("Fit-band");
   PlotBands(PsFile, 400, 3500, FitResult, ExpectedYields, WeightedYields, RCuts, "Fit");
   
   PsFile.AddTextPage("Smear A only");
   PlotBands(PsFile, 400, 3500, FitResult, ExpectedYields, WeightedYields, RCuts, "SmearA", 0.1, 0, 0);
   
   PsFile.AddTextPage("Smear A and B with correlation");
   PlotBands(PsFile, 400, 3500, FitResult, ExpectedYields, WeightedYields, RCuts, "SmearAB", 0.1, 0.4, -0.85);

   PsFile.AddTextPage("Smear A and B without correlation"); 
   PlotBands(PsFile, 400, 3500, FitResult, ExpectedYields, WeightedYields, RCuts, "NoCorrelation", 0.1, 0.4, 0);

   PsFile.AddTextPage("Smear A and B with full correlation");
   PlotBands(PsFile, 400, 3500, FitResult, ExpectedYields, WeightedYields, RCuts, "FullCorrelation", 0.1, 0.4, 1.00);
...
{
   ExpectedYields.push_back(0);
   double OverallFactor = WeightedYields / ExpectedYields[0];
   ...
   for(int i = 0; i < Tries; i++)
   {
      ...
      for(int j = 0; j < (int)RCuts.size(); j++)
         N[j] = DrawPoisson(ExpectedYields[j] - ExpectedYields[j+1]) * OverallFactor;

      double TotalN[10] = {0};
      TotalN[RCuts.size()-1] = N[RCuts.size()-1];
      for(int j = (int)RCuts.size() - 2; j >= 0; j--)
         TotalN[j] = TotalN[j+1] + N[j];

      double S[10] = {0};
      for(int j = 0; j < (int)RCuts.size(); j++)
         S[j] = AValue + BValue * RCuts[j];

      double Integral[10] = {0};
      for(int j = 0; j < (int)RCuts.size(); j++)
         Integral[j] = (exp(-Min * S[j]) - exp(-Max * S[j])) / S[j];

      double Factor[10] = {0};
      for(int j = 0; j < (int)RCuts.size(); j++)
         Factor[j] = TotalN[j] / Integral[j];

      for(int j = 0; j < (int)RCuts.size(); j++)
      {
         for(int k = 0; k < BinCount; k++)
         {
            double MRValue = Min + (k + 0.5) * (Max - Min) / BinCount;

            double FinalValue = Factor[j] * exp(-S[j] * MRValue);
            FinalValue = FinalValue / ((Max - Min) / BinCount);
            Bins[j][k].push_back(FinalValue);
         }
      }
   }
   ...
   for(int i = 0; i < (int)RCuts.size(); i++)
   {
      ...
      TH1D HFrame("HFrame", ";MR;Count / 1 GeV", 100, 400, 1500);
