...
RooFitResult *FitWithRCut(PsFileHelper &PsFile, string FileName, vector<double> RCuts, string WResult, string TResult)
{
   ...
   RooRealVar ParameterB2("ParameterB2", "s2 = a2 + \"b2\" R^2", 0.001, 0, 1);
   ...
   for(int i = 0; i < (int)RCuts.size(); i++)
   {
      ...
      else
      {
         ...
         WSingleBinYields.push_back(new RooRealVar(Form("WSingleBinYield_%d", i),
            Form("Yield with R in bin %d", i),
            WDHFile["CentralValue"][Form("SingleBinYield_%d", i)].GetDouble() / 2, 0, 10000));
      }
   }
   ...
   RooRealVar WFinalYield("WFinalYield", "WFinalYield", 1009.20);
   RooRealVar TTbarFinalYield("TTbarFinalYield", "TTbarFinalYield", 1050.75);
   RooRealVar QCDFinalYield("QCDFinalYield", "QCD final yield", 22000, 0, 100000);
