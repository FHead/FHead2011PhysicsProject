...
int main(int argc, char *argv[])
{
   ...
   PsFile.AddTextPage("Fit ttbar in the (almost) signal selection");
...
RooFitResult *FitWithRCut(PsFileHelper &PsFile, string FileName, vector<double> RCuts)
{
   ...
   RooRealVar ParameterA1Val("ParameterA1Val", "Parameter A1 value", 0.0137 * 0.063);
   ...
   RooProdPdf FinalModel("FinalModel", "Real final model",
      RooArgList(AlmostFinalModel, ParameterA1Penalty, ParameterA2Penalty, ParameterB1Penalty, ParameterA2Penalty));
   ...
   */
   ...
   double WeightedYields = 264.057;   // taking W.root, cut on R2 > 0.07, MR = 400-3500, then integrate
