...
RooFitResult *FitWithRCut(PsFileHelper &PsFile, string FileName, vector<double> RCuts, string WResult, string TResult)
{
   ...
   RooRealVar WFinalRawYield("WFinalRawYield", "WFinalYield", 2026.200);
   RooRealVar TTbarFinalRawYield("TTbarFinalRawYield", "TTbarFinalYield", 2114.120);
   RooRealVar QCDFinalRawYield("QCDFinalRawYield", "QCD final yield", 28245.7);
   ...
   RooPlot *FinalResultPlot = MR.frame(Bins(50), Name("FitResults"),
      Title("MR, Simultaneous fit, total model"), AutoRange(Dataset));
   Dataset.plotOn(FinalResultPlot, MarkerSize(0.9));
   ...
   */
   ...
   // double WeightedYields = 173.936;   // single-loose tag, electron
   // double WeightedYields = 17194.213;   // Exclusive single-loose tag, hadronic selection
   double WeightedYields = 4519.321;   // Double-loose tag, hadronic selection
