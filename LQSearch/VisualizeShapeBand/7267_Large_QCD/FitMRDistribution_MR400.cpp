...
RooFitResult *FitWithRCut(PsFileHelper &PsFile, string FileName, vector<double> RCuts, string WResult, string TResult)
{
   ...
   RooRealVar MR(MRString.c_str(), "Chris' kinematic variable", MRLowerBound, MRUpperBound, "GeV");
   RooRealVar R(RString.c_str(), "Chris' another kinematic variable", 0, 1.5);
   RooArgSet TreeVarSet(MR, R);

   RooDataSet Dataset("Dataset", "MR Dataset", Tree, TreeVarSet,
      Form("%s > %f && %s * %s < 0.5", RString.c_str(), RCuts[0], RString.c_str(), RString.c_str()));

   RooRealVar ParameterA1("ParameterA1", "s1 = \"a1\" + b1 R^2", 0.010, 0, 0.1);
   RooRealVar ParameterB1("ParameterB1", "s1 = a1 + \"b1\" R^2", 0.053, 0, 1);
   ...
   for(int i = 0; i < (int)RCuts.size(); i++)
   {
      ...
      else
      {
         TSingleBinYields.push_back(new RooRealVar(Form("TSingleBinYield_%d", i),
            ...
            TDHFile["CentralValue"][Form("SingleBinYield_%d", i)].GetDouble() / 2, 0, 10000));
         WSingleBinYields.push_back(new RooRealVar(Form("WSingleBinYield_%d", i),
            Form("Yield with R in bin %d", i),
            WDHFile["CentralValue"][Form("SingleBinYield_%d", i + 2)].GetDouble() / 2, 0, 10000));
      }
   }
   ...
   RooRealVar WFinalYield("WFinalYield", "WFinalYield", 989.201);
   RooRealVar TTbarFinalYield("TTbarFinalYield", "TTbarFinalYield", 875.066);
   RooRealVar QCDFinalYield("QCDFinalYield", "QCD final yield", 10000, 0, 100000);

   RooAddPdf FinalModel("FinalModel", "Final model!!",
      RooArgList(AlmostFinalModel, WFinalModel, TFinalModel),
      RooArgList(TTbarFinalYield, WFinalYield, TTbarFinalYield));
   ...
   */
   ...
   double WeightedYields = 974.487;   // taking TTbar.root, cut on R2 > 0.07, MR = 400-3500, then integrate
   ...
   string Tag, double SigmaA, double SigmaB, double Correlation)
{
   ...
   for(int i = 0; i < (int)RCuts.size(); i++)
   {
      ...
      TH1D HFrame("HFrame", ";MR;Count", 100, 400, 1500);
