...
int main(int argc, char *argv[])
{
   ...
   PsFileHelper PsFile("QCDFitResultExportBand.ps");
   PsFile.AddTextPage("Fit QCD (MC) in the (almost) signal selection");

   RooFitResult *Result = FitWithRCut(PsFile, "QCDAll.root", CutsToTry);
...
RooFitResult *FitWithRCut(PsFileHelper &PsFile, string FileName, vector<double> RCuts)
{
   ...
   RooDataSet Dataset("Dataset", "MR Dataset", Tree, TreeVarSet, Form("%s > %f", RString.c_str(), RCuts[0]), "Factor");
   ...
   RooRealVar ParameterA1("ParameterA1", "s1 = \"a1\" + b1 R^2", 0.01, -0.1, 0.1);
   RooRealVar ParameterB1("ParameterB1", "s1 = a1 + \"b1\" R^2", 0.1, -1, 1);
   RooRealVar ParameterA2("ParameterA2", "s2 = \"a2\" + b2 R^2", 0.005, -0.1, 0.1);
   RooRealVar ParameterB2("ParameterB2", "s2 = a2 + \"b2\" R^2", 0.017, -1, 1);
   ...
   RooAddPdf FinalModel("FinalModel", "Almost final model!", ModelList, YieldList);
   ...
   RooProdPdf FinalModel("FinalModel", "Real final model",
      RooArgList(AlmostFinalModel, ParameterB2Penalty, ParameterA2Penalty));
   */
   ...
   */
   ...
   double WeightedYields = 264.057;   // taking W.root, cut on R2 > 0.07, MR = 300-3500, then integrate
