...
RooFitResult *FitWithRCut(PsFileHelper &PsFile, string FileName, vector<double> RCuts, string WResult)
{
   ...
   RooRealVar WFinalYield("WFinalYield", "WFinalYield", 1876.186);
   RooRealVar TTbarFinalYield("TTbarFinalYield", "TTbarFinalYield", 2500.255, 0, 5000);
   ...
   RooPlot *FinalResultPlot = MR.frame(Bins(25), Name("FitResults"),
      Title("MR, Simultaneous fit, total model"), Range(300, 2000));
   Dataset.plotOn(FinalResultPlot, MarkerSize(0.9));
   FinalModel.plotOn(FinalResultPlot, LineColor(kBlue));
   FinalModel.plotOn(FinalResultPlot, LineColor(kRed), Components("AlmostFinalModel"));
   ...
   /*
   RooDataSet FullDataset("FullDataset", "Full MR Dataset", Tree, TreeVarSet,
      Form("%s * %s > 0.07", RString.c_str(), RString.c_str()), "Factor");

   double WeightedYields = FullDataset.sumEntries(Form("R > %f && MR > %f && MR < %f",
      RCuts[0], MRLowerBound, MRUpperBound));

   cout << "W --- " << WeightedYields << endl;
   */

   // double WeightedYields = 1245.126;   // taking TTbar.root, cut on R2 > 0.14, MR = 300-3500, then integrate
   // double WeightedYields = 1282.316;   // Had + exclusive single tag
   double WeightedYields = 818.361;   // Had + double tag
