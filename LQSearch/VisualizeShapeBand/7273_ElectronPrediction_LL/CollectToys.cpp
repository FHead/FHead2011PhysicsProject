...
int main()
{
   ...
   for(int i = 1; i <= 1000; i++)
   {
      ...
      double QPrediction = 0;

      double WSignal = WToyS->sumEntries("MR > 400 && R * R > 0.30");
      double TSignal = TToyS->sumEntries("MR > 400 && R * R > 0.30");
      // double QSignal = QToyS->sumEntries("MR > 400 && R * R > 0.30");
      double QSignal = 0;

      if(WPrediction > 0 && WSignal > 0)
      {
         HWSideBand.Fill(WPrediction);
         HWSignal.Fill(WSignal);
      }
      if(TPrediction > 0 && TSignal > 0)
      {
         HTSideBand.Fill(TPrediction);
         HTSignal.Fill(TSignal);
      }
      if(QPrediction > 0 && QSignal > 0)
      {
         HQSideBand.Fill(QPrediction);
         HQSignal.Fill(QSignal);
      }
      if(WPrediction > 0 && WSignal > 0 && TPrediction > 0 && TSignal > 0 && QPrediction > 0 && QSignal > 0)
      {
