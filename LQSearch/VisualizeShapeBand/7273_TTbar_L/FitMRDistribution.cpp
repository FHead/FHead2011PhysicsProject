...
RooFitResult *FitWithRCut(PsFileHelper &PsFile, string FileName, vector<double> RCuts, string WResult)
{
   ...
   RooDataSet Dataset("Dataset", "MR Dataset", Tree, TreeVarSet,
      Form("%s > %f && %s * %s < 0.5 && %s > %f", RString.c_str(), RCuts[0], RString.c_str(), RString.c_str(),
      MRString.c_str(), MRLowerBound));
   ...
   RooRealVar WFinalRawYield("WFinalRawYield", "WFinalYield", 938.755);
   RooRealVar TTbarFinalRawYield("TTbarFinalRawYield", "TTbarFinalYield", 888.255);
   ...
   RooPlot *FinalResultPlot = MR.frame(Bins(25), Name("FitResults"),
      Title("MR, Simultaneous fit, total model"), Range(400, 2000));
   Dataset.plotOn(FinalResultPlot, MarkerSize(0.9));
   ...
   RooPlot *FinalResultPlot2 = MR.frame(Bins(25), Name("FitResults2"), Title("MR, Simultaneous fit"),
      Range(400, 2000));
   ...
   // double TTbarRestrictedYield = 304.352;   // MR > 400, R2 > 0.20, Electron (L)
   // double TTbarRestrictedYield = 151.838;   // MR > 400, R2 > 0.20, Electron (LL)
   // double TTbarRestrictedYield = 189.666;   // MR > 400, R2 > 0.20, Had+1L
   // double TTbarRestrictedYield = 176.547;   // MR > 400, R2 > 0.20, Had+LL
   double TTbarRestrictedYield = 133.363;   // MR > 400, R2 > 0.20, Had+1L, semi-large
   // double TTbarRestrictedYield = 124.138;   // MR > 400, R2 > 0.20, Had+LL, semi-large
