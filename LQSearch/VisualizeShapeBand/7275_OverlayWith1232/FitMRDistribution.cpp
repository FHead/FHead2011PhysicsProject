   ...
   string WResult, string TResult, string QCDResult)
{
   ...
   RooDataSet Dataset("Dataset", "MR Dataset", Tree, TreeVarSet,
      Form("%s > %f && %s * %s < 1.0", RString.c_str(), RCuts[0], RString.c_str(), RString.c_str()));
   ...
   RooRealVar WFinalRawYield("WFinalRawYield", "WFinalYield", 76.81);
   RooRealVar TTbarFinalRawYield("TTbarFinalRawYield", "TTbarFinalYield", 80.88);
   RooRealVar QCDFinalRawYield("QCDFinalRawYield", "QCD final yield", 73.87);
   ...
   FinalModel.plotOn(FinalResultPlot, LineColor(kYellow - 3), Components("TFinalModel"));
   FinalResultPlot->SetMinimum(0.2);

   TH1D HDummy("HDummy", "", 1, 0, 1);
   TH1D HDummy0("HDummy0", "", 1, 0, 1);
   ...
   HDummy3.SetLineColor(kYellow - 3);
   HDummy.SetMarkerStyle(11);
   HDummy0.SetLineWidth(2);
   HDummy1.SetLineWidth(2);
   HDummy2.SetLineWidth(2);
   HDummy3.SetLineWidth(2);
   TLegend ModelLegend(0.5, 0.86, 0.8, 0.56);
   ModelLegend.SetFillStyle(0);
   ModelLegend.SetBorderSize(0);
   ModelLegend.SetTextFont(42);
   ModelLegend.AddEntry(&HDummy, "Data (1232/pb)", "pl");
   ModelLegend.AddEntry(&HDummy0, "Total SM", "l");
   ModelLegend.AddEntry(&HDummy1, "Multi-jets", "l");
   ModelLegend.AddEntry(&HDummy2, "W", "l");
   ModelLegend.AddEntry(&HDummy3, "TTbar", "l");
   ...
   C1.SaveAs("Plot1.pdf");
   C1.SetLogy();
   C1.SaveAs("Plot1Log.png");
   C1.SaveAs("Plot1Log.C");
   C1.SaveAs("Plot1Log.eps");
   C1.SaveAs("Plot1Log.pdf");

   // Each bin, model vs. data
   RooPlot *FinalResultPlot2 = MR.frame(Bins(40), Name("FitResults2"), Title("MR, different R cuts"),
      Range(400, 1400));
