   ...
   string WResult, string TResult, string QCDResult)
{
   ...
   for(int i = 0; i < (int)RCuts.size(); i++)
   {
      ...
      TSingleBinYields.push_back(new RooRealVar(Form("TSingleBinYield_%d", i),
         Form("Yield with R in bin %d", i), TDHFile["CentralValue"][Form("SingleBinYield_%d", i + 3)].GetDouble()));
      WSingleBinYields.push_back(new RooRealVar(Form("WSingleBinYield_%d", i),
         Form("Yield with R in bin %d", i), WDHFile["CentralValue"][Form("SingleBinYield_%d", i + 1)].GetDouble()));
   }
   ...
   for(int i = 0; i < (int)RCuts.size(); i++)
   {
      ...
      WFraction.push_back(new RooRealVar(Form("WFraction_%d", i), Form("WFraction_%d", i),
         WDHFile["CentralValue"][Form("Fraction_%d", i + 1)].GetDouble()));
      TFraction.push_back(new RooRealVar(Form("TFraction_%d", i), Form("TFraction_%d", i),
         TDHFile["CentralValue"][Form("Fraction_%d", i + 3)].GetDouble()));
   ...
   RooRealVar WFinalRawYield("WFinalRawYield", "WFinalYield", 111.648 + 59.722);
   RooRealVar TTbarFinalRawYield("TTbarFinalRawYield", "TTbarFinalYield", 185.210);
   RooRealVar QCDFinalRawYield("QCDFinalRawYield", "QCD final yield", 244.288);
   ...
   FinalModel.plotOn(FinalResultPlot, LineColor(kBlue), Normalization(1));
   FinalModel.plotOn(FinalResultPlot, LineColor(kGreen), Components("AlmostFinalModel"), Normalization(1));
   FinalModel.plotOn(FinalResultPlot, LineColor(kRed), Components("WFinalModel"), Normalization(1));
   FinalModel.plotOn(FinalResultPlot, LineColor(kYellow), Components("TFinalModel"), Normalization(1));
   FinalResultPlot->SetMinimum(0.1);
   PsFile.AddPlot(FinalResultPlot, "", false);
   PsFile.AddPlot(FinalResultPlot, "", true);

   RooPlot *FinalResultPlotRestricted = MR.frame(Bins(50), Name("FitResultsRestricted"),
      Title("MR overlay, total model, no R2 > 0.5"), Range(400, 1500));
   Dataset.reduce("R*R < 0.5")->plotOn(FinalResultPlotRestricted, MarkerSize(0.9));
   ...
   double Factor = Dataset.sumEntries() / Dataset.sumEntries("R*R < 0.5");
   ...
   RooPlot *FinalResultPlot2Restricted = MR.frame(Bins(50), Name("FitResults2Restricted"),
      Title("MR, different R cuts, R2 < 0.5"), Range(400, 1400));
   Dataset.reduce("R*R < 0.5")->plotOn(FinalResultPlot2Restricted, MarkerSize(0.9), MarkerColor(1));
   for(int i = 0; i < (int)RCuts.size() - 1; i++)
   {
      RooArgList ComponentsToAdd;
