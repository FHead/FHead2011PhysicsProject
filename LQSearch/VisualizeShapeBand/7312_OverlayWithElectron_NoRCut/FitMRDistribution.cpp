   ...
   string WResult, string TResult, string QCDResult)
{
   ...
   RooRealVar WFinalRawYield("WFinalRawYield", "WFinalYield", 479.272);
   RooRealVar TTbarFinalRawYield("TTbarFinalRawYield", "TTbarFinalYield", 345.953);
   RooRealVar QCDFinalRawYield("QCDFinalRawYield", "QCD final yield", 24.461);
   ...
   TH1D HDummy1("HDumry1", "", 1, 0, 1);
   TH1D HDummy2("HDumry2", "", 1, 0, 1);
   TH1D HDummy3("HDumry3", "", 1, 0, 1);
   TH1D HDummy4("HDumry4", "", 1, 0, 1);

   HDummy1.SetFillColor(kBlue);
   HDummy2.SetFillColor(kGreen);
   HDummy3.SetFillColor(kRed);
   HDummy4.SetFillColor(kYellow - 3);

   TLegend legend1(0.55, 0.85, 0.85, 0.55);
   ...
   for(int i = 0; i < (int)RCuts.size(); i++)
   {
      ...
      if(i != (int)RCuts.size() - 1)
         legend2.AddEntry(DummyHistograms[i], Form("R^{2} > %.2f", RCuts[i] * RCuts[i]), "f");
   }
   legend.Draw();
