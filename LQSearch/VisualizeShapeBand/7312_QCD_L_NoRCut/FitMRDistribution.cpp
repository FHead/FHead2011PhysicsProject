...
RooFitResult *FitWithRCut(PsFileHelper &PsFile, string FileName, vector<double> RCuts, string WResult, string TResult)
{
   ...
   RooRealVar WFinalRawYield("WFinalRawYield", "WFinalYield", 1320.884 + 80.758);
   RooRealVar TTbarFinalRawYield("TTbarFinalRawYield", "TTbarFinalYield", 989.997);
   RooRealVar QCDFinalRawYield("QCDFinalRawYield", "QCD final yield", 7761.551);
   ...
   for(int i = 0; i < (int)RCuts.size(); i++)
   {
      ...
      if(RCuts[i] * RCuts[i] > 0.199 && RCuts[i] * RCuts[i] < 0.499)
