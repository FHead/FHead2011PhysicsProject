...
RooFitResult *FitWithRCut(PsFileHelper &PsFile, string FileName, vector<double> RCuts, string TResult);
...
int main(int argc, char *argv[])
{
   ...
   PsFile.AddTextPage("Fit WJet in the muon selection (fixing TTJet contribution)");

   RooFitResult *Result = FitWithRCut(PsFile, "DataPlots.root", CutsToTry, "TFitResult.dh");
...
RooFitResult *FitWithRCut(PsFileHelper &PsFile, string FileName, vector<double> RCuts, string TResult)
{
   ...
   RooRealVar ParameterA2("ParameterA2", "s2 = \"a2\" + b2 R^2", 0.004, 0, 0.1);
   RooRealVar ParameterB2("ParameterB2", "s2 = a2 + \"b2\" R^2", 0.007, 0, 1);
   ...
   for(int i = 0; i < (int)RCuts.size(); i++)
   {
      ...
      */
      ...
      if(RCuts[i] * RCuts[i] > 0.12)
         ...
         TFraction.push_back(new RooRealVar(Form("TFraction_%d", i), Form("TFraction_%d", i),
            TDHFile["CentralValue"][Form("Fraction_%d", i + 2)].GetDouble(), 0, 1));
   ...
   RooAddPdf WJetAlmostFinalModel("WJetAlmostFinalModel", "Final model!", ModelList, YieldList);
   RooAddPdf TFinalModel("TFinalModel", "Final model!", TModelList, TYieldList);

   RooRealVar TFinalRawYield("TFinalRawYield", "TFinalYield", 955.267);
   RooRealVar WJetFinalRawYield("WJetFinalRawYield", "WJetFinalYield", 1223.702);
   RooRealVar OverallFactor("OverallFactor", "OverallFactor", 1.0, 0, 100);
   RooFormulaVar TFinalYield("TFinalYield", "TFinalYield", "@0*@1", RooArgList(TFinalRawYield, OverallFactor));
   RooFormulaVar WJetFinalYield("WJetFinalYield", "WJetFinalYield", "@0*@1",
      RooArgList(WJetFinalRawYield, OverallFactor));

   RooAddPdf AlmostFinalModel("AlmostFinalModel", "Final model!!",
      RooArgList(WJetAlmostFinalModel, TFinalModel), RooArgList(WJetFinalYield, TFinalYield));
   ...
   */
   ...
   FinalModel.plotOn(FinalResultPlot, LineColor(kRed), Components("WJetAlmostFinalModel"));
   FinalModel.plotOn(FinalResultPlot, LineColor(kGreen), Components("TFinalModel"));
   ...
   double WJetRestrictedYield = 590.081 + 1104.595;
   ...
   PlotBands(PsFile, MRLowerBound, MRUpperBound, FitResult, FloatParameters, FloatNames,
      WJetAlmostFinalModel, MR, R, WeightedYields, RCuts, "Fit");
   ...
   */
   ...
   PlotBands(PsFile, MRLowerBound, MRUpperBound, FitResult, FloatParameters, FloatNames,
      WJetAlmostFinalModel, MR, R, WeightedYields, RCuts, "FullCorrelation", 0.1, 0.4, 1);
   ...
   */
   ...
   PlotBands(PsFile, MRLowerBound, MRUpperBound, FitResult, FloatParameters, FloatNames,
      WJetAlmostFinalModel, MR, R, WeightedYields, RCuts, "SmearAB", 0.1, 0.4, -0.85);
