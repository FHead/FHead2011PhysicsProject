...
RooFitResult *FitWithRCut(PsFileHelper &PsFile, string FileName, vector<double> RCuts)
{
   ...
   RooRealVar ParameterA2("ParameterA2", "s2 = \"a2\" + b2 R^2", 0.004, -0.1, 0.1);
   ...
   for(int i = 0; i < (int)RCuts.size(); i++)
   {
      double GuessYield = 0;
      if(i != RCuts.size() - 1)
         GuessYield = Dataset.sumEntries(Form("%s > %f && %s <= %f", RString.c_str(), RCuts[i],
            RString.c_str(), RCuts[i+1]));
      else
         GuessYield = Dataset.sumEntries(Form("%s > %f", RString.c_str(), RCuts[i]));
   ...
   for(int i = 0; i < (int)RCuts.size(); i++)
   {
      ...
      DHFile["CentralValue"][(string)SingleBinYields[i]->GetName() + "Error"] = SingleBinYields[i]->getError();
   }
   for(int i = 0; i < (int)RCuts.size(); i++)
   {
      DHFile["CentralValue"][Fraction[i]->GetName()] = Fraction[i]->getVal();
      DHFile["CentralValue"][(string)Fraction[i]->GetName() + "Error"] = Fraction[i]->getError();
   }
   for(int i = 0; i < (int)RCuts.size(); i++)
   ...
   for(int i = 0; i < (int)RCuts.size(); i++)
   {
      ...
      if(i != 9)
         FinalModel.plotOn(FinalResultPlot2, Components(ComponentsToAdd), LineColor(i + 1));
      else
         FinalModel.plotOn(FinalResultPlot2, Components(ComponentsToAdd), LineColor(1));
   }
   ...
   for(int i = 0; i < (int)RCuts.size(); i++)
   {
      ...
      if(i == 9)
      {
         DummyHistograms[i]->SetLineColor(1);
         DummyHistograms[i]->SetFillColor(1);
      }
   ...
   // double WJetRestrictedYield = 590.081 + 1104.595;   // Had + 1L, full, R2 = 0.2-0.5, MR > 400
   // double WJetRestrictedYield = 402.433;   // Electron + 1L, full, R2 = 0.2-0.5, MR > 400
   // double WJetRestrictedYield = 1937.405;   // Had+1L, full, R2 > 0.2, MR > 400
   // double WJetRestrictedYield = 385.572;   // Electron + 1L, full, R2 > 0.2, MR > 400, New

   // double WJetRestrictedYield = 647.141 + 1189.587;   // Had + 1L, R2 0.2, MR 400
   // double WJetRestrictedYield = 385.572;   // Electron + 1L, R2 0.2, MR 400
   // double WJetRestrictedYield = 697.362;   // Muon + L, MR > 400, R2 > 0.2
   // double WJetRestrictedYield = 647.141 + 292.713;   // Had + 1L, R2 0.2, MR 400

   double WJetRestrictedYield = 1818.072 + 842.937;   // Had + 1L, R2 0.2, MR 400
