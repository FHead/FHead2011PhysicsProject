...
SingleFitResult FitWithRCut(PsFileHelper &PsFile, string FileName, double RCut)
{
   ...
   TTree *Tree = (TTree *)F.Get("MRTreeDown");
   if(Tree == NULL)
...
RooFitResult *FitWithRCut(PsFileHelper &PsFile, string FileName, vector<double> RCuts)
{
   ...
   // double WJetRestrictedYield = 171.061;   // Had + LL, full, R2 > 0.2, MR > 400
   double WJetRestrictedYield = 114.284 + 29.992;   // Had + LL, full, R2 > 0.2, MR > 400
