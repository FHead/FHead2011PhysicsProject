...
int main()
{
   ...
   for(int i = 1; i <= 100; i++)
   {
      ...
      double TSignal = TToyS->sumEntries("MR > 400 && R * R >= 0.25");
      double QSignal = QToyS->sumEntries("MR > 400 && R * R >= 0.25");
   ...
   cout << "W SB " << HWSideBand.GetMean() << " " << HWSideBand.GetRMS() << endl;
   cout << "T SB " << HTSideBand.GetMean() << " " << HTSideBand.GetRMS() << endl;
   cout << "Q SB " << HQSideBand.GetMean() << " " << HQSideBand.GetRMS() << endl;
   cout << "W S " << HWSignal.GetMean() << " " << HWSignal.GetRMS() << endl;
   cout << "T S " << HTSignal.GetMean() << " " << HTSignal.GetRMS() << endl;
   cout << "Q S " << HQSignal.GetMean() << " " << HQSignal.GetRMS() << endl;
