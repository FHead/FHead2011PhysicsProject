...
int main()
{
   ...
   for(int i = 1; i <= 100; i++)
   {
      ...
      double TSignal = TToyS->sumEntries("MR > 400 && R * R >= 0.42");
      double QSignal = QToyS->sumEntries("MR > 400 && R * R >= 0.42");
      double TLowSignal = TToyS->sumEntries("MR > 400 && R * R < 0.20");
      // cout << TLowSignal << endl;
      HTLowSignal.Fill(TLowSignal);

      double WRestrictedSB = WToySB->sumEntries("MR > 400 && R * R >= 0.20");
      double TRestrictedSB = TToySB->sumEntries("MR > 400 && R * R >= 0.20");
      double QRestrictedSB = QToySB->sumEntries("MR > 400 && R * R >= 0.20");
      double WRestrictedS = WToyS->sumEntries("MR > 400 && R * R >= 0.20");
      double TRestrictedS = TToyS->sumEntries("MR > 400 && R * R >= 0.20");
      double QRestrictedS = QToyS->sumEntries("MR > 400 && R * R >= 0.20");
      ...
      if(TLowSignal > 70 && TLowSignal < 200)
