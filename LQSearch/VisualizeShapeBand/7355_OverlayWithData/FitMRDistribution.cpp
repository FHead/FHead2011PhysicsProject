   ...
   string WResult, string TResult, string QCDResult)
{
   ...
   TLegend Legend1(0.45, 0.85, 0.85, 0.55);
   ...
   Dummy6.SetFillColor(kMagenta);
   Legend1.AddEntry(&Dummy6, "Total + Signal (M_{LQ} = 250 GeV)", "f");

   TFile F2("LQ250.root");
   TH1D *HSignal = (TH1D *)F2.Get("HMRNew_RNew05_2");
   HSignal->SetLineWidth(2);
   HSignal->SetLineColor(kMagenta);
   TH1D *HSignal2 = (TH1D *)HSignal->Clone("HMRNew_RNew05_2_2");
   FinalModel.fillHistogram(HSignal2, RooArgList(MR), 326.976);
   HSignal->Add(HSignal2);
