...
RooFitResult *FitWithRCut(PsFileHelper &PsFile, string FileName, vector<double> RCuts, string WResult, string TResult)
{
   ...
   TFile F(FileName.c_str());
   TTree *Tree = (TTree *)F.Get("LooseMuL");
   if(Tree == NULL)
      return NULL;
   ...
   RooRealVar ParameterB1("ParameterB1", "s1 = a1 + \"b1\" R^2", 0.004, -1, 1);
   RooRealVar ParameterA2("ParameterA2", "s2 = \"a2\" + b2 R^2", TDHFile["CentralValue"]["ParameterA1"].GetDouble(), -0.1, 0.1);
   RooRealVar ParameterB2("ParameterB2", "s2 = a2 + \"b2\" R^2", TDHFile["CentralValue"]["ParameterB1"].GetDouble(), -1, 1);

   RooRealVar WParameterA1("WParameterA1", "s1 = \"a1\" + b1 R^2", WDHFile["CentralValue"]["ParameterA1"].GetDouble(), -0.1, 0.1);
   RooRealVar WParameterB1("WParameterB1", "s1 = a1 + \"b1\" R^2", WDHFile["CentralValue"]["ParameterB1"].GetDouble(), -1, 1);
   RooRealVar WParameterA2("WParameterA2", "s2 = \"a2\" + b2 R^2", WDHFile["CentralValue"]["ParameterA2"].GetDouble(), -0.1, 0.1);
   RooRealVar WParameterB2("WParameterB2", "s2 = a2 + \"b2\" R^2", WDHFile["CentralValue"]["ParameterB2"].GetDouble(), -1, 1);

   RooRealVar TParameterA1("TParameterA1", "s1 = \"a1\" + b1 R^2", TDHFile["CentralValue"]["ParameterA2"].GetDouble(), -0.1, 0.1);
   RooRealVar TParameterB1("TParameterB1", "s1 = a1 + \"b1\" R^2", TDHFile["CentralValue"]["ParameterB2"].GetDouble(), -1, 1);
   // RooRealVar TParameterA2("TParameterA2", "s2 = \"a2\" + b2 R^2", TDHFile["CentralValue"]["ParameterA1"].GetDouble());
   // RooRealVar TParameterB2("TParameterB2", "s2 = a2 + \"b2\" R^2", TDHFile["CentralValue"]["ParameterB1"].GetDouble());
   ...
   for(int i = 0; i < (int)RCuts.size(); i++)
   {
      ...
      else
      {
         TSingleBinYields.push_back(new RooRealVar(Form("TSingleBinYield_%d", i),
            ...
            TDHFile["CentralValue"][Form("SingleBinYield_%d", i)].GetDouble() * 3, 0, 1000000));
         WSingleBinYields.push_back(new RooRealVar(Form("WSingleBinYield_%d", i),
            Form("Yield with R in bin %d", i),
            WDHFile["CentralValue"][Form("SingleBinYield_%d", i + 2)].GetDouble() * 3, 0, 1000000));
      }
   }
   ...
   for(int i = 0; i < (int)RCuts.size(); i++)
   {
      ...
      if(i == 0)
         ...
         Fraction.push_back(new RooFormulaVar(Form("Fraction_%d", i), Form("Fraction_%d", i),
            "@0*@1", RooArgList(*FractionIncrement[i], *Fraction[i-1])));
      ...
      */
      ...
      if(RCuts[i] * RCuts[i] > 0.15)
      {
         ...
         TFraction.push_back(new RooRealVar(Form("TFraction_%d", i), Form("TFraction_%d", i),
            1 - TDHFile["CentralValue"][Form("Fraction_%d", i)].GetDouble()));
      }
      else
      {
         WFraction.push_back(new RooRealVar(Form("WFraction_%d", i), Form("WFraction_%d", i),
            WDHFile["CentralValue"][Form("Fraction_%d", i + 2)].GetDouble(), 0, 1));
         TFraction.push_back(new RooRealVar(Form("TFraction_%d", i), Form("TFraction_%d", i),
            1 - TDHFile["CentralValue"][Form("Fraction_%d", i)].GetDouble(), 0, 1));
      }
   ...
   RooRealVar WFinalRawYield("WFinalRawYield", "WFinalYield", 1177.328 + 48.370 + 40.294);
   RooRealVar TTbarFinalRawYield("TTbarFinalRawYield", "TTbarFinalYield", 906.141);
   RooRealVar QCDFinalRawYield("QCDFinalRawYield", "QCD final yield", 6996.756);

   RooRealVar TotalScalingFactor("ScalingFactor", "ScalingFactor", 1.00, 0, 20);
   RooRealVar QCDScalingFactor("QCDScalingFactor", "QCDScalingFactor", 2.78603);
   ...
   RooRealVar WParameterA2Val("WParameterA2Val", "Parameter A2 value",
      WDHFile["CentralValue"]["ParameterA2"].GetDouble());
   RooRealVar WParameterA2Error("WParameterA2Error", "Parameter A2 error",
      WDHFile["CentralValue"]["ParameterA2Error"].GetDouble());
   ...
   RooRealVar WParameterB2Val("WParameterB2Val", "Parameter B2 value",
      WDHFile["CentralValue"]["ParameterB2"].GetDouble());
   RooRealVar WParameterB2Error("WParameterB2Error", "Parameter B2 error",
      WDHFile["CentralValue"]["ParameterB2Error"].GetDouble());
   ...
   RooGaussian WParameterA2Penalty("WParameterA2Penalty", "Penalty for A2",
      WParameterA2, WParameterA2Val, WParameterA2Error);
   RooGaussian WParameterB1Penalty("WParameterB1Penalty", "Penalty for B1",
      WParameterB1, WParameterB1Val, WParameterB1Error);
   RooGaussian WParameterB2Penalty("WParameterB2Penalty", "Penalty for B2",
      WParameterB2, WParameterB2Val, WParameterB2Error);
   ...
   RooProdPdf FinalModel("FinalModel", "Real final model",
      RooArgList(ReallyAlmostFinalModel, WParameterA1Penalty, WParameterA2Penalty,
         WParameterB1Penalty, WParameterB2Penalty, TParameterA1Penalty, TParameterA2Penalty,
         TParameterB1Penalty, TParameterB2Penalty));
   ...
   // FitResult = FinalModel.fitTo(Dataset, Save(true), PrintEvalErrors(-1), NumCPU(3));
   ...
   DHFile["CentralValue"]["TParameterA2"] = ParameterA2.getVal();
   DHFile["CentralValue"]["TParameterB1"] = TParameterB1.getVal();
   DHFile["CentralValue"]["TParameterB2"] = ParameterB2.getVal();
   DHFile["CentralValue"]["TParameterA1Error"] = TParameterA1.getError();
   DHFile["CentralValue"]["TParameterA2Error"] = ParameterA2.getError();
   DHFile["CentralValue"]["TParameterB1Error"] = TParameterB1.getError();
   DHFile["CentralValue"]["TParameterB2Error"] = ParameterB2.getError();
   ...
   TLegend legend1(0.5, 0.85, 0.8, 0.55);
   legend1.SetFillStyle(0);
   legend1.SetBorderSize(0);
   legend1.SetTextFont(42);
   legend1.AddEntry(&HDummy1, "Total", "f");
   legend1.AddEntry(&HDummy2, "Multijets", "f");
   legend1.AddEntry(&HDummy3, "W and Z #rightarrow #nu#bar{#nu} + jets", "f");
   legend1.AddEntry(&HDummy4, "t#bar{t} + jets", "f");

   TCanvas FinalResultCanvasSquare("C", "C", 1024, 1024);
   FinalResultPlot->Draw();
   AddCMSPreliminary(0.1, 0.92, 1776);
   legend1.Draw();
   ...
   PlotBands(PsFile, MRLowerBound, MRUpperBound, FitResult, FloatParameters, FloatNames,
      AlmostFinalModel, MR, R, WeightedYields, RCuts, "FitMM");

   PsFile.AddTextPage("Smear A and B with full correlation");
   PlotBands(PsFile, MRLowerBound, MRUpperBound, FitResult, FloatParameters, FloatNames,
      AlmostFinalModel, MR, R, WeightedYields, RCuts, "FullCorrelationMM", 0.1, 0.4, 1);

   PsFile.AddTextPage("Smear A and B");
   PlotBands(PsFile, MRLowerBound, MRUpperBound, FitResult, FloatParameters, FloatNames,
      AlmostFinalModel, MR, R, WeightedYields, RCuts, "SmearABMM", 0.1, 0.4, -0.85);
