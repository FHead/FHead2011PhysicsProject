...
RooFitResult *FitWithRCut(PsFileHelper &PsFile, string FileName, vector<double> RCuts, string WResult);
...
int main(int argc, char *argv[])
{
   ...
   RooFitResult *Result = FitWithRCut(PsFile, "DataPlots.root", CutsToTry, "WFitResult.dh");
...
SingleFitResult FitWithRCut(PsFileHelper &PsFile, string FileName, double RCut)
{
   ...
   TTree *Tree = (TTree *)F.Get("TightMuMM");
   if(Tree == NULL)
...
RooFitResult *FitWithRCut(PsFileHelper &PsFile, string FileName, vector<double> RCuts, string WResult)
{
   ...
   RooDataSet Dataset("Dataset", "MR Dataset", Tree, TreeVarSet,
      Form("%s > %f && %s > %f", RString.c_str(), RCuts[0], MRString.c_str(), MRLowerBound));

   RooRealVar ParameterA1("ParameterA1", "s1 = \"a1\" + b1 R^2", 0.008, -0.1, 0.1);
   RooRealVar ParameterB1("ParameterB1", "s1 = a1 + \"b1\" R^2", 0.06, -1, 1);
   // RooRealVar ParameterA2("ParameterA2", "s2 = \"a2\" + b2 R^2", 0.005, 0, 0.1);
   // RooRealVar ParameterB2("ParameterB2", "s2 = a2 + \"b2\" R^2", 0.017, 0, 1);
   ...
   FloatParameters.push_back(&WParameterA1);
   FloatParameters.push_back(&WParameterB1);
   FloatNames.push_back("ParameterA1");
   FloatNames.push_back("ParameterB1");
   FloatNames.push_back("WParameterA1");
   FloatNames.push_back("WParameterB1");
   ...
   RooAddPdf TTbarAlmostFinalModel("TTbarAlmostFinalModel", "Final model!", ModelList, YieldList);
   RooAddPdf WFinalModel("WFinalModel", "Final model!", WModelList, WYieldList);
   
   RooRealVar WFinalRawYield("WFinalRawYield", "WFinalYield", 11.003);
   RooRealVar TTbarFinalRawYield("TTbarFinalRawYield", "TTbarFinalYield", 157.951);
   RooRealVar OverallFactor("OverallFactor", "OverallFactor", 1.0, 0, 100);
   RooFormulaVar WFinalYield("WFinalYield", "WFinalYield", "@0*@1", RooArgList(WFinalRawYield, OverallFactor));
   RooFormulaVar TTbarFinalYield("TTbarFinalYield", "TTbarFinalYield", "@0*@1",
      RooArgList(TTbarFinalRawYield, OverallFactor));

   RooAddPdf AlmostFinalModel("AlmostFinalModel", "Final model!!",
      RooArgList(TTbarAlmostFinalModel, WFinalModel), RooArgList(TTbarFinalYield, WFinalYield));

   RooRealVar ParameterA1Val("ParameterA1Val", "Parameter A1 value",
      WDHFile["CentralValue"]["ParameterA1"].GetDouble());
   RooRealVar ParameterA1Error("ParameterA1Error", "Parameter A1 error",
      WDHFile["CentralValue"]["ParameterA1Error"].GetDouble());
   RooRealVar ParameterA2Val("ParameterA2Val", "Parameter A2 value",
      WDHFile["CentralValue"]["ParameterA2"].GetDouble());
   RooRealVar ParameterA2Error("ParameterA2Error", "Parameter A2 error",
      WDHFile["CentralValue"]["ParameterA2Error"].GetDouble());
   RooRealVar ParameterB1Val("ParameterB1Val", "Parameter B1 value",
      WDHFile["CentralValue"]["ParameterB1"].GetDouble());
   RooRealVar ParameterB1Error("ParameterB1Error", "Parameter B1 error",
      WDHFile["CentralValue"]["ParameterB1Error"].GetDouble());
   RooRealVar ParameterB2Val("ParameterB2Val", "Parameter B2 value",
      WDHFile["CentralValue"]["ParameterB2"].GetDouble());
   RooRealVar ParameterB2Error("ParameterB2Error", "Parameter B2 error",
      WDHFile["CentralValue"]["ParameterB2Error"].GetDouble());
   RooGaussian ParameterA1Penalty("ParameterA1Penalty", "Penalty for A1", WParameterA1, ParameterA1Val, ParameterA1Error);
   RooGaussian ParameterA2Penalty("ParameterA2Penalty", "Penalty for A2", WParameterA2, ParameterA2Val, ParameterA2Error);
   RooGaussian ParameterB1Penalty("ParameterB1Penalty", "Penalty for B1", WParameterB1, ParameterB1Val, ParameterB1Error);
   RooGaussian ParameterB2Penalty("ParameterB2Penalty", "Penalty for B2", WParameterB2, ParameterB2Val, ParameterB2Error);

   // RooProdPdf FinalModel("FinalModel", "Real final model",
   //    RooArgList(AlmostFinalModel, ParameterA1Penalty, ParameterA2Penalty, ParameterB1Penalty, ParameterB2Penalty));
   RooProdPdf FinalModel("FinalModel", "Real final model",
      RooArgList(AlmostFinalModel, ParameterA1Penalty, ParameterB1Penalty));

   // RooAbsPdf &FinalModel = AlmostFinalModel;
   ...
   DHFile["CentralValue"]["ParameterA2"] = WParameterA1.getVal();
   DHFile["CentralValue"]["ParameterB1"] = ParameterB1.getVal();
   DHFile["CentralValue"]["ParameterB2"] = WParameterB1.getVal();
   DHFile["CentralValue"]["ParameterA1Error"] = ParameterA1.getError();
   DHFile["CentralValue"]["ParameterA2Error"] = WParameterA1.getError();
   DHFile["CentralValue"]["ParameterB1Error"] = ParameterB1.getError();
   DHFile["CentralValue"]["ParameterB2Error"] = WParameterB1.getError();
   for(int i = 0; i < (int)RCuts.size(); i++)
   ...
   DHFile["CentralValue"]["OverallFactor"] = OverallFactor.getVal();
   ...
   RooPlot *FinalResultPlot = MR.frame(Bins(25), Name("FitResults"),
      Title("  "), Range(400, 2000));
   Dataset.plotOn(FinalResultPlot, MarkerSize(1.8));
   FinalModel.plotOn(FinalResultPlot, LineColor(kBlue));
   FinalModel.plotOn(FinalResultPlot, LineColor(kRed), Components("TTbarAlmostFinalModel"));
   FinalModel.plotOn(FinalResultPlot, LineColor(kGreen), Components("WFinalModel"));
   FinalResultPlot->SetMinimum(0.3);
   PsFile.AddPlot(FinalResultPlot, "", true);

   TH1D HDummy1("HDummy1", "Aye-aye sir", 1, 0, 1);
   TH1D HDummy2("HDummy2", "Meow", 1, 0, 1);
   TH1D HDummy3("HDummy3", "Total", 1, 0, 1);

   HDummy1.SetFillColor(kGreen);
   HDummy2.SetFillColor(kRed);
   HDummy3.SetFillColor(kBlue);
   
   HDummy1.SetLineColor(kGreen);
   HDummy2.SetLineColor(kRed);
   HDummy3.SetLineColor(kBlue);

   HDummy1.SetLineWidth(2);
   HDummy2.SetLineWidth(2);
   HDummy3.SetLineWidth(2);
   
   TLegend legend1(0.4, 0.55, 0.85, 0.85);
   legend1.SetFillStyle(0);
   legend1.SetBorderSize(0);
   legend1.SetTextFont(42);
   legend1.AddEntry(&HDummy3, "Total", "l");
   legend1.AddEntry(&HDummy1, "W and Z #rightarrow #nu#bar{#nu} + jets", "l");
   legend1.AddEntry(&HDummy2, "t#bar{t} + jets", "l");
   ...
   RooPlot *FinalResultPlot2 = MR.frame(Bins(25), Name("FitResults2"), Title("  "),
      Range(400, 2000));
   RooPlot *FinalResultPlot3 = MR.frame(Bins(25), Name("FitResults3"), Title("  "),
      Range(400, 2000));
   ...
   for(int i = 0; i < (int)RCuts.size(); i++)
   {
      ...
      legend.AddEntry(DummyHistograms[i], Form("R^{2} > %.2f", RCuts[i] * RCuts[i]), "lf");
   }
   legend.Draw();
   ...
   TLegend legend3(0.45, 0.85, 0.85, 0.45);
   ...
   TCanvas FinalResultCanvas3Square("FinalResultcanvas3Square", "", 1024, 1024);
   FinalResultPlot3->Draw();
   FinalResultCanvas3Square.SetLogy();
   legend3.Draw();
   AddCMS(0.1, 0.92, 4800);
   FinalResultCanvas3Square.SaveAs("FinalResultCanvas3Square.png");
   FinalResultCanvas3Square.SaveAs("FinalResultCanvas3Square.C");
   FinalResultCanvas3Square.SaveAs("FinalResultCanvas3Square.eps");
   FinalResultCanvas3Square.SaveAs("FinalResultCanvas3Square.pdf");
   ...
   for(double x = R2Begin; x <= R2End; x = x + (R2End - R2Begin) / 100)
   {
      ...
      double Correlation2 = FitResult->correlation("WParameterA1", "WParameterB1");
      double Value1 = ParameterA1.getVal() + ParameterB1.getVal() * x;
      double Value2 = WParameterA1.getVal() + WParameterB1.getVal() * x;
      ...
      double Error2 = sqrt(WParameterA1.getError() * WParameterA1.getError()
         + WParameterB1.getError() * WParameterB1.getError() * x * x
         + 2 * WParameterA1.getError() * WParameterB1.getError() * x * Correlation1);
   ...
   for(double x = R2Begin; x <= R2End; x = x + (R2End - R2Begin) / 100)
   {
      ...
      double Error2 = sqrt(WParameterA1.getError() * WParameterA1.getError()
         + WParameterB1.getError() * WParameterB1.getError() * x * x
         + 2 * WParameterA1.getError() * WParameterB1.getError() * x * Correlation2);
   ...
   FitParameterA2Stream << "Parameter A2 from simultaneous fit = "
      << WParameterA1.getVal() << " +- " << WParameterA1.getError();
   ...
   FitParameterB2Stream << "Parameter B2 from simultaneous fit = "
      << WParameterB1.getVal() << " +- " << WParameterB1.getError();
   ...
   FitParameterCorrelationStream << "Correlation(a1, b1) = "
      ...
      << "Correlation(a2, b2) = " << FitResult->correlation("WParameterA1", "WParameterB1");
   ...
   RooPlot ParameterA2B2Plot(WParameterA1, WParameterB1,
      WParameterA1.getVal() * 0.5, WParameterA1.getVal() * 1.5,
      WParameterB1.getVal() * 0.5, WParameterB1.getVal() * 1.5);
   FitResult->plotOn(&ParameterA2B2Plot, WParameterA1, WParameterB1, "ME");
   PsFile.AddPlot(ParameterA2B2Plot);
   
   RooPlot ParameterA1A2Plot(ParameterA1, WParameterA1,
      ParameterA1.getVal() * 0.8, ParameterA1.getVal() * 1.2,
      WParameterA1.getVal() * 0.5, WParameterA1.getVal() * 1.5);
   FitResult->plotOn(&ParameterA1A2Plot, ParameterA1, WParameterA1, "ME");
   PsFile.AddPlot(ParameterA1A2Plot);
   
   RooPlot ParameterB1B2Plot(ParameterB1, WParameterB1,
      ParameterB1.getVal() * 0.8, ParameterB1.getVal() * 1.2,
      WParameterB1.getVal() * 0.5, WParameterB1.getVal() * 1.5);
   FitResult->plotOn(&ParameterB1B2Plot, ParameterB1, WParameterB1, "ME");
   ...
   // double TTbarRestrictedYield = 189.677;   // MR > 400, R2 = 0.20-0.50, Had+1L, full
   // double TTbarRestrictedYield = 302.418;   // MR > 400, R2 = 0.20-0.50, Electron+1L, full
   // double TTbarRestrictedYield = 335.012;   // MR > 400, R2 > 0.2, ElectronL
   // double TTbarRestrictedYield = 281.352;   // MR > 400, R2 > 0.2, ElectronL (NEW)
   // double TTbarRestrictedYield = 198.337;   // MR > 400, R2 > 0.2, HadL

   // double TTbarRestrictedYield = 201.575;   // MR > 400, R2 > 0.2, HadL
   // double TTbarRestrictedYield = 271.352;   // MR > 400, R2 > 0.2, ElectronL

   // double TTbarRestrictedYield = 220.646;   // MR > 400, R2 > 0.2, HadMM
   double TTbarRestrictedYield = 39.809 * 4.7;   // MR > 400, R2 > 0.2, EleMM

   double WeightedYields = TotalSingleBinYield / RestrictedSingleBinYield * TTbarRestrictedYield;

   PsFile.AddTextPage("Fit-band");
   PlotBands(PsFile, MRLowerBound, MRUpperBound, FitResult, FloatParameters, FloatNames,
      TTbarAlmostFinalModel, MR, R, WeightedYields, RCuts, "Fit");
   ...
   */
   ...
   PlotBands(PsFile, MRLowerBound, MRUpperBound, FitResult, FloatParameters, FloatNames,
      TTbarAlmostFinalModel, MR, R, WeightedYields, RCuts, "FullCorrelation", 0.1, 0.4, 1);
   ...
   */
   ...
   PlotBands(PsFile, MRLowerBound, MRUpperBound, FitResult, FloatParameters, FloatNames,
      TTbarAlmostFinalModel, MR, R, WeightedYields, RCuts, "SmearAB", 0.1, 0.4, -0.85);
   
   PlotBands(PsFile, MRLowerBound, MRUpperBound, FitResult, FloatParameters, FloatNames,
      TTbarAlmostFinalModel, MR, R, 2000, RCuts, "Meow", 0.1, 0.4, -0.85);
   ...
   string Tag, double SigmaA, double SigmaB, double Correlation)
{
   ...
   for(int i = 0; i < Tries; i++)
   {
      ...
      // double A2Value = 0;
      // double B2Value = 0;
      double A2Value = ((RooAbsReal *)Parameters.find("WParameterA1"))->getVal() * (1 + ChangeA2);
      double B2Value = ((RooAbsReal *)Parameters.find("WParameterB1"))->getVal() * (1 + ChangeB2);
      ...
      for(int j = 0; j < (int)FloatParameters.size(); j++)
      {
         ...
         else if(FloatNames[j] == "WParameterA1")
            FloatParameters[j]->setVal(A2Value);
         else if(FloatNames[j] == "WParameterB1")
