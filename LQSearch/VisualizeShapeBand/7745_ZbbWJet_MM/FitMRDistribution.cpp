...
SingleFitResult FitWithRCut(PsFileHelper &PsFile, string FileName, double RCut)
{
   ...
   RooPlot *FinalResultPlot = MR.frame(Bins(25), Name("FitResults"), Title("MR"), AutoRange(Dataset));
...
RooFitResult *FitWithRCut(PsFileHelper &PsFile, string FileName, vector<double> RCuts)
{
   ...
   RooPlot *FinalResultPlot = MR.frame(Bins(25), Name("FitResults"), Title("    "),
      Range(400, 1500));
   ...
   for(int i = 0; i < (int)RCuts.size(); i++)
   {
      if(i == 0 || i == 2 || i == 3 || i == 5 || i == 7 || i == 9)
      {
         ...
         FinalModel.plotOn(FinalResultPlot2, LineColor(kBlue), Components(ComponentsToAdd),
            LineColor((i == 9)?2:(i + 1)), MarkerSize(1.8));
      }
   }
   for(int i = 1; i < (int)RCuts.size(); i++)
   {
      if(i == 0 || i == 2 || i == 3 || i == 5 || i == 7 || i == 9)
         Dataset.reduce(Cut(Form("%s > %f", RString.c_str(), RCuts[i])))->plotOn(FinalResultPlot2,
            MarkerSize(1.8), MarkerColor((i == 9)?2:(i + 1)));
   }
   ...
   TLegend legend(0.60, 0.50, 0.90, 0.85);
   ...
   for(int i = 0; i < (int)RCuts.size(); i++)
   {
      ...
      if(i == 0 || i == 2 || i == 3 || i == 5 || i == 7 || i == 9)
      {
         ...
         if(LineColor == 10)
            LineColor = 2;
         DummyHistograms[i]->SetLineColor(LineColor);
   ...
   TH1D HR2ViewDataHist("HR2ViewDataHist", ";R^{2};a.u.", RCuts.size() - 1, R2Binning);
   ...
   RViewLegend.AddEntry(&HR2ViewDataHist, "Simulation", "lp");
   ...
   // double WJetRestrictedYield = 74.639;   // electron (LL), R2 > 0.20, MR > 400
   // double WJetRestrictedYield = 104.526 + 59.719;   // Had, LL, R2 > 0.20, MR > 400
   // double WJetRestrictedYield = 73.497 + 41.991;

   // double WJetRestrictedYield = 12.722 + 3.766;   // Had, MM, R2 > 0.20, MR > 400
   double WJetRestrictedYield = (0.519 + 2.215) * 4.7;   // Ele, MM, R2 > 0.20, MR > 400
