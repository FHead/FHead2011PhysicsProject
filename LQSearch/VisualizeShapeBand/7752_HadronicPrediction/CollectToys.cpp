...
int main()
{
   ...
   TH1D HWSideBand("HWSideBand", "W prediction in side band", 500, 0, 500);
   TH1D HTSideBand("HTSideBand", "Top prediction in side band", 500, 0, 500);
   TH1D HQSideBand("HQSideBand", "QCD prediction in side band", 500, 0, 500);
   TH1D HAllSideBand("HAllSideBand", "Total prediction in side band", 500, 0, 500);

   TH1D HWSignal("HWSignal", "W prediction in signal region", 500, 0, 500);
   TH1D HTSignal("HTSignal", "Top prediction in signal region", 500, 0, 500);
   TH1D HQSignal("HQSignal", "QCD prediction in signal region", 500, 0, 500);
   TH1D HAllSignal("HAllSignal", "Total prediction in signal region", 500, 0, 500);
   TH1D HAllHighSignal("HAllHighSignal", "Total prediction in high signal region", 50, 0, 50);

   TH1D HWRestrictedSB("HWRestrictedSB", "W in restricted region (SB)", 20000, 0, 20000);
   TH1D HTRestrictedSB("HTRestrictedSB", "T in restricted region (SB)", 20000, 0, 20000);
   TH1D HQRestrictedSB("HQRestrictedSB", "Q in restricted region (SB)", 20000, 0, 20000);
   TH1D HWRestrictedS("HWRestrictedS", "W in restricted region (S)", 20000, 0, 20000);
   TH1D HTRestrictedS("HTRestrictedS", "T in restricted region (S)", 20000, 0, 20000);
   TH1D HQRestrictedS("HQRestrictedS", "Q in restricted region (S)", 20000, 0, 20000);

   TH1D HTLowSignal("HTLowSignal", "TLowSignal", 20000, 0, 20000);
   ...
   for(int i = 1; i <= 200; i++)
   {
      ...
      double WHighSignal = WToyS->sumEntries("MR > 800 && R * R >= 0.25");
      double THighSignal = TToyS->sumEntries("MRL > 800 && RL * RL >= 0.25");
      double QHighSignal = QToyS->sumEntries("MRL >= 800 && RL * RL > 0.25");
      ...
      if(QPrediction > 5 && QPrediction < 50000 && QSignal > 1)
      {
      ...
      if(WSignal > 0 && TSignal > 0 && QSignal > 0 && TSignal < 900 && WHighSignal + THighSignal + QHighSignal < 200)
         HAllHighSignal.Fill(WHighSignal + THighSignal + QHighSignal);
   ...
   PsFile.AddPlot(HAllHighSignal, "", true);
   ...
   cout << "All " << HWSideBand.GetMean() + HTSideBand.GetMean() + HQSideBand.GetMean()
      << " +- " << sqrt(HWSideBand.GetRMS() * HWSideBand.GetRMS() + HTSideBand.GetRMS() * HTSideBand.GetRMS()
      + HQSideBand.GetRMS() * HQSideBand.GetRMS()) << endl;
   cout << endl;
   ...
   cout << "All " << HWSignal.GetMean() + HTSignal.GetMean() + HQSignal.GetMean()
      << " +- " << sqrt(HWSignal.GetRMS() * HWSignal.GetRMS() + HTSignal.GetRMS() * HTSignal.GetRMS()
      + HQSignal.GetRMS() * HQSignal.GetRMS()) << endl;
   cout << endl;
