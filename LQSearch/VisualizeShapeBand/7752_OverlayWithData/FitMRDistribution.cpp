...
int main(int argc, char *argv[])
{
   ...
   FitWithRCut(PsFile, "DataPlots2.root", CutsToTry, "WFitResult.dh", "TTbarFitResult.dh", "QCDFitResult.dh");
   ...
   string WResult, string TResult, string QCDResult)
{
   ...
   RooRealVar WFinalRawYield("WFinalRawYield", "WFinalYield", 1.72 + 0.818 + 0);
   RooRealVar TTbarFinalRawYield("TTbarFinalRawYield", "TTbarFinalYield", 25.62);
   RooRealVar QCDFinalRawYield("QCDFinalRawYield", "QCD final yield", 4.67163);
   ...
   // FinalModel.plotOn(FinalResultPlot, LineColor(kBlue), Normalization(224.0 / 200));
   // FinalModel.plotOn(FinalResultPlot, LineColor(kGreen), Components("AlmostFinalModel"), Normalization(224.0 / 200));
   // FinalModel.plotOn(FinalResultPlot, LineColor(kRed), Components("WFinalModel"), Normalization(224.0 / 200));
   // FinalModel.plotOn(FinalResultPlot, LineColor(kYellow), Components("TFinalModel"), Normalization(224.0 / 200));
   TH1D HTotal("HTotal", "", 25, 400 ,1400);
   TH1D HW("HW", "", 25, 400 ,1400);
   TH1D HT("HT", "", 25, 400 ,1400);
   TH1D HQ("HQ", "", 25, 400 ,1400);
   FinalModel.fillHistogram(&HTotal, RooArgSet(MR), 205.248838);
   WFinalModel.fillHistogram(&HW, RooArgSet(MR), 11.4 * 1.10184745);
   TFinalModel.fillHistogram(&HT, RooArgSet(MR), 140.271 * 1.10184745);
   AlmostFinalModel.fillHistogram(&HQ, RooArgSet(MR), 34.445 * 1.10184745);
   ...
   Legend1.AddEntry(&Dummy6, "Total prediction + LQ (350 GeV)", "l");

   TFile F1("LQToBNutau_250.root");
   TFile F2("LQToBNutau_350.root");
   TFile F3("LQToBNutau_450.root");
   TFile F4("LQToBNutau_550.root");
   TFile F5("T2bbSignal.root");
   TH1D *HSignalOnly250 = (TH1D *)F1.Get("HMR_R2025Restricted_Had_TwoMedium");
   TH1D *HSignalOnly350 = (TH1D *)F2.Get("HMR_R2025Restricted_Had_TwoMedium");
   TH1D *HSignalOnly450 = (TH1D *)F3.Get("HMR_R2025Restricted_Had_TwoMedium");
   TH1D *HSignalOnly550 = (TH1D *)F4.Get("HMR_R2025Restricted_Had_TwoMedium");
   TH1D *HSignalOnlyT2bb = (TH1D *)F5.Get("MRHistogram");
   HSignalOnly250->Scale(4.700);
   HSignalOnly350->Scale(4.700);
   HSignalOnly450->Scale(4.700);
   HSignalOnly550->Scale(4.700);
   HSignalOnlyT2bb->Scale(1.000);   // scaled already
   TH1D *HSignal = (TH1D *)HSignalOnly350->Clone("HSignal");
   FinalModel.fillHistogram(HSignal, RooArgList(MR), 205.248838);
   HSignal->Add(HSignalOnly350);
   HSignal->SetLineWidth(2);
   HSignal->SetLineColor(kMagenta);

   HSignalOnlyT2bb->SetStats(0);

   HSignalOnly350->SetLineWidth(2);
   HSignalOnly450->SetLineWidth(2);
   HSignalOnly550->SetLineWidth(2);
   HSignalOnlyT2bb->SetLineWidth(2);
   HSignalOnly350->SetLineColor(kBlue);
   HSignalOnly450->SetLineColor(kYellow);
   HSignalOnly550->SetLineColor(kCyan);
   HSignalOnlyT2bb->SetLineColor(kRed);
   HSignalOnlyT2bb->SetLineStyle(2);
   
   TH1D Dummy7("Dummy7", "dummy!", 2, 0, 1);
   Dummy7.SetLineColor(kBlue);
   Dummy7.SetLineWidth(2);
   // Legend1.AddEntry(&Dummy7, "LQ (350 GeV)", "l");
   
   TH1D Dummy8("Dummy8", "dummy!", 2, 0, 1);
   Dummy8.SetLineColor(kYellow);
   Dummy8.SetLineWidth(2);
   // Legend1.AddEntry(&Dummy8, "LQ (450 GeV)", "l");
   
   TH1D Dummy9("Dummy9", "dummy!", 2, 0, 1);
   Dummy9.SetLineColor(kCyan);
   Dummy9.SetLineWidth(2);
   Legend1.AddEntry(&Dummy9, "LQ (550 GeV)", "l");

   TH1D Dummy10("Dummy10", "dummy!", 2, 0, 1);
   Dummy10.SetLineColor(kRed);
   Dummy10.SetLineStyle(2);
   Dummy10.SetLineWidth(2);
   Legend1.AddEntry(&Dummy10, "SMS Signal (M_{#tilde{b}} = 300 GeV, M_{#tilde{#chi}_{1}^{0}}100 GeV)", "l");
   ...
   // HSignalOnly250->Draw("same");
   HSignalOnly350->Draw("same");
   HSignalOnly450->Draw("same");
   HSignalOnly550->Draw("same");
   HSignalOnlyT2bb->Draw("same");
   Legend1.Draw();
   ...
   HModelR2View.SetBinContent(1, 205.248838 - 99.7816527);
   HModelR2View.SetBinContent(2, 99.7816527 - 55.9289284);
   HModelR2View.SetBinContent(3, 55.9289284 - 42.3857136);
   HModelR2View.SetBinContent(4, 42.3857136 - 24.7014586);
   HModelR2View.SetBinContent(5, 24.7014586);
   HModelR2View.SetBinError(1, sqrt(30.4038008 * 30.4038008 - 16.8650758 * 16.8650758));
   HModelR2View.SetBinError(2, sqrt(16.8650758 * 16.8650758 - 12.4926384 * 12.4926384));
   HModelR2View.SetBinError(3, sqrt(12.4926384 * 12.4926384 - 9.06631906 * 9.06631906));
   HModelR2View.SetBinError(4, sqrt(9.06631906 * 9.06631906 - 7.07717948 * 7.07717948));
   HModelR2View.SetBinError(5, 7.07717948);
   ...
   HLQR2View.SetBinContent(1, 4.99998474121093750);
   HLQR2View.SetBinContent(2, 4.21698379516601562);
   HLQR2View.SetBinContent(3, 1.92453134059906006);
   HLQR2View.SetBinContent(4, 2.64151406288146973);
   HLQR2View.SetBinContent(5, 9.05649566650390625);
   HLQR2View.Scale(4.700);
   HLQR2View.SetLineWidth(2);
   HLQR2View.SetLineColor(kMagenta);
   PsFile.AddPlot(HLQR2View);

   TH1D *RHistogram = (TH1D *)F5.Get("RHistogram");
   RHistogram->SetLineStyle(9);
   RHistogram->SetLineColor(kRed);
   RHistogram->SetLineWidth(2);
   RHistogram->SetStats(0);

   HLQR2View.Add(&HModelR2View);
   // RHistogram->Add(&HModelR2View);
   ...
   RViewLegend.AddEntry(RHistogram, "Signal (M_{#tilde{b}} = 300 GeV, M_{#tilde{#chi}_{1}^{0}} = 100 GeV)", "l");

   TCanvas FinalResult3Canvas;
   HDataR2View.SetMinimum(0);
   HDataR2View.Draw("error");
   HModelR2View.Draw("same hist");
   RHistogram->Draw("same");
   RViewLegend.Draw();
