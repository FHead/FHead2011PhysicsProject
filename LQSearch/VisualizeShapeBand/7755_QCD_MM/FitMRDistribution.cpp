...
RooFitResult *FitWithRCut(PsFileHelper &PsFile, string FileName, vector<double> RCuts, string WResult, string TResult);
...
int main(int argc, char *argv[])
{
   ...
   PsFileHelper PsFile("FitResultExportBand.ps");
   PsFile.AddTextPage("Fit ttbar in the muon selection (fixing WJet contribution)");

   RooFitResult *Result = FitWithRCut(PsFile, "DataPlots.root", CutsToTry, "WFitResult.dh", "TFitResult.dh");
...
SingleFitResult FitWithRCut(PsFileHelper &PsFile, string FileName, double RCut)
{
   ...
   TTree *Tree = (TTree *)F.Get("LooseMuL2");
   if(Tree == NULL)
   ...
   RooPlot *ParameterPlot = MR.frame(Bins(40), Name("FitParameters"), Title("Fit parameters"));
   Model->paramOn(ParameterPlot, Format("NELU", AutoPrecision(3)), Layout(0.1, 0.4, 0.9));
   PsFile.AddPlot(ParameterPlot, "", false);

   RooPlot *FinalResultPlot = MR.frame(Bins(40), Name("FitResults"), Title("MR"), AutoRange(Dataset));
...
RooFitResult *FitWithRCut(PsFileHelper &PsFile, string FileName, vector<double> RCuts, string WResult, string TResult)
{
   ...
   TFile F(FileName.c_str());
   TTree *Tree = (TTree *)F.Get("LooseMuMM");
   if(Tree == NULL)
      return NULL;
   ...
   RooRealVar ParameterA1("ParameterA1", "s1 = \"a1\" + b1 R^2", 0.012, -0.1, 0.1);
   RooRealVar ParameterB1("ParameterB1", "s1 = a1 + \"b1\" R^2", 0.000);
   RooRealVar ParameterA2("ParameterA2", "s2 = \"a2\" + b2 R^2", 0.006, -0.1, 0.1);
   RooRealVar ParameterB2("ParameterB2", "s2 = a2 + \"b2\" R^2", 0.010, -1, 1);
   ...
   vector<RooFormulaVar *> Fraction;
   vector<RooRealVar *> FractionIncrement;
   ...
   for(int i = 0; i < (int)RCuts.size(); i++)
   {
      ...
      if(i == 0)   GuessYield = 2807;
      if(i == 1)   GuessYield = 473;
      if(i == 2)   GuessYield = 143;
      if(i == 3)   GuessYield = 22.2;
      if(i == 4)   GuessYield = 6.96;
      if(i == 5)   GuessYield = 4.20;

      if(i == 0)
         SingleBinYields.push_back(new RooRealVar(Form("SingleBinYield_%d", i),
            Form("Yield with R in bin %d", i), GuessYield, 0, GuessYield * 10 + 20));
      else if(i < 6)
         SingleBinYields.push_back(new RooRealVar(Form("SingleBinYield_%d", i),
            Form("Yield with R in bin %d", i), GuessYield, 0, GuessYield * 10 + 20));
      else
         SingleBinYields.push_back(new RooRealVar(Form("SingleBinYield_%d", i),
            Form("Yield with R in bin %d", i), 0));

      FloatParameters.push_back(SingleBinYields[i]);
      FloatNames.push_back(Form("SingleBinYield_%d", i));

      if(RCuts[i] * RCuts[i] > 0.15)
      {
         TSingleBinYields.push_back(new RooRealVar(Form("TSingleBinYield_%d", i),
            Form("Yield with R in bin %d", i), TDHFile["CentralValue"][Form("SingleBinYield_%d", i)].GetDouble()));
         WSingleBinYields.push_back(new RooRealVar(Form("WSingleBinYield_%d", i),
            Form("Yield with R in bin %d", i), WDHFile["CentralValue"][Form("SingleBinYield_%d", i + 2)].GetDouble()));
      }
      else
      {
         TSingleBinYields.push_back(new RooRealVar(Form("TSingleBinYield_%d", i),
            Form("Yield with R in bin %d", i),
            TDHFile["CentralValue"][Form("SingleBinYield_%d", i)].GetDouble(), 0, 100000));
         WSingleBinYields.push_back(new RooRealVar(Form("WSingleBinYield_%d", i),
            Form("Yield with R in bin %d", i),
            WDHFile["CentralValue"][Form("SingleBinYield_%d", i + 2)].GetDouble()));
      }
   }
   ...
   for(int i = 0; i < (int)RCuts.size(); i++)
   {
      ...
      if(i == 0)
      {
         FractionIncrement.push_back(new RooRealVar(Form("FractionIncrement_%d", i), Form("FractionIncrement_%d", i),
            0.70, 0.0, 1.0));
         FloatParameters.push_back(FractionIncrement[i]);
         FloatNames.push_back(Form("FractionIncrement_%d", i));
      }
      else if(i < 2)
      {
         FractionIncrement.push_back(new RooRealVar(Form("FractionIncrement_%d", i), Form("FractionIncrement_%d", i),
      ...
      if(i == 0)
         Fraction.push_back(new RooFormulaVar(Form("Fraction_%d", i), Form("Fraction_%d", i),
            "@0", RooArgList(*FractionIncrement[0])));
      else
         Fraction.push_back(new RooFormulaVar(Form("Fraction_%d", i), Form("Fraction_%d", i),
            "@0", RooArgList(*FractionIncrement[i])));
         // Fraction.push_back(new RooFormulaVar(Form("Fraction_%d", i), Form("Fraction_%d", i),
         //    "@0*@1", RooArgList(*Fraction[i-1], *FractionIncrement[i])));
      ...
      */
      ...
      if(RCuts[i] * RCuts[i] > 0.15)
      {
         WFraction.push_back(new RooRealVar(Form("WFraction_%d", i), Form("WFraction_%d", i),
            WDHFile["CentralValue"][Form("Fraction_%d", i + 2)].GetDouble()));
         TFraction.push_back(new RooRealVar(Form("TFraction_%d", i), Form("TFraction_%d", i),
            TDHFile["CentralValue"][Form("Fraction_%d", i)].GetDouble()));
      }
      else
      {
         WFraction.push_back(new RooRealVar(Form("WFraction_%d", i), Form("WFraction_%d", i),
   ...
   RooRealVar WFinalRawYield("WFinalRawYield", "WFinalYield", 10.535 + 0 + 0.217);
   RooRealVar TTbarFinalRawYield("TTbarFinalRawYield", "TTbarFinalYield", 175.822);
   RooRealVar QCDFinalRawYield("QCDFinalRawYield", "QCD final yield", 399.138);

   RooRealVar TotalScalingFactor("ScalingFactor", "ScalingFactor", 4.18469);
   RooRealVar QCDScalingFactor("QCDScalingFactor", "QCDScalingFactor", 1.44, 0, 100);
   FloatParameters.push_back(&QCDScalingFactor);
   FloatNames.push_back("QCDScalingFactor");

   RooFormulaVar WFinalYield("WFinalYield", "WFinalYield", "@0*@1", RooArgList(WFinalRawYield, TotalScalingFactor));
   RooFormulaVar TTbarFinalYield("TTbarFinalYield", "TFinalYield", "@0*@1", RooArgList(TTbarFinalRawYield, TotalScalingFactor));
   RooFormulaVar QCDFinalYield("QCDFinalYield", "QCDFinalYield", "@0*@1*@2", RooArgList(QCDFinalRawYield, TotalScalingFactor, QCDScalingFactor));

   RooAddPdf ReallyAlmostFinalModel("ReallyAlmostFinalModel", "almost final model!!",
      RooArgList(AlmostFinalModel, WFinalModel, TFinalModel),
      RooArgList(QCDFinalYield, WFinalYield, TTbarFinalYield));

   RooRealVar WParameterA1Val("WParameterA1Val", "Parameter A1 value",
      WDHFile["CentralValue"]["ParameterA1"].GetDouble());
   RooRealVar WParameterA1Error("WParameterA1Error", "Parameter A1 error",
      WDHFile["CentralValue"]["ParameterA1Error"].GetDouble());
   RooRealVar WParameterB1Val("WParameterB1Val", "Parameter B1 value",
      WDHFile["CentralValue"]["ParameterB1"].GetDouble());
   RooRealVar WParameterB1Error("WParameterB1Error", "Parameter B1 error",
      WDHFile["CentralValue"]["ParameterB1Error"].GetDouble());
   RooRealVar TParameterA1Val("TParameterA1Val", "Parameter A1 value",
      TDHFile["CentralValue"]["ParameterA1"].GetDouble());
   RooRealVar TParameterA1Error("TParameterA1Error", "Parameter A1 error",
      TDHFile["CentralValue"]["ParameterA1Error"].GetDouble());
   RooRealVar TParameterA2Val("TParameterA2Val", "Parameter A2 value",
      TDHFile["CentralValue"]["ParameterA2"].GetDouble());
   RooRealVar TParameterA2Error("TParameterA2Error", "Parameter A2 error",
      TDHFile["CentralValue"]["ParameterA2Error"].GetDouble());
   RooRealVar TParameterB1Val("TParameterB1Val", "Parameter B1 value",
      TDHFile["CentralValue"]["ParameterB1"].GetDouble());
   RooRealVar TParameterB1Error("TParameterB1Error", "Parameter B1 error",
      TDHFile["CentralValue"]["ParameterB1Error"].GetDouble());
   RooRealVar TParameterB2Val("TParameterB2Val", "Parameter B2 value",
      TDHFile["CentralValue"]["ParameterB2"].GetDouble());
   RooRealVar TParameterB2Error("TParameterB2Error", "Parameter B2 error",
      TDHFile["CentralValue"]["ParameterB2Error"].GetDouble());
   RooGaussian WParameterA1Penalty("WParameterA1Penalty", "Penalty for A1",
      WParameterA1, WParameterA1Val, WParameterA1Error);
   RooGaussian WParameterB1Penalty("WParameterB1Penalty", "Penalty for B1",
      WParameterB1, WParameterB1Val, WParameterB1Error);
   RooGaussian TParameterA1Penalty("TParameterA1Penalty", "Penalty for A1",
      TParameterA1, TParameterA1Val, TParameterA1Error);
   RooGaussian TParameterA2Penalty("TParameterA2Penalty", "Penalty for A2",
      ParameterA2, TParameterA2Val, TParameterA2Error);
   RooGaussian TParameterB1Penalty("TParameterB1Penalty", "Penalty for B1",
      TParameterB1, TParameterB1Val, TParameterB1Error);
   RooGaussian TParameterB2Penalty("TParameterB2Penalty", "Penalty for B2",
      ParameterB2, TParameterB2Val, TParameterB2Error);

   RooProdPdf FinalModel("FinalModel", "Real final model",
      RooArgList(ReallyAlmostFinalModel, TParameterA2Penalty, TParameterB2Penalty));
   
   // RooAbsPdf &FinalModel = ReallyAlmostFinalModel;

   RooFitResult *FitResult = FinalModel.fitTo(Dataset, Save(true), PrintEvalErrors(-1), NumCPU(4));
   FitResult = FinalModel.fitTo(Dataset, Save(true), PrintEvalErrors(-1), NumCPU(4));
   ...
   DHFile["CentralValue"]["ParameterA1Error"] = ParameterA1.getError();
   DHFile["CentralValue"]["ParameterA2Error"] = ParameterA2.getError();
   DHFile["CentralValue"]["ParameterB1Error"] = ParameterB1.getError();
   DHFile["CentralValue"]["ParameterB2Error"] = ParameterB2.getError();
   DHFile["CentralValue"]["TParameterA1"] = TParameterA1.getVal();
   DHFile["CentralValue"]["TParameterA2"] = TParameterA2.getVal();
   DHFile["CentralValue"]["TParameterB1"] = TParameterB1.getVal();
   DHFile["CentralValue"]["TParameterB2"] = TParameterB2.getVal();
   DHFile["CentralValue"]["TParameterA1Error"] = TParameterA1.getError();
   DHFile["CentralValue"]["TParameterA2Error"] = TParameterA2.getError();
   DHFile["CentralValue"]["TParameterB1Error"] = TParameterB1.getError();
   DHFile["CentralValue"]["TParameterB2Error"] = TParameterB2.getError();
   DHFile["CentralValue"]["WParameterA1"] = WParameterA1.getVal();
   DHFile["CentralValue"]["WParameterA2"] = WParameterA2.getVal();
   DHFile["CentralValue"]["WParameterB1"] = WParameterB1.getVal();
   DHFile["CentralValue"]["WParameterB2"] = WParameterB2.getVal();
   DHFile["CentralValue"]["WParameterA1Error"] = WParameterA1.getError();
   DHFile["CentralValue"]["WParameterA2Error"] = WParameterA2.getError();
   DHFile["CentralValue"]["WParameterB1Error"] = WParameterB1.getError();
   DHFile["CentralValue"]["WParameterB2Error"] = WParameterB2.getError();
   for(int i = 0; i < (int)RCuts.size(); i++)
   ...
   for(int i = 0; i < (int)RCuts.size(); i++)
      DHFile["CentralValue"][string(Fraction[i]->GetName())+"Error"] = FractionIncrement[i]->getError();
   for(int i = 0; i < (int)RCuts.size(); i++)
      DHFile["CentralValue"][Form("R2Cut%d", i)] = RCuts[i] * RCuts[i];
   DHFile["CentralValue"]["OverallFactor"] = TotalScalingFactor.getVal();
   DHFile["CentralValue"]["QCDOverallFactor"] = QCDScalingFactor.getVal();
   DHFile["CentralValue"]["CovQual"] = FitResult->covQual();
   ...
   RooPlot *FinalResultPlot = MR.frame(Bins(25), Name("FitResults"), Title("  "), Range(400, 2000));
   ...
   TH1D HDummy1("HDummy1", "...", 1, 0, 1);
   TH1D HDummy2("HDummy2", "...", 1, 0, 1);
   TH1D HDummy3("HDummy3", "...", 1, 0, 1);
   TH1D HDummy4("HDummy4", "...", 1, 0, 1);

   HDummy1.SetFillColor(kBlue);
   HDummy2.SetFillColor(kGreen);
   HDummy3.SetFillColor(kRed);
   HDummy4.SetFillColor(kYellow);

   HDummy1.SetLineColor(kBlue);
   HDummy2.SetLineColor(kGreen);
   HDummy3.SetLineColor(kRed);
   HDummy4.SetLineColor(kYellow);

   HDummy1.SetLineWidth(2);
   HDummy2.SetLineWidth(2);
   HDummy3.SetLineWidth(2);
   HDummy4.SetLineWidth(2);

   TLegend legend1(0.35, 0.85, 0.8, 0.55);
   legend1.SetFillStyle(0);
   legend1.SetBorderSize(0);
   legend1.SetTextFont(42);
   legend1.AddEntry(&HDummy1, "Total", "l");
   legend1.AddEntry(&HDummy2, "Multijets", "l");
   legend1.AddEntry(&HDummy3, "W and Z #rightarrow #nu#bar{#nu} + jets", "l");
   legend1.AddEntry(&HDummy4, "t#bar{t} + jets", "l");

   TCanvas FinalResultCanvasSquare("C", "C", 1024, 1024);
   FinalResultPlot->Draw();
   AddCMS(0.1, 0.92, 4700);
   legend1.Draw();
   FinalResultCanvasSquare.SaveAs("QCDFit1.png");
   FinalResultCanvasSquare.SaveAs("QCDFit1.C");
   FinalResultCanvasSquare.SaveAs("QCDFit1.eps");
   FinalResultCanvasSquare.SaveAs("QCDFit1.pdf");
   FinalResultCanvasSquare.SetLogy();
   FinalResultCanvasSquare.SaveAs("QCDFit1Log.png");
   FinalResultCanvasSquare.SaveAs("QCDFit1Log.C");
   FinalResultCanvasSquare.SaveAs("QCDFit1Log.eps");
   FinalResultCanvasSquare.SaveAs("QCDFit1Log.pdf");

   // Each bin, model vs. data
   RooPlot *FinalResultPlot2 = MR.frame(Bins(25), Name("FitResults2"), Title("  "), Range(400, 2000));
   RooPlot *FinalResultPlot3 = MR.frame(Bins(25), Name("FitResults2"), Title("  "), Range(400, 2000));
   Dataset.plotOn(FinalResultPlot2, MarkerSize(1.8), MarkerColor(1));
   Dataset.plotOn(FinalResultPlot3, MarkerSize(1.8), MarkerColor(1));
   for(int i = 0; i < (int)RCuts.size(); i++)
   {
      ...
      if(i == 0 || i == 1 || i == 3 || i == 5 || i == 7)
         FinalModel.plotOn(FinalResultPlot3, LineColor(kBlue), Components(ComponentsToAdd), LineColor(i + 1));

      RooArgList ComponentsToAdd2;
      for(int j = i; j < (int)RCuts.size(); j++)
      {
         ComponentsToAdd2.add(*WTopLevelModels[j]);
         ComponentsToAdd2.add(*TTopLevelModels[j]);
      }
      FinalModel.plotOn(FinalResultPlot2, LineColor(kBlue), Components(ComponentsToAdd2), LineColor(i + 1));
      
      // RooArgList ComponentsToAdd3;
      // for(int j = i; j < (int)RCuts.size(); j++)
      //    ComponentsToAdd3.add(*WTopLevelModels[j]);
      // FinalModel.plotOn(FinalResultPlot2, LineColor(kBlue), Components(ComponentsToAdd3), LineColor(i + 1));
   }
   for(int i = 1; i < (int)RCuts.size(); i++)
   {
      ...
      if(i == 0 || i == 1 || i == 3 || i == 5 || i == 7)
         Dataset.reduce(Cut(Form("%s > %f", RString.c_str(), RCuts[i])))->plotOn(FinalResultPlot3,
            MarkerSize(1.8), MarkerColor(i + 1));
   }
   ...
   TLegend legend(0.65, 0.85, 0.9, 0.45);
   ...
   TCanvas FinalResultCanvas2Square("C", "C", 1024, 1024);
   FinalResultPlot2->Draw();
   legend.Draw();
   AddCMS(0.1, 0.92, 4700);
   FinalResultCanvas2Square.SaveAs("QCDFit.png");
   FinalResultCanvas2Square.SaveAs("QCDFit.C");
   FinalResultCanvas2Square.SaveAs("QCDFit.eps");
   FinalResultCanvas2Square.SaveAs("QCDFit.pdf");
   FinalResultCanvas2Square.SetLogy();
   FinalResultCanvas2Square.SaveAs("QCDFitLog.png");
   FinalResultCanvas2Square.SaveAs("QCDFitLog.C");
   FinalResultCanvas2Square.SaveAs("QCDFitLog.eps");
   FinalResultCanvas2Square.SaveAs("QCDFitLog.pdf");
   
   TCanvas FinalResultCanvas3;

   FinalResultPlot3->SetMinimum(0.3);
   FinalResultPlot3->Draw();

   TLegend legend3(0.50, 0.85, 0.85, 0.45);
   ...
   for(int i = 0; i < (int)RCuts.size(); i++)
   {
      ...
      DummyHistograms[i]->SetLineWidth(2);
      if(i == 0 || i == 1 || i == 3 || i == 5 || i == 7)
         legend3.AddEntry(DummyHistograms[i], Form("R^{2} > %.2f", RCuts[i] * RCuts[i]), "l");
   }
   legend3.Draw();

   PsFile.AddCanvas(FinalResultCanvas3);
   FinalResultCanvas3.SetLogy();
   PsFile.AddCanvas(FinalResultCanvas3);

   TCanvas FinalResultCanvas3Square("C", "C", 1024, 1024);
   FinalResultPlot3->Draw();
   legend3.Draw();
   AddCMS(0.1, 0.92, 4700);
   FinalResultCanvas3Square.SaveAs("QCDFit2.png");
   FinalResultCanvas3Square.SaveAs("QCDFit2.C");
   FinalResultCanvas3Square.SaveAs("QCDFit2.eps");
   FinalResultCanvas3Square.SaveAs("QCDFit2.pdf");
   FinalResultCanvas3Square.SetLogy();
   FinalResultCanvas3Square.SaveAs("QCDFit2Log.png");
   FinalResultCanvas3Square.SaveAs("QCDFit2Log.C");
   FinalResultCanvas3Square.SaveAs("QCDFit2Log.eps");
   FinalResultCanvas3Square.SaveAs("QCDFit2Log.pdf");

   for(int i = 0; i < (int)RCuts.size(); i++)
      delete DummyHistograms[i];
   DummyHistograms.clear();

   double RBinning[100] = {0};
   double R2Binning[100] = {0};
   for(int i = 0; i < (int)RCuts.size(); i++)
   {
      RBinning[i] = RCuts[i];
      R2Binning[i] = RCuts[i] * RCuts[i];
   }
   RBinning[RCuts.size()] = 1.5;

   TH1D HRViewDataHist("HRViewDataHist", ";R^{2};a.u.", RCuts.size(), RBinning);
   Dataset.fillHistogram(&HRViewDataHist, RooArgList(R));

   TH1D HR2ViewDataHist("HR2ViewDataHist", ";R^{2}", RCuts.size() - 1, R2Binning);
   for(int i = 1; i < RCuts.size(); i++)
      HR2ViewDataHist.SetBinContent(i, HRViewDataHist.GetBinContent(i));
   HR2ViewDataHist.SetStats(0);
   HR2ViewDataHist.SetMarkerStyle(20);
   HR2ViewDataHist.SetMarkerSize(1.6);
   HR2ViewDataHist.SetLineWidth(2);

   TH1D HRViewModel("HRViewModel", "", RCuts.size(), RBinning);
   FinalModel.fillHistogram(&HRViewModel, RooArgSet(R));

   TH1D HR2ViewModel("HR2ViewModel", "", RCuts.size() - 1, R2Binning);
   for(int i = 1; i < RCuts.size(); i++)
      HR2ViewModel.SetBinContent(i,
         HRViewModel.GetBinContent(i) * (RCuts[i] - RCuts[i-1]));
   HR2ViewModel.SetStats(0);
   HR2ViewModel.Scale(HR2ViewDataHist.Integral() / HR2ViewModel.Integral());

   TLegend RViewLegend(0.65, 0.85, 0.85, 0.75);
   RViewLegend.SetFillStyle(0);
   RViewLegend.SetBorderSize(0);
   RViewLegend.SetTextFont(42);
   RViewLegend.AddEntry(&HR2ViewDataHist, "Data", "lp");
   RViewLegend.AddEntry(&HR2ViewModel, "Fit result", "l");

   TCanvas CRView("CRView", "", 1024, 1024);
   HR2ViewDataHist.Draw("error");
   HR2ViewModel.Draw("same hist");
   RViewLegend.Draw();
   CRView.SaveAs("RView.png");
   CRView.SaveAs("RView.C");
   CRView.SaveAs("RView.eps");
   CRView.SaveAs("RView.pdf");
   ...
   TFile BandFile("BandsJunk.root", "recreate");

   PsFile.AddTextPage("Now, try to get ALL the bands");

   // WSingleBinYields[0]->setConstant();
   ...
   // double QCDRestrictedYield = 23.329;   // electron, R2 > 0.20, MR > 400
   // double QCDRestrictedYield = 800.74;   // had, 1L, R2 > 0.20, MR > 400
   // double QCDRestrictedYield = 243.500;   // had, LL, R2 > 0.20, MR > 400
   // double QCDRestrictedYield = 171.216;   // had, 1232, LL, R2 > 0.20, MR > 400
   // double QCDRestrictedYield = 563.093;   // had, 1232, L, R2 > 0.20, MR > 400
   // double QCDRestrictedYield = 243.514;   // had, LL, R2 0.2-0.5, MR > 400, full
   // double QCDRestrictedYield = 800.787;   // had, 1L, R2 0.2-0.5, MR > 400, full
   // double QCDRestrictedYield = 21.52;   // Electron, L, R2 0.2-0.5, MR > 400
   // double QCDRestrictedYield = 1.785;   // Electron, LL, R2 0.2-0.5, MR > 400
   // double QCDRestrictedYield = 24.326;   // ElectronL, R2 0.2, MR 400
   // double QCDRestrictedYield = 1.785;   // ElectronLL, R2 0.2, MR 400

   // double QCDRestrictedYield = 246.153;   // Had LL, R2 0.2, MR 400
   // double QCDRestrictedYield = 822.744;   // Had, L, R2 0.2, MR 400

   double QCDRestrictedYield = 85.73737;   // Had MM, R2 0.2, MR 400
   // double QCDRestrictedYield = (1.425) * 4.7;   // Ele MM, R2 0.2, MR 400

   double WeightedYields = TotalSingleBinYield / RestrictedSingleBinYield * QCDRestrictedYield
      * QCDScalingFactor.getVal();

   PsFile.AddTextPage("Fit-band");
   PlotBands(PsFile, MRLowerBound, MRUpperBound, FitResult, FloatParameters, FloatNames,
      AlmostFinalModel, MR, R, WeightedYields, RCuts, "Fit");

   PsFile.AddTextPage("Smear A and B with full correlation");
   PlotBands(PsFile, MRLowerBound, MRUpperBound, FitResult, FloatParameters, FloatNames,
      AlmostFinalModel, MR, R, WeightedYields, RCuts, "FullCorrelation", 0.1, 0.4, 1);

   PsFile.AddTextPage("Smear A and B");
   PlotBands(PsFile, MRLowerBound, MRUpperBound, FitResult, FloatParameters, FloatNames,
      AlmostFinalModel, MR, R, WeightedYields, RCuts, "SmearAB", 0.1, 0.4, -0.85);
   
   QCDRestrictedYield = 2088.571;   // Had L, R2 0.2, MR 400

   WeightedYields = TotalSingleBinYield / RestrictedSingleBinYield * QCDRestrictedYield
      * QCDScalingFactor.getVal();

   PsFile.AddTextPage("Fit-band");
   PlotBands(PsFile, MRLowerBound, MRUpperBound, FitResult, FloatParameters, FloatNames,
      AlmostFinalModel, MR, R, WeightedYields, RCuts, "FitL");

   PsFile.AddTextPage("Smear A and B with full correlation");
   PlotBands(PsFile, MRLowerBound, MRUpperBound, FitResult, FloatParameters, FloatNames,
      AlmostFinalModel, MR, R, WeightedYields, RCuts, "FullCorrelationL", 0.1, 0.4, 1);

   PsFile.AddTextPage("Smear A and B");
   PlotBands(PsFile, MRLowerBound, MRUpperBound, FitResult, FloatParameters, FloatNames,
      AlmostFinalModel, MR, R, WeightedYields, RCuts, "SmearABL", 0.1, 0.4, -0.85);
   ...
   for(int i = 0; i < (int)RCuts.size(); i++)
   {
      ...
      if(FractionIncrement[i] != NULL) delete FractionIncrement[i];
   }
   ...
   string Tag, double SigmaA, double SigmaB, double Correlation)
{
   ...
   vector<double> OriginalNumbers(FloatParameters.size());
   for(int j = 0; j < (int)FloatParameters.size(); j++)
   {
      if(FloatParameters[j] != NULL)
         OriginalNumbers[j] = FloatParameters[j]->getVal();
   }
   ...
   for(int i = 0; i < Tries; i++)
   {
      ...
      {
         if(FloatParameters[j] != NULL)
            FloatParameters[j]->setVal(OriginalNumbers[j]);
         if(FloatNames[j] == "SingleBinYield_1")
            S1 = OriginalNumbers[j];
         if(FloatNames[j] == "SingleBinYield_2")
            S2 = OriginalNumbers[j];
      }

      // RooDataSet *ToyDataSet = (RooDataSet *)PDF.generate(RooArgSet(R, MR), (long long)DrawPoisson(WeightedYields));
      // RooDataSet *ToyDataSet = (RooDataSet *)PDF.generate(RooArgSet(R, MR), (long long)WeightedYields);

      cout << "O " << S1 << " " << S2 << endl;
      const RooArgList Parameters = Result->randomizePars();
      cout << "V " << ((RooAbsReal *)Parameters.find("SingleBinYield_1"))->getVal() << " "
         << ((RooAbsReal *)Parameters.find("SingleBinYield_2"))->getVal() << endl;
      
      /*
      for(int j = 0; j < (int)FloatParameters.size(); j++)
      {
         if(FloatNames[j].find("SingleBinYield") != string::npos)
            FloatParameters[j]->setVal(OriginalNumbers[j]);
      }
      */
      ...
      for(int j = 0; j < (int)FloatParameters.size(); j++)
      {
         ...
         /*
         if(FloatNames[j][0] == 'S' && FloatNames[j][1] == 'i' && Parameters.find(FloatNames[j].c_str()) != NULL)
         {
            cout << FloatNames[j] << " "
               << ((RooAbsReal *)Parameters.find(FloatNames[j].c_str()))->getVal()
               * ((RooAbsReal *)Parameters.find("QCDScalingFactor"))->getVal() << endl;
         }
         cout << endl;
         */
      }
