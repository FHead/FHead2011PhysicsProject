...
SingleFitResult FitWithRCut(PsFileHelper &PsFile, string FileName, double RCut)
{
   ...
   TTree *Tree = (TTree *)F.Get("LooseMuLL");
   if(Tree == NULL)
...
RooFitResult *FitWithRCut(PsFileHelper &PsFile, string FileName, vector<double> RCuts, string WResult, string TResult)
{
   ...
   RooRealVar ParameterA1("ParameterA1", "s1 = \"a1\" + b1 R^2", 0.012, 0, 0.1);
   RooRealVar ParameterB1("ParameterB1", "s1 = a1 + \"b1\" R^2", 0.003, 0, 1);
   RooRealVar ParameterA2("ParameterA2", "s2 = \"a2\" + b2 R^2", TDHFile["CentralValue"]["ParameterA2"].GetDouble());
   RooRealVar ParameterB2("ParameterB2", "s2 = a2 + \"b2\" R^2", TDHFile["CentralValue"]["ParameterB2"].GetDouble());
   ...
   for(int i = 0; i < (int)RCuts.size(); i++)
   {
      ...
      */
      ...
      else
      {
         ...
         TFraction.push_back(new RooRealVar(Form("TFraction_%d", i), Form("TFraction_%d", i),
            TDHFile["CentralValue"][Form("Fraction_%d", i)].GetDouble(), 0, 1));
      }
   ...
   RooRealVar WFinalRawYield("WFinalRawYield", "WFinalYield", 177.145);
   RooRealVar TTbarFinalRawYield("TTbarFinalRawYield", "TTbarFinalYield", 352.246);
   RooRealVar QCDFinalRawYield("QCDFinalRawYield", "QCD final yield", 1715.218);

   RooRealVar TotalScalingFactor("ScalingFactor", "ScalingFactor", 4.32816);
   RooRealVar QCDScalingFactor("QCDScalingFactor", "QCD scaling factor", 1.2, 0, 20);
   ...
   DHFile["CentralValue"]["QCDScaling"] = QCDScalingFactor.getVal();
   ...
   double QCDRestrictedYield = 143.748 * 4.7;   // HadLL, R2 0.2 MR 400

   double WeightedYields = TotalSingleBinYield / RestrictedSingleBinYield * QCDRestrictedYield * QCDScalingFactor.getVal();
