...
SingleFitResult FitWithRCut(PsFileHelper &PsFile, string FileName, double RCut)
{
   ...
   TTree *Tree = (TTree *)F.Get("TightMuLL");
   if(Tree == NULL)
...
RooFitResult *FitWithRCut(PsFileHelper &PsFile, string FileName, vector<double> RCuts, string WResult)
{
   ...
   RooRealVar ParameterA1("ParameterA1", "s1 = \"a1\" + b1 R^2", 0.0083648, 0, 0.1);
   RooRealVar ParameterB1("ParameterB1", "s1 = a1 + \"b1\" R^2", 0.0610108, 0, 1);
   RooRealVar ParameterA2("ParameterA2", "s2 = \"a2\" + b2 R^2", 0.00389341, 0, 0.1);
   RooRealVar ParameterB2("ParameterB2", "s2 = a2 + \"b2\" R^2", 0.0177789, 0, 1);
   
   RooRealVar WParameterA1("WParameterA1", "s1 = \"a1\" + b1 R^2", WDHFile["CentralValue"]["ParameterA1"].GetDouble(), 0, 0.1);
   RooRealVar WParameterB1("WParameterB1", "s1 = a1 + \"b1\" R^2", WDHFile["CentralValue"]["ParameterB1"].GetDouble(), 0, 1);
   RooRealVar WParameterA2("WParameterA2", "s2 = \"a2\" + b2 R^2", WDHFile["CentralValue"]["ParameterA2"].GetDouble(), 0, 0.1);
   RooRealVar WParameterB2("WParameterB2", "s2 = a2 + \"b2\" R^2", WDHFile["CentralValue"]["ParameterB2"].GetDouble(), 0, 1);
   ...
   for(int i = 0; i < (int)RCuts.size(); i++)
   {
      ...
      if(RCuts[i] * RCuts[i] > 0.10)
         ...
         WSingleBinYields.push_back(new RooRealVar(Form("WSingleBinYield_%d", i),
            Form("Yield with R in bin %d", i), WDHFile["CentralValue"][Form("SingleBinYield_%d", i + 2)].GetDouble() / 10,
            0, 10000));
   }
   ...
   RooRealVar WFinalRawYield("WFinalRawYield", "WFinalYield", 163.197);
   RooRealVar TTbarFinalRawYield("TTbarFinalRawYield", "TTbarFinalYield", 313.911);
   RooRealVar OverallFactor("OverallFactor", "OverallFactor", 4.5, 0, 100);
   ...
   RooProdPdf FinalModel("FinalModel", "Real final model",
      RooArgList(AlmostFinalModel, ParameterA1Penalty, ParameterB1Penalty, ParameterA2Penalty, ParameterB2Penalty));
   ...
   legend1.AddEntry(&HDummy3, "Total", "f");
   legend1.AddEntry(&HDummy1, "W and Z #rightarrow #nu#bar{#nu} + jets", "f");
   legend1.AddEntry(&HDummy2, "t#bar{t} + jets", "f");

   TCanvas FinalResultCanvas1Square("FinalResultcanvas1Square", "", 1024, 1024);
   FinalResultPlot->Draw();
   legend1.Draw();
   FinalResultCanvas1Square.SetLogy();
   AddCMSPreliminary(0.1, 0.92, 4991);
   ...
   legend.Draw();
   AddCMSPreliminary(0.1, 0.92, 1770);
   ...
   // double TTbarRestrictedYield = 548.030;   // MR > 400, R2 > 0.2, Had+1L

   double TTbarRestrictedYield = 98.762 * 4.7;   // MR > 400, R2 > 0.2, HadLL
