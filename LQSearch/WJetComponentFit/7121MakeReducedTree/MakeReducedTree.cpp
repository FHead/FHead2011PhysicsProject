...
{
   // Get input
   TChain *InputTree = ChainAll("WJets.list", "ntp1");

   NTPTreeMessenger M;
   M.SetBranchAddress(InputTree);

   // Prepare output
   TFile OutputFile("WJetReducedTree.root", "RECREATE");
   ...
   // counting histogram
   TH2D HCategoryCount("HCategoryCount", "Count of events in each category;MC Category;Box", 4, 0, 4, 3, 0, 3);
   HCategoryCount.GetXaxis()->SetBinLabel(1, "e");
   HCategoryCount.GetXaxis()->SetBinLabel(2, "mu");
   HCategoryCount.GetXaxis()->SetBinLabel(3, "tau");
   HCategoryCount.GetXaxis()->SetBinLabel(4, "had");
   ...
   for(int iEntry = 0; iEntry < EntryCount; iEntry++)
   {
      ...
      {
         cout << "Number of jets = " << InputJets.size() << endl;
         continue;
      }
      if(InputJets.size() < 2)
         continue;
      ...
      /*
      cout << Hemispheres[0][0] << " " << Hemispheres[0].GetP() << endl;
      cout << Hemispheres[1][0] << " " << Hemispheres[1].GetP() << endl;
      cout << Hemispheres[0][0] << " " << Hemispheres[0][1]
         << " " << Hemispheres[0][2] << " " << Hemispheres[0][3] << endl;
      cout << Hemispheres[1][0] << " " << Hemispheres[1][1]
         << " " << Hemispheres[1][2] << " " << Hemispheres[1][3] << endl;
      cout << BetaR << endl;
      */
