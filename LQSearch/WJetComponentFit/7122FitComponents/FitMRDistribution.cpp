...
int main(int argc, char *argv[])
{
   ...
   Decays.push_back("E");
   Decays.push_back("Mu");
   Decays.push_back("Tau");
   Decays.push_back("Had");

   for(int i = 0; i < 3; i++)
   {
      for(int j = 0; j < 4; j++)
      {
         if(Decays[j] == "Tau")   // no W tau nu in the sample...!?
            continue;

         PsFileHelper PsFile(Decays[j] + "_" + Boxes[i] + "Box.ps");
         PsFile.AddTextPage("Fitting W to " + Decays[j] + " in the " + Boxes[i] + " box");
         FitWithRCut(PsFile, "WJetReducedTree.root", CutsToTry, j, i);
