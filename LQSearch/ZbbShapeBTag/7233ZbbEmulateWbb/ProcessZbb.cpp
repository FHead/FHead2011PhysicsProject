      ...
      for(int i = 0; i < (int)VBChildren.size(); i++)
      {
         ...
         for(int j = 0; j < M.nAK5Jet; j++)
         {
            ...
            if(fabs(JetP.GetEta()) > 3)
               continue;
            if(MCTree[VBChildren[i]].PDGID == 11 || MCTree[VBChildren[i]].PDGID == 15 ||
               MCTree[VBChildren[i]].PDGID == -11 || MCTree[VBChildren[i]].PDGID == -15)
            {
         ...
         if(JetMatched == false && fabs(MCTree[VBChildren[i]].PDGID) == 13
            && fabs(FourVector(MCTree[VBChildren[i]].P).GetEta()) < 2.4)
            PFMET = PFMET + LeptonP;
         */
