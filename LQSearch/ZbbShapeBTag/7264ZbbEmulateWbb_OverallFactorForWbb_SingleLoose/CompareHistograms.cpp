...
RatioResult Compare1DHistogram(PsFileHelper &PsFile, TFile &FZbb, TFile &FInclusive, string HistogramName)
{
   ...
   legend.AddEntry(HZbb, "(Z to ll) + bb, scaled to 1/fb with #sigma = \"1pb\", x1500", "l");
   legend.Draw();
   ...
   HInclusive->SetTitle(TString(HInclusive->GetTitle()).ReplaceAll(" (new)", ""));
   HInclusive->GetXaxis()->SetTitle(TString(HInclusive->GetXaxis()->GetTitle()).ReplaceAll(" (new)", ""));
   ...
   C1.SaveAs(Form("%s.png", HistogramName.c_str()));
   C1.SaveAs(Form("%s.C", HistogramName.c_str()));
   C1.SaveAs(Form("%s.eps", HistogramName.c_str()));
   C1.SaveAs(Form("%s.pdf", HistogramName.c_str()));

   HInclusive->Divide(HZbb);

   TCanvas C2("C2", "C2", 1024, 1024);

   HInclusive->GetYaxis()->SetTitle("Ratio (a.u.)");
   HInclusive->SetMaximum(2);
   HInclusive->SetMinimum(0);
   HInclusive->Draw();

   C2.SaveAs(Form("%s_Ratio.png", HistogramName.c_str()));
   C2.SaveAs(Form("%s_Ratio.C", HistogramName.c_str()));
   C2.SaveAs(Form("%s_Ratio.eps", HistogramName.c_str()));
   C2.SaveAs(Form("%s_Ratio.pdf", HistogramName.c_str()));
