...
int main(int argc, char *argv[])
{
   ...
   TTree OutputTreeUp("MRTreeUp", "MRTreeUp");
   double TreeMRUp = 0;
   double TreeRUp = 0;
   double TreeFactorUp = 0;
   OutputTreeUp.Branch("MR", &TreeMRUp, "MR/D");
   OutputTreeUp.Branch("R", &TreeRUp, "R/D");
   OutputTreeUp.Branch("Factor", &TreeFactorUp, "Factor/D");

   TTree OutputTreeDown("MRTreeDown", "MRTreeDown");
   double TreeMRDown = 0;
   double TreeRDown = 0;
   double TreeFactorDown = 0;
   OutputTreeDown.Branch("MR", &TreeMRDown, "MR/D");
   OutputTreeDown.Branch("R", &TreeRDown, "R/D");
   OutputTreeDown.Branch("Factor", &TreeFactorDown, "Factor/D");
   ...
   */
   ...
   for(int iEntry = 0; iEntry < EntryCount; iEntry++)
   {
      ...
      double JetUncertainty[1000] = {0};
      for(int i = 0; i < M.nAK5Jet; i++)
      {
         double PT = sqrt(M.pxAK5Jet[i] * M.pxAK5Jet[i] + M.pyAK5Jet[i] * M.pyAK5Jet[i]);
         CaloJetUncertainty.setJetEta(M.etaAK5Jet[i]);
         CaloJetUncertainty.setJetPt(PT);
         JetUncertainty[i] = PT * CaloJetUncertainty.getUncertainty(true);
      }
      ...
      for(int i = 0; i < (int)VBChildren.size(); i++)
      {
         ...
         FourVector PFMETUpDifference(0, 0, 0, 0);
         FourVector PFMETDownDifference(0, 0, 0, 0);

         // Calculate MR and R
         bool JetMatched = false;
         vector<FourVector> GoodJetsUp;
         vector<FourVector> GoodJetsDown;
         for(int j = 0; j < M.nAK5Jet; j++)
         {
            ...
            if(JetP.GetPT() + JetUncertainty[j] > 40)
            {
               PFMETUpDifference = PFMETUpDifference + JetP * JetUncertainty[j] / JetP.GetPT();
               GoodJetsUp.push_back(JetP * (1 + JetUncertainty[j] / JetP.GetPT()));
            }
            if(JetP.GetPT() - JetUncertainty[j] > 40)
            {
               PFMETDownDifference = PFMETDownDifference - JetP * JetUncertainty[j] / JetP.GetPT();
               GoodJetsDown.push_back(JetP * (1 - JetUncertainty[j] / JetP.GetPT()));
            }
         }

         vector<FourVector> GroupsUp, GroupsDown;
         if(GoodJetsUp.size() >= 2 && GoodJetsUp.size() < 20)
            GroupsUp = SplitIntoGroups(GoodJetsUp, true);
         if(GoodJetsDown.size() >= 2 && GoodJetsDown.size() < 20)
            GroupsDown = SplitIntoGroups(GoodJetsDown, true);
         ...
         FourVector PFMETUp = PFMET + PFMETUpDifference;
         FourVector PFMETDown = PFMET + PFMETDownDifference;

         double MRNewUp = 0;
         double RNewUp = 0;
         double R2NewUp = 0;
         double MegaJetDeltaPhiUp = 0;
         double MRNewDown = 0;
         double RNewDown = 0;
         double R2NewDown = 0;
         double MegaJetDeltaPhiDown = 0;

         if(GroupsUp.size() == 2)
         {
            MRNewUp = Get2011MR(GroupsUp[0], GroupsUp[1]);
            RNewUp = Get2011R(GroupsUp[0], GroupsUp[1], PFMETUp);
            R2NewUp = RNewUp * RNewUp;
            MegaJetDeltaPhiUp = fabs(GetDPhi(GroupsUp[0], GroupsUp[1]));
         }
         if(GroupsDown.size() == 2)
         {
            MRNewDown = Get2011MR(GroupsDown[0], GroupsDown[1]);
            RNewDown = Get2011R(GroupsDown[0], GroupsDown[1], PFMETDown);
            R2NewDown = RNewDown * RNewDown;
            MegaJetDeltaPhiDown = fabs(GetDPhi(GroupsDown[0], GroupsDown[1]));
         }

         if(MegaJetDeltaPhiDown > 0.25)
         {
            TreeFactorUp = WPU / 2;
            TreeMRUp = MRNewUp;
            TreeRUp = RNewUp;
         ...
         if(MegaJetDeltaPhiDown > 0.25)
         {
            TreeFactorDown = WPU / 2;
            TreeMRDown = MRNewDown;
            TreeRDown = RNewDown;
            OutputTreeDown.Fill();
         }  
