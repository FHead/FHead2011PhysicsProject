...
int main(int argc, char *argv[])
{
   ...
   for(int iEntry = 0; iEntry < EntryCount; iEntry++)
   {
      ...
      // jet energy modifier (and MET too)
      for(int j = 0; j < M.nAK5Jet; j++)
      {
         double Factor = DrawGaussian(1, 0.1);

         M.pxPFMet[0] = M.pxPFMet[0] - (Factor - 1) * M.pxAK5Jet[j];
         M.pyPFMet[0] = M.pyPFMet[0] - (Factor - 1) * M.pyAK5Jet[j];

         M.pxAK5Jet[j] = M.pxAK5Jet[j] * Factor;
         M.pyAK5Jet[j] = M.pyAK5Jet[j] * Factor;
         M.pzAK5Jet[j] = M.pzAK5Jet[j] * Factor;
         M.energyAK5Jet[j] = M.energyAK5Jet[j] * Factor;
      }
      ...
      for(int i = 0; i < (int)VBChildren.size(); i++)
      {
         ...
         double MegaJetDeltaPhi = fabs(GetDPhi(Groups[0], Groups[1]));
