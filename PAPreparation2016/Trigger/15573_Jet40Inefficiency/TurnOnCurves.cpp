...
int main(int argc, char *argv[])
{
   ...
   for(int i = 0; i < EntryCount; i++)
   {
      ...
      if(BestCaloJetPTIndex >= 0 && MTrigger.CheckTrigger("HLT_L1ZeroBias_BptxAND_v1") == 1)
      {
         ...
         {
            if(MCaloJet.JetRawPT[BestCaloJetPTIndex] > 70)
            {
               cout << "Event R" << MTrigger.Run << " E" << MTrigger.Event << " L" << MTrigger.Lumi << endl;
               cout << "Best PT = " << MCaloJet.JetRawPT[BestCaloJetPTIndex] << endl;
               cout << "But failed CaloJet40 trigger!" << endl;
               cout << endl;
            }
         }
      }
