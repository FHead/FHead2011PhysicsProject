...
int main(int argc, char *argv[])
{
   ...
   for(int i = 0; i < EntryCount; i++)
   {
      ...
      for(int j = 0; j < MCaloJet.JetCount; j++)
      {
         if(MCaloJet.JetEta[j] < -5.1 || MCaloJet.JetEta[j] > 5.1)
            continue;
      ...
      for(int j = 0; j < MPFJet.JetCount; j++)
      {
         if(MPFJet.JetEta[j] < -5.1 || MPFJet.JetEta[j] > 5.1)
            continue;
      ...
      if(BestCaloJetPTIndex >= 0)
      {
         ...
         if(MTrigger.CheckTrigger("HLT_PAAK4CaloJet40_Eta5p1_v2") == 1)
            HCaloJetBestPT_Passed40.Fill(MCaloJet.JetPT[BestCaloJetPTIndex]);
      }
      if(BestPFJetPTIndex >= 0)
      {
         ...
         if(MTrigger.CheckTrigger("HLT_PAAK4PFJet40_Eta5p1_v2") == 1)
            HPFJetBestPT_Passed40.Fill(MPFJet.JetPT[BestPFJetPTIndex]);
      }
   }
