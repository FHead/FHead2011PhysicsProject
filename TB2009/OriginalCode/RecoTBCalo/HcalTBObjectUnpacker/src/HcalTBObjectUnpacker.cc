...
#include <fstream>
  ...
  HcalTBObjectUnpacker::HcalTBObjectUnpacker(edm::ParameterSet const& conf):
    ...
    vlsbFed_(conf.getUntrackedParameter<int>("HcalVLSBFED",-1)),
    ...
    doRunData_(false),doTriggerData_(false),doEventPosition_(false),doTiming_(false),doSourcePos_(false),doBeamADC_(false),doVLSBData_(false)
  {
    ...
    if (vlsbFed_ >=0) {
      std::cout << "HcalTBObjectUnpacker will unpack VLSB FED ";
      std::cout << vlsbFed_ << endl;
      doVLSBData_=true;
    }
    ...
    if (doVLSBData_) produces<HcalTBVLSBData>();
...
//    if (doSourcePos_) produces<HcalSourcePositionData>();
  ...
  void HcalTBObjectUnpacker::produce(edm::Event& e, const edm::EventSetup&)
  {
    ...
    std::auto_ptr<HcalTBVLSBData>
      vlsbd(new HcalTBVLSBData);
    ...
    if (vlsbFed_ >=0) {
      // Step C: unpack all requested FEDs
      const FEDRawData& fed = rawraw->FEDData(vlsbFed_);
      vlsbUnpacker_.unpack(fed, *vlsbd);
    }
