...
void SimultaneousFit(string RootFile = "Samples/DataAll.root", int LastJetBin = 5, string JetFlavor = "Calo",
   ...
   string DataHelperFile)
{
   ...
   RooRealVar Beta("Beta", "Beta parameter in signal yield", 0);   // fix at zero during toy generation?
   ...
   sprintf(StateString, "%s Jet, %d, Eta range all, Isolation 0.30", JetFlavor.c_str(), EnergyThreshold);
   ...
   // Generate samples
   TFile SamplesFile(Form("Samples_%d.root", EnergyThreshold), "RECREATE");

   FitResult->Clone("InitialResult")->Write();

   for(int i = 0; i < 5000; i++)
   {
      RooDataSet *NewDataSet = (RooDataSet *)FinalModel.generate(RooArgSet(ZMass, NJet), DataSetTotalEntry);
      NewDataSet->SetName(Form("Samples_%d", i));
      NewDataSet->Write();
   }
