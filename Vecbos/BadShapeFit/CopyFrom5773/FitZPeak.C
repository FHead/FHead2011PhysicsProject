...
FitResult FitZPeak_Lorentzian(string Rootfile, string BackgroundRootFile);
FitResult FitZPeak_Voigtian(string Rootfile, string BackgroundRootFile);
FitResult FitZPeak_BifurGauss(string Rootfile, string BackgroundRootFile);
FitResult FitZPeak_CBShape(string Rootfile, string BackgroundRootFile);
FitResult FitZPeak_Landau(string Rootfile, string BackgroundRootFile);
...
void FitZPeak(string OutputFileName, string lumi)
{
   ...
   for(int i = 0; i < 4; i++)
   {
      for(unsigned int j = 0; j < SignalFileNames[i].size(); j++)
      {
...
FitResult FitZPeak_Cruijff(string RootFile, string BackgroundRootFile)
{
   ...
   RooRealVar AlphaR("AlphaR", "AlphaR", 0.45, 50.01, 1000000);
   RooRealVar AlphaL("AlphaL", "AlphaL", 0.56 * 0.99, 50.01, 1000000);
   RooRealVar SigmaR("SigmaR", "SigmaR", 2.27, 50.01, 1000000);
   RooRealVar SigmaL("SigmaL", "SigmaL", 2.30, 50.01, 1000000);
...
FitResult FitZPeak_Lorentzian(string RootFile, string BackgroundRootFile)
{
   ...
   RooRealVar PeakPosition("PeakPosition", "Z peak position", 90, 80, 100, "GeV/c^2");
   RooRealVar PeakWidth("PeakWidth", "PeakWidth", 2.27, 0.01, 1000000);
   RooLorentzian MainZPeak("MainZPeak", "MainZPeak", mll, PeakPosition, PeakWidth);
   RooRealVar PeakYield("PeakYield", "Peak Yield", 1000, 0, 1000000);
   ...
   result.PeakWidthValue = PeakWidth.getVal();
   result.PeakWidthError = PeakWidth.getError();
...
FitResult FitZPeak_Voigtian(string RootFile, string BackgroundRootFile)
{
   ...
   RooRealVar PeakPosition("PeakPosition", "Z peak position", 90, 80, 100, "GeV/c^2");
   RooRealVar PeakWidth("PeakWidth", "PeakWidth", 2.27, 0.01, 1000000);
   RooRealVar PeakSigma("PeakSigma", "PeakSigma", 2.27, 0.01, 1000000);
   RooVoigtian MainZPeak("MainZPeak", "MainZPeak", mll, PeakPosition, PeakWidth, PeakSigma);
   RooRealVar PeakYield("PeakYield", "Peak Yield", 1000, 0, 1000000);
   ...
   result.PeakWidthValue = PeakWidth.getVal();
   result.PeakWidthError = PeakWidth.getError();
   result.PeakSigmaValue = PeakSigma.getVal();
   result.PeakSigmaError = PeakSigma.getError();
...
FitResult FitZPeak_BifurGauss(string RootFile, string BackgroundRootFile)
{
   ...
   RooRealVar PeakPosition("PeakPosition", "Z peak position", 90, 80, 100, "GeV/c^2");
   RooRealVar SigmaR("SigmaR", "SigmaR", 2.27, 0.01, 1000000);
   RooRealVar SigmaL("SigmaL", "SigmaL", 2.30, 0.01, 1000000);
   RooBifurGauss MainZPeak("MainZPeak", "MainZPeak", mll, PeakPosition, SigmaL, SigmaR);
   RooRealVar PeakYield("PeakYield", "Peak Yield", 1000, 0, 1000000);
...
FitResult FitZPeak_CBShape(string RootFile, string BackgroundRootFile)
{
   ...
   TChain BackgroundTree("MllTree", "MllTree CBShape Fit Result");
   BackgroundTree.AddFile(BackgroundRootFile.c_str());
   RooDataSet BackgroundDataset("BackgroundCandidates", "BackgroundCandidates", &BackgroundTree, TreeVarSet);
   ...
   RooRealVar PeakPosition("PeakPosition", "Z peak position", 90, 80, 100, "GeV/c^2");
   RooRealVar Sigma("Sigma", "Sigma", 2.27, 0.01, 1000000);
   RooRealVar Alpha("Alpha", "Alpha", 85, 0.01, 1000000);
   RooRealVar n("n", "n", 1, -100, 100);
   RooCBShape MainZPeak("MainZPeak", "MainZPeak", mll, PeakPosition, Sigma, Alpha, n);
   RooRealVar PeakYield("PeakYield", "Peak Yield", 1000, 0, 1000000);
   ...
   result.PeakSigmaValue = Sigma.getVal();
   result.PeakSigmaError = Sigma.getError();
   result.PeakAlphaValue = Alpha.getVal();
   result.PeakAlphaError = Alpha.getError();
   result.NValue = n.getVal();
   result.NError = n.getError();
...
FitResult FitZPeak_Landau(string RootFile, string BackgroundRootFile)
{
   ...
   RooRealVar PeakPosition("PeakPosition", "Z peak position", 90, 80, 100, "GeV/c^2");
   RooRealVar Sigma("Sigma", "Sigma", 2.27, 0.01, 1000000);
   RooReverseLandau MainZPeak("MainZPeak", "MainZPeak", mll, PeakPosition, Sigma);
   RooRealVar PeakYield("PeakYield", "Peak Yield", 1000, 0, 1000000);
