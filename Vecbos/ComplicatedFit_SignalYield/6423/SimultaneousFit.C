...
void SimultaneousFit(string RootFile = "Samples/DataAll.root", int LastJetBin = 4, string JetFlavor = "Track",
   double AlphaLValue = 0.522, int EnergyThreshold = 30);
...
void SimultaneousFit(string RootFile, int LastJetBin, string JetFlavor, double AlphaLValue, int EnergyThreshold)
{
   // Dependent variables
   ...
   RooDataSet *ReducedDataSet = (RooDataSet *)DataSet.reduce(Form("N%sJet%d >= 1 && ZIsHighestPT != 0",
   ...
   RooRealVar Alpha("Alpha", "Alpha parameter in signal yield", 5, 1, 10);
   ...
   PsFileHelper PsFile("ResultPlots_" + JetFlavor + "_" + EnergyThresholdString + ".ps");
   ...
   FourPanelCanvas.SaveAs(Form("ComplicatedFit_%s_AllJets.png", JetFlavor.c_str()));
   FourPanelCanvas.SaveAs(Form("ComplicatedFit_%s_AllJets.C", JetFlavor.c_str()));
   FourPanelCanvas.SaveAs(Form("ComplicatedFit_%s_AllJets.eps", JetFlavor.c_str()));
