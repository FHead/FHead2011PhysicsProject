...
void SimultaneousFit(string RootFile, int LastJetBin, string JetFlavor, double AlphaLValue)
{
   // Dependent variables
   ...
   RooDataSet *ReducedDataSet = (RooDataSet *)DataSet.reduce(Form("N%sJet >= 1 && ZIsHighestPT != 0",
      JetFlavor.c_str()));
   ...
   for(int N = 1; N <= LastJetBin; N++)
   {
      ...
      BackgroundJetYields.push_back(new RooRealVar(Form("Background%dJetYield", N),
         Form("Background yield, %d jet", N), 1, 0, 10000000));
   ...
   PsFileHelper PsFile("ResultPlots_" + JetFlavor + ".ps");
