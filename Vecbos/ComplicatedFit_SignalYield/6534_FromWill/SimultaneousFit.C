...
void SimultaneousFit(string RootFile = "Samples/DataAll.root", int LastJetBin = 4, string JetFlavor = "Track",
   int EnergyThreshold = 30, string DataHelperFile = "Samples/DataAll.dh", bool MTCut = false);
...
void SimultaneousFit(string RootFile, int LastJetBin, string JetFlavor, int EnergyThreshold,
   string DataHelperFile, bool MTCut)
{
   string MTCutTag = "";
   if(MTCut == true)
      MTCutTag = "C";
   ...
   if(MTCut == false)
      ReducedDataSet = (RooDataSet *)DataSet.reduce(Form("N%s_%d >= 1 && cat_idx == 1 && Z_mass < 120 && Z_mass > 60", JetFlavor.c_str(), EnergyThreshold));
   else
      ReducedDataSet = (RooDataSet *)DataSet.reduce(Form("N%s_%d >= 1 && cat_idx == 1 && Z_mass < 120 && Z_mass > 60 && 80.399 / 91.187 * Mt_pf >= 20", JetFlavor.c_str(), EnergyThreshold));
   ...
   RooRealVar Alpha("Alpha", "Alpha parameter in signal yield", 2.62, 1.5, 6);
   RooRealVar Beta("Beta", "Beta parameter in signal yield", 0.28, -2, 2);
   ...
   PsFileHelper PsFile("ResultPlots" + MTCutTag + "_" + JetFlavor + "_" + EnergyThresholdString + ".ps");
   ...
   FourPanelCanvas.SaveAs(Form("ComplicatedFit%s_%s_%d_AllJets.png", MTCutTag.c_str(),
      JetFlavor.c_str(), EnergyThreshold));
   FourPanelCanvas.SaveAs(Form("ComplicatedFit%s_%s_%d_AllJets.C", MTCutTag.c_str(),
      JetFlavor.c_str(), EnergyThreshold));
   FourPanelCanvas.SaveAs(Form("ComplicatedFit%s_%s_%d_AllJets.eps", MTCutTag.c_str(),
      JetFlavor.c_str(), EnergyThreshold));
   ...
   YieldCanvas.SetLogy();
   YieldCanvas.SaveAs(Form("FitYieldsData%s.png", MTCutTag.c_str()));
   YieldCanvas.SaveAs(Form("FitYieldsData%s.C", MTCutTag.c_str()));
   YieldCanvas.SaveAs(Form("FitYieldsData%s.eps", MTCutTag.c_str()));
   ...
   TFile ResultFile(("ResultPlots" + MTCutTag + "_" + JetFlavor + "_" + EnergyThresholdString + ".root").c_str(),
      "RECREATE");
