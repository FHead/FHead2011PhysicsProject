...
void SimultaneousFit(string RootFile = "Samples/DataAll.root", int LastJetBin = 4, string JetFlavor = "Track",
   ...
   string SmearingMatrixFile = "Samples/PF30SmearingMatrix");
...
void SimultaneousFit(string RootFile, int LastJetBin, string JetFlavor, int EnergyThreshold,
   string DataHelperFile, bool Weighted, string SmearingMatrixFile)
{
   string WeightedTag = "";
   if(Weighted == true)
      WeightedTag = "W";
   ...
   double MixingA = 1;
   double MixingB = 1;
   RooRealVar AlphaPrime("AlphaPrime", "Alpha prime", 5.7, -10, 10);
   RooRealVar BetaPrime("BetaPrime", "Beta prime", 0.0, -10, 10);

   RooFormulaVar Alpha("Alpha", "AlphaPrime - BetaPrime", RooArgList(AlphaPrime, BetaPrime));
   
   RooFormulaVar Beta("Beta", "BetaPrime", RooArgList(BetaPrime));
   ...
   for(int N = 0; N <= LastJetBin; N++)
   {
      ...
      if(N == 1)
         UpperBound = 50;   // was 20.5 for 30 GeV
      if(N == 0)
         UpperBound = 5;   // was 6.6
   ...
   FinalModel.paramOn(ParametersPlot1, Format("NELU", AutoPrecision(2)), Layout(0.1, 0.9, 0.9),
      Parameters(RooArgSet(PeakPosition, AlphaL, AlphaR, SigmaL, SigmaR, AlphaPrime, BetaPrime,
      SignalInclusiveYield, BackgroundInclusiveYield)));
   ...
   SummaryValueTexts.push_back(ch);
   sprintf(ch, "Alpha = %f", Alpha.getVal());
   SummaryValueTexts.push_back(ch);
   sprintf(ch, "Beta = %f", Beta.getVal());
   SummaryValueTexts.push_back(ch);
