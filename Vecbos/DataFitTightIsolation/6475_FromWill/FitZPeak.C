...
void FitZPeak(string RootFile = "Samples/DataAll.root", string OutputBase = "DataFitTightIsolation",
   ...
   string DataHelperFile = "Samples/DataAll.dh", int JetCount = 1, bool Weighted = false);
FitResult FitZPeak(string RootFile, PsFileHelper &PsFile, int JetCount, double Isolation, string JetFlavor,
   int Threshold, bool Exclusive = false, bool Weighted = false);
...
void FitZPeak(string RootFile, string OutputBase, string JetFlavor, double Isolation, int Threshold,
   string DataHelperFile, int JetCount, bool Weighted)
{
   char ThresholdString[100] = "";
   sprintf(ThresholdString, "%d", Threshold);
   
   char JetCountString[100] = "";
   sprintf(JetCountString, "%d", JetCount);

   PsFileHelper PsFile(OutputBase + "_" + JetFlavor + "_" + ThresholdString + "_" + JetCountString + ".ps");
   PsFile.AddTextPage("Fit Z peak floating everything");

   FitResult Result = FitZPeak(RootFile, PsFile, JetCount, Isolation, JetFlavor, Threshold, false, Weighted);
   
   PsFile.AddTextPage("Fit zero bin!");
   FitResult ResultZeroJetBin = FitZPeak(RootFile, PsFile, 0, Isolation, JetFlavor, Threshold, true, Weighted);
   ...
   TFile F((OutputBase + "_" + JetFlavor + "_" + ThresholdString + "_" + JetCountString + ".root").c_str(),
      "RECREATE");
...
FitResult FitZPeak(string RootFile, PsFileHelper &PsFile, int JetCount, double Isolation, string JetFlavor,
   int Threshold, bool Exclusive, bool Weighted)
{
   ...
   RooArgSet TreeVarSet(ZMass, NCaloJets, Muon1Isolation, Category, W);
   ...
   RooDataSet dataset("ZCandidates", "ZCandidates", ZTree, TreeVarSet, "", WeightString.c_str());
   ...
   if(Weighted == true)
      MainZPeak.fitTo(*ReducedDataSet, SumW2Error(true));
   else
      MainZPeak.fitTo(*ReducedDataSet, SumW2Error(false));
