   ...
   int Threshold, bool Exclusive)
{
   ...
   RooRealVar NCaloJets(Form("N%sJet%d", JetFlavor.c_str(), Threshold), "Count of jets", 0, 100000);
   RooRealVar Muon1Isolation("Muon1CombinedIsolation", "Muon1 isolation", 0, 100000);
   RooRealVar Muon2Isolation("Muon2CombinedIsolation", "Muon2 isolation", 0, 100000);
   RooRealVar Muon1PT("Muon1PT", "Muon1 PT", 0, 100000);
   RooRealVar Muon2PT("Muon2PT", "Muon2 PT", 0, 100000);
   RooArgSet TreeVarSet(ZMass, NCaloJets, Muon1Isolation, Muon2Isolation, Muon1PT, Muon2PT);
   ...
   TCut NumberOfJetCut = "";
   if(Exclusive == false)
      NumberOfJetCut = Form("N%sJet%d >= %d", JetFlavor.c_str(), Threshold, JetCount);
   else
      NumberOfJetCut = Form("N%sJet%d == %d", JetFlavor.c_str(), Threshold, JetCount);

   TCut IsolationCut = Form("Muon1CombinedIsolation < %f * Muon1PT && Muon2CombinedIsolation < %f * Muon2PT", Isolation, Isolation);
   TCut ZMassRange = "ZMass > 60 && ZMass < 120";
