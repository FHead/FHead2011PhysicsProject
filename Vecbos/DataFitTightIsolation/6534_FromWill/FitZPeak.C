...
void FitZPeak(string RootFile = "Samples/DataAll.root", string OutputBase = "DataFitTightIsolation",
   ...
   string DataHelperFile = "Samples/DataAll.dh", int JetCount = 1, bool MTCut = false);
FitResult FitZPeak(string RootFile, PsFileHelper &PsFile, int JetCount, double Isolation, string JetFlavor,
   int Threshold, bool Exclusive = false, bool MTCut = false);
...
void FitZPeak(string RootFile, string OutputBase, string JetFlavor, double Isolation, int Threshold,
   string DataHelperFile, int JetCount, bool MTCut)
{
   ...
   FitResult Result = FitZPeak(RootFile, PsFile, JetCount, Isolation, JetFlavor, Threshold, false, MTCut);
   
   PsFile.AddTextPage("Fit zero bin!");
   FitResult ResultZeroJetBin = FitZPeak(RootFile, PsFile, 0, Isolation, JetFlavor, Threshold, true, MTCut);
...
FitResult FitZPeak(string RootFile, PsFileHelper &PsFile, int JetCount, double Isolation, string JetFlavor,
   int Threshold, bool Exclusive, bool MTCut)
{
   ...
   RooRealVar MTPF("Mt_pf", "MT PF", -100000, 100000);
   RooArgSet TreeVarSet(ZMass, NCaloJets, Muon1Isolation, Category, MTPF);
