   ...
   int EnergyThreshold, string DataHelperFile, bool Weighted, string SmearingMatrixFile, double Isolation)
{
   ...
   RooArgSet TreeVarSet(ZMass, NJet, Category, W, Iso_pf_relpt);
   ...
   RooDataSet *ReducedDataSet = (RooDataSet *)DataSet.reduce(Form("N%s_%d >= 0 && cat_idx == 1 && Z_mass < 120 && Z_mass > 60 && Iso_pf_relpt < %f", JetFlavor.c_str(), EnergyThreshold, Isolation));
