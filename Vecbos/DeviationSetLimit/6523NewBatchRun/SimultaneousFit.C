   ...
   string DataHelperFile, double ExtraTerm)
{
   ...
   // RooRealVar Alpha("Alpha", "Alpha parameter in signal yield", 3.9 + DrawGaussian(0.5), 1, 10);
   // RooRealVar Beta("Beta", "Beta parameter in signal yield", DrawGaussian(0.7), -3, 3);
   ...
   char ch[1024] = "";
   sprintf(ch, "AlphaPrime = %f +- %f", AlphaPrime.getVal(), AlphaPrime.getError());
   SummaryValueTexts.push_back(ch);
   sprintf(ch, "BetaPrime = %f +- %f", BetaPrime.getVal(), BetaPrime.getError());
   SummaryValueTexts.push_back(ch);
