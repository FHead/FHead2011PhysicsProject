...
double SimultaneousFit(string RootFile = "Samples/DataAll.root", int LastJetBin = 4, string JetFlavor = "Track",
...
double SimultaneousFit(string RootFile, int LastJetBin, string JetFlavor, int EnergyThreshold,
   string DataHelperFile, double ExtraTerm)
{
   ...
   // RooRealVar Alpha("Alpha", "Alpha parameter in signal yield", 3.4 + DrawGaussian(0.5), 1, 10);
   // RooRealVar Beta("Beta", "Beta parameter in signal yield", DrawGaussian(0.5), -3, 3);

   double SigmaAlpha = 0.23 + DrawGaussian(0.2);
   double SigmaBeta = 0.19 + DrawGaussian(0.2);
   double Denominator = SigmaAlpha * SigmaAlpha + SigmaBeta * SigmaBeta;

   RooRealVar AlphaPrime("AlphaPrime", "Alpha prime", SigmaBeta * (3.5 + DrawGaussian(0.2)), -10, 10);
   RooRealVar BetaPrime("BetaPrime", "Beta prime", SigmaAlpha * (3.5 + DrawGaussian(0.3)), -10, 10);

   RooFormulaVar Alpha("Alpha", "Alpha parameter in signal yield",
      Form("(%f * @0 + %f * @1) / %f", SigmaBeta, SigmaAlpha, Denominator), RooArgList(AlphaPrime, BetaPrime));
   RooFormulaVar Beta("Beta", "Beta parameter in signal yield",
      Form("(%f * @0 - %f * @1) / %f", SigmaAlpha, SigmaBeta, Denominator), RooArgList(AlphaPrime, BetaPrime));
   ...
   cout << "Correlation between alpha' and beta' = " << FitResult->correlation(AlphaPrime, BetaPrime) << endl;

   vector<string> SummaryValueTexts;
   char ch[1024] = "";
   sprintf(ch, "Alpha' = %f +- %f", AlphaPrime.getVal(), AlphaPrime.getError());
   SummaryValueTexts.push_back(ch);
   sprintf(ch, "Beta' = %f +- %f", BetaPrime.getVal(), BetaPrime.getError());
   SummaryValueTexts.push_back(ch);
   sprintf(ch, "Correlation between alpha' and beta' = %f", FitResult->correlation(AlphaPrime, BetaPrime));
   SummaryValueTexts.push_back(ch);
   ...
   FitResult->Clone(Form("ConstrainedResult_%s", EnergyThresholdString))->Write();
   ...
   char State[1000] = "";
   sprintf(State, "Energy %d, Extra term %.06f", EnergyThreshold, ExtraTerm);

   bool Write = true;
   if(DatabaseFile.Exist(State) == true)
   {
      if(DatabaseFile[State].Exist("MinNLL") == true)
      {
         double NewNLL = FitResult->minNll();
         double OldNLL = DatabaseFile[State]["MinNLL"].GetDouble();
   ...
   if(Write == true)
   {
      DatabaseFile[State]["AlphaPrime"] = AlphaPrime.getVal();
      DatabaseFile[State]["AlphaPrimeError"] = AlphaPrime.getError();
      DatabaseFile[State]["BetaPrime"] = BetaPrime.getVal();
      DatabaseFile[State]["BetaPrimeError"] = BetaPrime.getError();
      DatabaseFile[State]["AlphaPrimeBetaPrimeCorrelation"] = FitResult->correlation("AlphaPrime", "BetaPrime");
      ...
      DatabaseFile[State]["JetThreshold"] = EnergyThreshold;

      DatabaseFile.SaveToFile("Database.dh");
   }

   // result
   double result = FitResult->minNll();
   cout << "MinNLL = " << result << endl;
