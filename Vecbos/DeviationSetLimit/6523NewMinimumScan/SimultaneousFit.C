   ...
   string DataHelperFile, double ExtraTerm)
{
   ...
   // RooRealVar Alpha("Alpha", "Alpha parameter in signal yield", 3.5 + DrawGaussian(0.5), 1, 10);
   // RooRealVar Beta("Beta", "Beta parameter in signal yield", DrawGaussian(0.4), -3, 3);

   double SigmaAlpha = 0.23;
   double SigmaBeta = 0.19;
   double Denominator = SigmaAlpha * SigmaAlpha + SigmaBeta * SigmaBeta;

   RooRealVar AlphaPrime("AlphaPrime", "Alpha prime", SigmaBeta * 3.5, -10, 10);
   RooRealVar BetaPrime("BetaPrime", "Beta prime", SigmaAlpha * 3.5, -10, 10);
   ...
   if(Write == true)
   {
      ...
      DatabaseFile[State]["SigmaAlphaInput"] = SigmaAlpha;
      DatabaseFile[State]["SigmaBetaInput"] = SigmaBeta;
