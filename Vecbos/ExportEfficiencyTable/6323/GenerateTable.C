...
void GenerateTable()
{
   ...
   {
      Records[i].push_back(ReadSample("Samples/ZJetsAll.root", "ZJets", 800, JetFlavors[i]));
   ...
   for(unsigned int i = 0; i < JetFlavors.size(); i++)
   {
      ...
      PlotAbsoluteYieldTable(F, PsFile, Records[i], 2.66, ch, JetFlavors[i]);
      
      sprintf(ch, "%s Jet Relative Yield -- the jet part is relative to events after selection", JetFlavors[i].c_str());
      PlotRelativeYieldTable(F, PsFile, Records[i], 2.66, ch, JetFlavors[i]);
   }
