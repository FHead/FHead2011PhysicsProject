...
struct SampleRecord;
class EventIndex;
void GenerateInclusiveJetTable();
void PlotInclusiveNJetRelativeYieldTable(TFile &F, PsFileHelper &File, vector<SampleRecord> &Records,
   double Luminosity = 10, string title = "", string JetFlavor = "");
SampleRecord ReadSample(string RootFile, string Name, double CrossSection, string JetFlavor, int JetCount);
//---------------------------------------------------------------------------
struct SampleRecord
{
public:
...
void GenerateInclusiveJetTable()
{
   ...
   vector<SampleRecord> Records[2];

   for(unsigned int i = 0; i < JetFlavors.size(); i++)
   {
      Records[i].push_back(ReadSample("Samples/ZJetsAll.root", "0Jet", 800, JetFlavors[i], 0));
      Records[i].push_back(ReadSample("Samples/ZJetsAll.root", "1Jet", 800, JetFlavors[i], 1));
      Records[i].push_back(ReadSample("Samples/ZJetsAll.root", "2Jets", 800, JetFlavors[i], 2));
      Records[i].push_back(ReadSample("Samples/ZJetsAll.root", "3Jets", 800, JetFlavors[i], 3));
      Records[i].push_back(ReadSample("Samples/ZJetsAll.root", "4Jets", 800, JetFlavors[i], 4));
   }

   PsFileHelper PsFile("InclusiveJetResultTable.ps");
   TFile F("InclusiveJetResultTable.root", "RECREATE");
   ...
   for(unsigned int i = 0; i < JetFlavors.size(); i++)
   {
      ...
      sprintf(ch, "%s Jet Relative Yield", JetFlavors[i].c_str());
      PlotInclusiveNJetRelativeYieldTable(F, PsFile, Records[i], 2.66, ch, JetFlavors[i]);
   }
...
//---------------------------------------------------------------------------
void PlotInclusiveNJetRelativeYieldTable(TFile &F, PsFileHelper &File,
...
SampleRecord ReadSample(string RootFile, string Name, double CrossSection, string JetFlavor, int JetCount)
{
   ...
   TH1D *CountEventsSelectionCaloJet = (TH1D *)F.Get(Form("CountEventsSelection%sJet%d",
      JetFlavor.c_str(), JetCount));
