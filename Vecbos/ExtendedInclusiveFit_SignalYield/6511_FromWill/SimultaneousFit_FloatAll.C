...
void SimultaneousFit_FloatAll(string RootFile, int LastJetBin, string JetFlavor,
   int EnergyThreshold, string DataHelperFile, bool Weighted)
{
   string WeightString = "";
   if(Weighted == true)
      WeightString = "W";
   ...
   for(int N = 0; N <= LastJetBin; N++)
   {
      ...
      if(N == 1)
         UpperBound = 30;   // 10 for 15 GeV thresholds, 30 for 30 GeV thresholds
      BackgroundJetYields.push_back(new RooRealVar(Form("Background%dJetYield", N),
         Form("Background yield, %d jet", N), 0.3, 0.2, UpperBound));
   ...
   */
   ...
   for(int N = 0; N <= LastJetBin; N++)
   {
      ...
      if(N != LastJetBin)
         SeparateResultJetPlot = ZMass.frame(Bins(24), Name(Form("SeparateResult%dJetPlot", N)), Title(""));
      ...
      FinalModel.plotOn(SeparateResultJetPlot, Components(RooArgSet(*BackgroundJets[N], *SignalJets[N])),
         Normalization(DataSetTotalEntry), RooFit::DrawOption("L"), RooFit::LineColor(kOrange+7), RooFit::LineWidth(2),
         RooFit::MoveToBack());
      FinalModel.plotOn(SeparateResultJetPlot, Components(RooArgSet(*BackgroundJets[N], *SignalJets[N])),
         Normalization(DataSetTotalEntry), RooFit::DrawOption("F"), RooFit::LineColor(kOrange+4),
      ...
      FinalModel.plotOn(SeparateResultJetPlot, Components(*BackgroundJets[N]), Normalization(DataSetTotalEntry),
         RooFit::DrawOption("F"), RooFit::FillColor(kViolet));
      FinalModel.plotOn(SeparateResultJetPlot, Components(*BackgroundJets[N]), Normalization(DataSetTotalEntry),
         RooFit::LineColor(kViolet+3), RooFit::LineWidth(2));
      ...
      SeparateResultJetPlot->SetYTitle("Events / (2 GeV/c^{2})");
      SeparateResultJetPlot->Draw();
