...
void SimultaneousFit_FloatAll(string RootFile = "Samples/DataAll.root", int LastJetBin = 4,
   ...
   bool Weighted = false, string SmearingMatrixFile = "PF30SmearingMatrix");
...
void SimultaneousFit_FloatAll(string RootFile, int LastJetBin, string JetFlavor,
   int EnergyThreshold, string DataHelperFile, bool Weighted, string SmearingMatrixFile)
{
   string WeightString = "";
   if(Weighted == true)
      WeightString = "W";
   ...
   if(DHFile[StateString].Exist("ConstrainedFitSmearingUnfoldAlpha") == false
      || DHFile[StateString].Exist("ConstrainedFitSmearingUnfoldBeta") == false
      || DHFile[StateString].Exist("ConstrainedFitSmearingUnfoldAlphaError") == false
      || DHFile[StateString].Exist("ConstrainedFitSmearingUnfoldBetaError") == false
      || DHFile[StateString].Exist("ConstrainedFitSmearingUnfoldAlphaBetaCorrelation") == false)
   {
      cout << "Please do the constrained fit first (unfold with smearing matrix)!" << endl;
      return;
   }

   double AlphaFromComplicatedFit = DHFile[StateString]["ConstrainedFitSmearingUnfoldAlpha"].GetDouble();
   double SigmaAlphaFromComplicatedFit = DHFile[StateString]["ConstrainedFitSmearingUnfoldAlphaError"].GetDouble();
   double BetaFromComplicatedFit = DHFile[StateString]["ConstrainedFitSmearingUnfoldBeta"].GetDouble();
   double SigmaBetaFromComplicatedFit = DHFile[StateString]["ConstrainedFitSmearingUnfoldBetaError"].GetDouble();
   double ComplicatedCorrelation = DHFile[StateString]["ConstrainedFitSmearingUnfoldAlphaBetaCorrelation"].GetDouble();
   ...
   RooRealVar Mu2Eta("mu2Eta", "Muon 2 eta", -500, 500);
   RooArgSet TreeVarSet(ZMass, NJet, Category, W, Mu2Eta);
   ...
   RooDataSet *ReducedDataSet = (RooDataSet *)DataSet.reduce(Form("N%s_%d >= 0 && cat_idx == 1 && Z_mass < 120 && Z_mass > 60 && mu2Eta > -2.1 && mu2Eta < 2.1", JetFlavor.c_str(), EnergyThreshold));
   ...
   for(int N = 0; N <= LastJetBin; N++)
   {
      ...
      BackgroundJetYields.push_back(new RooRealVar(Form("Background%dJetYield", N),
         Form("Background yield, %d jet", N), 0.5, 0.1, UpperBound));
   ...
   */
   ...
   PsFileHelper PsFile("ResultPlots" + WeightString + "_FloatAll_" + JetFlavor + "_" + ThresholdString + ".ps");
   ...
   for(int N = 0; N <= LastJetBin; N++)
   {
      ...
      TFile FFF(Form("FFF_%d.root", N), "recreate");

      RooPlot *SeparateResultJetPlot;
      if(N != LastJetBin)
         SeparateResultJetPlot = ZMass.frame(Bins(60 / BinWidth),
            Name(Form("SeparateResult%dJetPlot", N)), Title(" "));
      else
         SeparateResultJetPlot = ZMass.frame(Bins(60 / BinWidth),
            Name(Form("SeparateResult%dJetPlot", N)), Title(" "));

      FinalModel.plotOn(SeparateResultJetPlot, Components(RooArgSet(*BackgroundJets[N], *SignalJets[N])),
         Normalization(DataSetTotalEntry / BinWidth), RooFit::DrawOption("L"), RooFit::LineColor(kOrange+7),
         RooFit::LineWidth(2), RooFit::MoveToBack());
      FinalModel.plotOn(SeparateResultJetPlot, Components(RooArgSet(*BackgroundJets[N], *SignalJets[N])),
         Normalization(DataSetTotalEntry / BinWidth), RooFit::DrawOption("F"), RooFit::LineColor(kOrange+4),
         RooFit::FillColor(kOrange), RooFit::LineWidth(2));
      FinalModel.plotOn(SeparateResultJetPlot, Components(*BackgroundJets[N]),
         Normalization(DataSetTotalEntry / BinWidth), RooFit::DrawOption("F"), RooFit::FillColor(kViolet));
      FinalModel.plotOn(SeparateResultJetPlot, Components(*BackgroundJets[N]),
         Normalization(DataSetTotalEntry / BinWidth), RooFit::LineColor(kViolet+3), RooFit::LineWidth(2));
      ...
      if(Weighted == false)
         ReducedJetDataSet->plotOn(SeparateResultJetPlot, Rescale(1.0 / BinWidth));
      else
         ReducedJetDataSet->plotOn(SeparateResultJetPlot, DataError(RooAbsData::SumW2), Rescale(1.0 / BinWidth));
      ...
      TCanvas TempCanvas("TempCanvas", "TempCanvas", 0, 0, 1024, 1024);
      SeparateResultJetPlot->SetXTitle("M_{#mu#mu} [GeV/c^{2}]");
      SeparateResultJetPlot->SetYTitle(Form("Events / (%.1f GeV/c^{2})", BinWidth));
      SeparateResultJetPlot->Draw();
      ...
      TempCanvas.Write();
      SeparateResultJetPlot->Write();

      PsFile.AddCanvas(TempCanvas);
   }

   PsFile.AddCanvas(FirstFourBinCanvas);
   FirstFourBinCanvas.SaveAs(Form("FitPlot%s_%s_%d.C", WeightString.c_str(), JetFlavor.c_str(), EnergyThreshold));
   FirstFourBinCanvas.SaveAs(Form("FitPlot%s_%s_%d.eps", WeightString.c_str(), JetFlavor.c_str(), EnergyThreshold));
   FirstFourBinCanvas.SaveAs(Form("FitPlot%s_%s_%d.png", WeightString.c_str(), JetFlavor.c_str(), EnergyThreshold));
   ...
   Canvas.SaveAs(Form("SummaryPlot%s_FloatAll_%s_%d.C", WeightString.c_str(), JetFlavor.c_str(), EnergyThreshold));
   Canvas.SaveAs(Form("SummaryPlot%s_FloatAll_%s_%d.png", WeightString.c_str(), JetFlavor.c_str(), EnergyThreshold));
   Canvas.SaveAs(Form("SummaryPlot%s_FloatAll_%s_%d.eps", WeightString.c_str(), JetFlavor.c_str(), EnergyThreshold));
   ...
   Canvas2.SaveAs(Form("FinalPlot%s_FloatAll_%s_%d.C", WeightString.c_str(), JetFlavor.c_str(), EnergyThreshold));
   Canvas2.SaveAs(Form("FinalPlot%s_FloatAll_%s_%d.png", WeightString.c_str(), JetFlavor.c_str(), EnergyThreshold));
   Canvas2.SaveAs(Form("FinalPlot%s_FloatAll_%s_%d.eps", WeightString.c_str(), JetFlavor.c_str(), EnergyThreshold));
   ...
   TFile F(Form("FitResult%s_%s_%d.root", WeightString.c_str(), JetFlavor.c_str(), EnergyThreshold), "RECREATE");
   ...
   // Writing out results
   TFile ResultFile(Form("ResultFile%s_%s_%d.root", WeightString.c_str(), JetFlavor.c_str(), EnergyThreshold),
      "RECREATE");
