...
void CollectResult(string Threshold)
{
   ...
   // TFile F11(("NoJEC_Z2Unfold_Weighted/ResultFileW_pf_" + Threshold + ".root").c_str());
   TFile F12(("NoJEC_D6TUnfold_Weighted/ResultFileW_pf_" + Threshold + ".root").c_str());
   TFile F13(("NoJEC_PythiaZ2Unfold_Weighted/ResultFileW_pf_" + Threshold + ".root").c_str());
   ...
   cout << "\\hline" << endl;
   cout << "   & $\\ge 0$ & $\\ge 1$ & $\\ge 2$ & $\\ge 3$ & $\\ge 4$ & $1 Jet/2 Jet$ \\\\\\hline\\hline" << endl;
   ...
   for(int i = 0; i < (int)Results.size(); i++)
   {
      ...
      for(int j = 0; j < 5; j++)
      {
         ...
         cout << setprecision(0) << " & $" << Value << " \\pm " << Error << "$ ";
      }

      double Value1Jet = ((RooRealVar *)(Results[i]->floatParsFinal().find("SignalInclusiveYield1Jet")))->getVal();
      double Value2Jet = ((RooRealVar *)(Results[i]->floatParsFinal().find("SignalInclusiveYield2Jet")))->getVal();
      cout << setprecision(3) << " & " << Value1Jet / Value2Jet;
