...
void SimultaneousFit_FloatAll(string RootFile = "Samples/DataAll.root", int LastJetBin = 4,
   string JetFlavor = "PF", double AlphaLValue = 0.485, int EnergyThreshold = 30, double AlphaFromComplicatedFit = -10,
   double BetaFromComplicatedFit = -10, double SigmaAlphaFromComplicatedFit = -10,
   double SigmaBetaFromComplicatedFit = -10, double ComplicatedCorrelation = 0);

void SimultaneousFit_FloatAll(string RootFile, int LastJetBin, string JetFlavor, double AlphaLValue,
   int EnergyThreshold, double AlphaFromComplicatedFit, double BetaFromComplicatedFit,
   double SigmaAlphaFromComplicatedFit, double SigmaBetaFromComplicatedFit, double ComplicatedCorrelation)
{
   // Dependent variables
   ...
   RooDataSet *ReducedDataSet = (RooDataSet *)DataSet.reduce(Form("N%sJet%d >= 1 && ZIsHighestPT != 0", JetFlavor.c_str(), EnergyThreshold));
   ...
   {
      int PointCount = 0;
      for(double x = 0.5; x < LastJetBin + 1; x = x + 0.05)
      {
