...
void PFThresholdScan()
{
   PsFileHelper PsFile("PFThresholdScan.ps");
   PsFile.AddTextPage("PF Jet Multiplicity Threshold Scan");

   vector<string> FileNames;
   FileNames.push_back("../FitResult_PF_15.root");
   FileNames.push_back("../FitResult_PF_17.root");
   FileNames.push_back("../FitResult_PF_20.root");
   FileNames.push_back("../FitResult_PF_25.root");
   FileNames.push_back("../FitResult_PF_30.root");
...
void MakeRatioPlots(PsFileHelper &PsFile, vector<string> FileNames, vector<string> Labels, vector<double> Thresholds)
{
   ...
   TH1D RangeSetter("RangeSetter", "Ratio summary for different jet threhsold (PF)", 7, 0.5, 7.5);
   ...
   for(int i = 0; i < (int)FileNames.size(); i++)
   {
      ...
      for(int j = 0; j < Graphs[i]->GetN(); j++)   // shift the point a little for a clearer view
      {
         double X, Y;
      ...
      double X, Y, EY;
      BandGraphs[i]->GetPoint(0, X, Y);
      EY = BandGraphs[i]->GetErrorY(0);
