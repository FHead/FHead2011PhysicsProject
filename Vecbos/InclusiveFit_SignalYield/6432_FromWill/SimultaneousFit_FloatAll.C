...
void SimultaneousFit_FloatAll(string RootFile = "Yay.root", int LastJetBin = 4,
   string JetFlavor = "PF", double AlphaLValue = 0.520);
...
void SimultaneousFit_FloatAll(string RootFile, int LastJetBin, string JetFlavor, double AlphaLValue)
{
   ...
   RooRealVar NJet("Npf_30", "Count of jets", 0, 20);
   RooRealVar cay_idx("cat_idx", "category", -1, 10);
   RooArgSet TreeVarSet(ZMass, NJet, cay_idx);

   // Import data
   TFile F1(RootFile.c_str());
   TTree *ZTree1 = (TTree *)F1.Get("TemporaryTree");
   RooDataSet DataSet("ZCandidates", "ZCandidates", ZTree1, TreeVarSet);
   
   RooDataSet *ReducedDataSet = (RooDataSet *)DataSet.reduce("Npf_30 >= 1 && cat_idx > 0.5");
   ...
   for(int N = 1; N <= LastJetBin; N++)
   {
      ...
      if(N != LastJetBin)
         SeparateResultJetPlot = ZMass.frame(Bins(20), Name(Form("SeparateResult%dJetPlot", N)),
      ...
      FinalModel.plotOn(SeparateResultJetPlot, Components(*BackgroundJets[N-1]),
         Normalization(DataSetTotalEntry));

      RooDataSet *ReducedJetDataSet;
      if(N != LastJetBin)
         ReducedJetDataSet = (RooDataSet *)ReducedDataSet->reduce(Form("Npf_30 == %d", N));
      else
         ReducedJetDataSet = (RooDataSet *)ReducedDataSet->reduce(Form("Npf_30 >= %d", N));
   ...
   FirstFourBinCanvas.SaveAs(Form("FitPlot_%s.C", JetFlavor.c_str()));
   FirstFourBinCanvas.SaveAs(Form("FitPlot_%s.eps", JetFlavor.c_str()));
   FirstFourBinCanvas.SaveAs(Form("FitPlot_%s.png", JetFlavor.c_str()));
