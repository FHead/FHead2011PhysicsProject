...
void SimultaneousFit_FloatAll(string RootFile = "Samples/DataAll.root", int LastJetBin = 4,
   string JetFlavor = "PF", int EnergyThreshold = 30, string EtaString = "",
   string DataHelperFile = "Samples/DataAll.dh");

void SimultaneousFit_FloatAll(string RootFile, int LastJetBin, string JetFlavor,
   int EnergyThreshold, string EtaString, string DataHelperFile)
{
   string EtaRange = "all";
   if(EtaString == "_Eta10")
      EtaRange = "1.0";
   ...
   sprintf(StateString, "%s Jet, %d, Eta range %s, Isolation 0.05",
      JetFlavor.c_str(), EnergyThreshold, EtaRange.c_str());
   ...
   sprintf(StateString, "%s Jet, %d, Eta range %s, Isolation 0.30",
      JetFlavor.c_str(), EnergyThreshold, EtaRange.c_str());
   cout << "Constrained fit state: \"" << StateString << "\"" << endl;
   ...
   RooRealVar IsHighestPTZ("ZIsHighestPT", "Highest PT candidate in event", -10, 10);
   RooArgSet TreeVarSet(ZMass, NJet, IsHighestPTZ);
   ...
   for(int N = 1; N <= LastJetBin; N++)
   {
      ...
      CurrentGuess = CurrentGuess / AlphaFromComplicatedFit;
   }
   ...
   PsFileHelper PsFile("ResultPlots_FloatAll_" + JetFlavor + "_" + ThresholdString + EtaString + ".ps");
   ...
   FirstFourBinCanvas.SaveAs(Form("FitPlot_%s_%d%s.C", JetFlavor.c_str(), EnergyThreshold, EtaString.c_str()));
   FirstFourBinCanvas.SaveAs(Form("FitPlot_%s_%d%s.eps", JetFlavor.c_str(), EnergyThreshold, EtaString.c_str()));
   FirstFourBinCanvas.SaveAs(Form("FitPlot_%s_%d%s.png", JetFlavor.c_str(), EnergyThreshold, EtaString.c_str()));
   ...
   Canvas.SaveAs(Form("SummaryPlot_FloatAll_%s_%d%s.C", JetFlavor.c_str(), EnergyThreshold, EtaString.c_str()));
   Canvas.SaveAs(Form("SummaryPlot_FloatAll_%s_%d%s.png", JetFlavor.c_str(), EnergyThreshold, EtaString.c_str()));
   Canvas.SaveAs(Form("SummaryPlot_FloatAll_%s_%d%s.eps", JetFlavor.c_str(), EnergyThreshold, EtaString.c_str()));
   ...
   Canvas2.SaveAs(Form("FinalPlot_FloatAll_%s_%d%s.C", JetFlavor.c_str(), EnergyThreshold, EtaString.c_str()));
   Canvas2.SaveAs(Form("FinalPlot_FloatAll_%s_%d%s.png", JetFlavor.c_str(), EnergyThreshold, EtaString.c_str()));
   Canvas2.SaveAs(Form("FinalPlot_FloatAll_%s_%d%s.eps", JetFlavor.c_str(), EnergyThreshold, EtaString.c_str()));
   ...
   TFile F(Form("FitResult_%s_%d%s.root", JetFlavor.c_str(), EnergyThreshold, EtaString.c_str()), "RECREATE");
