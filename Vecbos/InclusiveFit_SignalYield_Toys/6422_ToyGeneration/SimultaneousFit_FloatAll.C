...
void SimultaneousFit_FloatAll(string RootFile, int LastJetBin, string JetFlavor, double AlphaLValue)
{
   ...
   // Print out central values
   for(int N = 1; N <= LastJetBin; N++)
   {
      cout << N << " " << SignalJetInclusiveYields[N-1]->getVal()
         << " +- " << SignalJetInclusiveYields[N-1]->getError() << endl;
   }
   ...
   {
      RooDataSet *toydataset = FinalModel.generate(TreeVarSet,
         DataSetTotalEntry + DrawGaussian(sqrt(DataSetTotalEntry)));
