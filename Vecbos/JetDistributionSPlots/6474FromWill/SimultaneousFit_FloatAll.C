   ...
   int EnergyThreshold, string DataHelperFile)
{
   ...
   RooRealVar FirstJetPT("firstJet_pt", "First jet PT", 0, 10000);
   RooRealVar W("W", "Weight", -10000, 10000);
   // RooArgSet TreeVarSet(ZMass, NJet, Category);
   // RooArgSet TreeVarSet(ZMass, NJet, Category, FirstJetPT, W);
   RooArgSet TreeVarSet(ZMass, NJet, Category, W);

   // Import data
   TFile F1(RootFile.c_str());
   TTree *ZTree1 = (TTree *)F1.Get("tree");
   RooDataSet DataSet("ZCandidates", "ZCandidates", ZTree1, TreeVarSet, "", "W");   // weighted
   ...
   // Fit!
   RooFitResult *FitResult = FinalModel.fitTo(*ReducedDataSet, Verbose(false), Save(true), SumW2Error(true));
   ...
   for(int N = LastJetBin; N >= 0; N--)
   {
      ...
      SummaryPlot.SetBinContent(N + 1, 4, SignalJetYields[N]->getVal());
      SummaryPlot.SetBinError(N + 1, 4, 0);
