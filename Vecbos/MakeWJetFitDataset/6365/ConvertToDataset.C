...
void ConvertToDataset(string InputFile, string OutputFile, double CrossSection = 1)
{
   ...
   for(int i = 0; i < EntryCount; i++)
   {
      ...
      pair<long long, long long> index(Run + 0.0001, Event + 0.0001);

      if(Count.find(index) != Count.end())
      {
         Count[index] = Count[index] + 1;
         continue;
      }
      else
         Count.insert(pair<pair<long long, long long>, int>(index, 1));
   ...
   RooRealVar Var_NBTaggedCaloJet("NBTaggedCaloJet", "NBTaggedCaloJet", 0, 10000); Variables.add(Var_NBTaggedCaloJet);
   RooRealVar Var_NLooseBTaggedCaloJet("NLooseBTaggedCaloJet", "NLooseBTaggedCaloJet", 0, 10000);
      Variables.add(Var_NLooseBTaggedCaloJet);
   RooRealVar Var_NMatchedCaloJet("NMatchedCaloJet", "NMatchedCaloJet", 0, 10000); Variables.add(Var_NMatchedCaloJet);
   RooRealVar Var_NBTaggedMatchedCaloJet("NBTaggedMatchedCaloJet", "NBTaggedMatchedCaloJet", 0, 10000);
      Variables.add(Var_NBTaggedMatchedCaloJet);
   RooRealVar Var_NLooseBTaggedMatchedCaloJet("NLooseBTaggedMatchedCaloJet", "NLooseBTaggedMatchedCaloJet", 0, 10000);
