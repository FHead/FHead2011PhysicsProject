...
int main()
{
   TChain *Tree = ChainAll("Z.list", "ntp1");
   ...
   for(int iEntry = 0; iEntry < EntryCount; iEntry++)
   {
      ...
      for(int i = 0; i < nMc; i++)
      {
         if(idMc[i] == 13)
         {
            if(mothMc[i] != -1 && fabs(idMc[mothMc[i]]) == 23)
            {
               HasZMuMu = true;
               MuIndex = i;
            }
      ...
      if(MuIndex >= 0 && MuBarIndex >= 0)
      {
         double pxmu1 = energyMc[MuIndex] / cosh(etaMc[MuIndex]) * cos(phiMc[MuIndex]);
         double pymu1 = energyMc[MuIndex] / cosh(etaMc[MuIndex]) * sin(phiMc[MuIndex]);
         double pzmu1 = energyMc[MuIndex] * tanh(etaMc[MuIndex]);
         double pxmu2 = energyMc[MuBarIndex] / cosh(etaMc[MuBarIndex]) * cos(phiMc[MuBarIndex]);
         double pymu2 = energyMc[MuBarIndex] / cosh(etaMc[MuBarIndex]) * sin(phiMc[MuBarIndex]);
         double pzmu2 = energyMc[MuBarIndex] * tanh(etaMc[MuBarIndex]);

         double px = pxmu1 + pxmu2;
         double py = pymu1 + pymu2;
         double pz = pzmu1 + pzmu2;
         double E = energyMc[MuIndex] + energyMc[MuBarIndex];
         double Mass2 = E * E - px * px - py * py - pz * pz;

         if(Mass2 < 60 * 60 || Mass2 > 120 * 120)
            continue;
      }
      ...
      if(FirstLegMuonCount >= 1 && SecondLegMuonCount >= 2)
         HJetCount1.Fill(JetCount);
      if(FirstLegElectronCount >= 1 && SecondLegElectronCount >= 2)
