...
FitResult FitZPeak(string RootFile, int JetCount, double MuonPTMin, double MuonPTMax, PsFileHelper &PsFile);
...
void FitZPeak(string RootFile)
{
   ...
   MuonPTs.push_back(15);
   MuonPTs.push_back(20);
   MuonPTs.push_back(25);
   MuonPTs.push_back(30);
   MuonPTs.push_back(35);
   MuonPTs.push_back(40);
   MuonPTs.push_back(45);
   MuonPTs.push_back(50);
   MuonPTs.push_back(55);
   MuonPTs.push_back(60);
   MuonPTs.push_back(65);
   MuonPTs.push_back(70);
   MuonPTs.push_back(75);
   MuonPTs.push_back(80);
   MuonPTs.push_back(85);
   MuonPTs.push_back(90);
   MuonPTs.push_back(95);
   MuonPTs.push_back(100);
   MuonPTs.push_back(110);
   MuonPTs.push_back(120);
   MuonPTs.push_back(130);

   vector<FitResult> Results(MuonPTs.size());

   PsFileHelper FitPlots("FitPlots.ps");
   FitPlots.AddTextPage("Fits!");

   for(unsigned int i = 0; i < MuonPTs.size() - 1; i++)
      Results[i] = FitZPeak(RootFile, 1, MuonPTs[i], MuonPTs[i+1], FitPlots);
   ...
   for(unsigned int i = 0; i < MuonPTs.size() - 1; i++)
   {
      GAlphaL.SetPoint(i, (MuonPTs[i] + MuonPTs[i+1]) / 2, Results[i].AlphaL[0]);
      GAlphaL.SetPointError(i, (MuonPTs[i+1] - MuonPTs[i]) / 2, Results[i].AlphaL[1]);
      GAlphaR.SetPoint(i, (MuonPTs[i] + MuonPTs[i+1]) / 2, Results[i].AlphaR[0]);
      GAlphaR.SetPointError(i, (MuonPTs[i+1] - MuonPTs[i]) / 2, Results[i].AlphaR[1]);
      GSigmaL.SetPoint(i, (MuonPTs[i] + MuonPTs[i+1]) / 2, Results[i].SigmaL[0]);
      GSigmaL.SetPointError(i, (MuonPTs[i+1] - MuonPTs[i]) / 2, Results[i].SigmaL[1]);
      GSigmaR.SetPoint(i, (MuonPTs[i] + MuonPTs[i+1]) / 2, Results[i].SigmaR[0]);
      GSigmaR.SetPointError(i, (MuonPTs[i+1] - MuonPTs[i]) / 2, Results[i].SigmaR[1]);
      GMass.SetPoint(i, (MuonPTs[i] + MuonPTs[i+1]) / 2, Results[i].Mass[0]);
      GMass.SetPointError(i, (MuonPTs[i+1] - MuonPTs[i]) / 2, Results[i].Mass[1]);
   }
   ...
   */
   ...
   GAlphaL.GetHistogram()->GetXaxis()->SetTitle("First leg muon PT");
   GAlphaL.GetHistogram()->GetYaxis()->SetTitle("AlphaL");
   GAlphaR.GetHistogram()->GetXaxis()->SetTitle("First leg muon PT");
   GAlphaR.GetHistogram()->GetYaxis()->SetTitle("AlphaR");
   GSigmaL.GetHistogram()->GetXaxis()->SetTitle("First leg muon PT");
   GSigmaL.GetHistogram()->GetYaxis()->SetTitle("SigmaL");
   GSigmaR.GetHistogram()->GetXaxis()->SetTitle("First leg muon PT");
   GSigmaR.GetHistogram()->GetYaxis()->SetTitle("SigmaR");
   GMass.GetHistogram()->GetXaxis()->SetTitle("First leg muon PT");
...
FitResult FitZPeak(string RootFile, int JetCount, double MuonPTMin, double MuonPTMax, PsFileHelper &PsFile)
{
   ...
   TCut MuonPTCut = Form("Muon1PT < %f && Muon1PT > %f", MuonPTMax, MuonPTMin);
   RooDataSet *ReducedDataSet = (RooDataSet *)dataset.reduce(NumberOfJetCut && IsolationCut && MuonPTCut);
   ...
   RooPlot *FinalResult = ZMass.frame(Bins(50), Name("MLL"),
      Title(Form("Mass of lepton pair, muon pt = %f - %f", MuonPTMin, MuonPTMax)));
